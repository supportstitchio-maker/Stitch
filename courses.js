        ...
        function addLectureFloatingReaction(id, icon, name){
          lectureFloatingReactions.push({ id, icon, name });
          rerenderLectureScreen();
          setTimeout(() => {
        ...
        function lectureReactionsFloatHTML(){
          ...
          return lectureFloatingReactions.map(r => `
            <span class="lecture-reaction-float flex items-center gap-1.5 bg-white shadow-md rounded-full pl-1 pr-2.5 py-1">
              <span class="w-8 h-8 rounded-full bg-gray-50 flex items-center justify-center text-gray-700 flex-shrink-0">${Icon(r.icon,'w-4 h-4')}</span>
              <span class="text-xs font-semibold text-gray-700 truncate max-w-[110px]">${escapeHtml(r.name || '')}</span>
            </span>`).join('');
        }
        ...
        function handleLectureSignal(payload){
          ...
          if (payload.type === 'reaction') {
            addLectureFloatingReaction(payload.id, payload.icon, payload.name);
            return;
          }
