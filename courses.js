        ...
        // and the rest unlocked only after enrolling. Not wired to a real
        // backend yet -- see postCourseInsertRemote below for the same
        // best-effort Supabase pattern used for opportunities, and the SQL
        // comment above jobsData for how to actually set that up.
        ...
        // Every course is taught by "Team Stitch" by default; admins can
        // additionally invite other teachers onto a specific course (see
        // openCourseTeachers / inviteCourseTeacher below), who show up
        // alongside Team Stitch in that course's Teachers list.
        const TEAM_STITCH_TEACHER = 'Team Stitch';

        function resetNewCourseDraft(){
          newCourseDraft = { title:'', description:'', modules:[] };
        }
        ...
        function courseCardHTML(c){
          ...
          return `
            <div onclick="openCourseDetail('${c.id}')" class="bg-white rounded-3xl p-5 mb-3 shadow-sm cursor-pointer">
              <div class="flex items-center justify-between gap-2 mb-1">
                <div class="text-xs font-bold uppercase tracking-wide text-gray-400">${escapeHtml(c.org)}</div>
              ...
              <div class="text-lg font-bold font-display text-gray-800 mb-1">${escapeHtml(c.title)}</div>
              ...
              <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
                ${isCurrentUserAdmin() ? `
                  <div class="flex gap-3 mb-4">
                    <button onclick="openEditCourse('${c.id}')" class="flex-1 bg-gray-100 rounded-2xl py-2.5 font-semibold text-xs text-gray-700">${Icon('edit','w-3.5 h-3.5 inline mr-1')}Edit Course</button>
                    <button onclick="openCourseTeachers('${c.id}')" class="flex-1 bg-gray-100 rounded-2xl py-2.5 font-semibold text-xs text-gray-700">${Icon('personPlus','w-3.5 h-3.5 inline mr-1')}Teachers</button>
                    <button onclick="deleteCourse('${c.id}')" class="flex-1 bg-gray-100 rounded-2xl py-2.5 font-semibold text-xs text-red-500">${Icon('trash','w-3.5 h-3.5 inline mr-1')}Delete</button>
                  </div>` : ''}
                ...
                <button onclick="openCourseTeachers('${c.id}')" class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 flex items-center gap-1">${escapeHtml(c.org)}${(c.teachers && c.teachers.length) ? ` +${c.teachers.length}` : ''}</button>
                ...
                <div class="text-sm font-bold text-gray-700 mb-3">${escapeHtml(mod.title)}</div>
                ${locked ? `
          ...
          return `
            <div onclick="openCourseItemDetail('${course.id}','${item.id}')" class="bg-white rounded-3xl p-5 mb-3 shadow-sm border border-gray-100 cursor-pointer">
              <div class="flex items-center justify-between gap-2 mb-3">
                <div class="font-bold text-gray-800">${escapeHtml(item.title)}</div>
          ...
          return `
            <div class="flex-1 flex flex-col overflow-hidden">
              ...
              <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
                ...
                ${item.type === 'video' ? `
                  <div class="bg-black rounded-3xl mb-5 flex items-center justify-center" style="aspect-ratio:16/9;">
                    <span class="text-white">${Icon('video','w-10 h-10')}</span>
                  </div>` : ''}
                ${(item.type === 'reading' || item.type === 'assignment') ? `
        ...
        function openEditCourse(courseId){
          ...
          if (!c) return;
          newCourseDraft = JSON.parse(JSON.stringify({ title: c.title, description: c.description, modules: c.modules }));
        ...
        function addCourseDraftModule(){
          newCourseDraft.modules.push({ id:'mod' + Date.now() + Math.random().toString(36).slice(2,6), title:'Module ' + (newCourseDraft.modules.length + 1), items:[] });
          rerenderNewCourseOverlay();
        }

        function removeCourseDraftModule(mi){
          newCourseDraft.modules.splice(mi, 1);
          if (newCourseAddingItemModuleIndex === mi) { newCourseAddingItemModuleIndex = null; newCourseAddingItemDraft = null; }
          rerenderNewCourseOverlay();
        }
        ...
        function openAddCourseItemForm(mi){
          ...
          newCourseAddingItemDraft = { title:'', type:'video', duration:'', description:'', questions:[] };
          rerenderNewCourseOverlay();
        }
        ...
        function confirmAddCourseItem(){
          ...
          newCourseDraft.modules[newCourseAddingItemModuleIndex].items.push(item);
          newCourseAddingItemModuleIndex = null;
          newCourseAddingItemDraft = null;
          rerenderNewCourseOverlay();
        }

        function publishNewCourse(){
          if (!isCurrentUserAdmin()) return;
          ...
          if (newCourseEditingId) {
            ...
            if (c) {
              ...
              c.modules = d.modules;
              // Best-effort: keep the remote copy in sync too, same
              // pattern as a brand-new course below.
          ...
          const course = {
            ...
            description: d.description.trim(),
            modules: d.modules,
            teachers: [], // additional teachers invited on top of Team Stitch -- see openCourseTeachers
          };
          ...
          // course just stays local, same as opportunities above.
        ...
        // a course; Team Stitch is always the base teacher and can't be
        // removed) ----
        ...
        async function postCourseInsertRemote(course){
          ...
          try {
            ...
            const { error } = await sb.from(COURSES_TABLE).insert({
              id: course.id,
              data: course,
            ...
            <div class="bg-white rounded-3xl p-4 shadow-sm mb-3 border border-gray-100">
              ...
              ${m.items.map((it, ii) => `
                <div class="flex items-center justify-between gap-2 bg-gray-50 rounded-xl px-3 py-2 mb-2">
                  ...
                  <button onclick="removeCourseDraftItem(${mi},${ii})" class="text-red-500 flex-shrink-0">${Icon('close','w-4 h-4')}</button>
                </div>`).join('')}
        ...
        function addCourseItemFormHTML(){
          ...
          return `
            <div class="bg-blue-50 rounded-2xl p-3 mb-2">
              <input type="text" value="${escapeHtml(d.title)}" oninput="updateAddingItemField('title', this.value)" placeholder="Item title" class="w-full bg-white rounded-xl px-3 py-2 text-sm outline-none mb-2">
              ...
              ${!isQuiz ? `
                <textarea oninput="updateAddingItemField('description', this.value)" placeholder="${d.type === 'video' ? 'Video notes (optional)' : 'Content / instructions'}" rows="2" class="w-full bg-white rounded-xl px-3 py-2 text-sm outline-none resize-none mb-2">${escapeHtml(d.description)}</textarea>` : `
                <div class="mb-2">
              ...
              <div class="flex gap-2">
                ...
                <button onclick="confirmAddCourseItem()" class="flex-1 rounded-xl py-2 font-semibold text-xs text-white" style="background:${NAVY};">Add</button>
              </div>
            </div>`;
        }
        ...
        let myClasses = []; // { id, name, section, room, subject, code, role, students:[{name,email}] }
          ...
          return `
            ...
            <div class="p-4 flex-1 overflow-y-auto">
              ...
              <div class="flex items-center gap-3 mb-6">
                <span class="w-11 h-11 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 flex-shrink-0">${Icon('user','w-6 h-6')}</span>
                <div class="min-w-0">
                  <div class="font-semibold text-sm text-[${NAVY}] truncate">Stitch Nkrumah</div>
                  <div class="text-xs text-gray-500 truncate">stitch@example.com</div>
                </div>
              </div>
        ...
        function submitJoinClassroom(){
          ...
          const found = myClasses.find(c => c.code === code);
          if (!found) { alert("We couldn't find that class. Check the code and try again."); return; }
          found.role = 'student';
        ...
        function submitCreateClassroom(){
          ...
          const newClass = { id, name, section, subject, code, role: 'teacher', students: [], coTeachers: [], announcements: [], classwork: [], lectures: [], photo: null, notificationsEnabled: true, colorIndex, motifIndex };
          runClassActionLoading('Creating class', 'plus', () => {
        ...
        function leaveCurrentClass(){
          myClasses = myClasses.filter(c => c.id !== currentClassId);
          currentClassId = null;
          closeOverlay();
        }
            ...
            ${cls.announcements.length ? cls.announcements.map(a => `
              <div class="bg-white rounded-3xl p-4 mb-3 shadow-sm">
                <div class="flex items-center gap-3 mb-2.5">
                  <span class="w-10 h-10 rounded-full bg-blue-50 flex items-center justify-center text-[${NAVY}] flex-shrink-0">${Icon('user','w-5 h-5')}</span>
                  <div class="min-w-0">
                    <div class="font-semibold text-sm text-gray-800 truncate">Stitch Nkrumah</div>
                ...
                <div class="space-y-2.5 mb-3">
                  ${comments.map(c => `
                    <div class="flex items-start gap-2.5">
                      <span class="w-7 h-7 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 flex-shrink-0">${Icon('user','w-3.5 h-3.5')}</span>
                      <div class="min-w-0">
        ...
        function toggleAnnouncementRepost(announcementId){
          ...
          if (a.reposted) {
            ...
            const post = PostsAPI.create({
              avatarIcon:'bell', avatarBg:'bg-blue-50', name:'Stitch Nkrumah',
        ...
        // No backend means other devices can't actually be reached, so
        // classmates are shown as real roster entries (muted tiles) while
        // the host's own tile carries a genuine live camera feed.
        ...
        function lectureLocalMediaHTML(){
          ...
          return showLocalVideo
            ? `<video id="lecture-local-video" autoplay playsinline muted class="w-full h-full object-cover" style="transform:scaleX(-1);"></video>`
            : `<div class="w-full h-full flex items-center justify-center bg-gray-700">${Icon('user','w-10 h-10 text-white/70')}</div>`;
        }
        ...
        function lectureGridHTML(){
          const cls = myClasses.find(c => c.id === liveLectureState.classId);
          const students = ((cls && cls.students) || []).filter(s => !s.pending);
          ...
          const otherTiles = students.map((s, i) => `
            <div class="relative rounded-2xl overflow-hidden flex items-center justify-center ${LECTURE_TILE_COLORS[i % LECTURE_TILE_COLORS.length]}" style="aspect-ratio:4/3;">
              <span class="text-white font-bold text-2xl">${escapeHtml((s.name || '?').trim().charAt(0).toUpperCase())}</span>
              <div class="absolute bottom-1.5 left-1.5 text-xs font-semibold text-white bg-black/30 rounded-full px-2 py-0.5 max-w-[85%] truncate">${escapeHtml(s.name)}</div>
              <div class="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-black/40 flex items-center justify-center text-white">${Icon('micOff','w-3.5 h-3.5')}</div>
            </div>`).join('');
          ...
          return `
            ...
            ${!students.length ? `<div class="text-center text-xs text-gray-400 pb-2 flex-shrink-0">No other students in this class yet: invite them from the People tab.</div>` : ''}`;
        }
        ...
        function joinLectureSignaling(lectureId){
          const sb = getSupabaseClient();
          if (!sb) return; // no Supabase configured: whiteboard stays local-only
          lectureChannel = sb.channel(`lecture:${lectureId}`, { config: { broadcast: { self: false } } });
          lectureChannel.on('broadcast', { event: 'lecture-signal' }, ({ payload }) => handleLectureSignal(payload));
          lectureChannel.subscribe();
        }
        ...
        function broadcastLectureSignal(payload){
          ...
          lectureChannel.send({ type: 'broadcast', event: 'lecture-signal', payload: Object.assign({ lectureId: liveLectureState.lectureId }, payload) });
        }

        // Students (and any other participant) receive teacher-driven
        // updates here: switching into/out of the whiteboard, and every
        // stroke/undo/clear the teacher makes while on it.
          ...
          return `
            ...
            <div class="border-t border-gray-100 mb-4">
              <div class="flex items-center gap-3 py-3.5 ${(isTeacher && coTeachers.length) ? 'border-b border-gray-100' : ''}">
                <span class="w-11 h-11 rounded-full bg-blue-50 flex items-center justify-center text-[${NAVY}] flex-shrink-0">${Icon('user','w-6 h-6')}</span>
                <div class="font-semibold text-sm text-gray-800">Stitch Nkrumah</div>
              </div>
        ...
        function classSettingsHTML(){
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto px-5 pb-8">
              ...
              <button onclick="openLeaveClassModal(leaveCurrentClass, 'Leave ' + cls.name + '?', 'You\\'ll be removed from this classroom and its materials until you rejoin.')" class="w-full flex items-center justify-center gap-2 rounded-2xl py-3 font-semibold text-sm text-red-500 border border-red-100 bg-red-50">
        ...
        function updateClassName(value){
          const cls = myClasses.find(c => c.id === currentClassId);
          if (cls) cls.name = value;
        }

        function updateClassSection(value){
          const cls = myClasses.find(c => c.id === currentClassId);
          if (cls) cls.section = value;
        }
