        ...
        function syncAIClassKeyboardInset(){
          ...
          // Stitch Bot always reserves a gap the height of the Classroom
          // sub-nav underneath it (see openOverlay's keepsTaskbar block) so
          // that bar stays visible below the chat. This used to just be
          // overwritten with the raw keyboard inset -- which is 0 whenever
          // the keyboard is closed, and can also be a few px of browser
          // toolbar chrome even then -- so instead of the sub-nav peeking
          // through the reserved gap, whatever's on the Classroom screen
          // behind it (its content, not the nav bar) bled through. Once the
          // keyboard actually opens (or the composer is simply focused --
          // see aiChatInputFocused) it covers the sub-nav anyway, so it's
          // fine for the overlay to just track the keyboard at that point;
          // otherwise, keep the nav-bar-height gap intact.
          //
          // KEYBOARD_INSET_NOISE_FLOOR (px): that "few px of browser
          // toolbar chrome" comment above was underselling it -- on mobile
          // Chrome, simply scrolling the page (no keyboard, nothing
          // focused) makes the address bar collapse/expand, which shifts
          // visualViewport.height by up to ~80-100px on its own. With the
          // old `keyboardInset > 0` check, that alone was enough to flip
          // keyboardLikelyOpen to true: the Classroom sub-nav would blink
          // out and the overlay's bottom gap would collapse and un-collapse
          // as the chrome animated, which is what showed up as the whole
          // chat screen visibly shaking on scroll even with the keyboard
          // never actually open. Real on-screen keyboards are always
          // comfortably taller than that (200px+), so require the inset to
          // clear this floor before treating it as "keyboard open" --
          // aiChatInputFocused still covers the real "about to type" case
          // instantly, this only guards the visualViewport-derived signal.
          const KEYBOARD_INSET_NOISE_FLOOR = 120;
          const keyboardLikelyOpen = keyboardInset > KEYBOARD_INSET_NOISE_FLOOR || aiChatInputFocused;
          const navH = typeof activeNavBarHeight === 'function' ? activeNavBarHeight() : 0;
          ov.style.bottom = (keyboardLikelyOpen ? keyboardInset : navH) + 'px';
          // Breathing room under the composer: 20px normally, tightened to
          // 10px once the on-screen keyboard is up (so the bar doesn't sit
          // as far from the keyboard as it does from the bottom of the
          // screen when there's no keyboard eating into the space).
          ...
          // The Classroom sub-nav (Resources/Practice/Stitch Bot/Alerts)
          // is what the reserved bottom gap above is normally there to
          // show -- but once the on-screen keyboard is up, that gap is
          // handed over to the keyboard instead, and the nav bar has
          // nowhere to sit except awkwardly stacked on top of it. Hide it
          // outright while typing (or about to type -- see
          // keyboardLikelyOpen) so only the composer shows above the
          // keyboard; restore it the moment the keyboard closes again.
          const classroomNavEl = document.getElementById('classroom-nav');
          if (classroomNavEl) classroomNavEl.style.display = keyboardLikelyOpen ? 'none' : '';
        }
          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start">
              <div class="min-w-0" style="max-width:85%;">
                <div class="bg-gray-100 rounded-2xl px-4 py-3.5 text-[15px] text-gray-700 leading-relaxed">${isThinking ? aiThinkingIndicatorHTML() : formatAIText(text)}</div>
