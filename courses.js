        ...
        // Images get a real thumbnail (both in this pre-send strip and in
        // the sent bubble, see aiUserBubble) instead of a generic file
        // pill, so the student can actually see what they attached rather
        // than just its filename. previewUrl is a local blob: URL good
        // for this tab's session; it's set once at select time in
        // handleAIFileSelect and reused everywhere the file is rendered.
            ...
            <div class="px-4 pt-3 flex gap-2 flex-wrap">
              ${pendingAIAttachments.map((f,i) => f.previewUrl ? `
                <div class="relative flex-shrink-0" style="width:3.5rem;height:3.5rem;">
                  <img src="${f.previewUrl}" alt="${escapeHtml(f.name)}" class="w-full h-full object-cover rounded-xl" />
        ...
        function handleAIFileSelect(event){
          ...
          files.forEach(f => {
            ...
            if (okType) {
              if (f.type && f.type.startsWith('image/')) f.previewUrl = URL.createObjectURL(f);
        ...
        function aiUserBubble(text, attachments, id){
          // Images get an actual thumbnail here, same as the pre-send
          // attach strip (aiAttachStripHTML) -- they used to always fall
          // into the generic file-pill row below, so a sent photo showed
          // up as an unhelpful blue "image.jpg" chip instead of the
          // picture itself. Docs/PDFs/PPTX (no previewUrl) still use the
          // pill since there's nothing visual to preview.
          const imageAttachments = (attachments || []).filter(f => f.previewUrl);
          const fileAttachments = (attachments || []).filter(f => !f.previewUrl);
          // Images and files get their own tightly-fitted pill (a thin
          // p-1/px-2.5 border around the content, not a full chat-bubble
          // padding) instead of sharing the same px-4 py-3 padding as
          // real text -- that used to leave a big empty band of blue
          // around every attached photo. Any actual typed text still
          // gets the normal padded bubble underneath.
          const images = imageAttachments.length ? `
            <div class="flex flex-wrap gap-1 rounded-2xl p-1" style="background:${NAVY};">
              ${imageAttachments.map(f => `
                <img src="${f.previewUrl}" alt="${escapeHtml(f.name)}" class="rounded-xl object-cover block" style="width:9.5rem;height:9.5rem;max-width:100%;" />`).join('')}
            </div>` : '';
