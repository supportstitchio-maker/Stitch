        ...
        function confirmClearAIChat(){
          openAppConfirmModal('Clear this chat?', 'This will delete your conversation with StudyBot.', 'Clear', function(){ clearAIChat(); });
        }
        ...
        function aiClassHTML(){
          ...
          return `
            ...
            <div class="p-3 border-t border-gray-100 flex-shrink-0 flex items-center gap-1.5">
              ...
              <textarea id="ai-chat-input" rows="1" placeholder="Ask StudyBot anything..." onkeydown="handleAIChatInputKeydown(event)" oninput="autoGrowAIChatInput(this)" class="flex-1 min-w-0 bg-gray-100 rounded-3xl px-3 py-2 text-sm resize-none leading-snug" style="max-height:120px; overflow-y:auto;"></textarea>
        ...
        // Enter sends the message; Shift+Enter (or Alt/Ctrl+Enter) inserts a newline instead.
        ...
        function aiUserBubble(text, attachments, id){
          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start justify-end gap-3">
              <div class="text-white rounded-2xl px-4 py-3 text-sm" style="max-width:80%;background:${NAVY};white-space:pre-wrap;">${files}${escapeHtml(text)}</div>
            </div>`;
        }
        ...
        async function getAIResponse(text, attachments){
          ...
          try {
            ...
            const reply = await callClaude(system, text, 'chat');
        ...
        function resetNewCourseDraft(){
          newCourseDraft = { title:'', description:'', deliveryType:'uploaded', modules:[], resources:[], photo:null };
        }
        ...
        function courseCardHTML(c){
          ...
          return `
            <div onclick="openCourseDetail('${c.id}')" class="relative text-white rounded-3xl p-5 mb-3 shadow-sm cursor-pointer overflow-hidden" style="${c.photo ? `background-image:linear-gradient(rgba(10,37,64,0.45),rgba(10,37,64,0.45)),url('${c.photo}');background-size:cover;background-position:center;` : blueCardBackgroundStyle(c, idx === -1 ? 0 : idx)}">
              ...
              <div class="text-xs text-white/80 relative">${c.modules.length} module${c.modules.length === 1 ? '' : 's'} · ${total} lesson${total === 1 ? '' : 's'}${enrolled ? ` · ${done}/${total} completed` : ''}</div>
                ...
                ${courseDetailTab === 'resources' ? courseResourcesTabHTML(c, enrolled) : courseDetailTab === 'engage' ? courseEngageTabHTML(c) : (!c.modules.length ? `
                  ...
                  ${!enrolled ? `
                    ...
                    <div class="mb-5">
                      ...
                      <button onclick="leaveCourse('${c.id}')" class="w-full mt-3 flex items-center justify-center gap-2 rounded-2xl py-3 font-semibold text-sm text-white" style="background:rgba(239,68,68,0.75);">
          ...
          const thumb = item.mediaType === 'image' && item.mediaUrl
            ...
            : item.mediaType === 'video' && item.mediaUrl
              ...
              : item.mediaType === 'document' && item.mediaUrl
                ...
                : '';
          // Real download link when a file's attached (video/image/document);
          // stopPropagation so tapping it doesn't also open the item detail
          // overlay underneath it. Falls back to a plain (inert) icon when
          // there's nothing to download yet.
          const downloadControl = item.mediaUrl
            ? `<a href="${item.mediaUrl}" download="${escapeHtml(item.mediaFileName || item.title)}" target="_blank" rel="noopener" onclick="event.stopPropagation()" class="text-gray-400 flex-shrink-0">${Icon('download','w-4 h-4')}</a>`
            : `<span class="text-gray-300 flex-shrink-0">${Icon('download','w-4 h-4')}</span>`;
          return `
            <div onclick="openCourseItemDetail('${course.id}','${item.id}')" class="bg-white rounded-3xl p-5 mb-3 shadow-sm border border-gray-100 cursor-pointer">
              ...
              <div class="flex items-center gap-1.5 text-xs font-semibold ${item.completed ? 'text-emerald-700' : 'text-gray-500'}">
                <span class="inline-flex items-center gap-1">${item.completed ? Icon('check','w-3.5 h-3.5') : ''} ${courseItemTypeLabel(item.type)}</span>
          ...
          return `
            <div class="flex-1 flex flex-col overflow-hidden">
              ...
              <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
                ...
                ${item.type === 'document' ? `
                  <div class="bg-white rounded-3xl p-5 shadow-sm mb-3 border border-gray-100 flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                      ...
                      <div class="text-xs text-gray-400">${item.mediaUrl ? 'Ready to download' : 'No file uploaded yet'}</div>
                    </div>
                  </div>` : ''}
                ${item.type === 'audio' ? `
                ...
                ${item.mediaUrl ? `
                  <a href="${item.mediaUrl}" download="${escapeHtml(item.mediaFileName || item.title)}" target="_blank" rel="noopener" class="w-full flex items-center justify-center gap-2 rounded-2xl py-3 font-semibold text-sm mb-3" style="background:rgba(30,144,255,0.1);color:#1E90FF;">
                    ${Icon('download','w-4 h-4')} Download file
                  </a>` : ''}
            ...
            <div class="bg-white rounded-3xl p-5 shadow-sm mb-5">
              ...
              ${item.submissionUrl ? `
                <div class="flex items-center gap-2.5 bg-gray-50 rounded-xl p-3">
                  ...
                  <a href="${item.submissionUrl}" download="${escapeHtml(item.submissionFileName || 'submission')}" target="_blank" rel="noopener" class="flex-shrink-0" style="color:#1E90FF;">${Icon('download','w-4 h-4')}</a>
        ...
        function openEditCourse(courseId){
          ...
          const c = allCourses.find(x => x.id === courseId);
          if (!c) return;
          newCourseDraft = JSON.parse(JSON.stringify({ title: c.title, description: c.description, deliveryType: c.deliveryType || 'uploaded', modules: c.modules, resources: c.resources || [], photo: c.photo || null }));
        ...
        function handleAddingItemMediaSelected(e){
          ...
          if (!d) return;
          // The Part's chosen type (video/picture/document) decides how the
          // file is treated -- not the raw mime type -- so a docx uploaded
          // under "Document" stays a document rather than getting sniffed
          // into image/video.
          d.mediaType = d.type === 'video' ? 'video' : d.type === 'document' ? 'document' : 'image';
        ...
        function confirmAddCourseItem(){
          ...
          const item = {
            ...
            mediaUrl: (d.type === 'video' || d.type === 'image' || d.type === 'document') ? (d.mediaUrl || null) : null,
            mediaType: (d.type === 'video' || d.type === 'image' || d.type === 'document') ? d.mediaType : null,
            mediaFileName: (d.type === 'video' || d.type === 'image' || d.type === 'document') ? (d.mediaFileName || null) : null,
            completed:false,
          };
        ...
        function addCourseItemFormHTML(){
          ...
          const isQuiz = courseItemIsQuiz(d.type);
          const isMedia = d.type === 'video' || d.type === 'image' || d.type === 'document';
          return `
            <div class="bg-blue-50 rounded-2xl p-3 mb-2">
              ...
              ${isMedia ? `
                <input type="file" id="adding-item-media-input" accept="${d.type === 'video' ? 'video/*' : d.type === 'document' ? '.pdf,.doc,.docx,.ppt,.pptx,.xls,.xlsx' : 'image/*'}" class="hidden" onchange="handleAddingItemMediaSelected(event)">
                ${d.mediaUrl ? (d.mediaType === 'document' ? `
                  <div class="relative rounded-xl mb-2 bg-white border border-gray-200 p-3 flex items-center gap-2.5">
                    <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0" style="background:rgba(30,144,255,0.12);color:#1E90FF;">${Icon('doc','w-4.5 h-4.5')}</div>
                    <div class="flex-1 min-w-0 text-xs font-semibold text-gray-700 truncate">${escapeHtml(d.mediaFileName || 'Document uploaded')}</div>
                    ${d.mediaUploading ? `<span class="text-[10px] text-gray-400 flex-shrink-0">Uploading...</span>` : ''}
                    <button onclick="removeAddingItemMedia()" class="w-6 h-6 rounded-full bg-gray-100 text-gray-500 flex items-center justify-center flex-shrink-0">${IconBold('close','w-3.5 h-3.5')}</button>
                  </div>` : `
                  ...
                  <button onclick="document.getElementById('adding-item-media-input').click()" class="w-full flex items-center justify-center gap-2 rounded-xl py-3 font-semibold text-xs border border-dashed border-gray-300 text-gray-500 bg-white mb-2">
                    ${Icon('camera','w-4 h-4')} Upload ${d.type === 'video' ? 'a video' : d.type === 'document' ? 'a document' : 'a picture'}
                  </button>`}
              ` : ''}
              ...
              ${!isQuiz ? `
                <textarea oninput="updateAddingItemField('description', this.value)" placeholder="${isMedia ? 'Describe this ' + (d.type === 'video' ? 'video' : d.type === 'document' ? 'document' : 'picture') + ' (optional)' : 'Content / instructions'}" rows="2" class="w-full bg-white rounded-xl px-3 py-2 text-sm outline-none resize-none mb-2">${escapeHtml(d.description)}</textarea>` : `
                <div class="mb-2">
