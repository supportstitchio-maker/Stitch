        ...
        function courseCardHTML(c){
          ...
          return `
            <div onclick="openCourseDetail('${c.id}')" class="relative text-white rounded-3xl p-5 mb-3 shadow-sm cursor-pointer overflow-hidden" style="${blueCardBackgroundStyle(c, idx === -1 ? 0 : idx)}">
        ...
        function courseEngageQuizAssignmentSectionHTML(course){
          ...
          return `
            ...
            ${items.length ? items.map(({ mi, ii, moduleTitle, item }) => `
              ...
              <div class="bg-gray-50 border border-dashed border-gray-300 rounded-3xl p-6 text-center text-gray-500 text-sm mb-2">No quizzes, assignments, or projects yet. Add one to a module from Manage Course.</div>`}
          ...
          return `
            <div onclick="openCourseItemDetail('${course.id}','${item.id}')" class="bg-white rounded-3xl p-5 mb-3 shadow-sm border border-gray-100 cursor-pointer">
              ...
              <div class="flex items-center gap-2">
                <span class="inline-flex items-center gap-1.5 border rounded-full px-[10px] py-[5px] text-xs font-semibold ${item.completed ? 'border-emerald-600 text-emerald-700' : 'border-gray-300 text-gray-500'}">
                  ${item.completed ? Icon('check','w-3.5 h-3.5') : ''} ${courseItemTypeLabel(item.type)}
                </span>
                ${item.duration ? `<span class="border border-gray-200 rounded-full px-[10px] py-[5px] text-xs font-semibold text-gray-500">${escapeHtml(item.duration)}</span>` : ''}
              </div>
            </div>`;
        }
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
              ...
              <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
                <div class="flex items-center gap-2 mb-4">
                  <span class="border border-gray-200 rounded-full px-[10px] py-[5px] text-xs font-semibold text-gray-500">${courseItemTypeLabel(item.type)}</span>
                  ${item.duration ? `<span class="border border-gray-200 rounded-full px-[10px] py-[5px] text-xs font-semibold text-gray-500">${escapeHtml(item.duration)}</span>` : ''}
                </div>
                ${item.type === 'video' ? `
