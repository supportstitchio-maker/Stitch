        ...
        function aiChatLogHTML(){
          ...
          const bubbles = aiChatMessages.map((m, i) => m.role === 'bot' ? aiBotBubble(m.text, i) : aiUserBubble(m.text, m.attachments, i, i === aiEditingMessageId)).join('');
          return bubbles;
        }
          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start">
              <div class="min-w-0" style="max-width:85%;">
                ...
                ${isThinking ? '' : `<button onclick="copyAIMessage(${id})" title="Copy" class="mt-1.5 flex items-center justify-center text-gray-400">${Icon('copy','w-3.5 h-3.5')}</button>`}
              </div>
            </div>`;
        }
        ...
        function aiUserBubble(text, attachments, id, isEditing){
          ...
          const textBubble = text ? `
            <div class="rounded-2xl px-4 py-3 text-[15px] text-gray-800 leading-relaxed" style="background:rgba(30,144,255,0.14); white-space:pre-wrap;">${escapeHtml(text)}</div>` : '';
          if (isEditing) {
            return `
