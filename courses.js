        ...
        // a plain static image -- no animation, no motion. Each time this
        // screen mounts (openOverlay / startNewAIChat), one of four poses
        // is picked at random and just sits there. It steps aside
        // (hideAIRobot) the moment the student taps the composer to start
        // typing (handleAIChatInputFocus), and comes back (showAIRobot,
        // with a freshly re-rolled random pose) if they back out without
        // having typed anything (handleAIChatInputBlur).
        const AI_ROBOT_POSES = [
          'assets/stitch-robot-point.png',
          'assets/stitch-robot-wave-open.png',
          'assets/stitch-robot-wave-fingers.png',
          'assets/stitch-robot-cheer-fist.png',
        ];

        function aiRobotRandomPoseSrc(){
          return AI_ROBOT_POSES[Math.floor(Math.random() * AI_ROBOT_POSES.length)];
        }
        ...
        function showAIRobot(){
          ...
          if (img) img.src = aiRobotRandomPoseSrc();
          ...
          return `
            <div class="h-full min-h-full flex flex-col items-center justify-center text-center px-8">
              <div id="ai-robot-container" class="ai-robot-container">
                <img id="ai-robot-img" class="ai-robot-img" src="${aiRobotRandomPoseSrc()}" alt="Stitch Bot">
              </div>
            </div>`;
        }
