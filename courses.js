try {
          ...
          loadPaystackScript().then(() => {
            ...
            const handler = window.PaystackPop.setup({
              ...
              callback: function(response){
                ...
                verifyClassPayment(response.reference || reference, classId);
              },
        ...
        async function verifyClassPayment(reference, classId, attempt){
          attempt = attempt || 1;
          try {
            ...
            // A bare "Failed to fetch" means the request never made it to the server at all --
            // e.g. the connection blips for a moment while the browser hands control back from
            // the Paystack popup. That's transient, not a real verification failure, so retry a
            // couple of times before bothering the user -- a successful payment shouldn't get
            // stuck behind a network hiccup that clears up half a second later.
            const isNetworkError = (err instanceof TypeError) && /failed to fetch/i.test(err.message || '');
            if (isNetworkError && attempt < 3) {
              setTimeout(() => verifyClassPayment(reference, classId, attempt + 1), 1500 * attempt);
              return;
            }
            classPaymentBusy = false;
            window.reportError && window.reportError(err, { call: 'verifyClassPayment', classId, attempt });
            if (isNetworkError) {
              openAppConfirmModal('Payment not confirmed', "We couldn't reach the server to confirm your payment after a few tries. If you were charged, tap Retry -- your payment reference is saved and this won't charge you again.", 'Retry', () => verifyClassPayment(reference, classId, 1));
            } else {
