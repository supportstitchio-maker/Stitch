        ...
        // Mermaid needs a one-time init before mermaid.run() (used by
        // renderPendingMermaidDiagrams below) will draw anything.
        // startOnLoad stays false since diagrams only ever show up later,
        // inside a chat bubble, never at page load. Wrapped in try/catch
        // in case the CDN script didn't load (offline, blocked, etc.) --
        // formatAIText/renderPendingMermaidDiagrams already fall back
        // gracefully when mermaid is undefined, so a missing library just
        // means diagrams don't render, not a broken page.
        try {
        ...
        // ---- Stitch Bot voice input: speech-to-text only. Recording
        // uses the browser's built-in Web Speech API (SpeechRecognition)
        // rather than recording+uploading a raw audio clip, since
        // callClaude only ever forwards text/images to the ai-proxy Edge
        // Function (see its comment in study.js) -- there is no
        // server-side speech model to send actual audio to. Tapping the
        // mic transcribes what the student says straight into the
        // composer and sends it the moment they stop talking. Replies
        // are always shown as text, never read back aloud -- there used
        // to be a text-to-speech step here for messages sent by voice,
        // but it has been removed; viaVoice is now only used to tag the
        // student's own message bubble with a mic icon (see m.voice in
        // aiUserBubble) so it is still visually clear it was spoken
        // rather than typed.
        ...
        function archiveCurrentAIChat(){
          // Leaving/switching the live chat shouldn't leave a mic
          // recording still running in the background over whatever's
          // opened next.
          stopAIVoiceInput();
        ...
        // Classroom nav's Alerts / Stitch Bot buttons are shared between
        // the main Classroom tab (no overlay open yet) and being tapped
        // from *inside* another classroom overlay (my class's Stream/
        // Classwork/People, a classwork item, etc). In the second case,
        // remember what was open first so tapping the Classroom tab icon
        // again returns there instead of dropping all the way out to the
        // Classroom tab's default Resources/Practice view -- see the
        // classroomNoBackKinds check in switchTab (core.js).
        ...
        // Tapping the header's dashes icon used to open a one-item dropdown
        // ("Chat history") that had to be tapped again to actually get to
        // the history drawer. That extra step was pointless since the menu
        // never had a second item, so the icon now opens the drawer
        // directly (see aiChatHistoryDrawerHTML).
        ...
        // Search bar inside the Chat History drawer: tapping the search
        // icon swaps the "Chat History" title for a text input that
        // filters the Recents list below live, by each session's summary
        // (see aiSessionPreview). Mirrors the same tap-to-expand pattern
        // used on the Select Contact screen (see activateNewMessageSearch).
        ...
        // Chat History as a slide-in drawer -- overlays the live chat from
        // the left edge (backdrop + panel) instead of replacing the whole
        // Stitch Bot screen, matching the familiar "tap the menu, a
        // history rail slides in over the chat" pattern. Tapping a past
        // session, "New chat", the backdrop, or the close arrow all just
        // flip aiChatHistoryOpen back off and re-render.
        function aiChatHistoryDrawerHTML(){
          ...
          return `
            ...
            <div class="ai-history-drawer">
              <div class="flex items-center justify-between px-4 flex-shrink-0" style="padding-top:20px;padding-bottom:14px;">
        ...
        function aiClassHTML(){
          return `
            <div class="flex-1 relative overflow-hidden">
              <div class="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-5 pointer-events-none" style="padding-top:20px;">
        ...
        //
        // Whether the composer textarea currently has focus. The
        // visualViewport 'resize' event is what normally drives this sync,
        // but on some browsers it fires late (or the keyboard covers the
        // sub-nav without actually shrinking the reported viewport height
        // right away) -- so the Classroom sub-nav could still flash on top
        // of the keyboard for a moment. Tracking focus directly (see
        // handleAIChatInputFocus/Blur below) lets the nav bar hide the
        // instant the keyboard is requested, instead of waiting on the
        // viewport to catch up.
        ...
        function syncAIClassKeyboardInset(){
          ...
          // Without the composer actually focused, there is no way a real
          // on-screen keyboard can be up (typing requires focus first) --
          // so whether the Classroom sub-nav is shown or hidden is decided
          // purely by aiChatInputFocused, a plain boolean that only ever
          // changes on an actual focus/blur. Nothing here reads
          // visualViewport height while unfocused.
          //
          // This used to instead be inferred from visualViewport height
          // (a raw "keyboard inset" px value, later with a noise-floor
          // threshold on top of that) -- but on mobile Chrome, plain
          // scrolling of the page collapses/expands the address bar on its
          // own, which shifts that measurement with no keyboard or focus
          // involved at all. That kept crossing whatever threshold was in
          // place, so the sub-nav flickered in and out and the overlay's
          // bottom gap kept snapping between two sizes purely from
          // scrolling -- which is what actually showed up as the chat
          // screen shaking. Gating on focus instead removes that false
          // signal entirely: nothing here can fire while you're just
          // scrolling with the composer unfocused.
          ...
          // Composer is focused, so a keyboard opening (or already open)
          // is genuinely possible -- track its real height here to
          // position the composer just above it, and keep the sub-nav
          // hidden the whole time (it has nowhere to sit above a real
          // keyboard anyway).
        ...
        // iOS/Android fire visualViewport 'resize' and 'scroll' many times
        // in quick succession while the on-screen keyboard is animating
        // open or closed (each keyboard-open to ask Stitch Bot something,
        // and each send once the keyboard closes again, could trigger a
        // burst of a dozen+ events). syncAIClassKeyboardInset writes to
        // ov.style.bottom/composerWrap.style.paddingBottom synchronously,
        // so answering every single event in the burst forces that many
        // separate layout passes back to back -- which is what showed up
        // as the chat screen visibly shaking/jittering right when asking
        // Stitch Bot something. Coalescing the burst into at most one
        // style write per animation frame (rAF) keeps the inset in sync
        // just as accurately, but as one smooth adjustment instead of many.
        ...
        // Tapping into the composer textarea: hide the Classroom sub-nav
        // and let the overlay claim its space immediately, rather than
        // waiting for the visualViewport 'resize' event the keyboard
        // animation fires -- which on some browsers lags just long enough
        // for the nav bar to be visibly sitting on top of the keyboard for
        // a moment.
        ...
        // Leaving the composer: only bring the sub-nav back once the
        // keyboard has actually finished closing (the viewport briefly
        // still reports itself as covered right as blur fires), so give
        // it a beat to settle before re-checking.
        function handleAIChatInputBlur(){
          aiChatInputFocused = false;
          setTimeout(syncAIClassKeyboardInset, 60);
          // Bring the mascot back if the student backed out without
          // actually typing anything -- if there's real text pending
          // they're still mid-conversation, so leave it hidden.
        ...
        // ---- Stitch Bot's idle mascot ----
        // The robot on the empty-chat screen (see aiChatEmptyStateHTML) is
        // a plain static image -- no animation, no motion. One of four
        // poses is picked at sign-in (see rollAIRobotPoseForNewLogin,
        // called from authEnterApp) and then held for the rest of that
        // login -- reopening the chat, backing out of the composer, etc.
        // all reuse that same pose instead of re-rolling, so the mascot
        // only actually changes the next time the student logs in.
        ...
        // Persisted the same way the Supabase session itself is (see
        // getSupabaseClient in core.js): localStorage in the native app
        // (survives restarts, same login until sign-out), sessionStorage
        // on the web (cleared when the browser fully closes, same as the
        // session it's paired with). Falling back to a plain variable
        // keeps this working even if storage access throws (private
        // browsing, etc.) -- it just won't survive a page refresh then.
        ...
        // Called once per sign-in from authEnterApp() (games.js) -- picks
        // a fresh pose and persists it, so it's the same pose all
        // through this login and only changes on the next one.
        ...
        // Warms the browser's image cache for artwork that used to only
        // start loading the moment a student actually opened the screen
        // that shows it -- Stitch Bot's mascot poses, the Practice/
        // Challenge Arena/Flashcards/Upgrade card art (study.js), and the
        // subscription plan badges (study.js's planCardBadges). That made
        // those images visibly pop in/flash on first paint of Study Mode
        // or the plans screen. Called once from authEnterApp (games.js)
        // right after sign-in, so by the time the student actually
        // navigates to those screens the images are already sitting in
        // cache and just paint instantly. Fire-and-forget: creating an
        // Image() and setting .src is enough to trigger the fetch and
        // populate the cache -- nothing needs to await it or insert the
        // element anywhere.
        function preloadAppImageAssets(){
          const srcs = [
            ...AI_ROBOT_POSES.map(p => p.src),
            // Mirrors the four feedFeatureCard image paths in study.js's
            // renderExamsStudyMode (Practice Tests, Challenge Arena,
            // Flashcards, Upgrade).
            ...
            // study.js's own planCardBadges -- reused here instead of
            // duplicated, since courses.js loads after study.js.
        ...
        // Which of the user's own messages (by index into aiChatMessages)
        // is currently being edited, if any. Only one message can be in
        // edit mode at a time.
        ...
        // Mascot shown in place of the chat log for a brand-new,
        // still-empty conversation -- centered in the available space the
        // same way Claude/Gemini/ChatGPT show a prompt on a fresh chat.
        // Once the first message is sent, aiChatLogHTML stops calling this
        // (aiChatMessages is no longer empty) and it's gone for good for
        // that session. A random pose is chosen fresh every time this
        // markup is built, then held static.
        ...
        // Re-paints the chat log (both the main overlay's log and, if
        // present, the split-view's copy) after anything that mutates
        // aiChatMessages outside of sendAIMessage -- edits, cancels, etc.
        ...
        // real HTML: **bold** becomes <strong>, consecutive "- "/"* " or
        // "1. " lines become an actual <ul>/<ol>, $$...$$ / $...$ math
        // becomes real typeset equations and fractions via KaTeX (instead
        // of the raw "$$\frac{585}{-111}$$" text just sitting there
        // unrendered), and a fenced ```mermaid code block becomes an
        // actual rendered diagram via Mermaid (see
        // renderPendingMermaidDiagrams, called after this HTML lands in
        // the DOM -- Mermaid needs a live element to draw into, it can't
        // render to a plain string up front the way KaTeX can). Any
        // stray, unmatched "**" left over (the model starting a bold span
        // it never closed, etc.) is stripped rather than shown literally.
        // Doesn't HTML-escape the rest of the input -- getAIResponse's
        // attachment "ackLine" prefix is trusted markup (an icon + text)
        // baked into the same string, so escaping here would mangle that;
        // this matches how the bubble already treated its text before
        // this formatting was added. The math/diagram source text itself
        // is escaped before being handed to KaTeX/Mermaid below.
        let aiMermaidIdCounter = 0;
        function formatAIText(raw){
          if (!raw) return '';
          let text = String(raw);

          // Fenced ```mermaid blocks first -- they can span many lines
          // and would otherwise get chopped up by the line-by-line list
          // logic further down. Each becomes a placeholder token holding
          // a fresh, empty container div that renderPendingMermaidDiagrams
          // fills in once it's actually attached to the page.
          const mermaidBlocks = [];
          ...
          // Fenced ```graph blocks -- math/science plots (function curves,
          // lines, circles, polygons, points) for word problems and worked
          // examples. Unlike Mermaid, this renders straight to an inline
          // <svg> string right here (see renderGraphBlock) instead of
          // needing a live DOM element to draw into later: the model does
          // the actual math (sampling curve points, computing shape
          // coordinates) the same way it already writes Mermaid syntax or
          // LaTeX, and this just plots the numbers it hands back. A
          // malformed block quietly falls back to a small "couldn't be
          // drawn" note rather than breaking the rest of the reply.
          const graphBlocks = [];
          ...
          // Display math ($$...$$) then inline math ($...$), rendered up
          // front via KaTeX into real equations/fractions. Falls back to
          // showing the raw expression (still better than throwing) if
          // KaTeX failed to load or the expression is malformed.
          const mathBlocks = [];
          ...
          // A reply can arrive truncated (the ai-proxy Edge Function's
          // response length cap cutting it off mid-answer, most often on
          // a long worked-math reply) -- when that cut happens partway
          // through an equation, its opening $$ or $ never finds a
          // matching close, so the two regexes above skip over it and it
          // would otherwise sit there as raw, unrendered "$$...\frac{..."
          // source text for the rest of the message. Since there is no
          // way to finish rendering an equation that never finished being
          // written, just drop the stray marker(s) rather than showing
          // literal dollar signs -- the underlying LaTeX source is still
          // visible as plain text either way, this only removes the $ /
          // $$ that would otherwise look like a rendering bug.
          text = text.replace(/\$\$/g, '');
          ...
          // Swap the placeholder tokens back in for the real math/diagram
          // markup now that list/paragraph structure has been built
          // around them (done last so nothing above mangles KaTeX's own
          // HTML output or the diagram container).
        ...
        // ---------------- Stitch Bot: graph/shape plotting ----------------
        // Turns a ```graph fenced block's JSON spec into a self-contained
        // inline SVG string. Schema (all fields optional except
        // "elements"):
        //   {
        //     "title": "y = x^2",
        //     "xRange": [-5, 5], "yRange": [-2, 25],
        //     "elements": [
        //       {"type":"curve","points":[[x,y],...],"label":"y = x^2"},
        //       {"type":"line","from":[x,y],"to":[x,y],"dashed":true},
        //       {"type":"circle","center":[x,y],"radius":r,"fill":true},
        //       {"type":"polygon","points":[[x,y],...],"label":"Triangle ABC"},
        //       {"type":"point","at":[x,y],"label":"P(2,4)"}
        //     ]
        //   }
        // xRange/yRange are auto-computed from the elements (with padding)
        // when omitted. x and y always share the same pixels-per-unit
        // scale (never stretched independently), so a circle drawn this
        // way actually looks round and a right angle actually looks
        // square -- important for geometry, not just function plots.
        ...
        function renderGraphBlock(rawSpec){
          try {
            ...
            // Three flavors share this one fenced ```graph block: the
            // original numeric coordinate-plane plot (default, used when
            // "type" is omitted, for function curves/lines/circles/
            // polygons/points), plus two non-coordinate layouts added for
            // things that were never x-y curves to begin with -- a
            // game-theory/decision "matrix" (payoff matrices, truth
            // tables, comparison grids) and a categorical "bar" chart
            // (comparing values across labeled groups). Dispatch on
            // spec.type before falling into the coordinate-plane path
            // below so those two never get forced through axis/range
            // math that doesn't apply to them.
            ...
            // Bounding box across every coordinate actually used, as a
            // fallback for whichever of xRange/yRange the model left out.
        ...
        // Renders a payoff/decision matrix (game theory 2x2s, truth
        // tables, any row-by-column comparison grid) as an actual styled
        // table instead of the coordinate-plane SVG above -- there is no
        // x-y curve to plot for something like "Coke advertises / Coke
        // doesn't", it is fundamentally a grid of outcomes. Spec shape:
        // {"type":"matrix","title":"optional","corner":"optional label for
        // the blank top-left cell (e.g. two player names)","rowPlayer":
        // "optional label above the row headers","colPlayer":"optional
        // label above the column headers","rows":["Row A","Row B",...],
        // "cols":["Col A","Col B",...],"cells":[["r0c0","r0c1",...],
        // ["r1c0","r1c1",...],...],"highlight":[[rowIndex,colIndex],...]}.
        // cells[r][c] is shown as-is (the model writes it exactly how it
        // should read, e.g. "(50, 50)" or "Both spend big, market stays
        // split"), and any [r,c] pair listed in "highlight" (e.g. a Nash
        // equilibrium) gets a distinct background so it visually stands
        // out rather than the model having to describe "the highlighted
        // cell" in prose. Falls back to the same "couldn't be drawn" note
        // as the coordinate plot on malformed input.
        ...
        // Renders a categorical bar chart (comparing values across
        // labeled groups, e.g. survey results, one-off data comparisons)
        // -- again something the coordinate-plane plot above was never
        // meant for, since its x-axis is numeric, not a list of labels.
        // Spec shape: {"type":"bar","title":"optional","yLabel":
        // "optional axis label","categories":["Q1","Q2",...],"series":
        // [{"name":"optional legend label","values":[10,20,...],
        // "color":"optional hex"}]}. Multiple series render as grouped
        // bars per category (each gets its own legend swatch); a single
        // series with no "name" renders as plain unlabeled bars with no
        // legend.
        ...
        // Mermaid can only draw into a live DOM element, not render to a
        // plain string up front the way KaTeX does -- so formatAIText just
        // leaves an empty <div class="ai-mermaid-diagram"> holding the
        // raw diagram source as text, and this is what actually fills it
        // in with the rendered diagram, once that div is attached to the
        // page (see refreshAIChatLog). data-mermaid-rendered marks a div
        // as done so re-running this after every chat log refresh doesn't
        // re-render (and flicker) diagrams from earlier messages. Quietly
        // does nothing if Mermaid failed to load (offline, CDN blocked,
        // etc.) or a diagram's syntax is invalid, rather than breaking the
        // rest of the chat log.
        ...
        // The "Thinking" state used to just render the literal '···'
        // string sitting flat in the bubble. This swaps it for three
        // dots that bounce in sequence (see .ai-typing-dots in
        // styles.css), so it actually reads as a live, in-progress state
        // instead of a static glyph. No "Thinking..." label -- just the dots.
        ...
        function aiBotBubble(text, id, promptLimit, image, mediaType){
          ...
          // A generated image renders as its own rounded tile above any
          // accompanying text (there's usually none besides a possible
          // ackLine for a doc scan happening alongside it), same visual
          // slot a sent/received photo gets elsewhere in the app. Given
          // as a data URL straight from the base64 the ai-proxy Edge
          // Function returns, so no separate upload/fetch step is needed
          // before it can show up on screen.
        ...
        // Lets the student save a generated image straight from the chat
        // bubble, same as they'd expect from any image they receive.
        ...
        // Saves an edited user message, then -- since everything after it
        // in the conversation was a response to the old wording -- drops
        // whatever followed and asks Stitch Bot for a fresh reply, the
        // same way a brand-new message would be sent.
        ...
        function aiUserBubble(text, attachments, id, isEditing, voice){
          ...
          // Per-request, the user's own text now renders inside its own
          // soft, translucent blue bubble (instead of plain right-aligned
          // text with no background) -- text sits left-aligned within the
          // bubble, same as Stitch Bot's own gray bubble, while the bubble
          // itself floats right via the flex column's items-end.
          // A message sent by tapping the mic (see toggleAIVoiceInput)
          // gets a small mic glyph ahead of its text -- the text itself
          // is still the real transcript (that's what actually gets sent
          // to Stitch Bot), this just marks that it arrived by voice
          // rather than typing, the same way DM voice notes are visually
          // distinct from typed messages.
        ...
        async function sendAIMessage(preset, viaVoice){
          ...
          // Free-tier daily prompt cap (see DAILY_PROMPT_LIMIT/
          // canSendAIPrompt in study.js) -- unlimited during the 7-day
          // trial or on any active paid plan. Checked before anything is
          // pushed to aiChatMessages so a blocked message doesn't clutter
          // the transcript with a student message and no real reply.
          ...
          // autoGrowAIChatInput only ever grows the textarea as you type
          // (it never shrinks it back down mid-typing, by design -- see
          // its comment), so a message typed across several lines left
          // the box sitting tall even after being sent, floating above
          // where the single-line composer normally lives instead of
          // collapsing back down flush with the bottom of the screen.
          // Resetting the height here and re-syncing the keyboard inset
          // (rather than waiting on the next focus/blur/viewport event)
          // makes it retract back down to the bottom immediately, in the
          // same frame the message is sent.
          if (inputEl) { inputEl.value = ''; }
          resetAIChatInputHeight();
          if (typeof syncAIClassKeyboardInset === 'function') syncAIClassKeyboardInset();
          refreshAIChatLog(true);
        ...
        // ---- Stitch Bot voice input (speech-to-text) ----
        // Tapping the mic starts the browser's built-in SpeechRecognition,
        // live-fills the composer with what it hears (interim results
        // included, so the text visibly builds up while talking, not just
        // once it's done), and sends automatically the moment the browser
        // decides the student has stopped talking (onend). Tapping the mic
        // again mid-recording stops it early instead of waiting.
        ...
        // Stops an in-progress recording early (mic tapped again, or the
        // student navigates away from Stitch Bot -- see archiveCurrentAIChat).
        // recognition.onend still fires after stop(), so whatever was heard
        // up to that point still gets sent.
        function stopAIVoiceInput(){
          if (aiVoiceRecognition) { try { aiVoiceRecognition.stop(); } catch (err) {} }
        }

        // Idle: plain mic. Recording: solid red, pulsing, stop-square icon
        // (mirrors the DM voice-note mic in chat.js/setConvoMicRecordingUI).
        ...
        // Briefly swaps the composer placeholder to show a mic error
        // (permission denied, unsupported browser, etc.), same pattern as
        // flashConvoMicMessage for DM voice notes.
        ...
        // Gives Stitch Bot actual memory across the conversation instead of
        // answering each message in total isolation: pulls in the last
        // several turns of the *current* session (aiChatMessages, minus the
        // user message and "···" placeholder sendAIMessage just pushed for
        // the turn being answered right now) plus short topic summaries
        // from the student's *past* sessions (aiChatSessions, already
        // generated by generateAIChatSummary for Chat History). Without
        // this, callClaude only ever saw the single latest message, so the
        // bot had no way to follow a "what about the second one?" style
        // follow-up or recall what an earlier chat was about.
        ...
        // Simple phrase-based trigger: catches the common ways a student
        // actually asks for a picture ("draw...", "generate an image of
        // ...", "make me a picture of ...") without needing a whole
        // intent-classification round trip through Claude first. Checked
        // only when there's no image attachment on this message (an
        // attached image means they want Stitch Bot to look at a photo,
        // not generate a new one) and the message has no other purpose
        // that would be lost by swapping to image generation, like a doc
        // upload being scanned in parallel.
        ...
        async function getAIResponse(text, attachments, viaVoice){
          ...
          // Route to image generation instead of a text reply when the
          // student is clearly asking for a picture and hasn't also sent
          // their own image for Stitch Bot to look at (that case stays a
          // normal vision chat, handled below).
        ...
        // Folds in any classes this account was invited to by email (see
        // the class_invites/claim_class_invites SQL above) -- run before
        // the select() in loadMyClasses so a class someone was invited to
        // shows up the very first time they sign in, not just after a
        // second refresh. Best-effort: if the RPC doesn't exist yet (older
        // backend not migrated) or the call fails, just skip it silently
        // and fall back to whatever the person can already see.
        ...
        // ---- Live lecture link sharing (guest join, no class membership
        // needed) ----
        // Lets anyone with the link -- including people who aren't in this
        // class at all -- drop straight into this one lecture's video
        // call, without being added to the roster or getting access to
        // its Classwork/Stream/People tabs. Built the same way
        // buildChallengeInviteLink does (a query param on the app's own
        // URL, see quizzesAndGames.js), read back out by
        // checkPendingLectureLinkJoin() below.
        ...
        // True while the current lecture call is being attended by someone
        // who followed a shared lecture link (see shareLectureLink above /
        // checkPendingLectureLinkJoin below) rather than being a signed-in
        // member of this class. They're still a real signed-in Stitch
        // account -- the auth gate handles that -- just not on this
        // class's roster, so this only changes where leaving the call
        // routes back to (there's no Stream/Classwork/People tab for them
        // to land on) and skips ringing the rest of the class.
        let guestLectureMode = false;
        // { classId, lectureId } while the "you're not in this class"
        // confirm screen is up, between checkPendingLectureLinkJoin()
        // finding the link and submitGuestLectureJoin() actually
        // connecting.
        ...
        // Same connection path as joinLiveLecture, but for someone who
        // isn't a member of this class -- reached only via a shared
        // lecture link (see checkPendingLectureLinkJoin /
        // submitGuestLectureJoin below). Skips the class-membership
        // lookup entirely since a guest, by definition, won't be found in
        // myClasses; lectureCallHTML/lectureIsTeacher already fall back
        // gracefully when that lookup comes back empty.
        ...
        // Shared by joinLiveLecture and joinLectureAsGuest: resets any
        // state left over from a previous lecture, opens the call screen,
        // connects the Realtime signaling channel, and requests
        // camera/mic. Nothing in here touches myClasses, so it works
        // identically whether or not the caller is actually on this
        // class's roster.
        ...
        // Reads a `?joinLecture=classId:lectureId` param off the URL (see
        // buildLectureInviteLink above) and routes accordingly. Already a
        // member of that class? Just join the normal way, with full
        // Resources/Whiteboard rights, same as tapping "Join" on the
        // Stream tab. Otherwise, show the guest confirm screen first --
        // mirrors checkPendingChallengeLinkJoin in quizzesAndGames.js.
        // Called once, right after the app shell is up (see
        // authEnterApp() in games.js), and clears the param from the URL
        // either way so a refresh or reshare of the current tab doesn't
        // re-trigger it.
        ...
        function guestLectureJoinHTML(){
          ...
          return `
            <div class="flex-1 flex flex-col items-center justify-center px-8 text-center" style="padding-top:20px;">
        ...
        function endLecture(remoteEnded){
          ...
          lectureAttachments = [];
          pendingAutoOpenAttachmentId = null;
          // A guest was never viewing this class's Stream/Classwork/People
          // tabs to begin with (they aren't on its roster), so there's no
          // classDetail to fall back into -- just close straight back to
          // whatever tab they were on before they opened the lecture link.
        ...
        // Records a real class_invites row (see the SQL above) so that
        // whenever the invited person actually signs in under this email,
        // claim_class_invites() adds them to `members` for real -- not
        // just a cosmetic roster entry. Best-effort/fire-and-forget, same
        // convention as queueSaveClassRemote: the local roster row is what
        // the teacher sees immediately regardless of whether this
        // round-trip has landed yet.
        async function recordClassInviteRemote(classId, email){
          // A Course's invite flow never grants direct membership -- see
          // the "pending" branch below -- so there's nothing to record
          // here for those; only plain classes use class_invites.
          const sb = getSupabaseClient();
          if (!sb) return;
          try {
        ...
        async function submitInviteStudents(){
          ...
          emails.forEach(email => {
            ...
            // For a real class (not a Course), also record a real invite
            // so the person actually gets `members` access once they sign
            // in under this address -- see recordClassInviteRemote above.
            // The class code / invite link still works too; this just
            // means the email path grants real access as well now,
            // instead of only ever being a cosmetic roster row.
        ...
        function cancelStudentInvite(classId, index){
          ...
          queueSaveClassRemote(cls);
          // Also clear the matching pending class_invites row (if any) so
          // cancelling an invite in the UI actually stops that email from
          // being granted access later via claim_class_invites() -- best
          // effort, same as the roster removal above.
          if (removed && removed.email && !cls.isCourse) {
            const sb = getSupabaseClient();
            if (sb) {
              // Escape ilike's "%"/"_" wildcards -- email addresses
              // routinely contain literal underscores/dots (see the same
              // escaping in isUsernameTaken, profile.js) so this needs to
              // match the address exactly, not as a LIKE pattern.
