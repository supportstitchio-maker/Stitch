            ...
            <div class="flex-shrink-0">
              <div class="w-full px-5 pb-3 relative" style="padding-top:20px;">
                <div class="flex items-center justify-between">
                  <div class="flex items-center flex-shrink-0" style="gap:5px;">
                    <button onclick="openAIChatHistory()" title="Chat history" class="w-8 h-8 flex items-center justify-center flex-shrink-0">${gradIcon(IconBold('dashes','w-5 h-5'))}</button>
                    <button onclick="startNewAIChat()" title="New chat" class="w-8 h-8 flex items-center justify-center">${gradIcon(IconBold('plus','w-5 h-5'))}</button>
                  </div>
                  <div class="flex items-center gap-1.5 flex-shrink-0">
                    <h1 class="text-3xl font-bold font-display grad-text" style="max-width:60vw;">Stitch Bot</h1>
                  </div>
                </div>
              </div>
            </div>
        ...
        // Greeting shown in place of the chat log for a brand-new,
        ...
        function aiChatEmptyStateHTML(){
          const first = studentFirstName();
          return `
            <div class="h-full min-h-full flex flex-col items-center justify-center text-center gap-3 px-8">
              ${gradIcon(IconBold('bot','w-9 h-9'))}
              <div class="text-base font-normal grad-text leading-snug" style="font-family:'Montserrat',sans-serif;">${first ? `What's up, ${escapeHtml(first)}....` : "What's up...."}</div>
            </div>`;
        }
