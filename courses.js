        ...
        // Dropdown for the AI Class kebab menu: opens the sliding Chat
        // History drawer (see aiChatHistoryDrawerHTML). Clearing the
        // current chat now lives as a small trash icon inside that
        // drawer's own header instead of sitting here, since this menu
        // only ever needs the one entry.
        function aiClassMenuDropdownHTML(){
          return `
            <div class="absolute bg-white rounded-2xl border border-gray-100 py-2 z-20 menu-dropdown-inset" style="right:1.25rem;top:3.5rem;width:12rem;box-shadow:0 10px 30px rgba(0,0,0,.14);">
              <button onclick="aiClassMenuOpen=false; openAIChatHistory();" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 menu-item-pill">${Icon('comment','w-4 h-4')} Chat history</button>
            </div>`;
        }
        ...
        function aiChatHistoryDrawerHTML(){
          ...
          ).join('') : `<div class="text-center text-gray-400 text-sm py-10 px-3">No past conversations yet.</div>`;
          return `
            ...
            <div class="ai-history-drawer">
              <div class="flex items-center justify-between px-4 flex-shrink-0" style="padding-top:20px;padding-bottom:14px;">
                ...
                <div class="flex items-center gap-1">
                  <button onclick="confirmClearAIChat()" title="Clear current chat" class="w-8 h-8 flex items-center justify-center flex-shrink-0 text-red-400">${Icon('trash','w-4 h-4')}</button>
          ...
          return `
              ...
              <div class="w-full px-5 pb-3 relative" style="padding-top:20px;">
                <div class="flex items-center justify-between">
                  <div class="flex items-center flex-shrink-0" style="gap:5px;">
                    <button onclick="toggleAIClassMenu()" class="w-8 h-8 flex items-center justify-center flex-shrink-0">${gradIcon(IconBold('dashes','w-5 h-5'))}</button>
                    ...
                    <button onclick="openAIChatHistory()" title="Chat history" class="w-8 h-8 flex items-center justify-center">${gradIcon(IconBold('comment','w-5 h-5'))}</button>
                  </div>
            ...
            <div id="ai-composer-wrap" class="flex-shrink-0 px-3 pt-2" style="padding-bottom:20px;">
              ...
              <div class="flex items-center gap-2 rounded-3xl px-2 py-1.5" style="background:#ffffff;border:1.5px solid rgba(10,37,64,0.10);box-shadow:0 8px 24px rgba(10,37,64,0.10);">
                ...
                <textarea id="ai-chat-input" rows="1" placeholder="Ask Stitch Bot anything..." onkeydown="handleAIChatInputKeydown(event)" oninput="autoGrowAIChatInput(this)" class="flex-1 min-w-0 bg-transparent text-sm resize-none leading-snug self-center" style="max-height:120px; overflow-y:auto;"></textarea>
        ...
        function syncAIClassKeyboardInset(){
          ...
          // keyboard actually opens it covers the sub-nav anyway, so it's
          ...
          ov.style.bottom = (keyboardInset > 0 ? keyboardInset : navH) + 'px';
          ...
          if (composerWrap) composerWrap.style.paddingBottom = (keyboardInset > 0 ? 10 : 20) + 'px';
          ...
          // nowhere to sit except awkwardly stacked on top of the
          // keyboard. Hide it outright while typing; restore it the
          // moment the keyboard closes again.
          const classroomNavEl = document.getElementById('classroom-nav');
          if (classroomNavEl) classroomNavEl.style.display = keyboardInset > 0 ? 'none' : '';
        }
