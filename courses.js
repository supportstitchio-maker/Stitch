        ...
        function openAIClass(){
          if (aiChatMessages.length === 0) {
            aiChatMessages.push({ role:'bot', text: "Welcome to Stitch Bot! Ask me to explain a topic, quiz you, summarise your slides, or turn a resource into notes. What would you like to do?" });
          }
        ...
        function aiUserBubble(text, attachments, id){
          ...
          const images = imageAttachments.length ? `
            <div class="flex flex-wrap gap-1.5 ${text || fileAttachments.length ? 'mb-1.5' : ''}">
              ${imageAttachments.map(f => `
                <img src="${f.previewUrl}" alt="${escapeHtml(f.name)}" class="rounded-xl object-cover" style="width:8.5rem;height:8.5rem;" />`).join('')}
            </div>` : '';
          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start justify-end gap-3">
              <div class="text-white rounded-2xl px-4 py-3 text-sm" style="max-width:80%;background:${NAVY};white-space:pre-wrap;">${images}${files}${escapeHtml(text)}</div>
            </div>`;
        }
