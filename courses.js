          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start">
              <div class="min-w-0" style="max-width:85%;">
                ${imageHTML}
                ${(!isThinking && !text) ? '' : `<div class="ai-msg-bubble bg-gray-100 rounded-[24px] px-4 py-3.5 text-[15px] text-gray-700 leading-relaxed">${isThinking ? aiThinkingIndicatorHTML() : formatAIText(text)}</div>`}
                ${isThinking ? '' : `
        ...
        function aiUserBubble(text, attachments, id, isEditing, voice){
          ...
          const textBubble = text ? `
            <div class="rounded-[24px] px-4 py-3 text-[15px] text-gray-800 leading-relaxed" style="background:rgba(30,144,255,0.14); white-space:pre-wrap;">${voiceBadge}${escapeHtml(text)}</div>` : '';
          if (isEditing) {
            return `
