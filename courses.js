        ...
        // assistant would, and any bot reply -- typed or spoken -- can
        // be replayed on demand via the speaker icon under its bubble
        // (see aiBotBubble/toggleAISpeakMessage).
        let aiVoiceRecognition = null;
        let aiVoiceRecording = false;
        // Index into aiChatMessages of whichever bot reply is currently
        // being read aloud, if any -- drives the speaker/stop icon state
        // in aiBotBubble.
        ...
        function aiBotBubble(text, id){
          ...
          const isSpeaking = aiSpeakingMessageId === id;
          return `
            <div id="ai-msg-${id}" class="flex items-start">
              <div class="min-w-0" style="max-width:85%;">
                ...
                <div class="mt-1.5 flex items-center gap-3">
                  ...
                  <button onclick="toggleAISpeakMessage(${id})" title="${isSpeaking ? 'Stop reading' : 'Read aloud'}" class="flex items-center justify-center" style="color:${isSpeaking ? NAVY : '#9ca3af'};">${Icon(isSpeaking ? 'square' : 'volume','w-3.5 h-3.5')}</button>
                </div>`}
              </div>
            </div>`;
        }
        ...
        // The speaker icon under every bot bubble (see aiBotBubble): lets
        // the student replay any reply on demand -- not just ones they
        // asked by voice -- which is also how "read my slides" or
        // "summarise this" answers get read back aloud.
        function toggleAISpeakMessage(id){
          if (aiSpeakingMessageId === id) { stopAISpeaking(); return; }
          const msg = aiChatMessages[id];
          if (msg && msg.text && msg.text !== '···') speakAIText(msg.text, id);
        }
