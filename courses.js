          ...
          return `
            <div class="flex-1 relative overflow-hidden">
              <div class="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-5 pointer-events-none" style="padding-top:20px;">
                <button onclick="openAIChatHistory()" title="Chat history" class="ai-header-pill pointer-events-auto">${gradIcon(IconBold('dashes','w-5 h-5'))}</button>
        ...
        function showAIRobot(){
          ...
          if (container) container.dataset.pose = pose.id;
          if (container) container.classList.remove('ai-robot-hidden');
          startAIRobotFaceText();
        }

        // ---- Pixelated greeting on the robot's face ----
        // Small blocky readout that lives over the mascot's black
        // lens/screen and cycles through a short list of lines -- a
        // personalized greeting (using the student's own first name,
        // same one Stitch Bot's replies use -- see studentFirstName)
        // followed by a few stock lines of encouragement. Runs only
        // while the mascot itself is on screen (started by showAIRobot,
        // stopped by hideAIRobot/stopAIRobotFaceText) and re-measures
        // which pose is active each time so the text stays correctly
        // placed over the lens no matter which of the four poses is
        // showing (see the per-pose position rules in styles.css).
        let _aiRobotFaceTextTimer = null;
        function aiRobotFaceTextLines(){
          const name = (typeof studentFirstName === 'function') ? studentFirstName() : '';
          const lines = [];
          if (name) lines.push(`HI ${name.toUpperCase()}!`);
          lines.push("LET'S GO!!", 'YOU GOT THIS', 'READY?');
          return lines;
        }

        function startAIRobotFaceText(){
          stopAIRobotFaceText();
          const el = document.getElementById('ai-robot-face-text');
          if (!el) return;
          const lines = aiRobotFaceTextLines();
          let i = 0;
          const showLine = () => {
            el.textContent = lines[i % lines.length];
            el.classList.add('ai-robot-face-text-visible');
            i++;
          };
          showLine();
          _aiRobotFaceTextTimer = setInterval(showLine, 2600);
        }

        function stopAIRobotFaceText(){
          if (_aiRobotFaceTextTimer) { clearInterval(_aiRobotFaceTextTimer); _aiRobotFaceTextTimer = null; }
          const el = document.getElementById('ai-robot-face-text');
          if (el) el.classList.remove('ai-robot-face-text-visible');
        }
        ...
        function aiChatLogHTML(){
          ...
          const bubbles = aiChatMessages.map((m, i) => m.role === 'bot' ? aiBotBubble(m.text, i, m.promptLimit) : aiUserBubble(m.text, m.attachments, i, i === aiEditingMessageId, m.voice)).join('');
          return bubbles;
        }
        ...
        function aiChatEmptyStateHTML(){
          const pose = currentAIRobotPose();
          // Kicks off the face-text rotation right after this markup
          // actually lands in the DOM (the element doesn't exist yet at
          // the point this string is built).
          setTimeout(startAIRobotFaceText, 0);
          return `
            <div class="h-full min-h-full flex flex-col items-center justify-center text-center px-8">
              <div id="ai-robot-container" class="ai-robot-container" data-pose="${pose.id}">
                <img id="ai-robot-img" class="ai-robot-img" src="${pose.src}" alt="Stitch Bot">
                <div id="ai-robot-face-text" class="ai-robot-face-text"></div>
              </div>
            </div>`;
        }
        ...
        // styles.css) plus a "Thinking..." label, so it actually reads
        // as a live, in-progress state instead of a static glyph.
        function aiThinkingIndicatorHTML(){
          return `
            <div class="flex items-center gap-2 text-gray-500">
              <span class="ai-typing-dots">${'<span></span>'.repeat(3)}</span>
              <span class="text-[15px]">Thinking...</span>
            </div>`;
        }

        function aiBotBubble(text, id, promptLimit){
          const isThinking = text === '···';
          return `
            <div id="ai-msg-${id}" class="flex items-start">
              <div class="min-w-0" style="max-width:85%;">
                <div class="ai-msg-bubble bg-gray-100 rounded-2xl px-4 py-3.5 text-[15px] text-gray-700 leading-relaxed">${isThinking ? aiThinkingIndicatorHTML() : formatAIText(text)}</div>
                ${isThinking ? '' : `
                <div class="mt-1.5 flex items-center gap-3">
                  <button onclick="copyAIMessage(${id})" title="Copy" class="flex items-center justify-center text-gray-400">${Icon('copy','w-3.5 h-3.5')}</button>
                </div>`}
                ${promptLimit ? `
