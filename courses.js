        ...
        // starts out perfectly still; it only starts its gentle
        // float/sway/glow once the student actually engages -- focusing
        // the composer, typing, or sending a message (including a voice
        // note or a suggestion chip, which skip the composer entirely).
        // A single rAF loop is reused across all of those triggers
        // (aiRobotAnimating guards against starting a second one), and
        // the loop quietly stops itself the first frame it can't find
        // #ai-robot-img any more -- which happens as soon as the first
        // message is sent and aiChatLogHTML swaps the mascot out for
        // real chat bubbles.
        let aiRobotAnimating = false;
        let aiRobotAnimTime = 0;
        ...
        function stepAIRobotAnimation(){
          const robot = document.getElementById('ai-robot-img');
          if (!robot) { aiRobotAnimating = false; return; }

          aiRobotAnimTime += 0.05;

          // Float up and down
          const y = Math.sin(aiRobotAnimTime) * 10;
          // Gentle body sway
          const rotate = Math.sin(aiRobotAnimTime * 0.8) * 5;
          // Slight scale for a lively effect
          const scale = 1 + Math.sin(aiRobotAnimTime * 1.2) * 0.01;
          robot.style.transform = `translateY(${y}px) rotate(${rotate}deg) scale(${scale})`;

          // Blue glow pulse
          const glow = 8 + Math.sin(aiRobotAnimTime * 2) * 8;
        ...
        // that session. The robot sits still here; it only starts
        // floating once the student actually engages -- see
        // startAIRobotAnimation, hooked into the composer's focus/input
        // events and into sendAIMessage.
