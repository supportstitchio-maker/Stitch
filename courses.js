try {
        ...
        async function submitJoinClassroom(){
          ...
          if (error || !result) { openAppAlertModal("We couldn't find that class. Check the code and try again."); return; }

          if (result.status === 'not_found') { openAppAlertModal("We couldn't find that class. Check the code and try again."); return; }
          if (result.status === 'pending') { openAppAlertModal("Request sent -- the teacher needs to approve you before you can enter this class."); return; }
          ...
          return `
            ...
            <div class="p-4 flex-1 overflow-y-auto flex flex-col">
              <div class="rounded-3xl p-5 text-white relative overflow-hidden mb-5" style="${p.photo ? `background-image:linear-gradient(rgba(10,37,64,0.45),rgba(10,37,64,0.45)),url('${p.photo}');background-size:cover;background-position:center;` : 'background:linear-gradient(135deg,#2563eb,#1d4ed8);'}min-height:96px;">
                ...
                ${(p.section || p.subject) ? `<div class="text-sm text-white/85 truncate relative">${escapeHtml(p.section || p.subject)}</div>` : ''}
              </div>
              <div class="bg-amber-50 border border-amber-100 rounded-2xl p-4 mb-5 flex items-center justify-between">
                <div class="min-w-0">
                  <div class="text-xs font-semibold text-amber-700 mb-0.5">Entrance fee</div>
                  <div class="text-2xl font-bold text-[${NAVY}] truncate">${classPaymentFeeLabel(p)}</div>
                </div>
                <div class="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center text-amber-600 flex-shrink-0">${Icon('lock','w-5 h-5')}</div>
              </div>
              <div class="text-sm text-gray-500 mb-6 leading-relaxed">This class charges a one-time entrance fee before you can join. You'll pay securely through Paystack, and you'll get full access to the class as soon as your payment is confirmed.</div>
              <div class="mt-auto flex flex-col gap-2">
                <button onclick="confirmClassPayment()" class="w-full font-semibold py-3 rounded-2xl text-white" style="background:${NAVY};">Continue to Pay ${classPaymentFeeLabel(p)}</button>
        ...
        function startClassPayment(classId, className, amount, currency, joinedByCode){
          ...
          loadPaystackScript().then(() => {
            ...
            const handler = window.PaystackPop.setup({
              ...
              onClose: function(){ classPaymentBusy = false; },
            });
            pushModalBackHandler(() => { teardownPaystackPopup(false); classPaymentBusy = false; });
            handler.openIframe();
          }).catch(() => {
        ...
        async function submitCreateClassroom(){
          ...
          const classData = {
            name, section, subject, students: [], coTeachers: [], announcements: [], classwork: [], lectures: [], photo: null, notificationsEnabled: true, colorIndex, motifIndex,
        ...
        async function loadClassStudentProfiles(cls){
          if (!cls) return;
          const ids = (cls.students || []).map(s => s.id).filter(id => id && !classStudentProfiles[id]);
          ...
          return `
            ...
            <div class="border-t border-gray-100">
              ${cls.students.length ? cls.students.map((s, i) => `
                <div class="flex items-center gap-4 py-4 ${i < cls.students.length - 1 ? 'border-b border-gray-100' : ''}">
                  ...
                  <div class="min-w-0 flex-1">
                    ...
                    ${s.pending ? `<div class="text-xs text-gray-400 mt-0.5">Invite pending &middot; must enroll to get access</div>` : `<div class="text-xs text-gray-400 mt-0.5 truncate">${s.email}</div>`}
                  </div>
