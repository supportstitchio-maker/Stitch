try {
        ...
        async function getAIResponse(text, attachments, viaVoice){
          ...
          try {
            ...
            const msg = (err && err.message) || '';
            if (msg.startsWith('Too many AI requests')) return { text: ackLine + msg };
            // The raw Anthropic/Edge Function error used to be shown directly
            // to the student here (handy for debugging, but not something a
            // student should ever see). It's still logged for us via
            // reportError -- just no longer put in the chat bubble.
        ...
        function courseItemCardHTML(course, item){
          ...
          // Flush list items on the page background (divider between
          // items, no floating white card/shadow) -- matches every other
          // list in the app, including this same item's own detail view.
          if (isBigMedia) {
            return `
        ...
        async function submitJoinClassroom(){
          ...
          runClassActionLoading('Adding you to class', 'users', () => {
            // This already played the class's one-time "getting it ready"
            // animation, so opening it again later this session shouldn't
            // repeat it -- see preparedClassIds in study.js.
        ...
        // Joined students carry their real account id in s.id (pending
        // invites don't yet), so once someone has joined we can batch-fetch
        // their public profile photo the same way loadClassTeacherProfile
        // already does for the teacher, and cache it here keyed by id.
