        ...
        // ---- Stitch Bot's idle mascot animation ----
        // The robot on the empty-chat screen (see aiChatEmptyStateHTML)
        // walks back and forth, hops at each end of its little walk, and
        // wiggles as it goes -- kicked off from openOverlay and
        // startNewAIChat right after they set innerHTML. It steps aside
        // (hideAIRobot) the moment the student taps the composer to
        // start typing (handleAIChatInputFocus), and comes back
        // (showAIRobot) if they back out without having typed anything
        // (handleAIChatInputBlur). A single rAF loop is reused across
        // show/hide cycles (aiRobotAnimating guards against starting a
        // second one); it also quietly stops itself the first frame it
        // can't find #ai-robot-img any more, which happens for good as
        // soon as the first message is sent and aiChatLogHTML swaps the
        // mascot out for real chat bubbles.
        let aiRobotAnimating = false;
        let aiRobotStartTs = null;

        function startAIRobotAnimation(){
          if (aiRobotAnimating) return;
          if (!document.getElementById('ai-robot-img')) return;
          aiRobotAnimating = true;
          aiRobotStartTs = null;
          requestAnimationFrame(stepAIRobotAnimation);
        }

        function stopAIRobotAnimation(){
          aiRobotAnimating = false;
        }
        ...
        function hideAIRobot(){
          ...
          stopAIRobotAnimation();
        }

        function showAIRobot(){
          const container = document.getElementById('ai-robot-container');
          if (container) container.classList.remove('ai-robot-hidden');
          startAIRobotAnimation();
        }

        function stepAIRobotAnimation(ts){
          const robot = document.getElementById('ai-robot-img');
          if (!robot || !aiRobotAnimating) { aiRobotAnimating = false; return; }
          if (aiRobotStartTs === null) aiRobotStartTs = ts;
          const t = (ts - aiRobotStartTs) / 1000;

          // One lap: walk right, hop, walk back left, hop -- then repeat.
          const legDuration = 2.2;
          const hopDuration = 0.45;
          const cycle = (legDuration + hopDuration) * 2;
          const walkRange = 46;
          const ct = t % cycle;

          let x = 0, y = 0, rotate = 0, scaleX = 1, scaleY = 1, facing = 1;

          if (ct < legDuration) {
            // Walking right, little footstep bounces underfoot
            const p = ct / legDuration;
            x = -walkRange + p * walkRange * 2;
            y = -Math.abs(Math.sin(p * Math.PI * 7)) * 7;
            rotate = Math.sin(p * Math.PI * 7) * 4;
            facing = 1;
          } else if (ct < legDuration + hopDuration) {
            // Hop in place at the right end
            const p = (ct - legDuration) / hopDuration;
            x = walkRange;
            y = -Math.sin(p * Math.PI) * 30;
            scaleY = 1 - Math.sin(p * Math.PI) * 0.12;
            scaleX = 1 + Math.sin(p * Math.PI) * 0.10;
            facing = 1;
          } else if (ct < legDuration * 2 + hopDuration) {
            // Walking back left
            const p = (ct - legDuration - hopDuration) / legDuration;
            x = walkRange - p * walkRange * 2;
            y = -Math.abs(Math.sin(p * Math.PI * 7)) * 7;
            rotate = -Math.sin(p * Math.PI * 7) * 4;
            facing = -1;
          } else {
            // Hop in place at the left end
            const p = (ct - legDuration * 2 - hopDuration) / hopDuration;
            x = -walkRange;
            y = -Math.sin(p * Math.PI) * 30;
            scaleY = 1 - Math.sin(p * Math.PI) * 0.12;
            scaleX = 1 + Math.sin(p * Math.PI) * 0.10;
          ...
          // A little wiggle riding on top of everything else, like an arm
          // swinging, so it never looks perfectly rigid mid-stride.
          rotate += Math.sin(t * 5) * 2;

          robot.style.transform = `translate(${x}px, ${y}px) rotate(${rotate}deg) scale(${scaleX * facing}, ${scaleY})`;

          // Blue glow pulse
          const glow = 8 + Math.sin(t * 4) * 8;
          robot.style.filter = `drop-shadow(0 0 ${glow}px rgba(30,144,255,0.8))`;
        ...
        // that session. It starts walking/hopping (see
        // startAIRobotAnimation) as soon as this screen is mounted, and
        // steps aside the moment the student taps the composer to type
        // (see hideAIRobot/showAIRobot, hooked into the composer's
        // focus/blur).
          ...
          return `
            <div class="h-full min-h-full flex flex-col items-center justify-center text-center px-8">
              <div id="ai-robot-container" class="ai-robot-container">
                <img id="ai-robot-img" class="ai-robot-img" src="assets/stitch-robot.png" alt="Stitch Bot">
              </div>
            </div>`;
        }
