        ...
        function lectureReactionsFloatHTML(){
          ...
          return lectureFloatingReactions.map(r => `
            <span class="lecture-reaction-float ${r.big ? 'pl-1.5 pr-3 py-1.5' : 'pl-1 pr-2.5 py-1'} flex items-center gap-1.5 bg-white shadow-md rounded-full">
              <span class="${r.big ? 'w-11 h-11' : 'w-8 h-8'} rounded-full flex items-center justify-center flex-shrink-0" style="${r.icon === 'handRaised' ? `background:${NAVY}1a;color:${NAVY};` : 'background:#f9fafb;color:#374151;'}">${Icon(r.icon, r.big ? 'w-6 h-6' : 'w-4 h-4')}</span>
              <span class="${r.big ? 'text-sm' : 'text-xs'} font-semibold text-gray-700 truncate max-w-[130px]">${escapeHtml(r.name || '')}${r.icon === 'handRaised' ? ' raised a hand' : ''}</span>
            </span>`).join('');
        }
        ...
        function lectureCommentsFloatHTML(){
          ...
          return lectureFloatingComments.map(c => `
            <span class="lecture-reaction-float flex items-start gap-1.5 bg-white shadow-md rounded-2xl px-3 py-2" style="max-width:220px;">
          ...
          return `
            <div id="lecture-call-screen" class="flex-1 flex flex-col text-gray-800" style="padding-top:20px;background:#ffffff;">
              ...
              <div class="flex-1 flex flex-col min-h-0 relative">
                ...
                <div id="lecture-comments-float" class="absolute bottom-3 left-3 flex flex-col items-start gap-1 z-10" style="pointer-events:none;max-width:220px;">${lectureCommentsFloatHTML()}</div>
        ...
        function lectureCommentSheetHTML(){
          return `
            <div class="absolute left-1/2 z-30 flex items-center gap-2 rounded-full py-1.5 pl-4 pr-1.5 shadow-lg bg-white" style="bottom:78px;transform:translateX(-50%);width:min(calc(100vw - 60px),380px);">
