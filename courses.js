        ...
        // ---- Stitch Bot voice assistant: speech-to-text on the way in,
        // text-to-speech on the way out. Recording uses the browser's
        // built-in Web Speech API (SpeechRecognition) rather than
        // recording+uploading a raw audio clip, since callClaude only
        // ever forwards text/images to the ai-proxy Edge Function (see
        // its comment in study.js) -- there is no server-side speech
        // model to send actual audio to. Tapping the mic transcribes
        // what the student says straight into the composer and sends it
        // the moment they stop talking; the reply is then read back
        // aloud automatically (see speakAIText), the same way a voice
        // assistant would. Stitch Bot only ever talks in response to a
        // question that was actually asked by voice (viaVoice) -- a
        // typed message never gets read back, and there is no manual
        // "read aloud" control in the UI.
        let aiVoiceRecognition = null;
        let aiVoiceRecording = false;
        // Index into aiChatMessages of whichever bot reply is currently
        // being read aloud, if any. Only used internally now (there's no
        // speaker icon in the UI) to know whether speech is in progress
        // so a new message being sent can cut it off -- see stopAISpeaking
        // in sendAIMessage.
        let aiSpeakingMessageId = null;
        ...
        function archiveCurrentAIChat(){
          ...
          // recording running or Stitch Bot still talking in the
          // background over whatever's opened next.
          stopAIVoiceInput();
          stopAISpeaking();
        ...
        function aiChatLogHTML(){
          ...
          const bubbles = aiChatMessages.map((m, i) => m.role === 'bot' ? aiBotBubble(m.text, i) : aiUserBubble(m.text, m.attachments, i, i === aiEditingMessageId, m.voice)).join('');
          return bubbles;
        }
        ...
        async function sendAIMessage(preset, viaVoice){
          ...
          // Sending a message -- by typing or by voice -- always cuts off
          // whatever Stitch Bot might still be reading aloud from an
          // earlier reply. Otherwise the old answer keeps talking over
          // the new question being asked/typed.
          stopAISpeaking();
          ...
          if (typeof canSendAIPrompt === 'function' && !canSendAIPrompt()) {
            ...
            aiChatMessages.push({ role:'bot', text: `You've used all ${DAILY_PROMPT_LIMIT} of your free Stitch Bot prompts for today. They reset tomorrow, or subscribe to Stitch Pro for unlimited prompts any time.` });
          ...
          const thinkingMsg = { role:'bot', text: '···' };
          aiChatMessages.push(thinkingMsg);
          const botMsgId = aiChatMessages.length - 1;
          ...
          thinkingMsg.text = reply;
          refreshAIChatLog(true);
          // Asked by voice -> answered by voice: read the reply straight
          // back, the way an actual voice assistant would, instead of
          // making the student tap the speaker icon themselves.
          if (viaVoice) speakAIText(reply, botMsgId);
          if (typeof queueSaveUserState === 'function') queueSaveUserState();
        }
        ...
        // ---- Stitch Bot voice output (text-to-speech) ----
        // Strips markdown/LaTeX/diagram source down to plain speakable
        // prose -- code fences (mermaid/graph blocks) are dropped entirely
        // since there is nothing sensible to read aloud for a diagram, and
        // "$$...$$"/"$...$" math delimiters are unwrapped rather than read
        // literally as dollar signs.
        function plainTextForSpeech(text){
          ...
          t = t.replace(/^[-*]\s+/gm, '').replace(/^\d+[.)]\s+/gm, '');
          return t.replace(/\s+/g, ' ').trim();
        }

        // Breaks a reply into sentence-sized chunks so it can be spoken
        // one sentence at a time with a small gap in between (see
        // speakAIText) instead of one flat, run-on utterance -- that gap
        // is what actually reads as a person talking rather than a page
        // being read aloud.
        function splitTextForSpeech(text){
          return text.match(/[^.!?]+[.!?]*(?:\s+|$)/g) ? text.match(/[^.!?]+[.!?]*(?:\s+|$)/g).map(s => s.trim()).filter(Boolean) : [text];
        }

        // speechSynthesis.getVoices() can come back empty the first time
        // it's called (voices load async in some browsers), so this waits
        // for the 'voiceschanged' event once rather than just grabbing
        // whatever's there on the first call. Result is cached on window
        // so every reply after the first doesn't re-wait.
        function getSpeechVoices(){
          ...
          return new Promise(resolve => {
            let settled = false;
            const finish = () => { if (settled) return; settled = true; resolve(window.speechSynthesis.getVoices()); };
            window.speechSynthesis.onvoiceschanged = finish;
            setTimeout(finish, 300);
          });
        }

        // Picks the least robotic-sounding voice available for the
        // student's language, rather than whatever the browser defaults
        // to (often the flattest, most obviously synthetic one). Modern
        // browsers usually ship at least one "Natural"/"Neural"/"Online"
        // voice alongside the classic compact ones -- this scores each
        // voice and takes the best match, falling back to the browser
        // default (returns null) if nothing stands out.
        function pickNaturalVoice(voices){
          ...
          voices.forEach(v => {
            ...
            else if (v.lang && v.lang.split('-')[0] === base) score += 2;
            else return;
            if (/natural|neural|online|premium|enhanced/i.test(v.name)) score += 4;
        ...
        // Reads a bot reply aloud via the browser's SpeechSynthesis, in
        // its own conversational voice rather than a flat text-to-speech
        // reading: it speaks one sentence at a time with a short natural
        // pause between them (a beat longer after a question, since
        // that's how people actually pace speech), with small random
        // variation in rate/pitch per sentence so it doesn't sound like
        // the exact same robotic loop on every line. msgId (the reply's
        // index in aiChatMessages) drives aiSpeakingMessageId, which
        // sendAIMessage checks to cut a reply off the moment a new
        // message is sent; only one reply speaks at a time, so starting
        // a new one stops whatever was already playing.
        let aiSpeechSession = null;
        async function speakAIText(text, msgId){
          if (!('speechSynthesis' in window)) { if (msgId !== undefined) flashAIMicMessage("Reading aloud isn't supported in this browser"); return; }
          const clean = plainTextForSpeech(text);
          if (!clean) return;
          stopAISpeaking();
          const sentences = splitTextForSpeech(clean);
          if (!sentences.length) return;
          const session = { cancelled: false, timeoutId: null };
          aiSpeechSession = session;
          aiSpeakingMessageId = msgId !== undefined ? msgId : null;
          refreshAIChatLog(false);
          const voice = pickNaturalVoice(await getSpeechVoices());
          let i = 0;
          const speakNext = () => {
            if (session.cancelled) return;
            if (i >= sentences.length) {
              if (aiSpeechSession === session) { aiSpeechSession = null; aiSpeakingMessageId = null; refreshAIChatLog(false); }
              return;
            }
            const sentence = sentences[i++];
            const utter = new SpeechSynthesisUtterance(sentence);
            if (voice) utter.voice = voice;
            // A touch of per-sentence variation, same range a person's
            // pace/pitch naturally drifts in over a few sentences.
            utter.rate = 1 + (Math.random() * 0.08 - 0.04);
            utter.pitch = 1 + (Math.random() * 0.08 - 0.04);
            utter.onend = () => {
              if (session.cancelled) return;
              const pause = /[?!]\s*$/.test(sentence) ? 320 : 150;
              session.timeoutId = setTimeout(speakNext, pause);
            };
            utter.onerror = () => {
              if (aiSpeechSession === session) { aiSpeechSession = null; aiSpeakingMessageId = null; refreshAIChatLog(false); }
            };
        ...
        function stopAISpeaking(){
          if (aiSpeechSession) { aiSpeechSession.cancelled = true; if (aiSpeechSession.timeoutId) clearTimeout(aiSpeechSession.timeoutId); aiSpeechSession = null; }
          if ('speechSynthesis' in window) window.speechSynthesis.cancel();
          if (aiSpeakingMessageId !== null) { aiSpeakingMessageId = null; refreshAIChatLog(false); }
        }
        ...
        async function getAIResponse(text, attachments, viaVoice){
          ...
          try {
            ...
            const system = 'You are Stitch Bot, a warm, encouraging AI tutor and study buddy inside a study app for students across any subject. ' +
              ...
              (viaVoice
                ? 'This question was asked out loud and your reply gets read back with text-to-speech, so write it exactly like your side of a real spoken conversation, not an essay or an article being narrated. Talk the way you actually would out loud: short, natural sentences, contractions, the odd "so", "yeah", "honestly", or "okay so" where a person would actually say it, and let one thought lead into the next instead of a tidy, self-contained paragraph. Never use bullet points, numbered lists, headers, or bold text here, none of that exists when spoken, it just comes out as a flat, run-on list; if you have a few points to make, say them the way you would actually say them one after another in a sentence or two. Lean into your sense of humor a bit more here too, a quick witty remark or a fun line, since a spoken answer that is all business feels flat out loud. Keep it short and punchy, this is a back-and-forth, not a lecture. '
                : '') +
