          ...
          return `
            <div onclick="openCourseItemDetail('${course.id}','${item.id}')" class="bg-white rounded-3xl p-5 mb-3 shadow-sm border border-gray-100 cursor-pointer">
              ...
              <div class="flex items-center gap-2">
                <span class="inline-flex items-center gap-1.5 border rounded-full px-2.5 py-2.5 text-xs font-semibold ${item.completed ? 'border-emerald-600 text-emerald-700' : 'border-gray-300 text-gray-500'}">
                  ${item.completed ? Icon('check','w-3.5 h-3.5') : ''} ${courseItemTypeLabel(item.type)}
                </span>
                ${item.duration ? `<span class="border border-gray-200 rounded-full px-2.5 py-2.5 text-xs font-semibold text-gray-500">${escapeHtml(item.duration)}</span>` : ''}
              </div>
            </div>`;
        }
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
              ...
              <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
                <div class="flex items-center gap-2 mb-4">
                  <span class="border border-gray-200 rounded-full px-2.5 py-2.5 text-xs font-semibold text-gray-500">${courseItemTypeLabel(item.type)}</span>
                  ${item.duration ? `<span class="border border-gray-200 rounded-full px-2.5 py-2.5 text-xs font-semibold text-gray-500">${escapeHtml(item.duration)}</span>` : ''}
                </div>
                ${item.type === 'video' ? `
        ...
        function handleQuizSubmissionFile(e, itemId){
          ...
          uploadCourseItemMediaToStorage(file, itemId + '-submission').then(remoteUrl => {
            if (remoteUrl) item.submissionUrl = remoteUrl;
