try {
        ...
        function newClassJoinPolicyHTML(){
          ...
          return `
            <div id="create-class-join-policy" class="mb-5">
              ...
              <div class="flex gap-2">
                <button onclick="setNewClassJoinPolicy('open')" class="flex-1 px-3 py-2.5 rounded-2xl text-xs font-semibold text-center ${d.joinPolicy==='open' ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.joinPolicy==='open' ? `background:rgba(30,144,255,0.1);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">Open to all with the code</button>
                <button onclick="setNewClassJoinPolicy('approved')" class="flex-1 px-3 py-2.5 rounded-2xl text-xs font-semibold text-center ${d.joinPolicy==='approved' ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.joinPolicy==='approved' ? `background:rgba(30,144,255,0.1);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">Approve each student</button>
              </div>
            </div>`;
        }
          ...
          return `
            <div id="create-class-payment" class="mb-5">
              ...
              <div class="flex gap-2 mb-3">
                <button onclick="setNewClassPaymentEnabled(false)" class="flex-1 px-3 py-2.5 rounded-2xl text-xs font-semibold text-center ${!d.paymentEnabled ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${!d.paymentEnabled ? `background:rgba(30,144,255,0.1);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">Free for everyone</button>
                <button onclick="setNewClassPaymentEnabled(true)" class="flex-1 px-3 py-2.5 rounded-2xl text-xs font-semibold text-center ${d.paymentEnabled ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.paymentEnabled ? `background:rgba(30,144,255,0.1);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">Paid entrance</button>
              </div>
              ${d.paymentEnabled ? `
                <div class="flex gap-2">
        ...
        async function verifyClassPayment(reference, classId){
          try {
            ...
            classPaymentBusy = false;
            window.reportError && window.reportError(err, { call: 'verifyClassPayment', classId });
              ...
              <div class="border-t border-gray-100 mb-5">
                ${cls.pendingRequests.map((r, i) => `
                  <div class="flex items-center gap-4 py-4 ${i < cls.pendingRequests.length - 1 ? 'border-b border-gray-100' : ''}">
                    <span class="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 flex-shrink-0 overflow-hidden">${r.photo ? `<img src="${r.photo}" class="w-full h-full object-cover">` : Icon('user','w-6 h-6')}</span>
                    <div class="min-w-0 flex-1">
