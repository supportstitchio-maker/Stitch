        ...
        // Reads a bot reply aloud via the browser's SpeechSynthesis.
        // msgId (the reply's index in aiChatMessages) drives the speaker
        // icon's "reading.../stop" state in aiBotBubble via
        // aiSpeakingMessageId; only one reply speaks at a time, so
        // starting a new one cancels whatever was already playing.
        function speakAIText(text, msgId){
          ...
          utter.rate = 1;
          utter.onend = () => { aiSpeakingMessageId = null; refreshAIChatLog(false); };
          utter.onerror = () => { aiSpeakingMessageId = null; refreshAIChatLog(false); };
        ...
        async function getAIResponse(text, attachments, viaVoice){
          ...
          try {
            ...
            const system = 'You are Stitch Bot, a warm, encouraging AI tutor and study buddy inside a study app for students across any subject. ' +
              ...
              (viaVoice
                ? 'This question was asked out loud and your reply will be read back with text-to-speech, like talking to a voice assistant: lean into your sense of humor a bit more here, a quick witty remark or a fun line, since a spoken answer that is all business feels flat out loud. Keep it snappy and easy to follow by ear (short sentences, nothing that only lands when read on a page), and never let the joke get in the way of actually answering. '
                : '') +
