              ...
              <div class="w-full px-5 pb-3 relative" style="padding-top:20px;">
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-1.5 flex-shrink-0">
                    ...
                    <button onclick="closeOverlay()" class="flex items-center gap-1 flex-shrink-0 pl-0.5">
                      ${gradIcon(IconBold('back','w-5 h-5'))}
                      <span class="text-sm font-semibold" style="color:${NAVY};">Back</span>
                    </button>
                  </div>
                </div>
        ...
        function aiBotBubble(text, id){
          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start gap-4">
        ...
        function aiUserBubble(text, attachments, id, isEditing){
          ...
          // Per-request, only Stitch Bot's own replies keep the filled
          // "pill" background now -- the user's own text renders as plain,
          // right-aligned text with no colored bubble behind it.
          const textBubble = text ? `
            <div class="text-gray-800 text-[15px] leading-relaxed text-right" style="white-space:pre-wrap;">${escapeHtml(text)}</div>` : '';
          const actions = (text && !isEditing) ? `
            <div class="flex items-center gap-3">
              <button onclick="copyAIUserMessage(${id})" title="Copy" class="flex items-center justify-center text-gray-400">${Icon('copy','w-3.5 h-3.5')}</button>
              <button onclick="startEditAIUserMessage(${id})" title="Edit" class="flex items-center justify-center text-gray-400">${Icon('edit','w-3.5 h-3.5')}</button>
            </div>` : '';
          if (isEditing) {
            return `
          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start justify-end gap-3">
              <div class="min-w-0 flex flex-col items-end gap-1.5" style="max-width:80%;">
                ${images}${files}${textBubble}${actions}
              </div>
            </div>`;
        }
