        // ---------------- AI INTERACTIVE CLASS ---------------- Mermaid needs a one-time init before mermaid.run() (used by renderPendingMermaidDiagrams...
        try {
        ...
        // ---- Stitch Bot voice input: speech-to-text only.
        let aiVoiceRecognition = null;
        let aiVoiceRecording = false;

        // First name Stitch Bot should call the student by -- pulled from their profile (see profileData.name in profile.js), same name shown everywhere else...
        ...
        // Past conversations, one entry per session (everything you sent between opening Stitch Bot and leaving it).
        let aiChatSessions = [];
        let aiChatSessionIdCounter = 0;

        // Files away whatever's currently in aiChatMessages as a session (as long as you actually said something -- an empty chat with just the bot's welcome...
        function archiveCurrentAIChat(){
          // Leaving/switching the live chat shouldn't leave a mic recording still running in the background over whatever's opened next.
          stopAIVoiceInput();
        ...
        // Chat History used to just show each session's very first message verbatim, which was usually a bare greeting ("Yoo Sup") that says nothing about what...
        ...
        // Classroom nav's Alerts / Stitch Bot buttons are shared between the main Classroom tab (no overlay open yet) and being tapped from *inside* another...
        ...
        function openAIClass(){
          // Always land back on the live chat, not wherever the mobile full-screen history view was left open last time (it never got reset on close, so...
        ...
        // Tapping the header's dashes icon used to open a one-item dropdown ("Chat history") that had to be tapped again to actually get to the history drawer.
        ...
        // Resumes a past session as the live conversation: whatever's currently open gets archived first (so it isn't lost), then the picked session is pulled...
        ...
        // "+ New chat" in Chat History: archives whatever's currently live (only if anything was actually said, same as leaving Stitch Bot normally would) and...
        ...
        // Search bar inside the Chat History drawer: tapping the search icon swaps the "Chat History" title for a text input that filters the Recents list...
        ...
        // Chat History as a slide-in drawer -- overlays the live chat from the left edge (backdrop + panel) instead of replacing the whole Stitch Bot screen,...
        ...
        // Desktop right-panel version of the same list: the panel is already titled "Chat History" (see rightPanelTitle), so this is just the rows, and tapping...
        ...
        // Images (and video, if attached) get a real thumbnail (both in this pre-send strip and in the sent bubble, see aiUserBubble) instead of a generic file...
        ...
        // On a physical keyboard, Enter sends and Shift+Enter (or Alt/Ctrl+Enter) inserts a newline.
        ...
        // ---- Keep the composer bar pinned above the on-screen keyboard ---- Mobile browsers resize the *visual* viewport (not the layout viewport) when the...
        ...
        function syncAIClassKeyboardInset(){
          ...
          // Without the composer actually focused, there is no way a real on-screen keyboard can be up (typing requires focus first) -- so whether the Classroom...
          ...
          // Composer is focused, so a keyboard opening (or already open) is genuinely possible -- track its real height here to position the composer just above...
        ...
        // iOS/Android fire visualViewport 'resize' and 'scroll' many times in quick succession while the on-screen keyboard is animating open or closed (each...
        ...
        // Tapping into the composer textarea: hide the Classroom sub-nav and let the overlay claim its space immediately, rather than waiting for the...
        ...
        // Leaving the composer: only bring the sub-nav back once the keyboard has actually finished closing (the viewport briefly still reports itself as...
        function handleAIChatInputBlur(){
          aiChatInputFocused = false;
          setTimeout(syncAIClassKeyboardInset, 60);
          // Bring the mascot back if the student backed out without actually typing anything -- if there's real text pending they're still mid-conversation, so...
        ...
        // ---- Stitch Bot's idle mascot ---- The robot on the empty-chat screen (see aiChatEmptyStateHTML) is a plain static image -- no animation, no motion.
        ...
        // Persisted the same way the Supabase session itself is (see getSupabaseClient in core.js): localStorage in the native app (survives restarts, same...
        ...
        // Called once per sign-in from authEnterApp() (games.js) -- picks a fresh pose and persists it, so it's the same pose all through this login and only...
        ...
        // Warms the browser's image cache for artwork that used to only start loading the moment a student actually opened the screen that shows it -- Stitch...
        function preloadAppImageAssets(){
          const srcs = [
            ...AI_ROBOT_POSES.map(p => p.src),
            // Mirrors the four feedFeatureCard image paths in study.js's renderExamsStudyMode (Practice Tests, Challenge Arena, Flashcards, Upgrade).
            'assets/practice/practice-tests.png',
            'assets/practice/challenge-arena.png',
            'assets/practice/flashcards.png',
            'assets/practice/upgrade.png',
            // study.js's own planCardBadges -- reused here instead of duplicated, since courses.js loads after study.js.
            ...(typeof planCardBadges !== 'undefined' ? Object.values(planCardBadges) : []),
          ];
        ...
        // Which of the user's own messages (by index into aiChatMessages) is currently being edited, if any.
        ...
        // Mascot shown in place of the chat log for a brand-new, still-empty conversation -- centered in the available space the same way Claude/Gemini/ChatGPT...
        ...
        // Re-paints the chat log (both the main overlay's log and, if present, the split-view's copy) after anything that mutates aiChatMessages outside of...
        ...
        // Turns Stitch Bot's plain-text/lightweight-markdown replies into real HTML: **bold** becomes <strong>, consecutive "- "/"* " or "1.
        let aiMermaidIdCounter = 0;
        function formatAIText(raw){
          if (!raw) return '';
          let text = String(raw);

          // Fenced ```mermaid blocks first -- they can span many lines and would otherwise get chopped up by the line-by-line list logic further down.
          const mermaidBlocks = [];
          ...
          // Fenced ```graph blocks -- math/science plots (function curves, lines, circles, polygons, points) for word problems and worked examples.
          const graphBlocks = [];
          ...
          // Display math ($$...$$) then inline math ($...$), rendered up front via KaTeX into real equations/fractions.
          const mathBlocks = [];
          ...
          // A reply can arrive truncated (the ai-proxy Edge Function's response length cap cutting it off mid-answer, most often on a long worked-math reply) --...
          text = text.replace(/\$\$/g, '');
          ...
          // Swap the placeholder tokens back in for the real math/diagram markup now that list/paragraph structure has been built around them (done last so...
        ...
        // ---------------- Stitch Bot: graph/shape plotting ---------------- Turns a ```graph fenced block's JSON spec into a self-contained inline SVG string.
        ...
        function renderGraphBlock(rawSpec){
          try {
            ...
            // Three flavors share this one fenced ```graph block: the original numeric coordinate-plane plot (default, used when "type" is omitted, for function...
            ...
            // Bounding box across every coordinate actually used, as a fallback for whichever of xRange/yRange the model left out.
            ...
            // Padding so a curve/shape doesn't sit flush against the edge.
            const padX = (xmax - xmin) * 0.1, padY = (ymax - ymin) * 0.1;
            xmin -= padX; xmax += padX; ymin -= padY; ymax += padY;

            const rangeX = xmax - xmin, rangeY = ymax - ymin;
            const MAX_PX = 240;
            const ppu = MAX_PX / Math.max(rangeX, rangeY); // shared x/y scale
            const chartW = rangeX * ppu, chartH = rangeY * ppu;
            const pad = 26; // room for axis tick labels
            ...
            // Axes (only drawn where 0 actually falls within range).
            ...
            svg += '</g>';
            // Tick number labels along each axis (skip the 0 label twice).
            ...
            // Elements, in the order given (later ones draw on top).
            let colorI = 0;
            const legend = [];
            elements.forEach(el => {
        ...
        // Renders a payoff/decision matrix (game theory 2x2s, truth tables, any row-by-column comparison grid) as an actual styled table instead of the...
        ...
        // Renders a categorical bar chart (comparing values across labeled groups, e.g.
        function renderBarBlock(spec){
          try {
            ...
            let svg = '';
            // Zero line + a couple of horizontal gridlines for scale.
        ...
        // Mermaid can only draw into a live DOM element, not render to a plain string up front the way KaTeX does -- so formatAIText just leaves an empty <div...
        ...
        // Strips markup and any leftover markdown bold markers so the clipboard gets clean plain text, not raw HTML tags.
        ...
        // The "Thinking" state used to just render the literal '···' string sitting flat in the bubble.
        ...
        function aiBotBubble(text, id, promptLimit, image, mediaType){
          ...
          // A generated image renders as its own rounded tile above any accompanying text (there's usually none besides a possible ackLine for a doc scan...
          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start">
              <div class="min-w-0" style="max-width:85%;">
                ${imageHTML}
                ${(!isThinking && !text) ? '' : `<div class="ai-msg-bubble bg-gray-100 rounded-2xl rounded-bl-md px-4 py-3.5 text-[15px] text-gray-700 leading-relaxed">${isThinking ? aiThinkingIndicatorHTML() : formatAIText(text)}</div>`}
                ${isThinking ? '' : `
                ...
                ${promptLimit ? `
                <button onclick="openOverlayFrom('aiClass','subscriptionSettings')" class="mt-2 w-full font-semibold py-2.5 rounded-2xl text-center text-[13.5px] text-white flex items-center justify-center gap-1.5" style="background:${NAVY};">${Icon('crown','w-4 h-4')} Upgrade plan</button>` : ''}
              </div>
            </div>`;
        }

        // Lets the student save a generated image straight from the chat bubble, same as they'd expect from any image they receive.
        ...
        // Saves an edited user message, then -- since everything after it in the conversation was a response to the old wording -- drops whatever followed and...
        ...
        function aiUserBubble(text, attachments, id, isEditing, voice){
          // Images (and video, if ever attached) get an actual thumbnail here, same as the pre-send attach strip (aiAttachStripHTML) -- they used to always fall...
          const imageAttachments = (attachments || []).filter(f => f.previewUrl);
          const fileAttachments = (attachments || []).filter(f => !f.previewUrl);
          // The navy rounded-2xl block is meant to be the media's actual frame/outline, not a padded border sitting around a separately-rounded thumbnail -- that...
          ...
          // Per-request, the user's own text now renders inside its own soft, translucent blue bubble (instead of plain right-aligned text with no background) --...
          ...
          const textBubble = text ? `
            <div class="rounded-2xl rounded-br-md px-4 py-3 text-[15px] text-gray-800 leading-relaxed" style="background:rgba(30,144,255,0.14); white-space:pre-wrap;">${voiceBadge}${escapeHtml(text)}</div>` : '';
          if (isEditing) {
            return `
        ...
        async function sendAIMessage(preset, viaVoice){
          ...
          // Free-tier daily prompt cap (see DAILY_PROMPT_LIMIT/ canSendAIPrompt in study.js) -- unlimited during the 7-day trial or on any active paid plan.
          if (typeof canSendAIPrompt === 'function' && !canSendAIPrompt()) {
            aiChatMessages.push({ role:'user', text, attachments, voice: !!viaVoice });
            aiChatMessages.push({ role:'bot', text: `You've used all your daily Stitch Bot credits for today. They reset tomorrow, or subscribe to Stitch Pro for unlimited prompts any time.`, promptLimit: true });
          ...
          const thinkingMsg = { role:'bot', text: '···' };
          aiChatMessages.push(thinkingMsg);
          // Clear the composer and let it shrink back to a single line.
          if (inputEl) { inputEl.value = ''; }
          resetAIChatInputHeight();
          if (typeof syncAIClassKeyboardInset === 'function') syncAIClassKeyboardInset();
          refreshAIChatLog(true);
          ...
          // Any PDF/DOCX/PPTX attached here gets scanned exactly like a Resources-tab upload; same flashcards, practice questions, and it becomes part of what...
        ...
        // ---- Stitch Bot voice input (speech-to-text) ---- Tapping the mic starts the browser's built-in SpeechRecognition, live-fills the composer with what...
        ...
        // Stops an in-progress recording early (mic tapped again, or the student navigates away from Stitch Bot -- see archiveCurrentAIChat).
        function stopAIVoiceInput(){
          if (aiVoiceRecognition) { try { aiVoiceRecognition.stop(); } catch (err) {} }
        }

        // Idle: plain mic. Recording: solid red, pulsing, stop-square icon (mirrors the DM voice-note mic in chat.js/setConvoMicRecordingUI).
        ...
        // Briefly swaps the composer placeholder to show a mic error (permission denied, unsupported browser, etc.), same pattern as flashConvoMicMessage for...
        ...
        // Reads a File into raw base64 (no "data:...;base64," prefix) so it can ride along in the JSON body callClaude sends to the ai-proxy Edge Function.
        ...
        // Gives Stitch Bot actual memory across the conversation instead of answering each message in total isolation: pulls in the last several turns of the...
        ...
        // Simple phrase-based trigger: catches the common ways a student actually asks for a picture ("draw...", "generate an image of ...", "make me a...
        ...
        // Talks to Claude for real, grounding the answer in excerpts from whatever the student has uploaded under Resources (when relevant), and now actually...
        async function getAIResponse(text, attachments, viaVoice){
          ...
          // Route to image generation instead of a text reply when the student is clearly asking for a picture and hasn't also sent their own image for Stitch...
          ...
          try {
            ...
            // Surface the rate-limit message directly rather than masking it behind a generic canned reply -- the student needs to know to slow down/wait, not...
            const msg = (err && err.message) || '';
            if (msg.startsWith('Too many AI requests')) return { text: ackLine + msg };
        ...
        // A course is either taught live (scheduled sessions, like myClasses' live-lecture feature) or a self-paced set of uploaded material a learner works...
        ...
        // Which way the classroom-tab-panel should slide in on its next render -- 'left' when moving classes->courses (courses tab sits to the right of My...
        ...
        // 'content' (Modules/Items) | 'resources' (supplementary video & audio) | 'announcements' -- which tab of the course detail screen is showing.
        let courseDetailTab = 'content';
        // Whether the three-dot admin menu (Edit Course / Teachers / Delete) is open on the course detail screen -- see courseDetailMenuDropdownHTML.
        let courseDetailMenuOpen = false;
        let courseQuizAnswers = {}; // itemId -> [selectedOptionIndex, ...]
        let courseQuizSubmitted = {}; // itemId -> bool
        // Which lesson/item ids THIS student has marked complete.
        ...
        // When set (to the item's index within the module), the Add Part form below is in edit mode -- confirmAddCourseItem() overwrites that existing item...
        ...
        // Every course is taught by "Stitch Team" by default; admins can additionally invite other teachers onto a specific course (see openCourseTeachers /...
        ...
        // Same simple base64-data-URL pattern as a class photo (handleClassPhotoSelected) rather than a Storage upload -- it's one small image on a course's...
        ...
        function setClassroomAreaTab(tab){
          ...
          // 'classes' is the left-hand pill and 'courses' is the right-hand pill, so moving into 'courses' slides the panel in from the right (direction 'left')...
        ...
        // Which course card (in the Courses list, not the detail overlay) has its admin quick-menu open -- lets an admin Edit/manage Teachers/Delete a course...
        ...
        // A module counts as "done" (gets the Coursera-style checkmark pill) once it has at least one item and every item in it is completed.
        ...
        function courseCardHTML(c){
          ...
          // Same gradient-banner-plus-motif treatment as a My Classes card (see myClassesListHTML in study.js), but blue-only (blueCardPalette in core.js)...
        ...
        // Dropdown for the course detail three-dot menu: consolidates the admin-only actions (Edit Course -- add/remove modules, lessons, and quizzes --...
        ...
        // Enrolling used to be a single tap that unlocked every module instantly -- now it requires filling in a short form and explicitly agreeing to the...
        ...
        // Ends a student's own enrollment -- distinct from deleteCourse (admin-only, removes the course for everyone).
        ...
        // Resources are supplementary video/audio, kept separate from the graded Module/Item curriculum above -- same enrollment gate as module 2+, since...
        ...
        // Quick-add shortcut from the Resources tab -- same underlying newCourseDraft/publishNewCourse pipeline as Manage Course, see...
        ...
        // ---- Engage tab: everyone's Announcements (as before), plus -- for admins only -- a quick-reach list of every Quiz/Assignment/ Project already in...
        ...
        // Jumps an admin straight from the Engage tab into editing one specific quiz/assignment/project, skipping the module-list screen entirely: loads the...
        ...
        // Quick-add shortcuts from the Engage tab -- same underlying newCourseDraft/publishNewCourse pipeline as Manage Course (so a module/item added this way...
        ...
        // ---- Course Announcements (admin-posted updates shown to everyone browsing the course, enrolled or not -- e.g.
        ...
        // ---- Quiz "final work" file submission ---- Separate from the auto-graded multiple-choice answers above -- this lets a learner attach the actual file...
        ...
        // Local preview shows immediately (object URL) while the real file uploads to Storage in the background, same pattern as Part media uploads -- see...
        ...
        // Loads an existing course into the same draft/form used for creating one, so admins can add/remove modules and items or change the title/description...
        ...
        function rerenderNewCourseOverlay(){
          const ov = document.getElementById('overlay');
          if (!ov) return;
          // Every small interaction on this screen (toggling course/resource type, adding or removing a module/resource, picking a file to upload) re-renders the...
        ...
        // ---- Course Resources (admin only) -- supplementary videos and audio, separate from the graded Module/Item curriculum above.
        ...
        // Course-level items (assignments/projects/quizzes placed under "Final Project" instead of inside a specific module) -- kept in their own array so...
        ...
        // Opens the same Add Part form, but for an assignment/project/quiz that belongs to the course as a whole rather than any single module -- defaults its...
        ...
        // Admin editing an existing Part (video/reading/assignment/project/ quiz, etc.) inside a module -- reopens the same Add Part form, pre-filled from the...
        ...
        // Same as above, but for editing an item that's already placed under Final Project instead of inside a module.
        ...
        // Called from the "Where should this go?" picker in the Add Part form (assignment/project/quiz types only) -- just moves which section the open form...
        ...
        // Picking a video/picture file shows an instant local preview (object URL, so it appears immediately with no network wait) while the real file uploads...
        function handleAddingItemMediaSelected(e){
          const file = e.target.files && e.target.files[0];
          if (!file) return;
          const d = newCourseAddingItemDraft;
          if (!d) return;
          // The Part's chosen type (video/picture) decides how the file is treated -- not the raw mime type.
          ...
          uploadCourseItemMediaToStorage(file, itemId).then(remoteUrl => {
            ...
            if (remoteUrl) {
              ...
              // Upload failed -- keep the local preview so it isn't lost, but tell the admin now instead of letting them publish a file that will look broken to...
        ...
        function confirmAddCourseItem(){
          ...
          // Placement -- either a specific module's items array, or the course-level finalItems array when placed under Final Project (see...
          ...
          if (newCourseEditingItemIndex !== null) {
            // Editing in place -- keep the original id, overwrite everything else with the edited values.
            ...
            if (originalList === targetList) {
              ...
              // Placement changed while editing (e.g. moved from a module to Final Project, or vice versa) -- remove from where it used to live and add it to its new...
        ...
        // Leaving the editor (back arrow, or the phone/browser back button -- see the 'newCourse' entry in overlayBackAction) used to just close straight out,...
        function closeCourseEditorGuard(fromPopState){
          if (!newCourseDraft.title.trim()) {
            if (fromPopState) {
              // The hardware/browser back button has already popped our history entry by the time this fires -- push it back on so the editor stays open instead of...
        ...
        async function publishNewCourse(){
          ...
          if (newCourseEditingId) {
            ...
            if (c) {
              ...
              // Keep the remote copy in sync too, same pattern as a brand-new course below -- awaited now so we can warn if it silently failed instead of assuming it...
              ...
              // This course may have originated from a "Course" opportunity listing (see createLinkedCourse in jobs.js), which keeps its own title/org/description in...
              if (c.opportunityId) {
          ...
          const course = {
            ...
            // Fixed at creation time, same pattern as a class's colorIndex/motifIndex -- never re-rolled on edit, since the newCourseEditingId branch above only...
          ...
          // Persist it so other users see it too, and so it's still there the next time this admin signs in -- awaited so a failure can actually be surfaced...
        ...
        // ---- Course Teachers (admin can invite additional teachers onto a course; Stitch Team is always the base teacher and can't be removed) ----
        ...
        async function postCourseInsertRemote(course){
          ...
          try {
            ...
            // upsert, not insert: this is called both when a brand-new course is published AND when an existing one is edited (see publishNewCourse above).
        ...
        // Loads every course from Supabase into allCourses -- called once on sign-in (see authEnterApp in games.js).
        ...
        // One Realtime subscription per signed-in session, same pattern as subscribeToIncomingMessages in chat.js.
        ...
        // Fallback for when the `alter publication supabase_realtime add table courses;` step (see the courses table SQL comment near the top of this file)...
        ...
        function applyRemoteCourseChange(payload){
          ...
          // Only touch the DOM for screens actually on-screen right now -- the Classroom tab's course catalog, and/or an open course detail overlay for this same...
          if (typeof currentTab !== 'undefined' && currentTab === 2 && typeof classroomAreaTab !== 'undefined' && classroomAreaTab === 'courses') {
            const studyContentEl = document.getElementById('study-content');
            if (studyContentEl) studyContentEl.innerHTML = studyContent();
            else if (typeof renderStudy === 'function') renderStudy();
        ...
        // Same best-effort pattern in reverse: a real delete needs its own Postgres RLS "delete" policy on COURSES_TABLE restricted to admins (mirroring the...
        ...
        // Course-level "Final Project" section, shown once below every Module -- assignments/projects/quizzes placed here (via the placement picker in...
        ...
        function addCourseItemFormHTML(){
          ...
          // Assignments/projects/quizzes are the types that can live either inside a specific module or under the course-wide Final Project section -- everything...
        ...
        //
        //   -- Inviting a student by email (submitInviteStudents below)
        //   -- previously only added a cosmetic roster row -- it never
        //   -- touched `members`, so RLS still blocked the invited person
        //   -- from actually reading the class until they separately used
        //   -- the class code/link. This table + function closes that
        //   -- gap: an invite is recorded here, and whenever someone signs
        //   -- in, claim_class_invites() looks for invites matching their
        //   -- own verified email and folds them into `members` for them.
        ...
        //     created_at timestamptz not null default now()
        //   );
        //   create index class_invites_email_idx on class_invites (lower(email));
        //   alter table class_invites enable row level security;
        //
        //   -- Only a class's teacher may create/view/remove its pending
        //   -- invites -- nobody else should be able to see who's been
        //   -- invited to a class they don't teach.
        //   create policy "Teacher can manage their class's invites"
        //     on class_invites for all
        //     using (exists (select 1 from classes where classes.id = class_invites.class_id and classes.teacher_id = auth.uid()))
        //     with check (exists (select 1 from classes where classes.id = class_invites.class_id and classes.teacher_id = auth.uid()));
        //
        //   -- Runs with elevated privileges just long enough to match
        //   -- auth.email() (the caller's own *verified* address -- not
        //   -- anything a client could spoof) against pending invites and
        //   -- add the caller to those classes' `members`, then clears the
        //   -- matched invite rows so they aren't reprocessed. Safe to call
        //   -- on every sign-in: with no matching invites it's a no-op.
        //   create or replace function claim_class_invites()
        ...
        //     joiner_name text := split_part(coalesce(joiner_email, 'Student'), '@', 1);
        //   begin
        //     if auth.uid() is null or joiner_email is null then
        //       return;
        //     end if;
        //     return query
        //       update classes
        ...
        //             where class_invites.class_id = classes.id
        //               and lower(class_invites.email) = lower(joiner_email)
        //           )
        //         returning *;
        //     delete from class_invites
        //       where lower(email) = lower(joiner_email)
        //         and class_id in (select id from classes where auth.uid() = any(members));
        //   end;
        //   $$;
        //   revoke all on function claim_class_invites() from public;
        //   grant execute on function claim_class_invites() to authenticated;
        let classesChannel = null;
        let classesSubscribedForUserId = null;
        // Live top-up for deletions specifically: a teacher deleting their own class (leaveCurrentClass, role==='teacher') removes the row outright, but every...
        async function subscribeToClassesRealtime(){
          ...
          classesChannel = sb.channel('classes-catalog:' + myId)
            ...
            // UPDATE was previously missing entirely: a teacher starting a lecture, posting an announcement, or changing anything else about the class only ever...
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: CLASSES_TABLE }, (payload) => applyRemoteClassUpdate(payload))
            .subscribe();
        }

        // Merges a remote class row update into the matching local myClasses entry (id/code/teacherId/role never change here, only the shared `data` blob and...
        function applyRemoteClassUpdate(payload){
          ...
          // Only students get alerted about classroom activity here -- the teacher who just posted the announcement/classwork already knows about it (this update...
          ...
          // Sync every classroom alert (new announcement, new classwork) into the same Notifications bell tab everything else uses -- this is on top of the...
          ...
          // Re-render the class detail overlay (Stream/Classwork/People) if it's open on this same class, same pattern as applyRemoteCourseChange -- this is what...
        ...
        function applyRemoteClassDelete(payload){
          ...
          queueSaveUserState();
          // If it's the exact class currently open, kick back out with a heads-up instead of leaving them staring at a now-nonexistent classroom (or letting them...
        ...
        // Turns a `classes` row into the shape the rest of this file already expects on a myClasses entry (name/section/announcements/ etc, spread out of...
        function classRowToLocal(row, myUserId){
          ...
          return Object.assign({
            ...
            // Carried over so a teacher starting a lecture can ring every student directly (see ringClassMembersForLecture) without a separate round-trip to fetch...
            members: Array.isArray(row.members) ? row.members : [],
            // Any array field can independently come back non-array from a stale/partial `data` blob (e.g.
        ...
        // Folds in any classes this account was invited to by email (see the class_invites/claim_class_invites SQL above) -- run before the select() in...
        async function claimPendingClassInvites(){
          const sb = getSupabaseClient();
          if (!sb) return;
          try {
            await sb.rpc('claim_class_invites');
          } catch (e) { /* no backend configured yet, or function not set up */ }
        }

        // Loads every class this signed-in account can see -- thanks to the RLS policy above, a plain select() already comes back filtered to just "I teach it"...
        ...
        // Debounced best-effort sync of a class's content (announcements, classwork, lectures, roster, settings) back to its shared row, same pattern as...
        ...
        async function saveClassRemoteNow(cls){
          const sb = getSupabaseClient();
          if (!sb) return;
          // Only the fields that live in `data` -- id/code/teacherId/role are either the primary key or derived, and shouldn't be written back as part of the...
        ...
        // Loaded fresh from Supabase on every sign-in (loadMyClasses above) -- starts empty so there's something defined for the rest of this file to read/push...
        ...
        // ---------------- CLASS CREATE/JOIN LOADING (mirrors the Classroom-tab and exam "loading then slide-away" curtain pattern) ----------------
        ...
        // Shows the loading curtain in the overlay, runs onDone (which should populate data and render the final overlay content), then slides the curtain away...
        function runClassActionLoading(text, iconName, onDone){
          ...
          setTimeout(async () => {
            ...
            // onDone may be async (e.g. it needs to hit Supabase first) -- await it so the curtain doesn't slide away and reveal stale or empty content underneath...
            try {
              await onDone();
            } catch (err) {
        ...
        // Joining now goes through the join_class_by_code() function (see the schema comment above myClasses) instead of searching this browser's own local...
        ...
        async function submitCreateClassroom(){
          ...
          runClassActionLoading('Creating class', 'plus', async () => {
            // members starts empty -- the teacher is already covered by teacher_id in the RLS policies, no need to also list them as a member.
        ...
        // Classes (as opposed to self-paced Courses, which already use TEAM_STITCH_TEACHER above) show the actual teacher's name to a teacher looking at their...
        ...
        // Real per-teacher public profile (name + photo), keyed by teacherId, for students looking at someone else's class -- teacherId is a genuine auth user...
        ...
        // Real photo for whoever's "the teacher" in this class -- your own for a class you teach, or the real teacher's (once loaded by...
        ...
        // A teacher "leaving" their own class deletes it for everyone (there'd be no teacher left otherwise); a student leaving just removes themself from...
        ...
        // Cancel enrollment in a course that was enrolled into from the Explore/Opportunities tab (see enrollCourseIntoClassroom in jobs.js), which mints a...
        ...
        // Wrapper functions instead of building the modal's title/message by concatenating cls.name straight into an onclick="..." attribute string (see below)...
        ...
        function classDetailHTML(){
          ...
          // The Dashboard tab is for admins looking at a classroom they teach -- a plain teacher (non-admin) still just gets the regular Stream/Classwork/People...
        ...
        // ---- Admin Dashboard (visible to site admins inside a classroom they teach): an at-a-glance overview plus one-tap shortcuts to the actions they reach...
        ...
        // Class stream (announcements + their comments) used to stamp every one with the literal string 'now' at creation time and never touch it again -- so a...
        function formatClassStreamTime(item){
          if (item.createdAt) return formatNotifTime(item.createdAt);
          return item.time || 'now';
        }

        // Mirrors startNotifTimeTicker (overlays.js): walks whatever announcement/comment time labels are currently on screen and refreshes their text in place...
        ...
        // Read-only attachment chips shown under a posted announcement (tapping one opens the staged blob URL in a new tab).
        ...
        // Renders the "Class comments" thread beneath a stream announcement, plus an input box to add a new one, mirroring the Google Classroom comment section...
        ...
        // Reposting a class-stream announcement: pushes a copy into feedPosts (marked reposted:true) so it shows up in the main feed and under the "Reposts"...
        function toggleAnnouncementRepost(announcementId){
          ...
          if (a.reposted) {
            ...
            a.reposted = true;
            // PostsAPI.create() already ran synchronously by this point (see mockRequest), so the post's real id is available immediately; .then() here just reads...
        ...
        function addStreamComment(announcementId){
          ...
          document.getElementById('overlay').innerHTML = classDetailHTML();
          // Without this, the comment only ever existed in this device's local copy of `cls` -- it never reached Supabase, so no other member's app (including...
          queueSaveClassRemote(cls);
        }

        // ---------------- LECTURES: Start Live / Schedule (teacher-only controls) ---------------- Two pill buttons shown right under the class banner,...
        ...
        // Lists live + upcoming lectures at the top of the Stream tab so students can see and join them too, not just the teacher.
        ...
        // ---- Live lecture link sharing (guest join, no class membership needed) ---- Lets anyone with the link -- including people who aren't in this class...
        ...
        // ---------------- LIVE LECTURE (Zoom-style call screen) ---------------- view: 'grid' (participant tiles) | 'whiteboard' | 'slides'
        ...
        // True while the current lecture call is being attended by someone who followed a shared lecture link (see shareLectureLink above /...
        let guestLectureMode = false;
        // { classId, lectureId } while the "you're not in this class" confirm screen is up, between checkPendingLectureLinkJoin() finding the link and...
        ...
        // While a live lecture is open, the Classroom right panel's usual "Notebook" mode is relabelled "Class Pad" instead (see...
        let inLectureCall = false;
        // While a practice exam (or quiz/challenge using the shared exam engine) is open, the Classroom right *panel* relabels itself "Annotate" -- the...
        ...
        // For strings that get embedded inside an inline event-handler attribute as a JS string literal, e.g.
        ...
        // ---- Lecture ringing (teacher -> every student in the class) ---- Starting a lecture previously only wrote it into cls.lectures and saved it to...
        ...
        // One-shot broadcast to a single student's ring inbox -- same disposable-channel trick as broadcastToUserChannel in chat.js.
        ...
        // Rings every student in the class (cls.members -- populated by the join_class RPC, see the CLASSES_TABLE SQL comment above; the teacher is never in...
        async function ringClassMembersForLecture(cls, lecture){
          ...
          members.forEach(uid => {
            ...
            // Same reasoning as sendCallRing in chat.js: the broadcast only reaches students actively signed in with the app open right now -- this also lets...
            sendPushTo(uid, {
        ...
        // A teacher just started/resumed a lecture we're a member of.
        function handleIncomingLectureRing(payload){
          ...
          // Also logged to the Notifications bell tab, same as every other classroom alert -- so a lecture that was missed/ declined (or rang while this account...
        ...
        function acceptIncomingLectureCall(){
          ...
          // The class's local lectures list might not have caught up yet (applyRemoteClassUpdate should normally beat the ring here, but don't depend on...
          if (!cls.lectures) cls.lectures = [];
        ...
        // fromPopState is threaded through to closeOverlay -- same reason as declineIncomingCall's in chat.js.
        ...
        // Same connection path as joinLiveLecture, but for someone who isn't a member of this class -- reached only via a shared lecture link (see...
        ...
        // Shared by joinLiveLecture and joinLectureAsGuest: resets any state left over from a previous lecture, opens the call screen, connects the Realtime...
        async function connectLectureCall(classId, lectureId){
          clearLectureTimer();
          stopLectureLocalStream();
          // Fresh session: clear any whiteboard drawings/slides left over from a previous lecture, and release any object URLs created for previously...
        ...
        // Reads a `?joinLecture=classId:lectureId` param off the URL (see buildLectureInviteLink above) and routes accordingly.
        ...
        // ---- Guest confirm screen (link-only, no class membership) ----
        function guestLectureJoinHTML(){
          ...
          return `
            <div class="flex-1 flex flex-col items-center justify-center px-8 text-center" style="padding-top:var(--top-safe-pad);">
              ...
              <button onclick="submitGuestLectureJoin()" class="w-full max-w-xs font-semibold py-3 rounded-2xl text-white mb-3" style="background:${NAVY};">Join lecture</button>
        ...
        function toggleLectureControl(key){
          ...
          if (key === 'camOff' && liveLectureState.view === 'grid') {
            // Camera on/off is the one case that genuinely has to swap what's displayed (video vs. placeholder avatar), but it only needs to touch the local tile's...
        ...
        function toggleLectureHand(){
          ...
          // Only burst on the rising edge (hand going up), not when it's lowered again -- and reuse the exact same Reactions flow (local echo + broadcast) as the...
        ...
        // Re-publishes this participant's mute/camera/hand state via Presence so every other tile in the grid (see lectureGridHTML) shows accurate badges for...
        ...
        // Refreshes just the small overlay badges on the local camera tile (mute/camera-off/hand-raised icons) without ever touching the <video> element...
        ...
        // Refreshes only the control-bar button appearance (icon + active styling) for camera/mute/hand/more, in place, instead of re-rendering the whole...
        ...
        // Refreshes just the floating Reactions/More sheets (and their dimmed backdrop) without rebuilding the grid/whiteboard/slides stage sitting underneath...
        ...
        // ---- Mobile call bar: "Reactions" and "More" bottom sheets ---- Mirrors the reference: the main bar only shows End / Reactions / Mute / Raise hand /...
        ...
        // "T" button in the More sheet -- lets anyone (student or teacher) type a quick comment during the lecture that everyone else in the call sees too,...
        ...
        function toggleLectureResourcesPanel(){
          // On desktop, Class Pad lives in the static right-hand panel (matching Profile/Discover/Notebook/File) instead of the mobile-style slide-in overlay, so...
        ...
        // Reactions used to only ever be added to this participant's own lectureFloatingReactions and re-rendered locally -- nothing was ever sent over the...
        ...
        // `big` renders this burst larger than a normal tap reaction -- used for the raised-hand burst below so it stands out from a regular clap/heart/etc....
        function addLectureFloatingReaction(id, icon, name, big){
          lectureFloatingReactions.push({ id, icon, name, big: !!big });
          // Only touch the floating-bubble layer itself, not the whole lecture screen -- rerenderLectureScreen() replaces the entire #lecture-call-screen (video...
        ...
        // Comments typed via the "T" button -- same local-echo + broadcast pattern as reactions above, but shown as a short-lived text bubble (longer on-screen...
        ...
        function addLectureFloatingComment(id, text, name){
          ...
          // Same targeted update as reactions above -- avoid a full rerenderLectureScreen() so the video tiles don't shake.
        ...
        function toggleLectureWhiteboard(){
          // Only the teacher can open/close the whiteboard; students' screens just follow along via the broadcast below.
        ...
        function toggleLecturePresent(){
          // Match the pattern every other "More" sheet action uses (lectureAttachDocuments/lectureAttachMedia/toggleLectureWhiteboard): actually close the sheet...
          closeLectureSheets();
        ...
        // Re-renders the whole lecture screen, then re-wires whatever non-serializable bits (camera stream, canvas, pdf page) live in that view, since...
        ...
        // Rebuilds just the grid/whiteboard/slides stage (video tiles, canvas, etc.) in place, leaving the header, control bar, and floating reaction/comment...
        ...
        function endLecture(remoteEnded){
          ...
          // The teacher ending the lecture ends it for every student still on the call too -- broadcast it on the same lecture-signal channel before tearing our...
          if (isTeacher && !remoteEnded) broadcastLectureSignal({ type: 'end' });
          teardownLectureSignaling();
          inLectureCall = false;
          // Only the teacher ending the lecture actually ends it for the class.
          if (cls && isTeacher) {
          ...
          lectureAttachments = [];
          pendingAutoOpenAttachmentId = null;
          // A guest was never viewing this class's Stream/Classwork/People tabs to begin with (they aren't on its roster), so there's no classDetail to fall back...
          if (wasGuest) { closeOverlay(); return; }
          classDetailTab = 'stream';
          openOverlay('classDetail');
        }

        // ---- Zoom-style participant grid ---- Other participants' tiles now come from lecturePresence (who's actually in the call right now, tracked via...
        ...
        // The local camera tile is split into two independently-updatable pieces (media = video-or-avatar, badges = mute/camOff/hand overlay icons) so that...
        function lectureLocalMediaHTML(){
          ...
          // This tile is always you (the other participants get their own tiles elsewhere) -- show your real profile photo when the camera's off, same as the 1:1...
        ...
        // A single tile: the video/avatar square itself, plus a name caption rendered as its own row underneath (not overlaid on top of the video) -- so a name...
        ...
        // Just the mute/hand-raised badge markup for a remote participant's tile, split out so it can be patched in place (see updateLectureRemoteBadges)...
        ...
        // Patches each already-rendered remote tile's badge markup in place.
        ...
        function lectureGridHTML(){
          ...
          // The teacher's tile always leads the grid -- whether that's this participant's own local tile (handled by localTeacherFirst below) or one of the...
          ...
          // If I'm the teacher, my own tile is the host tile and belongs first.
          const orderedTiles = iAmTeacher ? (localTile + otherTiles) : (otherTiles + localTile);
          // On desktop, a fixed 2-column grid meant a single tile (just the lecturer, before any students joined) stretched to fill half the whole stage.
        ...
        // ---- Lecture realtime signaling (teacher -> students) ---- Reuses the same Supabase Realtime broadcast channel pattern as the 1:1 call signaling...
        ...
        // Called whenever Presence reports who's actually in the room right now (fires on join, leave, and periodic resync).
        function syncLecturePresence(){
          ...
          // Someone actually joining or leaving needs real tiles added or removed, so rebuild the stage.
          if (newlyJoined.length || anyoneLeft) {
            updateLectureStageContent();
          } else {
            updateLectureRemoteBadges();
          }
          // A student who joins (or reconnects) AFTER the teacher already switched to the whiteboard previously landed on the plain grid view and stayed there --...
        ...
        // One mesh connection per other participant.
        ...
        // Adds the local camera/mic stream to every already-open lecture peer connection.
        ...
        // Students (and any other participant) receive teacher-driven updates here: switching into/out of the whiteboard, and every stroke/undo/clear the...
        function handleLectureSignal(payload){
          ...
          // Reactions aren't teacher-authoritative state like the whiteboard/attachments below -- anyone can send one, and everyone (including the teacher)...
          if (payload.type === 'reaction') {
            addLectureFloatingReaction(payload.id, payload.icon, payload.name, payload.big);
            return;
          }
          // Comments, like reactions, are anyone-can-send and everyone (including the teacher) should see them.
          if (payload.type === 'comment') {
            addLectureFloatingComment(payload.id, payload.text, payload.name);
            return;
          }
          // The teacher just ended the lecture -- close it out for us too, the same as tapping "Leave" ourselves, instead of staying connected to a call the...
          if (payload.type === 'end') {
            endLecture(true);
          ...
          if (payload.type === 'view') {
            ...
            // If the file hasn't finished downloading yet, once it lands (see handleRemoteAttachmentAdd) it opens itself automatically.
            else pendingAutoOpenAttachmentId = payload.id;
          }
        }

        // Downloads a file another participant shared into the lecture and adds it to this screen's own Resources list, so it appears and opens exactly like...
        ...
        function handleWbAttachFile(event){
          ...
          // Stays on the whiteboard: this just attaches the file for reference, it does not switch the view over to the slide presenter.
        ...
        function initWhiteboardCanvas(){
          ...
          if (!canvas.dataset.wired && lectureIsTeacher()) {
            ...
            // The canvas itself gets recreated (and re-wired) on every whiteboard re-render -- switching tools, colors, etc. -- but `window` doesn't, so this one...
        ...
        function wbPointerDown(e){
          ...
          // Without this, the canvas (which can't itself hold focus) still runs its default mousedown handling right after this handler returns, which blurs...
        ...
        // Tapping the board with the "T" tool selected drops a real, editable text box right where you tapped -- a transparent- background input sitting on top...
        ...
        // Tapping the trash icon no longer wipes the whole board -- it arms "delete select" mode.
        ...
        // PPTX has no simple client-side visual renderer, so this reads the real slide XML out of the .pptx zip and rebuilds each slide's title + bullet text...
        ...
        // The floating Reactions/More sheets plus their dimming backdrop, pulled out into their own function so they can be refreshed in isolation (see...
        ...
        // ---- "Reactions" popover: a horizontal rounded pill of icons that floats just above the control bar (not a vertical sheet).
        ...
        // ---- "T" (comment) popover: a quick text box so anyone can type something during the lecture -- shown to every participant as a bubble over the stage...
        ...
        // ---- "More" bottom sheet (camera, whiteboard, present: everything that doesn't fit on the compact main bar) ----
        function lectureMoreSheetHTML(view){
          // Single inline row (icon + label stacked, no tile/box background) instead of a 2x2 grid of tiles, sitting directly under the Whiteboard pill.
        ...
        // ---- "More" sheet: attach a document, or a photo/video, to the lecture.
        ...
        // Adds the file to this participant's own screen immediately (so it opens with zero delay for whoever picked it), then uploads it to shared storage in...
        ...
        // Tapping a chip (not the ✕) opens that exact file so it can actually be viewed, live, by whoever is in the lecture: a PDF shows its pages, a PPTX its...
        function openLectureAttachment(id){
          const a = lectureAttachments.find(x => x.id === id);
          if (!a) return;
          lectureResourcesPanelOpen = false;
          openFilePreview(a.name, a.url, a.file, a.kind === 'photo' ? 'image' : a.kind === 'video' ? 'video' : null);
          // When the teacher opens a shared file, every student's screen should follow along and open the same file automatically.
          if (lectureIsTeacher()) broadcastLectureSignal({ type: 'attachment-open', id });
        }

        // ---- Resources panel: every file shared into this lecture (via the More > Documents/Photos menu, or attached straight to the whiteboard) lives here...
        ...
        // Same items list as the slide-in Resources panel, rendered inline instead -- used to surface lecture resources on the Class Pad (the desktop right...
        ...
        // Second Class Pad section: every file from the app-wide Resources library (Study > Resources -- PDFs/DOCX/PPTX the student has uploaded and had...
        ...
        // Opens an app-wide Resource in the same full-screen/right-panel preview lecture attachments use.
        ...
        // Keeps the Uploaded Resources section of every open Class Pad surface (desktop right panel and/or mobile slide-in panel) in sync as resources finish...
        ...
        // ---- Lecture file preview: opens any shared attachment (lecture "Documents"/"Photos & videos" chips, or the whiteboard's attached PDF/PPTX) as a...
        ...
        // Renders an opened document (lecture attachment or app-wide Resource) right over the call stage -- the same spot the video grid/whiteboard/slides...
        function renderLecturePreviewRegion(){
          const el = document.getElementById('lecture-preview-region');
          if (!el) return;
          el.innerHTML = lecturePreviewHTML();
          // Set the inline style directly (not just a class) -- the region's clickability is controlled via inline style on first render, and an inline style...
        ...
        // ---------------- CLASS-SCOPED NOTIFICATIONS ---------------- Unlike the app-wide Notice Board, this only surfaces items that belong to whichever...
        ...
        // Synchronous current-user id for keying per-student submissions -- _cachedAuthUser (core.js) is populated early in authEnterApp's boot sequence...
        function myClassworkUserId(){
          return (typeof _cachedAuthUser !== 'undefined' && _cachedAuthUser) ? _cachedAuthUser.id : null;
        }

        // Assignment/question submissions used to be a single `w.submission` field shared by the whole class -- fine for one student, but with a real class...
        ...
        // Small status pill: what state this piece of classwork is in for the current student (not submitted / turned in / graded / a quiz score) -- for a...
        ...
        function classClassworkTabHTML(cls){
          ...
          // Same gradient-banner-plus-motif card treatment as an Opportunities card (see jobCard in jobs.js) -- shades of blue only (blueCardPalette in core.js),...
          const row = w => {
        ...
        // Files picked here get an instant local blob preview (so the chip shows up immediately with no network wait) while the real bytes upload to Storage in...
        ...
        function submitNewAnnouncement(){
          ...
          // Only id/name/url are ever kept -- the raw File object can't survive being saved to Supabase's jsonb `data` column anyway, and by this point url...
        ...
        // ---------------- NEW CLASSWORK (Google Classroom-style) ---------------- classworkDraftType decides both the label on the creation form and what...
        let classworkDraftType = 'assignment';
        // Files picked via "Add attachment" while drafting an Assignment, Question, or Material.
        ...
        // ---- Draft attachments for the Assignment/Question/Material form: picking files here just stages them (chip strip below the form); they're only...
        ...
        function submitNewClasswork(){
          ...
          cls.classwork.unshift({
            ...
            submissions: {},
            // Only id/name/url are kept, same reasoning as submitNewAnnouncement -- the File object never survives the round trip to Supabase, and url should...
        ...
        // ---------------- NEW QUIZ (question builder) ---------------- Question blocks are built directly in the DOM (rather than kept in a JS array...
        ...
        function classworkAssignmentDetailHTML(w){
          ...
          if (isTeacher) {
            // Every student's submission is kept in its own slot (w.submissions[studentId]) instead of a single shared field, so the teacher sees and grades each...
          ...
          // Student view: turn work in, or see what was already turned in; the grade (once given) is shown read-only, never editable here.
          const mine = myClassworkSubmission(w);
        ...
        function submitClassworkAssignment(id){
          ...
          const now = new Date();
          // Keyed by student id (see classworkSubmissionsMap) instead of a single shared field -- otherwise one student turning work in would silently overwrite...
          ...
          // Without this, "Turn in" only ever updated this student's own local copy of `cls` -- it never reached Supabase, so the teacher (on their own device)...
          const cls = myClasses.find(c => c.id === currentClassId);
        ...
        function saveClassworkGrade(id, studentId){
          ...
          sub.graded = true;
          document.getElementById('overlay').innerHTML = classworkDetailHTML();
          // So the grade/remark the teacher just saved actually reaches the student instead of staying stuck on the teacher's device.
          const cls = myClasses.find(c => c.id === currentClassId);
          if (cls) queueSaveClassRemote(cls);
        }
        ...
        function classworkQuizDetailHTML(w){
          ...
          // Same per-student fix as the assignment view above: quiz submissions are keyed by student id (w.quizSubmissions[id]) instead of one shared field, so...
        ...
        function submitClassworkQuiz(id){
          ...
          document.getElementById('overlay').innerHTML = classworkDetailHTML();
          // Same fix as submitClassworkAssignment -- a quiz submission has to be pushed to Supabase for the teacher to ever see the score.
          const cls = myClasses.find(c => c.id === currentClassId);
          if (cls) queueSaveClassRemote(cls);
        }
        ...
        function submitPollVote(id){
          ...
          w.myVote = oi;
          document.getElementById('overlay').innerHTML = classworkDetailHTML();
          // Otherwise this vote only ever counted on the voter's own device -- everyone else's tally (and the teacher's) never moved, since it never made it to...
          const cls = myClasses.find(c => c.id === currentClassId);
          if (cls) queueSaveClassRemote(cls);
        }
        ...
        // A co-teacher invite is a lightweight, real piece of class state: it's stored on the class, shown in the Teachers list as "Invite pending", and can be...
        ...
        // Records a real class_invites row (see the SQL above) so that whenever the invited person actually signs in under this email, claim_class_invites()...
        async function recordClassInviteRemote(classId, email){
          // A Course's invite flow never grants direct membership -- see the "pending" branch below -- so there's nothing to record here for those; only plain...
          const sb = getSupabaseClient();
          if (!sb) return;
          try {
        ...
        async function submitInviteStudents(){
          ...
          emails.forEach(email => {
            ...
            // A Course works differently from a regular class: an invited learner doesn't get a roster spot (and the access that comes with it) right away.
            cls.students.push(cls.isCourse ? { name: nameGuess, email, pending: true } : { name: nameGuess, email });
            // For a real class (not a Course), also record a real invite so the person actually gets `members` access once they sign in under this address -- see...
        ...
        function cancelStudentInvite(classId, index){
          ...
          queueSaveClassRemote(cls);
          // Also clear the matching pending class_invites row (if any) so cancelling an invite in the UI actually stops that email from being granted access...
          if (removed && removed.email && !cls.isCourse) {
            const sb = getSupabaseClient();
            if (sb) {
              // Escape ilike's "%"/"_" wildcards -- email addresses routinely contain literal underscores/dots (see the same escaping in isUsernameTaken, profile.js)...
        ...
        // Finds the soonest upcoming occurrence of any class this week, rolling over to next week if every occurrence this week has passed.
        ...
        // In-app banner used whenever browser notifications aren't available or haven't been granted; keeps the "you will be notified" promise reliable even...
        ...
        function fireStudyNotification(title, body){
          // Real push -- reaches this same account on another device, or even this device once the tab is closed, unlike the in-tab Notification()/banner below...
        ...
        // Ticks every 30s, checking whether any class starts within the next ~10 minutes or any reminder is due within the next ~hour, and fires a one-time...
        ...
        // Same idea as checkStudyPlannerNotifications above, but covers anything scheduled inside a class: upcoming lectures (reminder ~10 min before start)...
