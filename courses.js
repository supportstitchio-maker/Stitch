          ...
          return `
            <div id="ai-msg-${id}" class="flex items-start">
              <div class="min-w-0" style="max-width:85%;">
                ...
                ${promptLimit ? `
                <button onclick="openOverlayFrom('aiClass','subscriptionSettings')" class="mt-2 w-full font-semibold py-2.5 rounded-full text-center text-[13.5px] text-white flex items-center justify-center gap-1.5" style="background:linear-gradient(135deg, ${NAVY} 0%, ${ROYAL} 100%);box-shadow:0 4px 14px rgba(65,105,225,0.35);">${Icon('crown','w-4 h-4')} Upgrade plan</button>` : ''}
              </div>
            </div>`;
        }
