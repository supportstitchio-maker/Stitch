        ...
        // idles through six random little animations, one arm/joint
        // motion at a time -- kicked off from openOverlay and
        ...
        // (handleAIChatInputBlur).
        //
        // The mascot itself is a real rigged 3D model (see robot3d.js --
        // window.StitchRobot3D), not swapped photos: shoulders, elbows,
        // wrists and fingers are actual jointed meshes that get rotated
        // every frame, cycling at random between six procedural
        // animations (wave, wave, leap, point, cheer x2).
        function startAIRobotAnimation(){
          const canvas = document.getElementById('ai-robot-canvas');
          if (!canvas || typeof StitchRobot3D === 'undefined') return;
          StitchRobot3D.mount(canvas);
        }

        function stopAIRobotAnimation(){
          if (typeof StitchRobot3D === 'undefined') return;
          StitchRobot3D.unmount();
        }
        ...
        // that session. It starts animating (see startAIRobotAnimation,
        // which mounts the 3D rig from robot3d.js into the canvas div
        // below) as soon as this screen is mounted, and steps aside the
        // moment the student taps the composer to type (see
        // hideAIRobot/showAIRobot, hooked into the composer's
        // focus/blur).
          ...
          return `
            <div class="h-full min-h-full flex flex-col items-center justify-center text-center px-8">
              <div id="ai-robot-container" class="ai-robot-container">
                <div id="ai-robot-canvas" class="ai-robot-canvas" role="img" aria-label="Stitch Bot"></div>
              </div>
            </div>`;
        }
