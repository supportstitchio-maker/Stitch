try {
        ...
        // Image/video attachments start out as blob: preview URLs (fast, local-only). Those blob
        // URLs die the moment the tab/session ends, so if a message carrying one gets archived
        ...
        async function getAIResponse(text, attachments, viaVoice){
          ...
          try {
            ...
            // (handy for debugging, but not something a student should ever see). It's still logged
        ...
        function courseDetailMenuDropdownHTML(courseId){
          return `
            <div class="absolute bg-white rounded-2xl border border-gray-100 py-2 z-20 menu-dropdown-inset" style="right:1.25rem;top:3.5rem;width:12rem;box-shadow:0 10px 30px rgba(0,0,0,.14);">
              ...
              <button onclick="courseDetailMenuOpen=false; openCourseEnrolledPeople('${courseId}')" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 menu-item-pill">${Icon('personPlus','w-4 h-4')} People</button>
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
              ...
              <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
                ...
                <div class="bg-gray-50 rounded-2xl p-4 text-xs text-gray-500 leading-relaxed mb-5">
                  By enrolling you agree to complete coursework honestly, follow ${escapeHtml(c.org)}'s classroom guidelines, and understand your progress may be visible to the course's organizers. You can end your enrollment at any time from the course page.
                </div>
        ...
        async function submitCourseEnroll(){
          ...
          if (!wasEnrolled) {
            ...
            if (c) {
              ...
              // Record who enrolled -- this is what powers the course's People view
              // (courseEnrolledPeopleHTML) for the course creator. Enrolling used to only ever bump a
            ...
            function(){
              ...
              if (c) {
                ...
                getCurrentUserId().then(myId => {
                  if (myId && c.enrolledUsers) c.enrolledUsers = c.enrolledUsers.filter(u => u.id !== myId);
                  postCourseInsertRemote(c);
                });
              }
            }
        ...
        function courseDetailHTML(){
          ...
          return `
            <div class="flex-1 flex flex-col overflow-hidden">
              ${isCurrentUserAdmin()
                ? menuOverlayHeader('Dashboard', courseDetailMenuOpen, 'toggleCourseDetailMenu', courseDetailMenuDropdownHTML(c.id))
                : overlayHeader(escapeHtml(c.title), '20px')}
              <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
                ${c.photo ? `<img src="${c.photo}" class="w-full rounded-3xl mb-4 object-cover" style="height:150px;margin-top:10px;" alt="">` : ''}
                <div class="flex items-center gap-2 mb-3">
                  <button onclick="openCourseEnrolledPeople('${c.id}')" class="text-xs font-bold uppercase tracking-wide text-gray-400 flex items-center gap-1">${escapeHtml(c.org)}</button>
        ...
        function returnToCourseEngage(courseId){
          currentCourseId = courseId;
          courseDetailTab = 'engage';
          openOverlay('courseDetail');
        }
        ...
        function confirmCourseAddModule(){
          ...
          const c = allCourses.find(x => x.id === courseAddModuleCourseId);
          if (!c) return;
          ...
          postCourseInsertRemote(c);
          returnToCourseEngage(c.id);
        }

        function courseAddModuleHTML(){
          const d = courseAddModuleDraft;
          return `
            ${overlayHeader('Add Module', '20px', 'cancelCourseAddModule()')}
            <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
              <div class="mb-4">
        ...
        // ---- Standalone "Add/Edit Part" page (video, picture, reading, assignment, project, or quiz) ----
        ...
        function deleteCourseItemPage(){
          ...
          openAppConfirmModal('Delete this item?', "This can't be undone.", 'Delete', function(){
            const c = allCourses.find(x => x.id === courseItemPageCourseId);
            if (!c) return;
        ...
        function courseAddItemPageHTML(){
          ...
          return `
            ${overlayHeader(isEditing ? 'Edit Part' : 'Add Part', '20px', 'cancelCourseItemPage()')}
            <div class="flex-1 overflow-y-auto no-scrollbar px-5 pb-8">
              <div class="mb-4">
              ...
              <button onclick="confirmCourseItemPage()" class="w-full flex items-center justify-center gap-2 rounded-2xl py-3 font-semibold text-sm text-white" style="background:linear-gradient(135deg, ${NAVY} 0%, ${ROYAL} 100%);box-shadow:0 4px 14px rgba(65,105,225,0.35);">${d.mediaUploading ? 'Uploading...' : (isEditing ? 'Save Changes' : 'Add')}</button>
              <div style="height:50px;"></div>
            </div>`;
        }
        ...
        function courseItemCardHTML(course, item){
          ...
          // card/shadow) -- matches every other list in the app, including this same item's own
          if (isBigMedia) {
            return `
        ...
        function courseEnrolledPeopleHTML(){
          ...
          if (!c) return `${overlayHeader('People', '20px')}<div class="p-5 text-center text-gray-400 text-sm">This course is no longer available.</div>`;
          const people = c.enrolledUsers || [];
          return `
            <div class="flex-1 flex flex-col overflow-hidden">
              ${overlayHeader('People', '20px')}
        ...
        // but students who already enrolled keep full access to it in their courses. Use this
        ...
        function newCourseHTML(){
          ...
          return `
            ${overlayHeader(isEditing ? 'Edit Course' : 'Create a Course', '20px', 'closeOverlay()', null, { center: true })}
            <div class="flex-1 overflow-y-auto px-5 pb-8">
              ...
              <div class="mb-4">
                ...
                <div class="w-full bg-gray-100 rounded-2xl px-4 py-3 text-sm text-gray-500">${TEAM_STITCH_TEACHER}${isEditing ? ' -- see who has enrolled from the course\'s People button' : ''}</div>
              </div>
              <div class="mb-5">
              ...
              ${d.modules.length ? d.modules.map((m, mi) => courseDraftModuleHTML(m, mi)).join('') : `
                <div class="bg-gray-50 border border-dashed border-gray-300 rounded-3xl p-5 text-center text-gray-500 text-sm mb-6">No modules yet -- add one from the course's Engage tab.</div>`}
        ...
        function courseDraftModuleHTML(m, mi){
          ...
          return `
            <div class="bg-white rounded-3xl p-4 shadow-sm mb-3 border border-gray-100">
              ...
              <textarea oninput="updateCourseDraftModuleDescription(${mi}, this.value)" placeholder="Module description (optional) -- what will students do in this module?" rows="2" class="w-full bg-gray-100 rounded-xl px-3 py-2 text-sm outline-none resize-none mb-2">${escapeHtml(m.description || '')}</textarea>
              <div class="text-xs text-gray-400">${count} part${count === 1 ? '' : 's'} -- manage parts, quizzes &amp; assignments from the Engage tab</div>
            </div>`;
        }
        ...
        function applyRemoteClassUpdate(payload){
          ...
          // need to know the moment someone is waiting on them -- a join request only they can
          // see and approve. Snapshot who was already pending/enrolled before the merge below so
          // we can tell what's actually new.
          ...
          if (isTeacherHere && typeof addNotif === 'function') {
            ...
            // instant the request lands -- this only ever fires in the teacher's own session, so a
            // requesting student never sees "their own" approval notice.
            ...
            // A class with an entrance fee (or an open join policy) lets someone in the moment
            // they pay/join, with no approval step -- still worth letting the teacher know someone
            // new enrolled, since that's the only sign they'll get that the roster changed.
        ...
        async function claimPendingClassInvites(){
          ...
          try {
            ...
            if (error) {
              ...
              // in Supabase yet -- see the SQL in supabase-fixes.sql. Not fatal: invited students just
        ...
        // Draft for the two class-creation options below: who can join
        // (everyone vs. teacher-approved), and -- admins only -- whether
        // there's an entrance fee (wired to Paystack, see startClassPayment).
        ...
        function setNewClassJoinPolicy(policy){
          newClassDraft.joinPolicy = policy;
          const el = document.getElementById('create-class-join-policy');
          if (el) el.outerHTML = newClassJoinPolicyHTML();
        }
        function setNewClassPaymentEnabled(enabled){
          newClassDraft.paymentEnabled = enabled;
          const el = document.getElementById('create-class-payment');
          if (el) el.outerHTML = newClassPaymentHTML();
        }
        function updateNewClassPaymentField(field, value){
          newClassDraft[field] = value;
        }

        function newClassJoinPolicyHTML(){
          const d = newClassDraft;
          return `
            <div id="create-class-join-policy" class="mb-5">
              <label class="text-xs font-semibold text-gray-500 mb-1 block">Who can join</label>
              <div class="flex gap-2">
                <button onclick="setNewClassJoinPolicy('open')" class="flex-1 px-3 py-2.5 rounded-2xl text-xs font-semibold text-center class-choice-pill ${d.joinPolicy==='open' ? 'is-selected' : ''}">Open to all with the code</button>
                <button onclick="setNewClassJoinPolicy('approved')" class="flex-1 px-3 py-2.5 rounded-2xl text-xs font-semibold text-center class-choice-pill ${d.joinPolicy==='approved' ? 'is-selected' : ''}">Approve each student</button>
              </div>
            </div>`;
        }

        // Admin-only: an entrance fee, charged via Paystack before someone
        // can join (see startClassPayment + verify-paystack-payment).
        function newClassPaymentHTML(){
          if (!isCurrentUserAdmin()) return '';
          const d = newClassDraft;
          return `
            <div id="create-class-payment" class="mb-5">
              <label class="text-xs font-semibold text-gray-500 mb-1 block">Entrance fee</label>
              <div class="flex gap-2 mb-3">
                <button onclick="setNewClassPaymentEnabled(false)" class="flex-1 px-3 py-2.5 rounded-2xl text-xs font-semibold text-center class-choice-pill ${!d.paymentEnabled ? 'is-selected' : ''}">Free for everyone</button>
                <button onclick="setNewClassPaymentEnabled(true)" class="flex-1 px-3 py-2.5 rounded-2xl text-xs font-semibold text-center class-choice-pill ${d.paymentEnabled ? 'is-selected' : ''}">Paid entrance</button>
              </div>
              ${d.paymentEnabled ? `
                <div class="flex gap-2">
          ...
          return `
            <div class="w-full px-5 pb-3 relative" style="padding-top:var(--top-safe-pad);">
              <div class="flex items-center justify-between">
                ...
                <button onclick="toggleJoinClassMenu()" class="w-8 h-8 flex items-center justify-center flex-shrink-0" style="color:${NAVY};">${Icon('dashes','w-5 h-5')}</button>
              </div>
          ...
          return `
            <div class="w-full px-5 pb-3 relative" style="padding-top:var(--top-safe-pad);">
              <div class="flex items-center justify-between">
                ...
                <button onclick="toggleCreateClassMenu()" class="w-8 h-8 flex items-center justify-center flex-shrink-0" style="color:${NAVY};">${Icon('dashes','w-5 h-5')}</button>
              </div>
            ...
            <div class="p-4 flex-1 overflow-y-auto">
              ...
              <input type="text" id="create-subject-input" placeholder="e.g. Biology" class="w-full bg-gray-100 rounded-2xl px-4 py-3 text-sm mb-4">
              <label class="text-xs font-semibold text-gray-500 mb-1 block">Description (optional)</label>
              <textarea id="create-description-input" rows="3" placeholder="What's this class about?" class="w-full bg-gray-100 rounded-2xl px-4 py-3 text-sm mb-5 resize-none"></textarea>
              ${newClassJoinPolicyHTML()}
              ${newClassPaymentHTML()}
        ...
        async function submitJoinClassroom(){
          ...
          if (result.status === 'joined' || result.status === 'already_member') {
            ...
            runClassActionLoading('Adding you to class', 'users', async () => {
              // The RPC's own returned row is enough to open the class right away (above), but it
              // can be a beat ahead of the row Postgres will actually let us SELECT as a newly-added
              // member (RLS only starts allowing full reads once the membership insert has truly
              // landed). Re-pull this one class from the server before opening it so classwork,
              // lectures, and the member list are all the real thing immediately -- not the leaner
              // RPC snapshot -- and nobody has to sign out and back in to see the rest of the class.
              try {
              ...
              // again later this session shouldn't repeat it -- see preparedClassIds in study.js.
        ...
        // Mirrors the existing course-payment flow in jobs.js (same
        // PAYSTACK_PUBLIC_KEY, loadPaystackScript, and access-token/apikey
        // header pattern) -- verification happens server-side in a
        // separate function since this gates the classes table, not
        // opportunities (see verify-class-payment in
        // supabase-classroom-updates.sql's accompanying edge function).
        //
        // NOTE: the deployed Supabase Edge Function's URL slug is "bright-function" (its
        // auto-generated name from when it was created in the dashboard), while its display
        // name/title is "verify-class-payment" -- the two are independent in Supabase. The
        // fetch below must use the slug ("bright-function"), not the display name, or every
        // request 404s before it ever reaches the function (which shows up to the browser as a
        // bare "Failed to fetch", since a 404 on this route doesn't carry CORS headers). If the
        // function is ever redeployed under the slug "verify-class-payment" instead, update the
        // URL below to match.
        //
        // The verification call can legitimately fail to even reach the server for a moment
        // (e.g. the connection blips while the browser hands control back from the Paystack
        // popup, or the edge function is cold-starting) -- that's transient, not a declined
        // payment, and it should never look like a dead end to someone who was just charged.
        // So: retry quietly in the background (surviving a reload/app-kill via a small pending
        // record) instead of interrupting with a "Retry / Cancel" prompt. The person only ever
        // sees something once we have a definite answer -- a friendly "you're in" toast on
        // success, or (only for a real, non-network decline) a single dismissible notice.
        ...
        // Resumes a payment that never got confirmed before the tab closed/reloaded -- called once
        // on script load. Runs fully in the background: no loading UI, no error modal, ever.
        ...
        // Joining by code for a class that charges an entrance fee used to jump straight into
        // the Paystack popup the moment the RPC came back "payment_required" -- someone could be
        // charged before they'd even seen which class this was or what it cost. This screen sits
        // in between: it shows the class (name + section/subject, and its photo if it has one)
        // and the fee up front, and only "Continue to Pay" below launches startClassPayment;
        // "Cancel" (or the back button) returns to the Join a Class screen with nothing charged.
          ...
          return `
            ...
            <div class="w-full px-5 pb-3 relative flex-shrink-0" style="padding-top:var(--top-safe-pad);">
              <div class="flex items-center justify-between">
                ...
                <h1 class="text-base font-bold text-[${NAVY}] font-display absolute left-1/2 -translate-x-1/2 truncate" style="max-width:60%;">Entrance Fee</h1>
        ...
        function startClassPayment(classId, className, amount, currency, joinedByCode){
          ...
          loadPaystackScript().then(() => {
            ...
            const handler = window.PaystackPop.setup({
              ...
              ref: reference,
              metadata: { class_id: classId, class_name: className },
        ...
        // person navigated away) -- suppresses the loading overlay and, on failure, suppresses all
        // UI entirely so nothing pops up unexpectedly while they're doing something else.
        // opts.cycle: which slow-retry cycle we're on, once the quick attempts are exhausted.
        async function verifyClassPayment(reference, classId, attempt, opts){
          ...
          try {
            ...
            if (typeof preparedClassIds !== 'undefined') preparedClassIds.add(found.id);
            if (opts.silent) {
              // The person isn't sitting in a loading screen waiting for this -- just let them know
              // it landed. Non-blocking, self-dismissing (see openAppAlertModal), no action required.
            ...
            // A bare "Failed to fetch" (or not being able to read a session yet, right after
            // launch) means the request never really got a chance to succeed or fail -- that's
            // transient, not a real verification outcome, so keep retrying quietly rather than
            ...
            if (isNetworkError && cycle < CLASS_PAYMENT_SLOW_CYCLES) {
              // Quick retries didn't land it -- back off and keep trying in the background for as
              // long as the tab stays open, without ever interrupting the person to tell them so.
              classPaymentBusy = false;
            ...
            if (isNetworkError) {
              // Exhausted every quiet retry this session. Leave the pending record in place (it'll
              // resume automatically next launch, up to 48h) instead of surfacing an alarming
              // "payment not confirmed" dialog -- silence beats a scary false alarm here.
              return;
            }
            // A real, non-network decline (bad reference, actual verification failure) -- only worth
            // surfacing if the person is actively watching this happen.
            clearPendingClassPayment(reference);
            if (!opts.silent) {
              resetClassPaymentConfirmBtn();
        ...
        async function submitCreateClassroom(){
          ...
          // Admins can gate a class behind a Paystack entrance fee; this is
          // enforced server-side too (see join_class_by_code in
          // supabase-classroom-updates.sql), this is just what gets shown.
          const paymentEnabled = isCurrentUserAdmin() && !!newClassDraft.paymentEnabled;
        ...
        // ---- Student roster photos (People tab) ---- Joined students carry their real account id
        // in s.id (pending invites don't yet), so once someone has joined we can batch-fetch their
        let classStudentProfiles = {};
        async function loadClassStudentProfiles(cls){
          if (!cls) return;
          // Include cls.members too, not just cls.students -- a student who joined an open class,
          // was approved, or paid the entrance fee lands in members straight away and may not have
          // a matching cls.students entry yet, but still needs their name/photo resolved for the
          // People tab's merged roster (see classRosterList).
        ...
        // Fetches a single class row live (used when a ring/notification references a class
        // this device hasn't loaded into myClasses yet -- e.g. just approved or just paid its
        // way in earlier this session). Lets someone join a live lecture immediately instead of
        // being told to reopen/re-log-in to the app.
        ...
        async function acceptIncomingLectureCall(){
          ...
          if (!cls) {
            // Don't dead-end here: fetch the class live and join right away, so being mid-way
            // through something else -- or having just been let into the class -- never means
            // logging out and back in before you can get into the lecture.
        ...
        async function connectLectureCall(classId, lectureId){
          ...
          // contribution; the teacher (the one running the lecture) joins unmuted since they're the
        ...
        // Tapping "Back" during a live lecture used to call endLecture() outright -- one
        // accidental tap on the way to checking chat or another tab would hang up (or, for a
        ...
        function lectureCommentSheetHTML(){
          ...
          // the lecture screen's inner wrapper -- that's what lets syncLectureCommentKeyboardInset()
          return `
        ...
        function newAnnouncementHTML(){
          ...
          return `
            ...
            <div class="flex-shrink-0 flex justify-end" style="padding:0 1.25rem calc(env(safe-area-inset-bottom, 0px) + 16px);">
              <button onclick="submitNewAnnouncement()" ${canPost ? '' : 'disabled'} id="announcement-create-btn" class="font-semibold text-sm px-6 py-3 rounded-full flex-shrink-0 shadow-lg ${canPost ? 'text-white' : 'text-gray-400 bg-gray-100'}" style="${canPost ? `background:linear-gradient(135deg, ${NAVY} 0%, ${ROYAL} 100%);box-shadow:0 4px 14px rgba(65,105,225,0.35);` : ''}">Create</button>
            </div>`;
        }
        ...
        function newClassworkHTML(){
          ...
          return `
            ...
            <div class="flex-shrink-0 flex justify-end" style="padding:0 1.25rem calc(env(safe-area-inset-bottom, 0px) + 16px);">
              <button onclick="submitNewClasswork()" id="classwork-create-btn" class="font-semibold text-sm px-6 py-3 rounded-full flex-shrink-0 shadow-lg text-gray-400 bg-gray-100">Create</button>
            </div>`;
        }
        ...
        function newQuizHTML(){
          ...
          return `
            ...
            <div class="flex-shrink-0 flex justify-end" style="padding:0 1.25rem calc(env(safe-area-inset-bottom, 0px) + 16px);">
              <button onclick="submitNewQuiz()" class="font-semibold text-sm px-6 py-3 rounded-full flex-shrink-0 shadow-lg text-white" style="background:linear-gradient(135deg, ${NAVY} 0%, ${ROYAL} 100%);box-shadow:0 4px 14px rgba(65,105,225,0.35);">Create</button>
            </div>`;
        }
        ...
        function newPollHTML(){
          ...
          return `
            ...
            <div class="flex-shrink-0 flex justify-end" style="padding:0 1.25rem calc(env(safe-area-inset-bottom, 0px) + 16px);">
              <button onclick="submitNewPoll()" class="font-semibold text-sm px-6 py-3 rounded-full flex-shrink-0 shadow-lg text-white" style="background:linear-gradient(135deg, ${NAVY} 0%, ${ROYAL} 100%);box-shadow:0 4px 14px rgba(65,105,225,0.35);">Create</button>
            </div>`;
        }
        ...
        // in cls.students (the roster the People tab used to render from) when the teacher invited
        // them by email first. That left paid/enrolled students invisible on their own roster --
        // this builds the People tab's student list from members + students together instead, so
        // every enrolled student (paid entrance or not) is always visible to the teacher.
          ...
          return `
            ...
            <div class="p-4 flex-1 overflow-y-auto no-scrollbar">
              ...
              <div class="bg-white rounded-3xl p-5 mb-5 shadow-sm">
                ...
                <input type="text" id="class-room-input" placeholder="e.g. Lecture Hall 3" class="w-full bg-gray-100 border-2 border-gray-200 rounded-2xl px-4 py-3 text-sm mb-5">
            ...
            <div class="p-4 flex-1 overflow-y-auto no-scrollbar">
              ...
              <div class="bg-white rounded-3xl p-5 mb-5 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-3 mb-5">
                  <div>
                    <label class="text-xs font-semibold text-gray-500 mb-1 block">Date</label>
                    <input type="date" id="reminder-date-input" class="w-full bg-gray-100 border-2 border-gray-200 rounded-2xl px-4 py-3 text-sm">
                  </div>
                  <div>
                    <label class="text-xs font-semibold text-gray-500 mb-1 block">Time</label>
        ...
        function reminderRow(r){
          const typeColor = r.type === 'Exam' ? 'bg-rose-100 text-rose-700 border-2 border-rose-300' : r.type === 'Deadline' ? 'bg-amber-100 text-amber-700 border-2 border-amber-300' : 'bg-sky-100 text-sky-700 border-2 border-sky-300';
          return `
            <div class="bg-white rounded-3xl p-4 flex items-center gap-3 mb-3 shadow-sm">
              ...
              <div class="flex-1 min-w-0">
                ...
                <div class="text-xs text-gray-500 bg-gray-50 border-2 border-gray-200 rounded-full px-2 py-0.5 inline-block mt-1">${formatReminderDate(r.date)} · ${formatTime12(r.time)}</div>
              </div>
