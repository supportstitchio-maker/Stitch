const overlayBackKinds = ['discover', 'create', 'tagPeoplePicker', 'aiClass', 'conversation', 'addToCall', 'incomingCall', 'incomingLectureCall', 'joinClassroom', 'createClassroom', 'classDetail', 'inviteStudents', 'newAnnouncement', 'scheduleLecture', 'classworkCreateMenu', 'newClasswork', 'newQuiz', 'newPoll', 'classworkDetail', 'classSettings', 'studyTimetable', 'studyReminders', 'classAnnouncements', 'classNotifications', 'gamification', 'profileMenu', 'profileQR', 'newMessage', 'myContacts', 'newCollaboration', 'profileAnalytics', 'careerAnalytics', 'notifications', 'notificationSettings', 'savedItems', 'blockedAccounts', 'referrals', 'termsOfService', 'privacyPolicy', 'helpCenter', 'contactUs', 'reportIssue', 'practiceTests', 'examTake', 'jobDetail', 'jobApply', 'postOpportunity', 'courseDetail', 'courseItemDetail', 'courseEnroll', 'courseTeachers', 'newCourse', 'courseAddModule', 'courseAddItemPage', 'personProfile', 'personProfileQR', 'personNetwork', 'postFeed', 'careerStart', 'careerMatches', 'careerMatching', 'myFullProfile'];
        ...
        function personProfileDropdownMenuHTML(p){
          const items = [];
          if (p.connected) {
            items.push(`<button onclick="togglePersonProfileMenu(); messageViewedProfile();" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 menu-item-pill">${Icon('mail','w-4 h-4')} Message</button>`);
          } else if (p.requestSent) {
            items.push(`<button onclick="togglePersonProfileMenu(); cancelConnectionRequestWithViewedProfile();" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 menu-item-pill">${Icon('close','w-4 h-4')} Cancel request</button>`);
          } else {
            items.push(`<button onclick="togglePersonProfileMenu(); connectWithViewedProfile();" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 menu-item-pill">${Icon('personPlus','w-4 h-4')} Connect</button>`);
            items.push(`<button onclick="togglePersonProfileMenu(); messageAndConnectViewedProfile();" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 menu-item-pill">${Icon('mail','w-4 h-4')} Message</button>`);
          }
          ...
          return `
            <div class="px-5 pb-3 flex items-center justify-between relative" style="padding-top:var(--top-safe-pad);">
              ...
              <div class="flex items-center rounded-full" style="background:rgba(65,105,225,0.08)">
                <button onclick="togglePersonProfileMenu()" class="w-8 h-8 flex items-center justify-center">${gradIcon(IconBold('dashes','w-4 h-4'))}</button>
              </div>
