        ...
        function openOverlay(kind){
          ...
          if (kind === 'discover') { discoverSearchQuery = ''; discoverPeopleLoaded = false; ov.innerHTML = discoverHTML(); loadDiscoverPeople(); }
            ...
            selectedMediaHtml = null; selectedMediaType = null; selectedMediaFile = null;
            composeTaggedUsers = []; composeTagPickerOpen = false; composeTagSearchQuery = '';
        ...
        let composeTagPickerOpen = false;
        let composeTagSearchQuery = '';
          ...
          return `
            ...
            <div id="create-post-scroll" class="p-5 flex-1 overflow-y-auto no-scrollbar">
              ...
              <div class="relative mb-2">
                <textarea id="new-post-text" oninput="onComposeTextInput()" class="w-full h-24 p-4 border border-gray-200 rounded-2xl text-base" placeholder="Write a caption... Type @ to tag someone"></textarea>
                <div id="compose-mention-suggestions" class="hidden absolute left-2 right-2 bg-white rounded-2xl border border-gray-100 py-1 z-20" style="top:100%;margin-top:4px;overflow-y-auto;overscroll-behavior:contain;box-shadow:0 10px 30px rgba(0,0,0,.14);"></div>
              </div>
              <button type="button" onclick="toggleTagPeoplePicker()" class="text-xs font-semibold flex items-center gap-1.5 mb-2" style="color:${ROYAL}">${Icon('users','w-4 h-4')} Tag people</button>
              <div id="compose-tagged-chips" class="flex flex-wrap gap-2 ${composeTaggedUsers.length ? 'mb-3' : ''}">${composeTaggedChipsHTML()}</div>
              <div id="compose-tag-picker" class="${composeTagPickerOpen ? '' : 'hidden'} mb-3 bg-gray-50 rounded-2xl p-3">
                <input id="compose-tag-search" value="${escapeHtml(composeTagSearchQuery)}" oninput="onComposeTagSearchInput(this.value)" placeholder="Search people to tag" class="w-full border border-gray-200 rounded-2xl px-3 py-2 text-sm mb-2" autocomplete="off">
                <div id="compose-tag-picker-list" class="max-h-40 overflow-y-auto space-y-1"></div>
              </div>
            </div>
        ...
        // Sizes the "@" mention dropdown so its white/shadow box always
        // reaches down to the divider line just above the Post pill
        // (footer), instead of stopping at a fixed height and leaving
        // later suggestions to render with no background behind them.
        // Also why the box itself scrolls (overflow-y-auto +
        // overscroll-behavior:contain) rather than the page: once its
        // own scroll hits an end, overscroll-behavior:contain stops that
        // scroll gesture from "chaining" into the outer #create-post-scroll
        // container, which is what was dragging the whole New Post screen
        // down before.
        function positionComposeMentionBox(){
          const box = document.getElementById('compose-mention-suggestions');
          const footer = document.getElementById('create-post-footer');
          if (!box || !footer) return;
          const boxTop = box.getBoundingClientRect().top;
          const footerTop = footer.getBoundingClientRect().top;
          const available = footerTop - boxTop - 12;
        ...
        // Opens/closes the "Tag people" picker panel below the caption --
        // a simple searchable list (reusing discoverPeople) as an
        // alternative to typing "@" directly in the caption.
        function toggleTagPeoplePicker(){
          composeTagPickerOpen = !composeTagPickerOpen;
          const el = document.getElementById('compose-tag-picker');
          if (el) el.classList.toggle('hidden', !composeTagPickerOpen);
          if (composeTagPickerOpen) {
            if (!discoverPeopleLoaded) loadDiscoverPeople();
            renderComposeTagPickerList();
          }
        }

        function onComposeTagSearchInput(val){
          composeTagSearchQuery = val;
          renderComposeTagPickerList();
        }

        function renderComposeTagPickerList(){
          const list = document.getElementById('compose-tag-picker-list');
          if (!list) return;
          if (!discoverPeopleLoaded) { list.innerHTML = `<div class="text-center text-gray-400 text-xs py-4">Loading people...</div>`; return; }
          const q = composeTagSearchQuery.trim().toLowerCase();
          ...
          if (!filtered.length) { list.innerHTML = `<div class="text-center text-gray-400 text-xs py-4">No one found.</div>`; return; }
          list.innerHTML = filtered.map(p => {
            const tagged = composeTaggedUsers.some(u => u.id === p.id);
            const avatarInner = p.photo ? `<img src="${p.photo}" class="w-full h-full object-cover">` : Icon('user','w-3.5 h-3.5');
            return `
              <button type="button" onclick="toggleComposeTagPerson('${escapeForJsAttr(p.id)}')" class="w-full flex items-center gap-2.5 px-2 py-2 rounded-xl text-left ${tagged ? 'bg-blue-50' : ''}">
                <div class="w-7 h-7 rounded-full bg-blue-50 overflow-hidden flex items-center justify-center flex-shrink-0">${avatarInner}</div>
                <div class="flex-1 min-w-0">
                  <div class="text-xs font-semibold truncate">${escapeHtml(p.name)}</div>
                  ${p.username ? `<div class="text-[11px] text-gray-400 truncate">@${escapeHtml(p.username)}</div>` : ''}
                </div>
        ...
        function removeComposeTag(id){
          ...
          refreshComposeTagChips();
          if (composeTagPickerOpen) renderComposeTagPickerList();
        }

        // Drops "@username " into the caption at the end of whatever's
        // already typed -- used when a tag comes from the "Tag people"
        // picker rather than typed inline (see applyMentionSuggestion for
        // the inline-typing path, which inserts at the cursor instead).
        function insertMentionIntoCaption(username){
          const ta = document.getElementById('new-post-text');
          if (!ta) return;
          const mention = '@' + username;
          const text = ta.value;
          const needsSpaceBefore = text.length && !/\s$/.test(text);
          ta.value = text + (needsSpaceBefore ? ' ' : '') + mention + ' ';
        }

        // Live "@" mention autocomplete while typing the caption: looks
        // at the text immediately before the cursor for an in-progress
        // "@handle" and, if found, shows matching people below the
        // textarea (see createPostHTML's #compose-mention-suggestions).
        function onComposeTextInput(){
          ...
          if (!match) { box.classList.add('hidden'); box.innerHTML = ''; return; }
          if (!discoverPeopleLoaded) { loadDiscoverPeople(); }
          const query = match[2].toLowerCase();
          const candidates = discoverPeople.filter(p => (p.username || '').trim());
          const filtered = (query ? candidates.filter(p => p.username.toLowerCase().startsWith(query)) : candidates).slice(0, 6);
          if (!filtered.length) { box.classList.add('hidden'); box.innerHTML = ''; return; }
          box.classList.remove('hidden');
          positionComposeMentionBox();
          box.innerHTML = filtered.map(p => {
            const avatarInner = p.photo ? `<img src="${p.photo}" class="w-full h-full object-cover">` : Icon('user','w-3 h-3');
            return `
              <button type="button" onclick="applyMentionSuggestion('${escapeForJsAttr(p.id)}')" class="w-full flex items-center gap-2 px-3 py-2 text-left">
                <div class="w-6 h-6 rounded-full bg-blue-50 overflow-hidden flex items-center justify-center flex-shrink-0">${avatarInner}</div>
                <div class="min-w-0">
                  <div class="text-xs font-semibold truncate">${escapeHtml(p.name)}</div>
                  <div class="text-[11px] text-gray-400 truncate">@${escapeHtml(p.username)}</div>
                </div>
              </button>`;
          }).join('');
        }

        // Replaces the in-progress "@handle" right before the cursor with
        // the full "@username " for whoever was tapped in the
        // autocomplete list, and records them as tagged.
        function applyMentionSuggestion(id){
          const p = discoverPeople.find(x => x.id === id);
          const ta = document.getElementById('new-post-text');
          const box = document.getElementById('compose-mention-suggestions');
          if (ta && p) {
            ...
            const newBefore = before.replace(/(^|\s)@([A-Za-z0-9._]*)$/, (m, pre) => `${pre}@${p.username} `);
            ta.value = newBefore + after;
            const newCursor = newBefore.length;
            ta.focus();
            if (ta.setSelectionRange) ta.setSelectionRange(newCursor, newCursor);
          }
          if (box) { box.classList.add('hidden'); box.innerHTML = ''; }
          if (p && !composeTaggedUsers.some(u => u.id === id)) {
            composeTaggedUsers.push({ id: p.id, name: p.name, username: p.username });
            refreshComposeTagChips();
          }
        }
        ...
        function submitPost(){
          ...
          PostsAPI.create({
            ...
            composeTaggedUsers = [];
            composeTagPickerOpen = false;
            composeTagSearchQuery = '';
            closeOverlay();
            renderFeed();
          });
        }
        ...
        function personProfileHTML(){
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto p-5">
              ...
              ${theirPosts.length
                ? `<div class="profile-thumb-grid">${theirPosts.map(p => profileGridItem(p, 'author:' + (p.authorId != null ? p.authorId : ''))).join('')}</div>`
        ...
        function notifTap(id){
          ...
          if ((n.type === 'post' || n.type === 'comment' || n.type === 'like' || n.type === 'repost' || n.type === 'tag') && n.postId != null && typeof openComments === 'function') {
            n.read = true;
            queueSaveUserState();
            closeOverlay();
            switchTab(0);
