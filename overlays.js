        ...
        function openOverlay(kind){
          ...
          // snapping straight into place -- only while a conversation is
          // open, so other overlay kinds that also move ov.style.bottom
          // around (call, right-panel toggles, etc.) aren't slowed down.
          ov.style.transition = kind === 'conversation' ? 'bottom .3s cubic-bezier(.22,.68,0,1)' : '';
        ...
        function closeOverlay(fromPopState){
          ...
          if (currentOverlayKind === 'examTake' && typeof examStage !== 'undefined' && examStage === 'take'
            ...
            finishExam(true);
            // The score is now locked in, but this path (unlike the
            // on-screen "Exit" button or the hardware back button) never
            // shows the result screen -- it jumps straight back to
            // whatever the person tapped instead. Surface the score in a
            // brief toast first so it isn't silently thrown away from
            // their point of view, then let the rest of this function
            // continue closing/navigating as normal.
            if (typeof examScore === 'function' && typeof pushInAppNotification === 'function') {
              const total = examTest.questions.length;
              const correct = examScore();
              pushInAppNotification('Test exited', `Your score: ${correct}/${total}. It's been saved to Practice Tests.`);
            }
          }
          ...
          return `
            <div class="w-full px-5 pb-3 relative" style="padding-top:20px;">
              <div class="flex items-center justify-between">
                ${backBtn}
                <h1 class="${titleClass}" style="max-width:${titleMaxWidth};">${title}</h1>
                <button onclick="${toggleFnName}()" class="w-8 h-8 flex items-center justify-center flex-shrink-0">${gradIcon(menuIcon)}</button>
              </div>
        ...
        function notifTap(id){
          ...
          if ((n.type === 'post' || n.type === 'comment' || n.type === 'like' || n.type === 'repost') && n.postId != null && typeof openComments === 'function') {
            n.read = true;
            queueSaveUserState();
            closeOverlay();
            switchTab(0);
        ...
        function renderClassAnnouncementsTab(){
          ...
          ov.innerHTML = `
            <div class="overflow-y-auto no-scrollbar flex-1 bg-gray-50">
              ${menuOverlayHeader('Notice Board', noticeBoardMenuOpen, 'toggleNoticeBoardMenu', notifActionsDropdownHTML('handleNoticeAction'), { hideBack: true, boldMenuIcon: true, titleLeft: true, titleSize: 'text-3xl' })}
              <div class="p-5">
                ${noticeSelected.size ? `
                <div class="flex items-center justify-between pb-4 mb-4 border-b border-gray-200">
