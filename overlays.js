        ...
        const overlayBackKinds = ['aiClass', 'gamification', 'classAnnouncements', 'classNotifications', 'notifications', 'notificationSettings', 'savedItems', 'blockedAccounts', 'termsOfService', 'privacyPolicy', 'helpCenter', 'contactUs', 'reportIssue', 'newMessage', 'newCollaboration', 'studyTimetable', 'studyReminders', 'practiceTests', 'examTake', 'profileQR', 'postOpportunity', 'jobDetail', 'jobApply', 'classDetail', 'inviteStudents', 'newAnnouncement', 'newClasswork', 'scheduleLecture', 'courseDetail', 'courseItemDetail', 'newCourse', 'personProfile', 'personNetwork'];
        ...
        function closeOverlay(fromPopState){
          // Bug fix: activeConvoId used to stay set to whatever thread was
          // last opened, even after backing out to the inbox list. That
          // made handleIncomingMessage() (chat.js) think the conversation
          // was still open, so new messages arriving while you were just
          // sitting on the inbox -- not the thread -- got silently marked
          // read instead of showing up as unread. Clear it here, but only
          // when we're actually closing the conversation window itself --
          // desktop split view swaps straight from one open conversation to
          // another via openConversation() without ever calling
          // closeOverlay(), so this doesn't affect that flow.
          if (currentOverlayKind === 'conversation' && typeof activeConvoId !== 'undefined') activeConvoId = null;
        ...
        async function removeConnectionWithViewedProfile(){
          if (!viewedProfile || !viewedProfile.connected) return;
          const id = viewedProfile.id;
          if (!confirm(`Remove ${viewedProfile.name || 'this person'} from your network?`)) return;
