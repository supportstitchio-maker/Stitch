        ...
        function closeOverlay(fromPopState){
          ...
          // Opening a post fully (see openPostDetail in feed.js) turns off
          // the Home feed's scroll-triggered video-autoplay observer for
          // as long as that full-screen view is up, so nothing behind it
          // can start playing on its own -- closing back out to the Home
          // feed this way (its own back arrow/hardware back, rather than
          // switchTab, which already re-arms it via renderFeed) needs to
          // turn it back on again, or scrolling the feed afterward would
          // never autoplay another video until the next full Home
          // re-render.
        ...
        // selectedMediaItems. Once the whole batch has finished being
        // read as data URLs, any photos in it (not videos -- see
        // beginMediaEditQueue) are routed one at a time through the Edit
        // Photo screen (crop/reposition/zoom) before landing back here,
        // same as tapping a photo on Instagram walks you through an edit
        // step before you get to add a caption and post.
        ...
        // ---------------- EDIT PHOTO (crop before posting) ----------------
        // A photo picked for a post is routed through this full-screen
        // editor -- reposition/zoom it within a chosen aspect ratio --
        // before landing back on the New Post screen to add a caption.
        // Videos skip this (client-side video cropping isn't supported)
        // and go straight into the preview strip as before.
        ...
        function openMediaEditorStep(){
          if (mediaEditIndexInQueue >= mediaEditQueue.length) {
            mediaEditQueue = [];
            syncSelectedMediaMirror();
            // Return to the New Post screen directly instead of through
            // openOverlay('create') -- that branch wipes selectedMediaItems,
            // the caption, and tags to start a fresh post (it's also what
            // the "+" nav button uses), which would throw away everything
            // just picked/edited/typed so far. Same approach as
            // closeTagPeoplePicker uses to return from Tag People.
        ...
        function mediaEditFrameAspectCss(){
          ...
          // 'original' -- matches the photo's own aspect ratio once its
          // natural size is known (initMediaEditTransform sets this on
          // the frame directly once the <img> has loaded); until then,
          // fall back to a square so the frame doesn't collapse to 0
          // height for the split second before that.
          if (mediaEditNatural.w && mediaEditNatural.h) return `aspect-ratio:${mediaEditNatural.w}/${mediaEditNatural.h};`;
          return 'aspect-ratio:1/1;';
        }

        // CSS aspect-ratio alone (the width:100%;max-width:420px;aspect-
        // ratio:... the frame starts with, see editMediaHTML) only ever
        // constrains the frame's *width* -- its height just follows from
        // the ratio, with nothing capping it. A very tall "Original"-
        // aspect photo (a phone screenshot, a long portrait shot, etc.)
        // could end up with a computed height taller than the space
        // actually available for it, pushing the aspect-ratio tabs and
        // zoom slider below it (both flex-shrink:0 siblings in the same
        // column) off the bottom of the screen entirely -- there's no
        // scroll here to reach them. This measures the actual available
        // box (the frame's own parent, which is the flexible middle
        // section between the header and the controls) and sets the
        // frame's width/height in real pixels so it always fits inside
        // that box -- exactly like object-fit:contain, just computed by
        // hand since it's the frame's own size being fit, not an image
        // inside a fixed frame. Called after every aspect-ratio switch
        // and once the photo's natural size is first known.
        ...
        function setMediaEditAspect(key){
          ...
          // The frame's rendered size only settles after this style
          // change is painted, so wait a frame before recomputing the
          // fit/cover scale/clamp -- doing it synchronously would measure
          // the frame's old (pre-switch) dimensions.
          requestAnimationFrame(() => { fitMediaEditFrame(); applyMediaEditTransform(); });
        }

        // Called once the picked photo's <img> has actually loaded, so
        // its real pixel dimensions are known (needed both to size the
        // "Original" frame and to compute the cover scale below).
        ...
        // Sizes and positions the <img> so it always fully covers the
        // crop frame (like CSS object-fit:cover) scaled further by
        // mediaEditZoom, then shifted by mediaEditPan -- clamped so the
        // photo's edges can never pull in past the frame and leave a gap.
        ...
        // "Cancel" leaves this one photo exactly as picked (uncropped)
        // and moves on -- there's already a separate way to drop a photo
        // entirely (the x on its thumbnail back on the New Post screen),
        // so Cancel here only backs out of *editing*, not the photo itself.
        function skipMediaEdit(){
          mediaEditIndexInQueue += 1;
          openMediaEditorStep();
        }

        // Bakes the current crop (aspect ratio + zoom + pan) into an
        // actual cropped image via canvas, replacing both this item's
        // preview url AND its file (submitPost/PostsAPI.create upload
        // item.file, not item.url -- item.url is only the local preview,
        // so leaving the original file in place here would crop the
        // on-screen preview but still post the uncropped original), then
        // moves to the next photo in the queue (or back to New Post once
        // the queue is empty).
        function doneMediaEdit(){
          ...
          try {
            ...
            // Map the visible frame back into the source photo's own
            // pixel space: its center sits over the photo's center,
            // offset by the current pan (also converted back into
            // photo pixels).
            ...
            // Output at the frame's own on-screen resolution (capped to
            // the source photo's actual resolution) rather than the full
            // source resolution -- plenty sharp for a feed photo, and
            // keeps the resulting file from ballooning.
            ...
            canvas.toBlob((blob) => {
              if (blob) {
                ...
                // oldUrl was either the original data: URL (nothing to
                // revoke) or, if this photo was edited more than once in
                // the same session, a blob: URL from a previous crop --
                // release that one now that nothing references it.
        ...
        function submitPost(){
          ...
          PostsAPI.create({
            ...
            // profileData.posts used to be hand-incremented here, but
            // nothing ever kept it in sync with deletes -- the "Posts"
            // stat is computed live from feedPosts instead (myPostsCount
            // in feed.js), so there's no separate counter to bump.
            selectedMediaItems = [];
            ...
            closeOverlay();
            renderFeed();
            // renderFeed() only rebuilds the Home feed's own #screen --
            // it never touches Profile's #profile-tab-content, which is
            // why a brand-new post wouldn't show up in your own Posts
            // grid (or update the Posts count) until you left and
            // re-opened the Profile tab. This keeps it live the moment
            // you post, same fix as deletePost in feed.js.
        ...
        // Session cache of profiles already fetched from Supabase this
        // visit, keyed by userId: { details: {...fields from loadPersonProfileDetails},
        // network: {...fields from loadPersonNetwork}, ts: Date.now() }.
        // Re-opening a profile already in here renders fully loaded on the
        // very first paint (no "Loading profile..." flash, no network
        // wait) instead of re-running both round trips every single tap.
        // Entries older than PERSON_PROFILE_CACHE_MS still get a fresh
        // background refetch (stale-while-revalidate) so a connect
        // request accepted elsewhere, or a bio edited moments ago, still
        // shows up -- the cache only removes the *wait*, it never keeps
        // showing permanently stale data.
        ...
        async function openPersonProfile(userId, fallback){
          ...
          // Cache hit: apply the remembered details/network straight onto
          // viewedProfile before the overlay ever paints, so this open
          // renders complete on frame one -- no placeholder, no wait.
          ...
          // Still fetch: on a cold/expired cache this is the only load
          // (identical to before), and on a fresh cache hit it's a silent
          // background refresh that re-renders in place if anything
          // actually changed, rather than trusting a 2-minute-old snapshot
          // forever.
          ...
          // "Is this actually my own profile" only matters for the rare
          // case of tapping your own avatar somewhere -- worth checking,
          // but not worth making every profile open wait on an auth
          // round trip first. Runs after the overlay is already showing;
          // if it turns out to be you, swap over to the real profile tab.
        ...
        async function loadPersonProfileDetails(userId, background){
          ...
          try {
            // getCachedAuthUser() (core.js) reuses the session already
            // held in memory -- by the time a profile is opened, the app
            // has already called it at least once (on load), so this
            // resolves instantly with no network trip. sb.auth.getUser()
            // (what this replaces) always re-verifies with Supabase's
            // auth server over the network, even for a check that had
            // already been done moments earlier -- and the old code
            // awaited that BEFORE the three queries below even started,
            // forcing two network round trips back-to-back for something
            // that only needed one. That's what was leaving "Loading
            // profile..." on screen for so long.
            ...
            // Built independent of viewedProfile (rather than writing
            // straight onto it like before) so this same fetch can also
            // be used to warm personProfileCache for someone whose card
            // isn't open at all -- see prefetchContactProfiles() below,
            // which calls this in the background right after login for
            // every contact, so the "Loading profile..." beat is already
            // done by the time a person actually taps in.
            ...
            // Only touch/repaint the live overlay if it's actually
            // showing this exact profile right now -- a background
            // prefetch for a contact nobody has opened yet has no
            // overlay to update, it's just warming the cache above.
            if (viewedProfile && viewedProfile.id === userId) {
              Object.assign(viewedProfile, details);
              // A silent background refresh only needs to repaint if it
              // actually turned up something different from the cached
              // snapshot already on screen -- otherwise re-rendering here
              // would just be an invisible no-op DOM replace on every
              // re-visit.
        ...
        function personProfilePostGroups(){
          ...
          // Sorted newest-first by post id (each post's creation
          // timestamp) rather than left in feedPosts' own order -- that
          // order gets reshuffled by rankFeedPosts() on every Home feed
          // refresh (see feed.js), which was otherwise making someone
          // else's video/picture grid visibly reorder itself too. Same
          // fix as byNewestFirst in profile.js's own profileTabContent.
        ...
        async function loadPersonNetwork(userId, background){
          ...
          try {
            ...
            // The number on the profile header only needs the count above
            // -- it used to sit on "···" until the member list (fetched
            // below) had *also* finished loading, forcing the header to
            // wait on a second, avoidable round trip. Render as soon as
            // the count itself is known (only meaningful when this
            // profile is the one actually open right now -- a background
            // prefetch has no overlay to paint into); the member list
            // keeps loading separately underneath for whoever actually
            // opens the Network tab. networkLoaded (below) still gates
            // just that list, same as before.
            if (viewedProfile && viewedProfile.id === userId) {
            ...
            // Cache written unconditionally (not just when this is the
            // profile currently on screen) -- this is what lets
            // prefetchContactProfiles() (below) warm every contact's
            // network in the background right after login.
        ...
        // ---- Warm the profile cache for known contacts right after login ----
        // Called once from authEnterApp's post-render batch (games.js),
        // after the incoming/outgoing connection requests and
        // collaborations have all been loaded -- so every person who'd
        // show up in Messaging's General/Requests/Collaborations lists
        // already has their bio and network sitting in
        // personProfileCache by the time anyone actually taps into their
        // profile. That's what turns the visible "Loading profile..."
        // beat (and the second, shorter wait for the network count) into
        // an instant, fully-loaded open -- the same round trips just
        // happen quietly in the background during login instead of
        // after the tap. Silent and best-effort: one contact's fetch
        // failing only leaves that contact falling back to the normal
        // on-open fetch, same as before this existed.
        async function prefetchContactProfiles(){
          ...
          // A handful at a time rather than every contact in one burst --
          // keeps a large contact list from firing dozens of Supabase
          // requests simultaneously on login, while still finishing well
          // before anyone has had a chance to open a profile by hand.
          const CONCURRENCY = 4;
          let next = 0;
        ...
        function personProfileHTML(){
          ...
          // The header only ever needs the count itself, not the full
          // member list -- typeof-checking networkCount directly (instead
          // of gating on networkLoaded, which also covers the separate
          // member-list fetch used by the Network tab) lets the number
          // show up as soon as loadPersonNetwork's count RPC resolves,
          // rather than waiting on that second, unrelated fetch too.
