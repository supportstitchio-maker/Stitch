        ...
        function openOverlay(kind){
          ...
          if (kind === 'discover') { discoverSearchQuery = ''; discoverPeopleLoaded = false; ov.innerHTML = discoverHTML(); loadDiscoverPeople(); }
            ...
            selectedMediaHtml = null; selectedMediaType = null; selectedMediaFile = null; selectedMediaPreviewHtml = null;
        ...
        // The already-styled (max-h-64, object-contain) preview markup
        // shown inside the New Post screen -- kept separate from
        // selectedMediaHtml (which uses the plainer w-full/h-auto markup
        // that actually ships with the post) so createPostHTML can restore
        // the same preview when it re-renders after a trip to the Tag
        // People page, instead of only ever setting it once from inside
        // handleMediaSelect.
        ...
        function createPostHTML(captionDraft){
          const hasMedia = !!selectedMediaPreviewHtml;
          return `
            ${overlayHeader('New post', '20px')}
            <div id="create-post-scroll" class="p-5 flex-1 overflow-y-auto no-scrollbar">
              <button type="button" onclick="pickMedia('image/*,video/*')" id="media-preview" class="${hasMedia ? 'w-full rounded-2xl overflow-hidden mb-4' : 'w-full h-48 bg-gray-100 rounded-2xl flex flex-col items-center justify-center gap-2 text-gray-400 mb-4'}">
                ${hasMedia ? selectedMediaPreviewHtml : `${Icon('upload','w-8 h-8')}<div class="text-sm">(tap to upload a post)</div>`}
              </button>
        ...
        function handleMediaSelect(event){
          ...
          reader.onload = function(e){
            ...
            selectedMediaPreviewHtml = isVideo
            ...
            if (preview) {
              ...
              preview.innerHTML = selectedMediaPreviewHtml;
            }
          };
