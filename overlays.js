        ...
        const overlayBackKinds = ['aiClass', 'gamification', 'classAnnouncements', 'classNotifications', 'notifications', 'notificationSettings', 'savedItems', 'blockedAccounts', 'termsOfService', 'privacyPolicy', 'helpCenter', 'contactUs', 'reportIssue', 'newMessage', 'newCollaboration', 'studyTimetable', 'studyReminders', 'practiceTests', 'examTake', 'profileQR', 'postOpportunity', 'jobDetail', 'jobApply', 'classDetail', 'inviteStudents', 'newAnnouncement', 'newClasswork', 'scheduleLecture', 'courseDetail', 'courseItemDetail', 'newCourse'];
        ...
        function openOverlay(kind){
          ...
          if (kind === 'profileMenu' || kind === 'profileAnalytics' || kind === 'call' || kind === 'profileQR' || kind === 'lectureCall') {
            ov.style.top = '0';
          }

          if (kind === 'discover') { discoverSearchQuery = ''; ov.innerHTML = discoverHTML(); }
          else if (kind === 'create') { selectedMediaHtml = null; selectedMediaType = null; ov.innerHTML = createPostHTML(); }
        ...
        // Sample people to discover/connect with. In a real backend these
        // would come from a users database; here they're seeded so the
        // Discover search bar and connect flow have real people to show
        // and match against (by name, username, or school/field).
        const discoverPeople = [
          { id:'d1',  name:'Ama Owusu',        username:'ama_owusu',        sub:'KNUST · Economics', icon:'user', avatarBg:'bg-blue-50' },
          { id:'d2',  name:'Kwame Boateng',     username:'kwame_boateng',    sub:'University of Ghana · Economics', icon:'user', avatarBg:'bg-emerald-50' },
          { id:'d3',  name:'Abena Asante',      username:'abena_asante',     sub:'KNUST · Banking & Finance', icon:'user', avatarBg:'bg-amber-50' },
          { id:'d4',  name:'Kofi Mensah',       username:'kofi_mensah',      sub:'UCC · Economics', icon:'user', avatarBg:'bg-rose-50' },
          { id:'d5',  name:'Efua Darko',        username:'efua_darko',       sub:'KNUST · Economics', icon:'user', avatarBg:'bg-purple-50' },
          { id:'d6',  name:'Yaw Appiah',        username:'yaw_appiah',       sub:'Legon · Statistics', icon:'user', avatarBg:'bg-blue-50' },
          { id:'d7',  name:'Adwoa Sarpong',     username:'adwoa_sarpong',    sub:'KNUST · Economics', icon:'user', avatarBg:'bg-emerald-50' },
          { id:'d8',  name:'Kwabena Frimpong',  username:'kwabena_frimpong', sub:'GIMPA · Economics', icon:'user', avatarBg:'bg-amber-50' },
          { id:'d9',  name:'Akosua Agyeman',    username:'akosua_agyeman',   sub:'KNUST · Actuarial Science', icon:'user', avatarBg:'bg-rose-50' },
          { id:'d10', name:'Nana Yeboah',       username:'nana_yeboah',      sub:'UCC · Economics', icon:'user', avatarBg:'bg-purple-50' },
          { id:'d11', name:'Esi Amoah',         username:'esi_amoah',        sub:'Legon · Economics', icon:'user', avatarBg:'bg-blue-50' },
          { id:'d12', name:'Kojo Addai',        username:'kojo_addai',       sub:'KNUST · Economics', icon:'user', avatarBg:'bg-emerald-50' },
          { id:'d13', name:'Maame Asare',       username:'maame_asare',      sub:'GIMPA · Economics', icon:'user', avatarBg:'bg-amber-50' },
          { id:'d14', name:'Fiifi Quaye',       username:'fiifi_quaye',      sub:'KNUST · Economics', icon:'user', avatarBg:'bg-rose-50' },
        ];
        let discoverSearchQuery = '';

        function discoverHTML(){
          return `
            ${overlayHeader('Discover')}
        ...
        function discoverPeopleHTML(){
          ...
          return filtered.length
            ? filtered.map(p => personCard(p)).join('')
            : `<div class="text-center text-gray-400 text-sm py-10">No one found for "${discoverSearchQuery}".</div>`;
        }
        ...
        function personCard(p){
          return `
            <div class="bg-white rounded-3xl p-4 flex items-center gap-4 mb-4 shadow-sm">
              <div class="w-11 h-11 ${p.avatarBg || 'bg-blue-50'} rounded-2xl flex items-center justify-center text-gray-600">${Icon(p.icon || 'user','w-5 h-5')}</div>
              <div class="flex-1">
                <div class="font-semibold text-sm">${escapeHtml(p.name)}</div>
                <div class="text-xs text-gray-500">${p.username ? `@${escapeHtml(p.username)} · ` : ''}${p.sub}</div>
              </div>
              ${p.connected
                ? `<span class="text-xs font-semibold px-4 py-2 rounded-full bg-gray-100 text-gray-400">Connected</span>`
                : `<button onclick="connectToDiscoverPerson('${p.id}')" class="text-xs font-semibold px-4 py-2 rounded-full" style="background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};">Connect</button>`}
            </div>`;
        }

        // Connecting from Discover is real: it adds the person to the
        // user's actual network (primaryConvos, flagged network:true) so
        // they show up in My Contacts / My Network, opens a starter chat
        // the same way any other connection would, and bumps the real
        // network counter shown across the profile, rather than just
        // alerting that a request was "sent" and forgetting about it.
        function connectToDiscoverPerson(id){
          const p = discoverPeople.find(x => x.id === id);
          if (!p || p.connected) return;
          p.connected = true;
          primaryConvos.unshift({
            id: 'net-' + id, icon: p.icon || 'user', avatarBg: p.avatarBg || 'bg-blue-100',
            name: p.name, preview: 'You are now connected.', time: 'now', unread: false, network: true
          });
          convoMeta['net-' + id] = { icon: p.icon || 'user', avatarBg: p.avatarBg || 'bg-blue-100', name: p.name };
          conversationMessages['net-' + id] = [{ from: 'them', text: `You and ${escapeHtml(p.name)} are now connected on Stitch.` }];
