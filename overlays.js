        ...
        async function loadPersonProfileDetails(userId, background){
          ...
          try {
            ...
            personProfileCache[userId] = {
              ts: Date.now(),
              network: prevCache ? prevCache.network : undefined,
              details: {
                name: viewedProfile.name, username: viewedProfile.username, bio: viewedProfile.bio,
                photo: viewedProfile.photo, links: viewedProfile.links,
                connected: viewedProfile.connected, requestSent: viewedProfile.requestSent, loaded: true,
              },
            };
            ...
            // re-visit.
            if (!background || JSON.stringify(prevCache && prevCache.details) !== JSON.stringify(personProfileCache[userId].details)) {
              renderPersonProfile();
            }
          } catch (e) {
        ...
        async function loadPersonNetwork(userId, background){
          ...
          try {
            ...
            // the count itself is known; the member list keeps loading
            // separately underneath for whoever actually opens the
            // Network tab. networkLoaded (below) still gates just that
            // list, same as before.
            const countChanged = !background || !personProfileCache[userId] || personProfileCache[userId].network.networkCount !== viewedProfile.networkCount;
            ...
            if (!otherIds.length) {
              ...
              personProfileCache[userId] = {
                ts: Date.now(),
                details: prevCache ? prevCache.details : undefined,
                network: { networkCount: viewedProfile.networkCount, network: [], networkLoaded: true },
              };
              if (countChanged) renderPersonProfile();
              return;
            }
            ...
            viewedProfile.networkLoaded = true;
            const prevCache2 = personProfileCache[userId];
            const newNetworkCache = { networkCount: viewedProfile.networkCount, network: viewedProfile.network, networkLoaded: true };
            const listChanged = !background || !prevCache2 || JSON.stringify(prevCache2.network) !== JSON.stringify(newNetworkCache);
            personProfileCache[userId] = { ts: Date.now(), details: prevCache2 ? prevCache2.details : undefined, network: newNetworkCache };
