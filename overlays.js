        ...
        function personNetworkRow(m){
          ...
          return `
            <button onclick="openPersonProfile('${m.id}', {name:'${escapeForJsAttr(m.name)}', photo:${m.photo ? `'${escapeForJsAttr(m.photo)}'` : 'null'}, icon:'${m.icon}', avatarBg:'${m.avatarBg}'})" class="w-full flex items-center gap-3 py-3 text-left">
        ...
        function personProfileHTML(){
          ...
          const theirPosts = p.id ? feedPosts.filter(post => post.mediaHtml && post.authorId === p.id) : [];
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto p-5">
              ...
              <div class="max-w-xs mx-auto mb-6">${actionHTML}</div>
              <div class="-mx-5">
                <div class="px-5 text-xs font-semibold text-gray-400 uppercase tracking-wide pb-2">Posts</div>
                ${theirPosts.length
                  ? `<div class="space-y-3">${theirPosts.map(feedPost).join('')}</div>`
