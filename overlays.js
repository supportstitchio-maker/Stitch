        ...
        function personProfileActionRowHTML(p){
          ...
          return `
            <div class="flex items-center gap-2 mb-4 relative">
              ...
              ${personProfileMenuOpen ? `
                <div onclick="togglePersonProfileMenu()" onwheel="togglePersonProfileMenu()" ontouchmove="togglePersonProfileMenu()" class="fixed inset-0 z-10 flex flex-col justify-end" style="padding-bottom:calc(env(safe-area-inset-bottom, 12px) + 20px);">
                  <div onclick="event.stopPropagation()">${personProfileDropdownMenuHTML(p)}</div>
                </div>` : ''}
            </div>`;
        }

        // ---- "More" menu for a viewed profile, presented as a bottom
        //      sheet that slides up from the bottom of the screen (same
        //      "share-sheet-panel" slide-up pattern used by Share and by
        //      the comment sheet) instead of a small dropdown pinned
        //      under the "..." button. Sits inside a bottom-padded
        //      backdrop (see caller) instead of being pinned flush to the
        //      literal screen edge, and each row is separated by a
        //      divider line rather than just spacing. ----
        function personProfileDropdownMenuHTML(p){
          const blocked = isPersonProfileBlocked(p);
          const rows = [];
          rows.push(`<button onclick="togglePersonProfileMenu(); openOverlayFrom('personProfile','personProfileQR');" class="w-full flex items-center gap-3 px-5 py-3.5 text-sm text-gray-700 menu-item-pill">${Icon('send','w-4 h-4')} Share profile</button>`);
          rows.push(`<button onclick="togglePersonProfileMenu(); togglePersonProfileBlock();" class="w-full flex items-center gap-3 px-5 py-3.5 text-sm text-red-500 menu-item-pill">${Icon('block','w-4 h-4')} ${blocked ? 'Unblock' : 'Block'}</button>`);
          if (p.connected) {
            rows.push(`<button onclick="togglePersonProfileMenu(); removeConnectionWithViewedProfile();" class="w-full flex items-center gap-3 px-5 py-3.5 text-sm text-red-500 menu-item-pill">${Icon('trash','w-4 h-4')} Remove</button>`);
          }
          ...
          return `
            <div class="w-full bg-white flex flex-col share-sheet-panel" style="border-radius:22px 22px 0 0;overflow:hidden;box-shadow:0 -8px 30px rgba(0,0,0,0.18);">
