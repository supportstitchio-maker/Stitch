        ...
        function setMediaEditAspect(key){
          ...
          if (frame) frame.style.cssText += mediaEditFrameAspectCss();
          ...
          // cover scale/clamp -- doing it synchronously would measure
          // the frame's old (pre-switch) dimensions.
          requestAnimationFrame(applyMediaEditTransform);
        }
        ...
        function initMediaEditTransform(){
          ...
          if (mediaEditAspect === 'original') {
            const frame = document.getElementById('edit-crop-frame');
            if (frame) frame.style.aspectRatio = `${mediaEditNatural.w} / ${mediaEditNatural.h}`;
          }
