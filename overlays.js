        ...
        async function loadDiscoverPeople(){
          ...
          try {
            ...
            const { data, error } = await sb.from(PUBLIC_PROFILES_TABLE).select('*');
            if (error || !data) { discoverPeopleLoaded = true; return; }
            let sentTo = new Set();
            if (me) {
              const { data: reqRows } = await sb.from(CONNECTION_REQUESTS_TABLE).select('to_user').eq('from_user', me.id);
              sentTo = new Set((reqRows || []).map(r => r.to_user));
            }
            discoverPeople = data
          ...
          } catch (e) { discoverPeopleLoaded = true; }
        }
        ...
        function personCard(p){
          ...
          return `
            <div class="bg-white rounded-3xl p-4 flex items-center gap-4 mb-4 shadow-sm">
              <div class="w-11 h-11 ${p.avatarBg || 'bg-blue-50'} rounded-2xl flex items-center justify-center text-gray-600 overflow-hidden">${avatarInner}</div>
        ...
        async function connectToDiscoverPerson(id){
          ...
          try {
            const { data: userRes } = await sb.auth.getUser();
            const me = userRes && userRes.user;
            if (!me) { p.requestSent = false; if (list) list.innerHTML = discoverPeopleHTML(); return; }
            ...
            if (error) { p.requestSent = false; if (list) list.innerHTML = discoverPeopleHTML(); }
          } catch (e) { p.requestSent = false; if (list) list.innerHTML = discoverPeopleHTML(); }
        }
