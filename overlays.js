const overlayBackKinds = ['discover', 'create', 'tagPeoplePicker', 'aiClass', 'conversation', 'addToCall', 'incomingCall', 'incomingLectureCall', 'joinClassroom', 'createClassroom', 'classDetail', 'inviteStudents', 'newAnnouncement', 'scheduleLecture', 'classworkCreateMenu', 'newClasswork', 'newQuiz', 'newPoll', 'classworkDetail', 'classSettings', 'studyTimetable', 'studyReminders', 'classAnnouncements', 'classNotifications', 'gamification', 'profileMenu', 'profileQR', 'newMessage', 'myContacts', 'newCollaboration', 'profileAnalytics', 'careerAnalytics', 'notifications', 'notificationSettings', 'savedItems', 'blockedAccounts', 'referrals', 'termsOfService', 'privacyPolicy', 'helpCenter', 'contactUs', 'reportIssue', 'practiceTests', 'examTake', 'jobDetail', 'jobApply', 'postOpportunity', 'courseDetail', 'courseItemDetail', 'courseEnroll', 'courseTeachers', 'newCourse', 'courseAddModule', 'courseAddItemPage', 'personProfile', 'personProfileQR', 'personNetwork', 'postFeed', 'careerStart', 'careerMatches', 'careerMatching', 'myFullProfile'];
        ...
        function personProfileActionRowHTML(p){
          ...
          const outline = 'border:1.5px solid #d1d5db;color:#6b7280;background:#fff;';
          let pills;
          if (p.connected) {
            ...
            pills = `
              ...
              <button onclick="confirmWithdrawViewedProfileRequest()" class="${pillBase}" style="${outline}">${Icon('clock','w-4 h-4')} Pending</button>`;
          } else {
            pills = `
              <button onclick="connectWithViewedProfile()" class="${pillBase}" style="${filled}">${Icon('personPlus','w-4 h-4')} Connect</button>
              <button onclick="messageAndConnectViewedProfile()" class="${pillBase}" style="${outline}">${Icon('mail','w-4 h-4')} Message</button>`;
          }
          return `
        ...
        function personProfileDropdownMenuHTML(p){
          ...
          const items = [];
          items.push(`<button onclick="togglePersonProfileMenu(); openOverlayFrom('personProfile','personProfileQR');" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 menu-item-pill">${Icon('send','w-4 h-4')} Share profile</button>`);
          items.push(`<button onclick="togglePersonProfileMenu(); togglePersonProfileBlock();" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-500 menu-item-pill">${Icon('block','w-4 h-4')} ${blocked ? 'Unblock' : 'Block'}</button>`);
          if (p.connected) {
            items.push(`<button onclick="togglePersonProfileMenu(); removeConnectionWithViewedProfile();" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-500 menu-item-pill">${Icon('trash','w-4 h-4')} Remove</button>`);
          }
          return `
