const overlayBackKinds = ['discover', 'create', 'tagPeoplePicker', 'aiClass', 'conversation', 'addToCall', 'incomingCall', 'incomingLectureCall', 'joinClassroom', 'classPaymentConfirm', 'createClassroom', 'classDetail', 'inviteStudents', 'newAnnouncement', 'scheduleLecture', 'classworkCreateMenu', 'newClasswork', 'newQuiz', 'newPoll', 'classworkDetail', 'classSettings', 'studyTimetable', 'studyReminders', 'classAnnouncements', 'classNotifications', 'gamification', 'profileMenu', 'profileQR', 'newMessage', 'myContacts', 'newCollaboration', 'profileAnalytics', 'careerAnalytics', 'notifications', 'notificationSettings', 'savedItems', 'blockedAccounts', 'referrals', 'termsOfService', 'privacyPolicy', 'helpCenter', 'contactUs', 'reportIssue', 'practiceTests', 'examTake', 'jobDetail', 'jobApply', 'jobDashboard', 'postOpportunity', 'courseDetail', 'courseItemDetail', 'courseEnroll', 'coursePeople', 'newCourse', 'courseAddModule', 'courseAddItemPage', 'personProfile', 'personProfileQR', 'personNetwork', 'postFeed', 'careerStart', 'careerMatches', 'careerMatching', 'myFullProfile', 'forwardMessage'];
        const overlayBackAction = {
          ...
          newCourse: (fromPopState) => closeOverlay(fromPopState),
        ...
        function pauseAllOverlayMedia(){
          ...
          // #screen, not #overlay -- opening any overlay (notifications, chat, create post, the
        ...
        function openOverlay(kind){
          ...
          // Re-pin (or clear) #overlay's height for whichever kind is opening now -- see
          // pinOverlayHeightAgainstKeyboard below -- instead of leaving a stale height from
          ...
          if (kind === 'discover') { discoverSearchQuery = ''; ov.innerHTML = discoverHTML(); if (!discoverPeopleLoaded) loadDiscoverPeople(); else renderDiscoverList(); }
            ...
            if (!canCurrentUserPost()) { closeOverlay(); return; }
        ...
        function closeOverlay(fromPopState){
          ...
          // unrelated screens' back buttons -- without these checks, closing some other overlay
          ...
          if (overlayHistoryPushed) {
            ...
            if (!fromPopState) {
              ...
              // resulting async popstate event doesn't get treated as a real back-press. closeOverlay()
              suppressNextPopstate = true;
              history.back();
            }
          }
          ...
          // sitting in the DOM from before the chat was opened. The underlying
          ...
          if (opts.center) {
            return `
              <div class="flex-shrink-0 w-full"${padStyle}>
                <div class="max-w-2xl mx-auto px-5${pbClass} relative flex items-center justify-center"${pbStyle}>
        ...
        function deleteCurrentPhotoInEditor(){
          ...
          // selectedMediaItems -- keep their indices pointing at the right item. Dropping this slot
        ...
        function submitPost(){
          ...
          // previously this fell through to PostsAPI.create(), which optimistically inserted the
          ...
          PostsAPI.create({
            ...
            // Don't clear the guard until the real upload+insert finishes -- see the note on
            // postSubmitInFlight above. The optimistic UI reset below still happens immediately either
            ...
            // desktop side nav on any tab, so currentTab isn't necessarily Home here. renderFeed()
        ...
        async function loadDiscoverPeople(retryCount){
          ...
          if (!sb) {
            ...
            // during a connectivity blip). That used to mark the list as "loaded" with zero people
        ...
        function personCard(p){
          // Falls back to the silhouette placeholder when the photo can't load (e.g. offline
          // and not cached yet) instead of showing the browser's broken-image icon.
        ...
        async function sendConnectionRequestTo(toUserId, message){
          ...
          // the request was sent from -- reverted below if the request turns out not to go through.
        ...
        async function cancelMyConnectionRequestTo(toUserId){
          ...
          try {
            ...
            // from_user = me. If the table has no RLS policy permitting deletes (only the
        ...
        // Every place a person's Connect/Pending status can be shown -- their posts in the home
        // feed, the Discover list, their profile pill, and any "network" list (theirs or a mutual
        ...
        function openPersonProfileForConvo(convoId){
          ...
          // photo, members) instead of a person's profile -- and it used to silently do nothing.
        ...
        async function loadPersonNetwork(userId, background){
          ...
          try {
            ...
            // / .eq('to_user', userId) for whichever OTHER person's profile we're viewing. Row Level
        ...
        // ---- Message request compose (replaces the old prompt()-based flow) -- tapping "Message"
        // on someone you're not connected with yet opens a small chat-style compose screen instead
        ...
        // button (Share/Block/Remove) on the right -- sits right below the bio instead of the old
        ...
        // right under the "..." button -- same "menu-dropdown-inset" pattern (drop-in animation,
        function personProfileDropdownMenuHTML(p){
          ...
          // Divider lines between the rows -- but not after the last one -- added as an inline
          // border rather than a Tailwind "last:" variant, since this app ships a statically pre-
        ...
        function playNotifChime(title, body){
          ...
          // browser's default) over a synthesized beep. Fall back to the old in-app chime only when
        ...
        function notifTap(id){
          ...
          if (n.type === 'classroom' && n.classId && typeof openOverlay === 'function') {
            ...
            if (n.workId && typeof openClassworkDetail === 'function') {
              ...
              // Join-request / new-enrollment notices set tab:'people' so tapping them lands
              // straight on the roster (where Approve/Deny live) instead of the stream.
        ...
        // #overlay fills its parent (top:0; bottom:0, see core.js) which shrinks whenever
        // the keyboard opens, so a form's Next/Continue/Post/Submit button (flex-shrink-0 at
        // the bottom of the overlay's flex column -- Apply to post, New post, Post an
        // Opportunity, Match with CV, etc.) visibly slides up with it. This mirrors
        // pinAuthGateHeight in games.js (which solves the identical problem for #auth-gate,
        // the pre-login onboarding screens): while a text field inside #overlay is focused,
        // pin #overlay's height to what it was with the keyboard closed, so the keyboard
        // simply appears over the lower part of the form instead of resizing it. The
        // few overlay kinds that deliberately want a chat-style composer gliding above the
        // keyboard (conversation, aiClass -- see syncConvoKeyboardInset / syncAIClassKeyboardInset)
        // manage #overlay's height/bottom themselves, so this leaves those alone entirely.
        (function pinOverlayHeightAgainstKeyboard(){
          ...
          // Exposed so openOverlay() (below) can re-pin/clear the height the instant a new
          // overlay kind opens, instead of waiting on the next resize/focus event -- without
          // this, an overlay opened right after one that pinned a height (or that needs to
          // track the keyboard itself) could briefly render at the wrong, stale height.
          ...
          // the overlay's own height isn't shrinking anymore, so the browser's automatic
          // "scroll the focused input above the keyboard" has nothing to react to.
          // Deliberately NOT scrollIntoView({block:'center'}): centering the field also
          // drags whatever sits just below it -- a form's Next/Continue/Post/Submit button
          // included -- up into the visible area above the keyboard. The button is meant to
          // stay buried under the keyboard (Enter/Go on the keyboard does the same thing as
          // tapping it), so this only scrolls the minimum amount needed to clear the field
          // itself, leaving anything after it alone.
          document.addEventListener('focusin', function(e){
            ...
            // Wait a beat for the keyboard (and visualViewport) to settle before measuring,
            // so this doesn't scroll against the still-full pre-keyboard viewport.
            setTimeout(function(){
          ...
          // Same debounced "did the keyboard actually close" check as pinAuthGateHeight,
          // for the system back-gesture/button case (no blur event fires there).
          if (vv) {
