const overlayBackKinds = ['discover', 'create', 'tagPeoplePicker', 'aiClass', 'conversation', 'addToCall', 'incomingCall', 'incomingLectureCall', 'joinClassroom', 'createClassroom', 'classDetail', 'inviteStudents', 'newAnnouncement', 'scheduleLecture', 'classworkCreateMenu', 'newClasswork', 'newQuiz', 'newPoll', 'classworkDetail', 'classSettings', 'studyTimetable', 'studyReminders', 'classAnnouncements', 'classNotifications', 'gamification', 'profileMenu', 'profileQR', 'newMessage', 'myContacts', 'newCollaboration', 'profileAnalytics', 'careerAnalytics', 'notifications', 'notificationSettings', 'savedItems', 'blockedAccounts', 'referrals', 'termsOfService', 'privacyPolicy', 'helpCenter', 'contactUs', 'reportIssue', 'practiceTests', 'examTake', 'jobDetail', 'jobApply', 'postOpportunity', 'courseDetail', 'courseItemDetail', 'courseEnroll', 'courseTeachers', 'newCourse', 'courseAddModule', 'courseAddItemPage', 'personProfile', 'personProfileQR', 'personNetwork', 'postFeed', 'careerStart', 'careerMatches', 'careerMatching', 'myFullProfile'];
        ...
        function classDetailHeaderHTML(title, isTeacher){
          return `
            <div class="w-full px-5 pb-3 relative" style="padding-top:var(--top-safe-pad);">
              ...
              ${classDetailMenuOpen ? `<div onclick="toggleClassDetailMenu()" onwheel="toggleClassDetailMenu()" ontouchmove="toggleClassDetailMenu()" class="fixed inset-0 z-10"></div>${classDetailMenuDropdown(isTeacher)}` : ''}
            </div>`;
        }
        ...
        // Add-another-photo, sitting right after the 16:9 pill, styled like
        // the bare (no circle background) plus icon used in the home
        // topbar rather than as a filled icon button.
        ...
        function deleteCurrentPhotoInEditor(){
          ...
          // Everything still queued after the one we just removed shifted
          // down by one slot in selectedMediaItems -- keep their indices
          // pointing at the right item. Dropping this slot from the queue
          // (rather than advancing mediaEditIndexInQueue) means whatever
          // was next slides into the current position automatically.
            ...
            reader.onload = function(e){
              ...
              if (remaining === 0) {
                ...
                const newPhotoIndices = newItems
                  .map((it, i) => (it.type === 'image' ? startIndex + i : -1))
                  .filter(i => i !== -1);
                // Tack the new photos onto the end of the current edit
                // session's queue instead of starting a separate one, so
                // they get their own crop step once you're done with
                // whatever's already left.
        ...
        // count -- including on the author's own device, since both rows
        // are genuinely "mine" and the "always keep my own posts" merge
        // rule keeps both forever.
        //
        // This flag has to stay true for the *entire* upload+insert, not
        // just until PostsAPI.create()'s promise settles -- create()
        // resolves right after its synchronous optimistic-UI step (by
        // design, so the post appears instantly), which used to release
        // this guard within a tick or two of the first tap. A real
        // double-tap 100-300ms later landed well after that, sailed
        // straight through the "guard", and created a second fully
        // independent post. See PostsAPI.create's post._pending below --
        // that's what this now actually waits on.
        ...
        function submitPost(){
          ...
          // Bail out before touching the network at all when the device is
          // known to be offline -- previously this fell through to
          // PostsAPI.create(), which optimistically inserted the post,
          // attempted the media upload anyway, and only then failed with a
          // generic "could not be saved" alert once the upload rejected.
          // navigator.onLine === false is a reliable "definitely offline"
          // signal (it's only ever unreliable in the other direction, i.e.
          // reporting online when a connection is actually unusable), so
          // checking it here first skips the doomed upload attempt entirely
          // and tells the person plainly what's wrong.
          ...
          PostsAPI.create({
            ...
            // see the note on postSubmitInFlight above. The optimistic UI
            // reset below still happens immediately either way.
        ...
        function onDiscoverSearchInput(val){
          ...
          const list = document.getElementById('discover-people-list');
          if (list) list.innerHTML = discoverPeopleHTML();
          // Debounced so we record what the person actually searched for
          // (a pause in typing), not every half-typed keystroke, as a
          // signal for which "Suggested for you" videos are relevant to them.
        ...
        async function sendConnectionRequestTo(toUserId, message){
          // Guard against duplicate requests / duplicate network rows: if
          // we're already connected to this person, or already have a
          // request pending to them (per our local, server-synced state),
          // don't insert another row -- just report success so the caller's
          // UI treats it the same as a freshly-sent request. Without this,
          // a stale button state, a double-tap, or the same person being
          // reachable from two different screens at once could fire two
          // separate INSERTs and leave two pending rows for one person,
          // which is what was producing duplicate "My Network" entries.
          ...
          // Flip every Connect tag for this person to Pending right away,
          // rather than only the one the request was sent from -- reverted
          // below if the request turns out not to go through.
          setConnectionTagEverywhere(toUserId, true);
          const sb = getSupabaseClient();
          if (!sb) { setConnectionTagEverywhere(toUserId, false); return false; }
          try {
            const { data: userRes } = await sb.auth.getUser();
            const me = userRes && userRes.user;
            if (!me) { setConnectionTagEverywhere(toUserId, false); return false; }
            // Belt-and-suspenders check against the DB itself, in case our
            // local sentRequestConvos/network state hasn't caught up yet
            // (e.g. right after app load, before loadMyPendingOutgoingRequests
            // has resolved) -- avoids inserting a second pending row for a
            // request that already exists server-side.
        ...
        async function cancelMyConnectionRequestTo(toUserId){
          ...
          if (!sb) return false;
          try {
            // FIX: this used to run a plain .delete() against
            // connection_requests filtered to from_user = me. If the table
            // has no RLS policy permitting deletes (only the select/update
            // policies accept/reject rely on), Postgres/PostgREST doesn't
            // return an error for that -- it just matches zero rows and
            // reports success. The UI would optimistically remove the
            // "Pending" row and show it as cancelled, but the request was
            // still sitting in the database, so the next refresh (tab
            // switch, realtime sync) pulled it back in and it reappeared
            // as pending again -- i.e. "cancel" didn't actually stick.
            // Routing this through a SECURITY DEFINER RPC (same pattern as
            // get_network_count/get_network_list) guarantees the row is
            // really deleted regardless of RLS. See supabase-fixes-
            // network-list.sql for the function definition to add.
        ...
        // their posts in the home feed, the Discover list, their profile
        // pill, and any "network" list (theirs or a mutual connection's)
        // sitting in personProfileCache -- keeps its own local copy of
        // requestSent/connected. setConnectionTagEverywhere() is the single
        // place that keeps all of those copies in sync for one user id, so
        // sending (or cancelling) a request from any one screen immediately
        // flips every tag for that person, everywhere in the app, to
        // "Pending" (or back to "Connect") instead of only the tag on the
        // screen the action was taken from.
        ...
        async function openPersonProfile(userId, fallback){
          if (!userId) return;
          // Looking someone up counts as a real interest signal for the
          // "Suggested for you" videos elsewhere in the feed.
        ...
        async function loadPersonNetwork(userId, background){
          ...
          try {
            // FIX: this used to select straight from connection_requests
            // with .eq('from_user', userId) / .eq('to_user', userId) for
            // whichever OTHER person's profile we're viewing. Row Level
            // Security on that table only lets you read rows where you
            // (the signed-in user) are the from_user or to_user -- so for
            // anyone besides yourself, that query silently came back
            // filtered down to just the one row (if any) connecting YOU to
            // them, which is why someone's network screen only ever showed
            // a single stray name with a bare Connect button instead of
            // their real connections. get_network_count already worked
            // around this the same way (a security-definer RPC that can
            // see the full table); get_network_list does the same thing
            // for the actual member list. See supabase-fixes.sql for the
            // function definition to add in Supabase.
            ...
            if (otherIds.length) {
              ...
              network = otherIds.map(id => {
                ...
                return {
                  ...
                  // FIX: these used to always initialize to connected:false/
                  // requestSent:false-ish state derived only from local convo
                  // caches computed at fetch time, so a person you'd already
                  // connected with (or already sent a request to) elsewhere
                  // in the app could still show a bare "Connect" button here
                  // the next time you viewed someone else's network list.
                  // Checking the live pending-request list too keeps this in
                  // sync with reality instead of resetting per view.
        ...
        // Tapping the "Pending" pill (LinkedIn-style) confirms before
        // withdrawing, instead of cancelling the request immediately.
        ...
        function removeConnectionWithViewedProfile(){
          ...
          openAppConfirmModal(`Remove ${name} from your network?`, '', 'Remove', async function(){
            ...
            // ...but if the backend removal didn't actually go through, put
            // it back and tell the person, instead of silently leaving a
            // stale "accepted" row in the database that comes back and
            // re-adds the connection on the next refresh/sync.
            if (!ok) {
        ...
        // Removing a mutual connection touches BOTH people's rows in
        // connection_requests (from_user=me/to_user=them AND
        // from_user=them/to_user=me), but a plain .update() can only ever
        // touch the row where from_user = me -- RLS blocks updating a row
        // that belongs to the other user, and PostgREST reports that as
        // "success, 0 rows matched" rather than an error. The old code
        // removed the connection locally regardless, so the other user's
        // row silently stayed "accepted" and the connection reappeared the
        // next time the network list synced. Routing this through a
        // SECURITY DEFINER RPC (same pattern as cancel_connection_request)
        // guarantees both rows are actually updated. Requires a
        // `remove_connection(target_user uuid)` function in Supabase --
        // see supabase-fixes-remove-connection.sql.
        ...
        // ---- LinkedIn-style pill row: Connect/Message/Pending on the
        //      left, a round "..." dots button (Share/Block/Remove) on the
        //      right -- sits right below the bio instead of the old
        //      header hamburger icon that hid Connect/Message inside it. ----
        ...
        // ---- "More" menu for a viewed profile: a small dropdown card
        //      anchored to the right, right under the "..." button --
        //      same "menu-dropdown-inset" pattern (drop-in animation,
        //      rounded card, border) used by every other dropdown in the
        //      app (post menu, chat options, etc.) instead of a bottom
        //      sheet. Rows are separated by a divider line. ----
        function personProfileDropdownMenuHTML(p){
          ...
          // Divider lines between the rows -- but not after the last one --
          // added as an inline border rather than a Tailwind "last:" variant,
          // since this app ships a statically pre-built Tailwind stylesheet
          // (no in-browser JIT), so any utility class not already compiled
          // into styles.css (like "last:border-b-0") would silently do
          // nothing at runtime.
