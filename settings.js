        // ---------------- APP PREFERENCES (theme / privacy) ----------------
        let appPrefs = {
          theme: 'light',
          accountPrivacy: 'Public',
          notifReminders: true,
          notifMessages: true,
          notifForumReplies: true,
          notifEmail: false
        };

        let accountPrivacyMenuOpen = false;

        // Gathers the user's own content into a plain object and triggers
        // a JSON file download via a throwaway Blob URL + anchor click
        // (no server round-trip needed -- everything is already in
        // memory, kept in sync with the account via the sync engine in
        // core.js).
        async function exportUserData(){
          const btn = document.getElementById('downloadDataRow');
          const subEl = btn ? btn.querySelector('.text-xs') : null;
          const subOriginal = subEl ? subEl.textContent : null;
          if (subEl) subEl.textContent = 'Preparing your export…';
          try {
            const gameProgress = getProgressSnapshot();
            ...
            const exportPayload = {
              ...
              notebookNotes,
              jobPlanNotes,
              savedPosts: feedPosts.filter(p => p.saved),
              savedJobs: allJobs().filter(j => j.saved),
              myGlimpses,
              blockedAccounts: blockedAccounts,
              favoriteConversations: Array.from(favoriteConvos),
              gameProgress
            };
            ...
            a.href = url;
            a.download = `stitch-data-export-${new Date().toISOString().slice(0,10)}.json`;
        ...
        function setAccountPrivacy(v){
          ...
          openOverlay('profileMenu');
          // Public accounts don't gate who joins your network; any pending
          // connect requests go through immediately. Private accounts keep
          // them sitting in Notifications until you Accept/Decline.
          autoAcceptConnectionsIfPublic();
          queueSaveUserState();
        }

        // Public: incoming connection requests are accepted automatically.
        // Private: they stay pending and require a manual Accept/Decline;
        // this is the actual behavioral difference the privacy toggle drives.
        //
        // This used to loop over notifData looking for a 'connect_request'
        // notification type that nothing in the app ever actually created
        // -- incoming requests live in requestConvos (see
        // loadIncomingConnectionRequests in chat.js), not notifData -- so
        // this silently matched zero notifications and never accepted
        // anything. Now it walks requestConvos and runs each one through
        // the same acceptRequest() used for a manual Accept tap, so it
        // actually creates the connection (moves it into Primary with
        // network:true, updates the "My Network" count, clears it from
        // Discover, and writes the accepted status back to Supabase).
        function autoAcceptConnectionsIfPublic(){
          if (appPrefs.accountPrivacy !== 'Public') return;
          // Snapshot the ids first -- acceptRequest() splices requestConvos
          // as it goes, so iterating the live array directly would skip
          // entries.
        ...
        // ---------------- Notification Settings screen (opened from the
        // "Notifications" row under Settings and activity) ----------------
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto p-5">
              ...
              <div class="rounded-2xl border border-gray-100 divide-y px-4 bg-white shadow-sm">
                ...
                ${notificationSettingsRow('users','bg-purple-100','text-purple-700','Forum replies','When someone replies to your posts', appPrefs.notifForumReplies, "toggleNotifPref('notifForumReplies','notificationSettings')")}
        ...
        // ---------------- Saved items screen (Posts / Career tabs) ----------------
        let savedTab = 'posts';
        ...
        function savedItemsHTML(){
          ...
          return `
            ...
            <div id="saved-items-body" class="flex-1 overflow-y-auto p-5">
              ${savedTab === 'posts'
                ? (savedPosts.length ? `<div class="profile-thumb-grid">${savedPosts.map(profileGridItem).join('')}</div>` : savedEmptyState('No saved posts yet. Tap the bookmark icon on a post to save it here.'))
        ...
        // Full Terms of Service text, mirrored (in short form) by the
        // pre-login auth-terms-modal shown on the register/login screen
        // (see #auth-terms-modal). Update the "Last Updated" date and the
        // contact email placeholder together with that modal when this
        // text changes.
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto p-5">
              <div class="bg-white rounded-3xl p-5 shadow-sm text-sm leading-relaxed text-gray-600 space-y-3">
                ...
                <p class="text-gray-400 italic">Stitch itself is free to use. Stitch hosts online courses uploaded by course creators, and pricing for each course is set by the creator, not by Stitch. Pricing will be shown before purchase, and payments are processed by a third-party provider; we do not store card or mobile money details. Refunds are generally not offered for course access already granted, except where required by law; contact us within 7 days of a disputed charge.</p>

                <h3 class="font-bold pt-1" style="background-image:linear-gradient(135deg, #1e90ff, #4169e1 60%, #0a2540);-webkit-background-clip:text;background-clip:text;color:transparent;">7. Acceptable Use</h3>
                ...
                <h3 class="font-bold pt-1" style="background-image:linear-gradient(135deg, #1e90ff, #4169e1 60%, #0a2540);-webkit-background-clip:text;background-clip:text;color:transparent;">8. Data &amp; Privacy</h3>
                <p>We collect account data, uploaded documents, content you create (notes, flashcards, quiz results, stories, messages), usage data, and call metadata (not recordings) to provide our service. We do not sell your personal data to third parties: see our Privacy Policy for full details. You may request deletion of your account and data at any time through the Profile page.</p>

                <h3 class="font-bold pt-1" style="background-image:linear-gradient(135deg, #1e90ff, #4169e1 60%, #0a2540);-webkit-background-clip:text;background-clip:text;color:transparent;">9. Intellectual Property</h3>
                ...
                <h3 class="font-bold pt-1" style="background-image:linear-gradient(135deg, #1e90ff, #4169e1 60%, #0a2540);-webkit-background-clip:text;background-clip:text;color:transparent;">10. Account Termination</h3>
                <p>You may delete your account at any time through the Profile page, which permanently removes your notes, exam history, and messages. We reserve the right to suspend or terminate accounts that violate these Terms without notice. No refund will be issued for course access already granted if your account is terminated.</p>

                <h3 class="font-bold pt-1" style="background-image:linear-gradient(135deg, #1e90ff, #4169e1 60%, #0a2540);-webkit-background-clip:text;background-clip:text;color:transparent;">11. Limitation of Liability</h3>
                ...
                <h3 class="font-bold pt-1" style="background-image:linear-gradient(135deg, #1e90ff, #4169e1 60%, #0a2540);-webkit-background-clip:text;background-clip:text;color:transparent;">12. Changes to These Terms</h3>
                <p>We may update these Terms from time to time, updating the "Last Updated" date above. Continued use of Stitch after changes are posted constitutes acceptance of the updated terms.</p>

                <h3 class="font-bold pt-1" style="background-image:linear-gradient(135deg, #1e90ff, #4169e1 60%, #0a2540);-webkit-background-clip:text;background-clip:text;color:transparent;">13. Contact Us</h3>
        ...
        // ---------------- Help Center (in-app) ----------------
        // FAQ content grouped by topic, mirroring the app's actual feature
        // set (StudyBot, Feed/Stories, Classes/Messaging, Practice Tests &
        // Past Papers, Opportunities). Each item is keyed by a stable id so
        // helpFAQOpenIds (a Set) can track which answers are expanded.
        const helpFAQData = [
          { category: 'Getting Started', items: [
            ...
            { id: 'gs-free', q: 'Is Stitch free to use?', a: 'Yes, Stitch itself is completely free. Some courses in the Opportunities tab are posted by outside creators who set their own price, and payments for those are handled by a third-party provider.' },
          ]},
          { category: 'StudyBot AI Tutor', items: [
            { id: 'sb-help', q: 'What can StudyBot help me with?', a: "StudyBot can explain concepts, generate notes and flashcards from your uploaded documents, quiz you on a topic, and answer questions about your course material in plain language." },
            { id: 'sb-docs', q: 'Can StudyBot use my uploaded documents?', a: "Yes. Once you upload a PDF or Word file, StudyBot can read its content and use it to generate summaries, flashcards, and practice questions, or to answer your questions directly from that document." },
          ]},
        ...
        // Which FAQ ids are currently expanded.
            ...
            <div class="flex-1 overflow-y-auto px-5 pb-8">
              ...
              <div class="rounded-3xl p-5 text-center mt-4" style="background-image:linear-gradient(135deg, rgba(30,144,255,0.09) 0%, rgba(65,105,225,0.09) 100%);background-color:#ffffff;">
                ...
                <div class="flex flex-col gap-2.5">
                  <button onclick="openOverlay('contactUs')" class="w-full font-semibold text-sm py-3 rounded-2xl text-white" style="background:rgba(30,144,255,0.5);">Contact Us</button>
                  <button onclick="openOverlay('reportIssue')" class="w-full font-semibold text-sm py-3 rounded-2xl border" style="color:${NAVY};border-color:rgba(30,144,255,0.09);background-image:linear-gradient(135deg, rgba(30,144,255,0.09) 0%, rgba(65,105,225,0.09) 100%);background-color:#ffffff;">Report an Issue</button>
                </div>
              </div>
            </div>`;
        }
        ...
        async function submitContactForm(){
          ...
          alert("Thanks for reaching out! We'll get back to you at " + d.email + " within 24 hours.");
          openOverlay('profileMenu');
        }
        function contactUsHTML(){
          const d = contactFormDraft;
          return `
            ${overlayHeader('Contact Us', '20px', "openOverlay('profileMenu')")}
        ...
        async function submitIssueReport(){
          ...
          if (!d.type) { alert('Please choose what type of issue this is.'); return; }
          if (!d.description.trim()) { alert('Please describe the issue.'); return; }
          ...
          alert("Thanks for the report. Our team will look into it" + (d.email ? (" and follow up at " + d.email) : "") + ".");
          openOverlay('profileMenu');
        }
        function reportIssueHTML(){
          const d = reportIssueDraft;
          return `
            ${overlayHeader('Report an Issue', '20px', "openOverlay('profileMenu')")}
            <div class="flex-1 overflow-y-auto px-5">
