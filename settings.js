let appPrefs = {
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto p-5">
              <div class="bg-white rounded-3xl p-5 shadow-sm text-sm leading-relaxed text-gray-600 space-y-3">
                ...
                <p>To keep your practice pool useful, Stitch automatically checks each upload's extracted content against your other uploaded resources and rejects a file if it detects duplicate content -- including the exact same document uploaded again under a different file name or file type. Rejected duplicates are not processed.</p>
                ...
                <p>You may delete your account at any time through the Profile page, which permanently removes your notes, exam history, and messages. We reserve the right to suspend or terminate accounts that violate these Terms without notice. No refund will be issued for course access if your account is terminated for a violation of these Terms.</p>
        ...
        const helpFAQData = [
          { category: 'Getting Started', items: [
            ...
            { id: 'gs-free', q: 'Is Stitch free to use?', a: "Yes, Stitch itself is free. New accounts get a 7-day free trial with unlimited practice questions, flashcards, and Challenge Arena questions. After the trial, every account gets 20 Stitch Bot prompts a day, plus a monthly allowance across all your uploaded resources: 50 practice questions and 50 flashcards a month (both reset on the 1st), and a pool of 20 Challenge Arena questions. Stitch Pro (GHS 20/month) raises that to 100 prompts a day and 100 questions/flashcards a month, and Stitch Pro Plus (GHS 25/month) raises it to 200 prompts a day and 150 questions/flashcards a month. Some courses in the Opportunities tab are also posted by outside creators who set their own price, and payments for both those and Stitch Pro/Pro Plus are handled securely by Paystack." },
          ]},
        ...
        function helpCenterHTML(){
          return `
            ...
            <div class="flex-1 overflow-y-auto px-5 pb-8">
              ...
              <div class="rounded-3xl p-5 text-center mt-4" style="background-image:linear-gradient(135deg, rgba(30,144,255,0.09) 0%, rgba(65,105,225,0.09) 100%);background-color:#ffffff;margin-bottom:50px;">
        ...
        function contactInfoRow(icon, label, value){
          return `
            <div class="flex items-center px-4 py-3 rounded-2xl mb-2.5 bg-white border border-gray-300">
              <div class="min-w-0">
            ...
            <div class="flex-1 overflow-y-auto px-5">
              ...
              <div class="mb-5 flex flex-col gap-2.5">
                ${reportIssueTypes.map(t => `
                  <button onclick="setReportIssueType('${t.id}')" class="w-full px-4 py-2.5 rounded-2xl text-sm font-semibold text-left bg-white border border-gray-300" style="${d.type === t.id ? `background:rgba(10,37,64,0.08);color:${NAVY};` : `color:#4b5563;`}">
                    <span>${t.label}</span>
                  </button>
                `).join('')}
              </div>
