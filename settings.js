        ...
        function autoAcceptConnectionsIfPublic(){
          ...
          for (let i = notifData.length - 1; i >= 0; i--){
            if (notifData[i].type === 'connect_request'){
              // "My Network" is computed live from actual connections (see
              // networkConnectionCount in chat.js) rather than a manually
              // incremented counter, so there's nothing to increment here
              // -- just clear the request notification.
              notifData.splice(i, 1);
            }
          }
        }
