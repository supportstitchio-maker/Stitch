        ...
        // (no server round-trip needed since everything already lives in
        // memory / window.storage on this device).
        async function exportUserData(){
          ...
          try {
            ...
            try {
              if (window.storage) {
                const res = await window.storage.get('gameProgress', false);
                gameProgress = res ? JSON.parse(res.value) : null;
              }
        ...
        function blockedAccountRow(b){
          return `
            <div class="flex items-center gap-3 py-3">
              <div class="w-11 h-11 rounded-full ${b.avatarBg} flex items-center justify-center text-gray-600 flex-shrink-0">${Icon(b.icon,'w-5 h-5')}</div>
