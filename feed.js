        ...
        function reelPostDetailHTML(post){
          ...
          // object-fit:contain has to be set as an inline style rather
          // than a Tailwind utility class (e.g. class="object-contain")
          // here -- this project's compiled styles.css only includes the
          // handful of Tailwind utility classes actually present in the
          // static markup at build time, and .object-contain was never
          // one of them, so that class silently did nothing and both
          // photos and videos fell back to object-fit's real default,
          // `fill` (stretch to the box, ignoring aspect ratio). Videos
          // mostly happened to already be a similar aspect ratio to the
          // screen so the stretch wasn't very noticeable; photos (all
          // kinds of aspect ratios) visibly distorted. An inline style
          // always applies regardless of what the build happened to
          // compile, and letterboxes both the same way -- black bars
          // (matching the existing bg-black behind them) rather than a
          // crop or a stretch.
          const containStyle = 'object-fit:contain;background:#000;';
          return `
            <div class="relative w-full h-full bg-black overflow-hidden" id="video-post-${post.id}" ontouchstart="vpdTouchStart(event)" ontouchend="vpdTouchEnd(event, ${post.id})">
              <div class="absolute inset-0 feed-video-wrap" id="${uid}" ${isVideo ? `onclick="toggleFeedVideoPlay('${uid}')"` : ''}>
                ${media ? (isVideo
                  ? `<video src="${media.url}" class="absolute inset-0 w-full h-full" style="${containStyle}" playsinline webkit-playsinline muted disablePictureInPicture controlsList="nodownload noplaybackrate nofullscreen" autoplay loop></video>`
                  : `<img src="${media.url}" class="absolute inset-0 w-full h-full" style="${containStyle}">`) : ''}
                ${isVideo ? `
