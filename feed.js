// ---- Feed render + pull-to-refresh ----
        ...
        // Set the moment the bare shimmer skeleton (feedSkeletonHTML) is
        // painted into #screen with nothing cached to show instead; see
        // the minimum-visible-time guard at the top of the "real content"
        // branch of renderFeed() below.
        let feedSkeletonShownAt = null;
        // How long the skeleton stays up at minimum once it's shown, so a
        // fast connection (or an already-warm Supabase session) doesn't
        // make it flash by too quickly to register as a loading state.
        ...
        // Single source of truth for "does this post belong in the home
        // feed right now" -- keeps renderFeed() and patchFeedInPlace() from
        // drifting apart. A stranger's video only qualifies once it lines
        // up with this account's own search/activity signals; everything
        // else (your own posts, network members' posts, strangers' photos)
        // is unaffected.
        ...
        function renderFeed() {
          ...
          // The bare skeleton (not a cached snapshot -- that path returns
          // above and is unaffected) was just shown very recently. Wait
          // out the rest of a minimum visible window before swapping to
          // real content, so it reads as an actual loading state instead
          // of an imperceptible flash on a fast connection.
        ...
        function patchFeedInPlace(){
          ...
          // Uses the same feedVisiblePosts() the initial render uses (see
          // above) so this patch path can't drift out of sync with it --
          // a post mid-upload has no mediaHtml yet, but it should already
          // occupy its slot in the feed (shown via its skeleton in
          // feedPost()) instead of popping in only once the upload finishes.
        ...
        // struck out.
        // Media loading previously worked like this: on the first error the
        // <video>/<img> just stayed in the DOM (collapsed to the browser's
        // tiny default broken-media size, with its overlay header's dark
        // gradient still showing since the code assumed it might still
        // recover) while up to 2 silent background retries ran; only once
        // those were exhausted did it finally swap to the correctly-sized,
        // gradient-free placeholder below. On a flaky connection that
        // meant a visible ~1.2-3.6s stretch of a wrong-sized box with a
        // stray dark gradient before things settled -- exactly the
        // "gradient screen first, then the right one" flash reported.
        // Fix: build the correctly-sized, gradient-free box immediately on
        // the very first error, then keep retrying quietly *behind* it --
        // swapping the live media back in if a retry succeeds, or turning
        // the same box into "couldn't load, tap to retry" if it doesn't.
        // The user never sees the collapsed/gradient in-between state.
        ...
        function feedMediaScheduleBackgroundRetry(placeholder, kind, baseUrl, attempt){
          ...
          setTimeout(() => {
            // If the person already tapped "retry" themselves (or
            // navigated away and this post got re-rendered), this
            // placeholder node is no longer attached -- stop.
            if (!placeholder.isConnected) return;
            const sep = baseUrl.indexOf('?') === -1 ? '?' : '&';
            const probeUrl = baseUrl + sep + '_retry=' + Date.now();
            const probe = kind === 'video' ? document.createElement('video') : new Image();
            const onOk = () => feedMediaSwapInLiveMedia(placeholder, kind, baseUrl);
            const onFail = () => feedMediaScheduleBackgroundRetry(placeholder, kind, baseUrl, attempt + 1);
            if (kind === 'video') {
              probe.preload = 'metadata';
              probe.muted = true;
              probe.onloadedmetadata = onOk;
        ...
        function feedMediaSwapInLiveMedia(placeholder, kind, baseUrl){
          if (!placeholder.isConnected) return;
          ...
          const sep = baseUrl.indexOf('?') === -1 ? '?' : '&';
          fresh.src = baseUrl + sep + '_retry=' + Date.now();
          if (isVideo) {
            const freshWrap = fresh.closest('.feed-media');
            const header = freshWrap && freshWrap.querySelector('[id^="post-header-"]');
            if (header) header.classList.remove('media-header-failed');
          }
        }
        ...
        function feedMediaFinalizePlaceholder(placeholder, kind, baseUrl){
          ...
          placeholder.classList.add('flex', 'flex-col', 'items-center', 'justify-center', 'gap-1.5', 'bg-gray-100', 'text-gray-400', 'text-xs', 'italic', 'text-center', 'px-2');
          ...
          placeholder.querySelector('button').addEventListener('click', () => feedMediaSwapInLiveMedia(placeholder, kind, baseUrl));
        }

        function feedMediaEnterPlaceholder(el, kind, baseUrl){
          const isVideo = kind === 'video';
          const host = isVideo ? (el.closest('.feed-video-wrap') || el) : el;
          // Video posts render their name/avatar header as an absolutely-
          // positioned bar floating ON TOP of the video, with a dark
          // gradient behind it so white text stays legible against
          // whatever's playing underneath (see feedPostHeaderHtml's
          // "overlay" mode). That header lives in .feed-media as a
          // *sibling* of .feed-video-wrap, not inside it, so replacing
          // the wrap below leaves the header (and its black gradient)
          // floating untouched over the now-light placeholder box --
          // looking like a stray dark smear with white text nobody can
          // read well. Tag it so styles.css can flip it back to a normal
          // in-flow, dark-on-light header once there's no video left to
          // float over.
          const mediaWrap = host.closest('.feed-media');
          if (isVideo && mediaWrap) {
            const header = mediaWrap.querySelector('[id^="post-header-"]');
            if (header) header.classList.add('media-header-failed');
          }
          // Nothing in this app records a post's real media width/height at
          // upload time, and .feed-media has no reserved aspect-ratio of its
          // own -- it's a flex box that simply sizes itself to whatever's
          // inside (see styles.css). That's fine while media is healthy
          // (the real image/video dictates the size), but on a hard,
          // zero-bytes-ever-arrived failure a <video> or <img> has no
          // intrinsic size at all, so it collapses to the browser's own
          // generic broken-media box (a plain <video> defaults to ~300x150
          // CSS px) rather than anything resembling the actual post -- that
          // generic box is what was showing before, not the post's own
          // size. A poster image (videos only) is the one place real
          // dimensions might still be available, since posters are fetched
          // as plain <img>s and often succeed even when the video itself
          // doesn't -- so prefer that when present, and otherwise fall back
          // to a fixed, deliberately-chosen post aspect ratio (4:5, the
          // standard portrait shape used across most social feeds) so every
          // failed post at least renders at one consistent, sane size
          // instead of whatever the browser happened to collapse it to.
          ...
          // While a background retry is still in flight, show the same
          // neutral shimmer used elsewhere for "still loading" (skel-
          // shimmer) rather than error text -- most flaky-connection
          // failures recover within the 2 retries, so most people never
          // see anything but a normal loading state at the right size.
        ...
        async function postInsertRemote(post){
          ...
          if (!sb) return false;
          try {
            // Use the shared getCachedAuthUser() helper (same one every other
            // "who am I" check in the app uses) instead of a fresh
            // sb.auth.getUser() call. A bare sb.auth.getUser() here could
            // transiently race the session (e.g. right after signup/login,
            // before the session has fully settled) and resolve to no user,
            // writing created_by: null. Once that happens the post can never
            // be recognized as "mine" again -- loadRemotePostsImpl compares
            // row.created_by to the *real* signed-in id, null never matches,
            // so the post permanently shows up as a stranger's ("Suggested
            // for you" + Connect) post in the feed and is excluded from the
            // profile's own-posts count/grid (both filter on post.mine).
        ...
        // `prefetch`, if given, is an in-flight promise from
        // fetchRemotePostsRaw() that was already kicked off earlier (see
        // authEnterApp in games.js) -- letting the network round-trip for
        // posts overlap with some other slow round-trip (like restoring
        // account state) instead of only starting once that other call
        // has fully finished. loadRemotePostsImpl still waits to *merge*
        // the results until it's actually called, so ordering guarantees
        // (like feedPosts being restored before ghosts are pruned) are
        // untouched -- only the network request itself moves earlier.
        ...
        // Fires the posts table request by itself, with no dependency on
        // feedPosts/auth state, so it can be started as early as possible
        // (e.g. right alongside ensureUserStateLoaded on boot) instead of
        // sitting queued behind another unrelated fetch. loadRemotePostsImpl
        // awaits this same promise rather than re-issuing the request.
        ...
        async function loadRemotePostsImpl(prefetch){
          ...
          try {
            // See the matching note in postInsertRemote: use the shared
            // cached-auth-user helper here too, so "who am I" resolves to
            // the exact same id every insert and every reload compare
            // against, instead of two independent sb.auth.getUser() calls
            // that can disagree under a session race.
            const me = await getCachedAuthUser();
            // Use the already-in-flight request if one was handed in
            // (see fetchRemotePostsRaw/authEnterApp) so this call doesn't
            // wait for -- or redundantly re-issue -- a fetch that was
            // already started earlier while some other boot task (like
            // restoring account state) was also in flight. Falls back to
            // firing the request itself for every other caller (polling,
            // pull-to-refresh, etc).
        ...
        // A stranger's video only gets the "Suggested for you" treatment
        // (and only shows up in the feed at all) when it lines up with
        // this account's own search/activity signals -- see
        // postMatchesContentInterest() in core.js. Everything else about a
        // stranger's post (photos, and anyone already in your network)
        // is unaffected.
...
const GOOGLE_DRIVE_LOGO_URI = "data:image/png;base64,";
        ...
        // Pulls a thumbnail straight out of a post's already-rendered media
        // HTML (same lightweight regex approach postMediaKind above uses)
        // instead of re-deriving it from raw upload data, so this works for
        // any post regardless of how its media was produced.
        //
        // FIX: this used to match `(?:poster|src)` in one combined regex.
        // A video tag is rendered as `<video src="VIDEO_URL" poster="POSTER_URL">`
        // -- src comes first in the markup, so that combined regex always
        // matched "src" before it ever got to "poster", handing back the
        // raw VIDEO FILE's own URL as the "thumbnail". A video file can't
        // be rendered through an <img> tag, so a shared video's preview
        // card (see convoSharedPostAttachmentsHTML) came out blank/broken
        // instead of showing a frame from the video the way a shared
        // picture does. Now each media tag is inspected on its own terms:
        // a picture always uses its own src (unchanged behavior), a video
        // always uses its poster frame -- the thumbnail image generated
        // from the video at upload time (see generateVideoPosterFile) --
        // and NEVER its src. A multi-photo/video post can have several
        // media tags in mediaHtml; they're walked in the order they
        // appear so the first one with a usable image wins, same as
        // before.
        function postShareThumbnailUrl(post){
          ...
          while ((match = tagPattern.exec(post.mediaHtml))) {
            ...
            if (/^<img/i.test(tag)) {
              ...
              // Video: only its poster frame counts as a thumbnail. A
              // video with no poster yet (e.g. an older post from before
              // poster generation existed) is skipped rather than
              // falling back to its own (unusable) src -- a later item
              // in a multi-media post might still have a real thumbnail.
        ...
        // Shape of a "shared post" chat attachment. This rides in the same
        // `attachments` JSON column messages already use for photos/files --
        // no backend schema change needed -- so it survives being sent,
        // stored, and re-delivered to the other person exactly like any
        // other attachment.
        ...
        function sendPostToSelectedContacts(postId){
          ...
          // The caption now travels inside the shared-post card itself, so
          // the plain-text bubble only needs to exist as an inbox preview /
          // fallback for clients that don't render the card.
        ...
        // Tapping a shared-post card in a chat -- opens straight to that
        // post using the same single-post "post feed" overlay the rest of
        // the app already uses for deep views, so the back button returns
        // cleanly to the conversation instead of leaving the modal stack.
        ...
        // Google Drive has no web share-target for a plain link the way
        // WhatsApp/SMS/Gmail do, so the closest useful action is copying
        // the link and pointing the person at Drive to paste it in.
        ...
        // Browsers can't push text/files over Bluetooth directly (no web
        // API for that), so Bluetooth routes through the OS share sheet,
        // same as the "Share" icon -- Bluetooth is one of the targets the
        // OS offers there on devices that support it.
