// ---- Feed render + pull-to-refresh ----
        ...
        function reorderStoriesByViewed(){
          ...
          // The admin's glimpse always sits first among "others", ahead of the normal
          // viewed/unviewed sort, so it's the first thing anyone sees in the story bar.
        ...
        async function loadRemoteGlimpses(){
          ...
          try {
            ...
            // Everyone else's glimpses still expire after 24h (created_at.gte cutoff), but the
            // admin account's glimpses never expire out of this query (user_id.eq admin, no
            // cutoff needed for those rows) -- so an admin glimpse stays visible to everyone
            // indefinitely, until the admin deletes it.
            const { data, error } = await sb.from(GLIMPSES_TABLE)
              .select('id, user_id, data, created_at')
              .or(`created_at.gte.${cutoffIso},user_id.eq.${ADMIN_USER_ID}`)
        ...
        function rankFeedPosts(){
          ...
          const scored = feedPosts.map(post => {
            let score = 0;
            // Admin posts always float to the very top of everyone's feed, no matter how old
            // they are or whether the viewer is connected to the admin account. This outweighs
            // every other factor below (age decay, likes, network affinity, jitter, etc.).
