          ...
          if (s.mine) {
            ...
            return `
              <div class="flex flex-col items-center gap-1.5 flex-shrink-0 w-20">
                <div class="relative">
                  <button onclick="openMyGlimpses()" class="w-20 h-20 rounded-full overflow-hidden flex items-center justify-center" style="padding:3.5px;background:${mineRingBg};">
        ...
        // Glimpses top out at 60 seconds each. A picked video longer than
        // that gets cut into consecutive 60-second glimpses (see
        // splitAndPostLongGlimpseVideo below) instead of being posted as
        // one long clip.
        ...
        function handleGlimpseMediaSelected(e){
          ...
          // even though the rest of this function runs async below.
          e.target.value = '';
          if (!file) return;
          const isVideo = file.type.startsWith('video/');
          if (!isVideo) {
            // Object URLs (short "blob:..." references) rather than
            // FileReader/base64 data URLs -- a data URL for anything more
            // than a couple MB gets embedded straight into the caption
            // screen's HTML string, and large ones (especially video,
            // see below) can blow past the JS engine's max string length
            // and crash the screen outright.
            openGlimpseMediaPreview(file, URL.createObjectURL(file), 'image');
            return;
          }
          // Probe the real duration via a throwaway <video> + object URL
          // (cheap -- no base64 involved) before deciding whether to post
          // it as one glimpse or split it.
          ...
          probe.onloadedmetadata = function(){
            ...
            if (!duration || !isFinite(duration) || duration <= GLIMPSE_MAX_SECONDS) {
              openGlimpseMediaPreview(file, URL.createObjectURL(file), 'video');
            } else {
              splitAndPostLongGlimpseVideo(file, duration);
            }
          };
          probe.onerror = function(){
            URL.revokeObjectURL(probeUrl);
            // Couldn't read its metadata -- fall back to posting it whole
        ...
        // Shared by both the plain (<=60s) video path and images: shows
        // the full-screen caption step with the picked media.
        function openGlimpseMediaPreview(file, url, type){
        ...
        // ---------------- Splitting long glimpse videos ----------------
        // A video longer than GLIMPSE_MAX_SECONDS gets cut into
        // back-to-back clips (no caption step for these -- there's no one
        // good caption for the whole set) and each is posted as its own
        // glimpse, in order, so the full video is still there for viewers
        // just spread across several glimpses instead of one over-length
        // one.
        let glimpseSplitInProgress = false;

        async function splitAndPostLongGlimpseVideo(file, duration){
          if (glimpseSplitInProgress) return;
          glimpseSplitInProgress = true;
          const segmentCount = Math.ceil(duration / GLIMPSE_MAX_SECONDS);
          showGlimpseSplitProgress(0, segmentCount);
          try {
            if (!requireCompleteProfile()) return;
            for (let i = 0; i < segmentCount; i++){
              updateGlimpseSplitProgress(i, segmentCount);
              const start = i * GLIMPSE_MAX_SECONDS;
              const end = Math.min(duration, start + GLIMPSE_MAX_SECONDS);
              const blob = await recordGlimpseSegment(file, start, end);
              if (!blob || !blob.size) continue; // skip a segment this browser couldn't capture rather than aborting the rest
              const ext = (blob.type || 'video/webm').split('/')[1] || 'webm';
              const baseName = (file.name || 'glimpse').replace(/\.[^.]+$/, '');
              const segFile = new File([blob], `${baseName}-part${i + 1}.${ext}`, { type: blob.type || 'video/webm' });
              myGlimpsesViewed = false;
              await GlimpsesAPI.create({
                caption: segmentCount > 1 ? `Part ${i + 1} of ${segmentCount}` : '',
                icon: 'camera',
                bgStyle: 'background:linear-gradient(135deg,#374151,#111827)',
                mediaUrl: URL.createObjectURL(segFile),
                mediaType: 'video',
                mediaFile: segFile,
              });
            }
            ...
            hideGlimpseSplitProgress();
            openMyGlimpses();
          }
        }

        // Plays `file` from `start` to `end` seconds and records just
        // that window via captureStream() + MediaRecorder, resolving with
        // the recorded Blob (or null if this browser can't capture video
        // streams at all, so the caller can skip that segment instead of
        // hanging forever).
        function recordGlimpseSegment(file, start, end){
          return new Promise((resolve) => {
            ...
            if (typeof video.captureStream !== 'function') { cleanup(null); return; }

            video.onloadedmetadata = () => { video.currentTime = start; };
            video.onerror = () => cleanup(null);
            video.onseeked = () => {
              let stream;
              try { stream = video.captureStream(); } catch(err){ cleanup(null); return; }
              let recorder;
              try { recorder = new MediaRecorder(stream, { mimeType: 'video/webm' }); }
              catch(err) {
                try { recorder = new MediaRecorder(stream); }
                catch(err2){ cleanup(null); return; }
              }
              const chunks = [];
              recorder.ondataavailable = (ev) => { if (ev.data && ev.data.size) chunks.push(ev.data); };
              recorder.onstop = () => cleanup(new Blob(chunks, { type: recorder.mimeType || 'video/webm' }));
              recorder.onerror = () => cleanup(chunks.length ? new Blob(chunks, { type: recorder.mimeType || 'video/webm' }) : null);
              recorder.start();
              video.play().catch(() => {});
              const stopWhenDone = () => {
              ...
              // Safety net in case timeupdate stalls for this segment.
              setTimeout(() => { if (recorder.state !== 'inactive') recorder.stop(); }, (end - start) * 1000 + 4000);
            };
          });
        }

        // Small full-screen progress indicator shown while a long video
        // is being split -- this runs in real time (each segment plays
        // out to be captured), so it can take a while for a long video.
        function showGlimpseSplitProgress(i, total){
          let el = document.getElementById('glimpse-split-progress');
          if (!el) {
            el = document.createElement('div');
            el.id = 'glimpse-split-progress';
            el.style.cssText = 'position:fixed;inset:0;z-index:70;background:rgba(10,15,25,0.82);display:flex;flex-direction:column;align-items:center;justify-content:center;color:#ffffff;font-family:inherit;text-align:center;padding:0 2rem;';
            document.body.appendChild(el);
          }
          updateGlimpseSplitProgress(i, total);
        }

        function updateGlimpseSplitProgress(i, total){
          const el = document.getElementById('glimpse-split-progress');
          if (!el) return;
          el.innerHTML = `
            <div style="width:2.75rem;height:2.75rem;border-radius:9999px;border:3px solid rgba(255,255,255,0.25);border-top-color:#ffffff;animation:classroom-spin 0.9s linear infinite;margin-bottom:1rem;"></div>
            <div style="font-size:0.95rem;font-weight:600;">Splitting your video into glimpses</div>
            <div style="font-size:0.8rem;color:rgba(255,255,255,0.7);margin-top:0.35rem;">Part ${i + 1} of ${total} &middot; this video is over 60 seconds</div>`;
        }

        function hideGlimpseSplitProgress(){
          const el = document.getElementById('glimpse-split-progress');
          if (el) el.remove();
        }
        ...
        // Very tall ("long") images -- a forwarded screenshot, a full
        // chat thread, anything much taller than a normal portrait photo
        // -- get cropped to fill the feed card at their real width
        // instead of shrinking down to fit and leaving big gray bars on
        // either side (see .feed-media-tall in styles.css). Fired via
        // each <img>'s onload once its real dimensions are known --
        // ordinary photos never cross the threshold and keep showing in
        // full. The badge is just a visual hint that there's more to see
        // on tap; post-detail-full-media hides it and drops the crop once
        // the post is actually opened.
        function markTallFeedMedia(img){
          if (!img || !img.naturalWidth || !img.naturalHeight) return;
          if (img.naturalHeight / img.naturalWidth <= 1.25) return;
          const wrap = img.closest('.feed-media');
          if (!wrap || wrap.classList.contains('feed-media-tall')) return;
          wrap.classList.add('feed-media-tall');
          if (!wrap.querySelector('.feed-media-tall-badge')) {
            wrap.insertAdjacentHTML('beforeend', '<span class="feed-media-tall-badge">Tap to view full</span>');
          }
        }
        ...
        function postMediaHtmlFromUploaded(items){
          const itemHtml = (it) => it.type === 'video'
            ? `<video src="${it.url}" controls class="w-full h-auto bg-black" onerror="this.onerror=null;this.replaceWith(Object.assign(document.createElement('div'),{className:'w-full py-10 flex items-center justify-center bg-gray-100 text-gray-400 text-xs italic',textContent:'Video no longer available'}))"></video>`
            : `<img src="${it.url}" class="w-full h-auto" onload="markTallFeedMedia(this)" onerror="this.onerror=null;this.replaceWith(Object.assign(document.createElement('div'),{className:'w-full py-10 flex items-center justify-center bg-gray-100 text-gray-400 text-xs italic',textContent:'Image no longer available'}))">`;
        ...
        const GlimpsesAPI = {
          ...
          create(data){
            ...
            return (async () => {
              ...
              if (mediaFile) {
                const uploaded = await uploadPostMediaToStorage(mediaFile, glimpse.id);
                if (uploaded) {
          ...
          return `
            <div class="bg-white overflow-hidden -mx-5 border-b border-gray-100" id="post-${post.id}">
              ...
              <div class="px-3.5 pt-1 text-[13px] leading-snug">
                ...
                <span class="inline-flex items-center justify-center rounded align-middle mr-1" style="width:1.15rem;height:1.15rem;padding:2px;background:rgba(30,144,255,0.12);color:${NAVY};" title="${post.tag}">${Icon(post.tag === 'Video' ? 'videoClip' : 'photoFrame','w-3 h-3')}</span>
        ...
        function postListForSource(source){
          if (source === 'reposts') return feedPosts.filter(p => p.reposted);
          if (source === 'saved') return feedPosts.filter(p => p.saved);
          if (typeof source === 'string' && source.indexOf('author-videos:') === 0) {
            const authorId = source.slice('author-videos:'.length);
            return feedPosts.filter(p => String(p.authorId) === authorId && !p.isRepost && postMediaKind(p) === 'video');
          }
          if (typeof source === 'string' && source.indexOf('author-pictures:') === 0) {
            const authorId = source.slice('author-pictures:'.length);
            return feedPosts.filter(p => String(p.authorId) === authorId && !p.isRepost && postMediaKind(p) === 'picture');
          }
          if (typeof source === 'string' && source.indexOf('author-reposts:') === 0) {
            const authorId = source.slice('author-reposts:'.length);
            return feedPosts.filter(p => String(p.authorId) === authorId && p.isRepost);
          }
          if (typeof source === 'string' && source.indexOf('author:') === 0) {
            const authorId = source.slice('author:'.length);
            return feedPosts.filter(p => String(p.authorId) === authorId);
          }
          return feedPosts.filter(p => p.mine);
        }
        ...
        function shareContactCell(postId, c){
          return `
            <button id="share-cell-${postId}-${c.id}" data-name="${escapeHtml(c.name.toLowerCase())}" onclick="sendPostToContact(${postId}, '${c.id}')" class="flex flex-col items-center gap-1.5 text-center">
              <div class="relative">
                <div class="w-14 h-14 ${c.avatarBg} rounded-full flex items-center justify-center text-gray-600 overflow-hidden">${avatarInnerHTML(c,'w-6 h-6')}</div>
                <div id="share-check-${postId}-${c.id}" class="hidden absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full bg-[${ROYAL}] border-2 border-white flex items-center justify-center text-white">${Icon('check','w-3 h-3')}</div>
              </div>
              <div id="share-label-${postId}-${c.id}" class="text-xs font-medium text-gray-700 truncate w-14">${escapeHtml(c.name.split(' ')[0])}</div>
            </button>`;
        }
