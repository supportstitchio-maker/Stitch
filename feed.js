// ---- Feed render + pull-to-refresh ----
        ...
        function feedMediaShowRetryPlaceholder(el, kind, baseUrl){
          ...
          // Capture the actual on-screen size of whatever's being replaced
          // *before* swapping it out, so the placeholder can be pinned to
          // those exact pixel dimensions below. Every post/video has its own
          // aspect ratio (portrait photo, square, 16:9 video, etc.) -- if the
          // placeholder just used a fixed generic height instead, it would
          // shrink or stretch posts to a one-size-fits-all box, causing the
          // whole feed to visibly reflow/jump as media fails and successive
          // posts jam together or leave awkward gaps.
          ...
          if (!isAbsolute && rect.width > 0 && rect.height > 0) {
