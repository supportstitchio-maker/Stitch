        ...
        function Icon(type, cls){
          const paths = {
            ...
            // Chunkier, flat-style thumbs-up (matches the reference icon
            // supplied for the feed "like" button) -- a solid thumb with
            // a separate rounded rectangle "handle" block beside it,
            // rather than the thinner single-outline glyph this used to
            // be.
        ...
        function myGlimpseRow(g){
          // The dropdown menu lives in its own container (#glimpse-menu-<id>)
          // instead of being part of the same template string that's
          // regenerated wholesale on every toggle. toggleMyGlimpseMenu used
          // to call refreshMyGlimpses(), which rebuilt the *entire* list
          // (myGlimpsesHTML()) from scratch just to open one row's menu --
          // every row's <img>/<video> thumbnail in the process, including
          // ones with nothing to do with the menu, reloaded and visibly
          // flashed. Only this small container is touched now.
          return `
        ...
        function toggleMyGlimpseMenu(id){
          ...
          // Patch just the menu container(s) that actually changed -- the
          // one being opened, and the one being closed if a different
          // row's menu was already open -- instead of refreshMyGlimpses()'s
          // full-list rebuild.
        ...
        async function loadGlimpseViewers(g){
          ...
          try {
            ...
            // Patch just the eye-button/viewers-panel container instead of
            // the whole viewer screen -- a full ov.innerHTML rebuild here
            // would tear down and reload the glimpse's <video>/<img> the
            // instant the real viewer list resolved, right after it had
        ...
        function toggleMyGlimpseViewers(id){
          ...
          const g = myGlimpses.find(x => x.id === id);
          if (!g) return;
          // Only re-render the eye-button/viewers-panel container, same
          // reasoning as loadGlimpseViewers above -- toggling "Viewed by"
          // open and closed used to rebuild the entire screen via
          // myGlimpseViewerHTML(), reloading (and restarting/flashing) the
          // glimpse's own video every single time.
        ...
        // The eye-count button plus (when expanded) the "Viewed by" sheet
        // over the glimpse. Split out of myGlimpseViewerHTML into its own
        // container so opening/closing it, or a viewer list finishing its
        // load, only ever touches this piece -- never the glimpse's own
        // <video>/<img> sitting above it. See toggleMyGlimpseViewers and
        // loadGlimpseViewers.
        ...
        // Glimpse videos are capped at this length. A longer clip is
        // posted as a single glimpse same as any other -- just capped to
        // its first GLIMPSE_MAX_SECONDS seconds during playback (see
        // glimpseVideoAttrs) -- rather than re-encoded/split into several
        // glimpses, which meant capturing the whole clip back in real
        // time and could take as long as the source video itself.
        const GLIMPSE_MAX_SECONDS = 60;

        // Shared <video> attributes for every place a glimpse's own video
        // renders (compose preview, thumbnail row, my/other viewers, story
        // viewer): loops normally under the cap, and silently restarts
        // from 0 the moment playback would cross GLIMPSE_MAX_SECONDS, so a
        // longer source clip is simply never seen past its first minute --
        // no upfront re-encoding, so nothing to wait on.
        ...
        function handleGlimpseMediaSelected(e){
          ...
          // once it's known -- doesn't block opening the preview. Uses
          // its own separate blob: URL rather than reusing previewUrl:
          // previewUrl is actively displayed by the two <video> elements
          // in glimpseMediaCaptionHTML (blurred background + foreground
          // preview), and revoking a blob: URL invalidates it everywhere
          // it's used, not just for this probe -- reusing it here made
          // the metadata callback below blank out the preview the user
          // was looking at (revoked the instant metadata loaded, often
          // before the visible video had even painted its first frame).
            ...
            <div class="flex flex-col h-full bg-black text-white relative">
              <div class="absolute inset-0 overflow-hidden">
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                  ${glimpseMediaPreviewType === 'video'
                    // Only the blurred background copy above stays muted
                    // (it's a decorative duplicate playing in lockstep --
                    // unmuting it too would just double up the audio).
                    // This foreground copy is the one actually being
                    // previewed before posting, so it plays with sound.
                    // Autoplay-with-sound is allowed here because opening
                    // this screen is a direct result of the file picker's
                    // onchange, which itself only fires from a user
                    // gesture (selecting a file) -- the same activation
                    // Chrome checks before permitting unmuted autoplay.
        ...
        // scrolled back out -- same as Instagram/TikTok feed video. There
        // is no progress/scrub bar and no overflow menu at all. A small
        // volume icon in the corner (see toggleFeedVideoMute below) is the
        // one other control -- feed video starts muted (autoplaying with
        ...
        // tap it to unmute.
        //
        // This wrap deliberately has no onclick of its own -- routing a
        // tap to play/pause vs. opening the full post depends on *where*
        // on the media someone tapped (see handleFeedMediaTap on the
        // outer .feed-media wrapper, which calls toggleFeedVideoPlay
        // directly for a center tap), and having a second, separate click
        // handler here as well meant every tap fired both at once: a tap
        // square in the middle -- meant only to pause the video -- also
        // opened the full post right along with it.
        ...
        // visible on screen, and pauses again once it's scrolled mostly
        // back out -- same behavior as Instagram/TikTok feed video --
        // rather than requiring an explicit tap every time. Muted by
        // default (browsers block autoplay-with-sound that isn't
        // triggered by a direct tap), same as simplePostVideoHtml's
        // default state; the corner mute button still works exactly the
        // same either way. Re-armed after every feed/post-list render
        // (renderFeed, patchFeedInPlace, postFeedHTML) since each of
        // those can add, remove, or move video elements the previous
        // observer wasn't watching.
        //
        // Play and pause each use a *different* visibility threshold
        // (0.6 to start, 0.15 to stop) rather than one shared cutoff --
        // a video sitting right at the edge of view, which happens
        // constantly during an ordinary slow scroll, would otherwise
        // cross a single threshold back and forth on every tiny scroll
        // jitter, restarting and stopping the video over and over --
        // which is what "the video keeps blinking" actually was. With
        // two thresholds a video has to clearly enter view to start and
        // clearly leave to stop, so hovering right at the edge just
        // holds whatever state it was already in instead of flickering.
        ...
        function setupFeedVideoAutoplay(container){
          ...
          feedVideoObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
              ...
              // Between 0.15 and 0.6: deliberately left alone -- whatever
              // state the video was already in stays, which is what
              // actually stops the flicker at the boundary.
            });
        ...
        // or more render as a static photo grid (one large tile plus the
        // rest stacked alongside it -- the same "1 big + up to 3 small"
        // layout LinkedIn/Facebook use for a multi-photo post) instead of
        // the old horizontally swipeable carousel. Each tile opens the
        // full media gallery (postMediaGalleryHTML) at its own index when
        // tapped, rather than the whole post's media only ever showing one
        // pane at a time in the feed. Each item keeps its own onerror
        // fallback so one broken file doesn't take out the whole post.
        ...
        // One grid tile: a fixed-position cell (its placement/span comes
        // from `areaStyle`, set by the caller below) showing the item
        // cropped to fill the cell (object-fit:cover -- unlike the single-
        // item path above, a grid cell has a fixed shape it needs to fill
        // evenly, the same tradeoff every app with a photo grid makes).
        // Tapping any tile opens the swipeable full gallery at exactly
        // that photo (see postMediaGalleryHTML) instead of always landing
        // on the first one. The very last visible tile can carry a
        // "+N" overlay for however many additional items didn't get a
        // tile of their own.
        ...
        // A grid tile's onclick above needs the id of the post it belongs
        // to, but postMediaHtmlFromUploaded/postMediaGridHtml only ever
        // receive the raw items -- not the post -- since they're also
        // used for the not-yet-saved local preview (which has no post id
        // yet at all). Reading it off the ancestor .feed-media wrapper's
        // id (id="post-<id>" is set two levels up on the card itself, see
        // feedPost) at click time avoids threading a post id through
        // every layer just for this.
        function postGridOwnerId(tileEl){
          const card = tileEl.closest('[id^="post-"]');
          return card ? Number(card.id.replace('post-', '')) : null;
        }

        // The "1 big + up to 3 stacked" grid itself: 2 items sit side by
        // side; 3 or more put one large tile on the left (spanning the
        // full height) with the rest stacked in a column on the right,
        // same as a LinkedIn/Facebook multi-photo post. A 5th-and-beyond
        // item never gets its own tile -- the 4th tile instead shows a
        // "+N" count over itself for however many are left, since a grid
        // only ever has room for 4 tiles before it'd need to keep growing
        // taller with every extra photo.
        function postMediaGridHtml(items){
          ...
          // Only the first 4 items get an actual grid tile -- but the
          // gallery viewer (postMediaItemsList) finds its list of photos
          // to swipe through by parsing every img/video tag out of this
          // same mediaHtml, so items 5+ need *some* tag here too, or
          // they'd have nowhere to come from once someone taps the "+N"
          // tile expecting to see the rest. Rendered hidden (not as a
          // tile of their own -- there's no room left in the grid for
          // them), just present in the markup so the gallery can page to
        ...
        // Pulls every photo/video out of a post's rendered mediaHtml,
        // whichever shape it's in -- a plain single img/video tag, or the
        // grid's several tiles -- so the full gallery viewer (see
        // openPostMediaGallery) can page through the actual list instead
        // of only ever knowing about post.mediaHtml as one opaque string.
        // Works on posts saved before the grid existed too (an old
        // carousel's panes are plain img/video tags same as the grid's),
        ...
        // Refreshes every on-screen copy of one specific post's card (its
        // dots menu, delete, "Not interested", Report, ...) without
        // rebuilding the whole feed or overlay around it. A post can be
        // showing in more than one place that shares the same
        // id="post-<id>" markup -- the Home feed's #feed-list, and the
        // scrollable #post-feed-list a profile's grid opens (see
        // openPostFeedFrom) -- and those live inside two different
        // containers, #screen and #overlay. renderFeed() only ever
        // rebuilds #screen's Home feed, so calling it after toggling a
        // post's menu did nothing at all when that post was actually
        // showing inside the profile's #overlay list instead -- the
        // dropdown just silently never appeared, which is what made the
        // three-dot menu (and Delete post inside it) look broken whenever
        // it was opened from Profile rather than Home. Patching every
        // matching element in place -- there can be zero, one, or two,
        // depending on what's currently on screen -- fixes that instead
        // of guessing which container to rebuild, and also avoids the
        // scroll-position/video-restart disruption a full rebuild causes.
        function refreshFeedPostCard(id){
          ...
          // Whichever list is actually visible just had its video(s)
          // recreated by the replace above -- hand them back to the
          // scroll-triggered autoplay observer, which only ever knows
          // about the specific <video> elements it was last told to
          // watch. Only the visible list matters here (see openPostDetail
          // for why a hidden one shouldn't have an active observer at
          // all): the overlay covers the Home feed whenever it's open, so
          // that's the one worth re-arming if it's showing.
        ...
        // The backdrop + dropdown markup for a post's three-dot menu,
        // shared between feedPost's initial render and the direct DOM
        // patching in togglePostMenu/reportPost below. Returns '' when
        // this post's menu isn't the one currently open.
        ...
        // Drops just the backdrop + dropdown nodes for a post's menu,
        // wherever it's currently showing (there can be zero, one, or
        // two copies -- see refreshFeedPostCard) -- without touching
        // anything else in that post's card.
        ...
        // Opening/closing used to call refreshFeedPostCard(), which
        // rebuilds the post's *entire* card -- including its image/video
        // in .feed-media -- just to show or hide a dropdown next to the
        // three-dot button. Replacing that media node made it reload from
        // scratch, which is what caused the visible blink/flash on the
        // post every time the menu was opened. Patching only the header
        // (where the dropdown actually lives, id="post-header-<id>")
        // leaves the media untouched, so no more blink.
        function togglePostMenu(id){
          ...
          // Watch whichever container is actually scrollable around this
          // post right now -- #screen's Home feed, or the profile's own
          // #post-feed-list inside the overlay -- rather than always
          // #screen, which never scrolls (and never fires a 'scroll'
          // event) while the overlay is open on top of it.
          const overlayEl = document.getElementById('overlay');
          const overlayVisible = overlayEl && !overlayEl.classList.contains('hidden');
        ...
        function deletePost(id){
          PostsAPI.remove(id).then(() => {
            ...
            // -- no separate counter to update here. refreshFeedPostCard
            // removes this post's own card wherever it's showing (Home
            // feed and/or a profile's postFeed overlay -- see its comment
            // above) instead of rebuilding the whole feed, which is what
            // let Delete post silently do nothing when opened from
            // Profile: renderFeed() only ever touches the Home feed's own
            // #screen, never the profile overlay's #post-feed-list.
            refreshFeedPostCard(id);
            // Previously only ran renderProfile() (a header-only patch,
            // see patchProfileHeaderInPlace in profile.js) and only when
            // Profile happened to be the active tab -- so the deleted
            // post's tile lingered in the Posts/Reposts/Saved grid until
            // you left and came back to Profile. refreshProfilePostsUI
            // updates both the count and the grid itself, and is a no-op
            // if Profile isn't currently mounted.
            if (typeof refreshProfilePostsUI === 'function') refreshProfilePostsUI();
          });
        }
        ...
        function reportPost(id){
          ...
          queueSaveUserState();
          openPostMenuId = null;
          // Just closing the dropdown here (not refreshFeedPostCard) --
          // the menu is going away either way, so there's no need to
          // rebuild the whole card (and its media) just to reflect the
          // reported state in a dropdown that's about to disappear. See
          // togglePostMenu for why that rebuild caused a blink.
          removePostMenuDom(id);
        ...
        // Routes a tap on a feed card's media area (the .feed-media
        // wrapper, whatever it contains) to opening the full post --
        // every tap opens it, whether it lands on a photo, a video, or a
        // carousel, the same way tapping a photo always has. The two
        // exceptions are the video's own small centered play/pause
        // button and its corner mute button (see simplePostVideoHtml):
        // both call event.stopPropagation() in their own onclick, so a
        // tap that actually lands on either of those never reaches this
        // handler at all -- only taps everywhere else on the media do,
        // and those always open the post.
        ...
        // the only place that renders their swipeable carousel. Reached
        // either through a side-tap on a video's media (see
        // handleFeedMediaTap above) or any tap on a photo/carousel's.
        //
        ...
        function openPostDetail(id, evt){
          ...
          // The scroll-triggered autoplay observer (setupFeedVideoAutoplay)
          // watching whichever list this post was opened from is still
          // live at this point -- just pausing the videos above doesn't
          // stop it from resuming one of them again on its own the next
          // time it fires, which is what let a video behind this
          // full-screen view start playing again even though nothing on
          // screen should be able to. Disconnecting it here means only
          // this one post's own player (autoplay="" on its <video>, see
          // reelPostDetailHTML/postDetailHTML) is ever playing while this
          // view is open; returning to the list re-arms it (see
          // openOverlay's 'postFeed' case and renderFeed/patchFeedInPlace).
          if (feedVideoObserver) { feedVideoObserver.disconnect(); feedVideoObserver = null; }
        ...
        function navigateReelPost(currentId, direction){
          ...
          const ov = document.getElementById('overlay');
          if (!ov) return;
          // Explicitly pause whatever's currently playing before swapping
          // it out -- innerHTML below tears the old <video> out of the
          // document either way, but pausing it first (rather than
          // relying on removal alone) guarantees it actually stops right
          // away instead of possibly bleeding a moment of sound/frames
          // into the next post's video.
        ...
        // The backdrop+foreground pair behind a full-bleed photo/video: a
        // photo that doesn't share the screen's aspect ratio used to just
        // letterbox against a flat black background (object-fit:contain
        // leaves the leftover top/bottom or left/right bars empty) --
        // this fills those bars with the same photo itself, scaled up and
        // blurred, so the space around it reads as part of the photo's
        // own frame instead of a generic black gap. The blurred copy is a
        // separate element behind the sharp one (not a filter on a shared
        // wrapper), since a CSS filter also blurs anything nested inside
        // it -- a wrapper-level blur would've taken the sharp photo down
        // with it. Video skips this: there's no cheap way to blur a
        // playing video's own frame as a background without a live canvas
        // capture, so it keeps the plain black backdrop it always had.
        ...
        // ---------------- Multi-photo post gallery (grid tap-to-open) ----------------
        // The full-bleed, swipeable viewer a grid tile (see
        // postMediaGridTile) opens into -- one photo/video per screen,
        // each framed on its own terms via framedMediaLayerHtml above,
        // paged by a left/right swipe between the *other items in this
        // same post* (unlike the reel view's up/down swipe, which pages
        ...
        // Touch tracking for the gallery's left/right swipe between this
        // post's own photos -- same swipe-threshold approach as
        // vpdTouchStart/vpdTouchEnd above, just watching dx instead of dy
        // (paging sideways through one post's items instead of vertically
        // between posts).
