// ---- Feed render + pull-to-refresh ----
        ...
        // mediaPath/mediaPaths). Same author + same caption, posted within
        // a few minutes of each other: keep one, drop the rest. Media isn't
        // part of the match on purpose -- two rows from one accidental
        // double-submit still count as the same post even though their
        // uploads landed at different paths. Reposts are left alone since
        // two people can legitimately repost the same original within the
        // same window, and a blank caption alone isn't specific enough to
        // treat as a match (two unrelated uncaptioned posts by the same
        // person minutes apart happen), so those still also compare media.
        function dedupeRemotePostRows(rows){
          ...
          rows.forEach(row => {
            ...
            const sig = body ? [row.created_by, body].join('|') : [row.created_by, body, mediaKey].join('|');
