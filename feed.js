            ...
            <div class="flex flex-col h-full bg-white relative">
              ...
              <div class="flex-1 overflow-y-auto">
                ...
                <div class="px-6 pt-2 pb-4 flex justify-center">
                  <button onclick="composeGlimpseMedia()" class="flex items-center gap-2 rounded-full pl-3 pr-4 py-2" style="color:${NAVY};background:rgba(10,37,64,0.08);">
        ...
        function shareContactCell(postId, c){
          return `
            <button id="share-cell-${postId}-${c.id}" data-name="${c.name.toLowerCase()}" onclick="sendPostToContact(${postId}, '${c.id}')" class="flex flex-col items-center gap-1.5 text-center">
              <div class="relative">
                <div class="w-20 h-20 ${c.avatarBg} rounded-full flex items-center justify-center text-gray-600 overflow-hidden">${avatarInnerHTML(c,'w-8 h-8')}</div>
        ...
        function extractPostMedia(post){
          ...
          const srcMatch = post.mediaHtml.match(/src="([^"]+)"/);
          if (!srcMatch) return null;
          const isVideo = /^\s*<video/i.test(post.mediaHtml);
          return { url: srcMatch[1], type: isVideo ? 'video' : 'image' };
        }
