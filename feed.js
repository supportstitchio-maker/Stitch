// ---- Feed render + pull-to-refresh ----
        ...
        function shareHTML(post){
          ...
          return `
            ...
            <div class="flex-shrink-0 w-full flex items-start no-scrollbar border-t border-gray-100" style="gap:22px;padding:16px 20px calc(env(safe-area-inset-bottom, 12px) + 16px) 20px;background:#fafafa;overflow-x:auto;">
              ${shareExternalOption('link','Copy link', `copyPostLink(${post.id})`)}
              ${shareExternalOption('send','Share', `sharePostExternally(${post.id})`)}
            </div>
        ...
        function shareExternalOption(icon,label,onclick){
          return `
            <div class="flex flex-col items-center gap-2 flex-shrink-0" style="width:60px;">
              <button onclick="${onclick}" title="${label}" class="rounded-full flex items-center justify-center flex-shrink-0" style="width:52px;height:52px;background:#eef0f4;color:${NAVY};">${Icon(icon,'w-5 h-5')}</button>
