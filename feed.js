        ...
        // wrapper, whatever it contains) to either play/pause a video in
        // place or open the full post, based on *where* the tap landed --
        // a center tap on a video plays/pauses it without opening
        // anything (matching what tapping a video normally does
        // everywhere else -- TikTok/Reels/Instagram all treat a center
        // tap as play/pause, not "open"), while a tap nearer either edge
        // opens the full post, same as tapping directly on a photo does
        // (photos have no play/pause of their own, so any tap on one
        // opens it). A single handler on the outer wrapper -- rather
        // than a separate onclick on the video wrap *and* one on the
        // outer .feed-media div -- is what keeps these from both firing
        // on the same tap.
        const FEED_MEDIA_SIDE_ZONE = 0.22; // left/right ~22% of the width each count as "the side"
        function handleFeedMediaTap(evt, postId){
          const container = evt.currentTarget;
          // A multi-photo/video carousel always opens as the regular
          // card view (see openPostDetail's isMulti check), never the
          // reel player, so there's no play/pause zone to route around --
          // and since only its first pane sits in the DOM at a fixed
          // spot regardless of which pane a swipe has scrolled to,
          // reliably telling whether the *currently visible* pane is a
          // video isn't something a simple querySelector here can do
          // safely anyway. Any tap just opens it, same as a single photo.
          const isCarousel = !!(container && container.querySelector('.post-media-carousel'));
          if (isCarousel) { openPostDetail(postId, evt); return; }
          const wrap = container && container.querySelector('.feed-video-wrap');
          const video = wrap && wrap.querySelector('video');
          if (!video) { openPostDetail(postId, evt); return; } // photo, or no media -- any tap opens it
          const rect = container.getBoundingClientRect();
          const clientX = evt.clientX != null ? evt.clientX : (evt.changedTouches && evt.changedTouches[0] && evt.changedTouches[0].clientX);
          const relX = (rect.width > 0 && clientX != null) ? (clientX - rect.left) / rect.width : 0.5;
          const isSideTap = relX < FEED_MEDIA_SIDE_ZONE || relX > (1 - FEED_MEDIA_SIDE_ZONE);
          if (isSideTap) { openPostDetail(postId, evt); return; }
          toggleFeedVideoPlay(wrap.id);
        }
          ...
          return `
            <div class="relative w-full h-full bg-black overflow-hidden" id="video-post-${post.id}" ontouchstart="vpdTouchStart(event)" ontouchend="vpdTouchEnd(event, ${post.id})">
              <div class="absolute inset-0 feed-video-wrap" id="${uid}" ${isVideo ? `onclick="toggleFeedVideoPlay('${uid}')"` : ''}>
                ${media ? (isVideo
                  ? `<video src="${media.url}" class="absolute inset-0 w-full h-full" style="${containStyle}" playsinline webkit-playsinline disablePictureInPicture controlsList="nodownload noplaybackrate nofullscreen" autoplay loop></video>`
