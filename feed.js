        ...
        // Tapping the video toggles play/pause and a simple centered play
        // icon fades in while paused (see toggleFeedVideoPlay below); there
        ...
        // one control that's kept, same as Instagram/TikTok-style feed
        // video -- starts unmuted, tap it to mute/unmute.
        ...
        function toggleFeedVideoMute(wrapId){
          ...
          if (!wrap) return;
          const video = wrap.querySelector('video');
          const btn = wrap.querySelector('.feed-video-mutebtn');
          if (!video) return;
        ...
        // Opens a full post detail window. Video posts open as a full-
        // bleed, Reels-style player (see videoPostDetailHTML) instead of
        // the card-plus-comments-list layout, since forcing a tall video
        // into that card meant a lot of scrolling to see anything else;
        // photo posts keep the original card (avatar, media, actions,
        // caption) followed by its comments and a composer, the way
        // tapping a post on Instagram opens its permalink page instead
        ...
        function openPostDetail(id){
          const post = findPost(id);
          if (!post) return;
          // Multi-photo/video carousel posts keep the regular card view --
          // it's the only place that renders their swipeable carousel, so
          // the full-screen video player (built for a single video) only
          // takes over for posts whose one piece of media is a video.
          const isMulti = !!(post.mediaHtml && /post-media-carousel/.test(post.mediaHtml));
          const isVideo = !isMulti && postMediaKind(post) === 'video';
          const ov = document.getElementById('overlay');
          ov.classList.remove('hidden');
          ov.style.top = isVideo ? '0' : OVERLAY_TOP;
          ov.style.paddingTop = '';
          ov.style.bottom = '0';
          ov.innerHTML = isVideo ? videoPostDetailHTML(post) : postDetailHTML(post);
        ...
        // Full-screen, edge-to-edge video player for a video post -- the
        // video fills the whole screen (tap to play/pause, same as the
        // in-feed player) with like/comment/repost/share/save stacked as
        // overlaid icons down the right edge and the author + caption
        // overlaid bottom-left, mirroring a Reels-style viewer instead of
        // the plain card-with-comments-below layout photo posts use.
        // Tapping the comment icon opens the existing comments sheet
        // (openComments) on top of it, same handler the card view uses.
        // The videos-by-this-author list a swipe on the full-screen player
        // moves through -- their own posted videos only (not reposts),
        // newest first via byNewestFirst so the swipe order matches
        // whatever their profile grid shows, and stays stable even if
        // feedPosts itself gets reshuffled by rankFeedPosts() underneath.
        function authorVideoQueue(authorId){
          return feedPosts.filter(p => String(p.authorId) === String(authorId) && !p.isRepost && postMediaKind(p) === 'video').sort(byNewestFirst);
        }

        // Swipes to the next/previous video from the same account as the
        // one currently open -- direction 1 = next (older), -1 = previous
        // (newer) -- letting someone land on one video from a profile's
        // grid and then keep swiping through the rest of that account's
        // videos without backing out to the grid each time, the way a
        // TikTok/Reels-style viewer works. Does nothing at either end of
        // that account's video list.
        function navigateVideoPost(currentId, direction){
          const post = findPost(currentId);
          if (!post || !post.authorId) return;
          const queue = authorVideoQueue(post.authorId);
          const idx = queue.findIndex(p => String(p.id) === String(post.id));
          if (idx === -1) return;
          const nextPost = queue[idx + direction];
          if (!nextPost) return; // no more videos from this account in that direction
          const ov = document.getElementById('overlay');
          if (!ov) return;
          ov.innerHTML = videoPostDetailHTML(nextPost);
        ...
        // Touch tracking for the full-screen video swipe above. Attached
        // to the whole player (not just the tap-to-play area) so a swipe
        ...
        // since its dy/dx never clear the swipe threshold below.
        ...
        function vpdTouchEnd(e, postId){
          ...
          navigateVideoPost(postId, dy < 0 ? 1 : -1); // swipe up -> next, swipe down -> previous
        }

        function videoPostDetailHTML(post){
          ...
          return `
            <div class="relative w-full h-full bg-black overflow-hidden" id="video-post-${post.id}" ontouchstart="vpdTouchStart(event)" ontouchend="vpdTouchEnd(event, ${post.id})">
              <div class="absolute inset-0 feed-video-wrap" id="${uid}" onclick="toggleFeedVideoPlay('${uid}')">
                ${media ? `<video src="${media.url}" class="absolute inset-0 w-full h-full object-contain" playsinline webkit-playsinline disablePictureInPicture controlsList="nodownload noplaybackrate nofullscreen" autoplay loop></video>` : ''}
              ...
              <button type="button" onclick="event.stopPropagation(); toggleFeedVideoMute('${uid}')" class="feed-video-mutebtn absolute z-20 flex items-center justify-center rounded-full" style="top:calc(env(safe-area-inset-top, 12px) + 12px);right:14px;width:2.25rem;height:2.25rem;background:rgba(0,0,0,0.35);">${Icon('volume','w-4 h-4 text-white')}</button>

              <div class="absolute z-20 flex flex-col items-center gap-5" style="right:12px;bottom:calc(env(safe-area-inset-bottom, 12px) + 100px);filter:drop-shadow(0 1px 3px rgba(0,0,0,0.85)) drop-shadow(0 0 10px rgba(0,0,0,0.55));">
                <button onclick="event.stopPropagation(); toggleLike(${post.id})" id="like-btn-${post.id}" class="flex flex-col items-center gap-1 text-white ${post.liked ? 'text-red-500' : ''}">
                  <span id="like-icon-${post.id}">${Icon(post.liked ? 'heart' : 'heartOutline','w-7 h-7')}</span>
                  <span class="text-[11px] font-semibold" id="like-inline-count-${post.id}">${formatCount(post.likes)}</span>
                </button>
                <button onclick="event.stopPropagation(); openComments(${post.id})" id="comment-btn-${post.id}" class="flex flex-col items-center gap-1 text-white">
                  ${Icon('comment','w-7 h-7')}
                  <span class="text-[11px] font-semibold" id="comment-inline-count-${post.id}">${formatCount(post.comments)}</span>
                </button>
                <button onclick="event.stopPropagation(); toggleRepost(${post.id})" id="repost-btn-${post.id}" class="flex flex-col items-center gap-1 text-white ${post.reposted ? 'text-emerald-400' : ''}">
                  ${Icon('repost','w-6 h-6')}
                  <span class="text-[11px] font-semibold" id="repost-inline-count-${post.id}">${formatCount(post.reposts)}</span>
                </button>
                <button onclick="event.stopPropagation(); openShare(${post.id})" id="share-btn-${post.id}" class="flex flex-col items-center gap-1 text-white">
                  ${Icon('send','w-7 h-7')}
                  <span class="text-[11px] font-semibold" id="share-inline-count-${post.id}">${formatCount(post.shares)}</span>
                </button>
                <button onclick="event.stopPropagation(); toggleSavePost(${post.id})" class="flex flex-col items-center text-white ${post.saved ? `text-[${ROYAL}]` : ''}">${Icon('bookmark','w-7 h-7')}</button>
              </div>

              <div class="absolute z-20" style="left:16px;right:88px;bottom:calc(env(safe-area-inset-bottom, 12px) + 20px);filter:drop-shadow(0 1px 3px rgba(0,0,0,0.85)) drop-shadow(0 0 10px rgba(0,0,0,0.55));">
                <div class="flex items-center gap-2 ${captionHtml ? 'mb-1.5' : ''} cursor-pointer" onclick="event.stopPropagation(); openPersonProfileForPost(${post.id})">
                  <div class="w-8 h-8 rounded-full ${post.avatarBg} overflow-hidden flex items-center justify-center flex-shrink-0" style="box-shadow:0 0 0 1px rgba(255,255,255,0.25);">${(post.mine && profileData.photo) ? `<img src="${profileData.photo}" class="w-full h-full object-cover">` : (!post.mine && post.photo) ? `<img src="${post.photo}" class="w-full h-full object-cover">` : Icon(post.avatarIcon,'w-4 h-4 text-white')}</div>
                  <span class="text-white font-semibold text-sm">${escapeHtml(post.name)}</span>
                  ${post.verified ? verifiedBadge() : ''}
                </div>
                ${captionHtml ? `<div class="text-white text-[13px] leading-snug">${captionHtml}</div>` : ''}
              </div>
            </div>`;
        }
        ...
        // (and the swipeable video order it drives -- see
        // navigateVideoPost) silently reorder itself underneath someone
        ...
        function addComment(id){
          ...
          PostsAPI.addComment(id, text).then(post => {
            ...
            // page (opened by tapping a photo post) keeps its media/caption
            // above the comments, the full-screen video player keeps its
            // own overlay chrome, and the plain comments sheet (opened via
            // the comment icon) only shows the list.
            const isDetailOpen = !!document.getElementById('post-' + id);
            const isVideoDetailOpen = !!document.getElementById('video-post-' + id);
            if (isDetailOpen) {
              const ov = document.getElementById('overlay');
              if (ov) ov.innerHTML = postDetailHTML(post);
            } else if (isVideoDetailOpen) {
              const ov = document.getElementById('overlay');
              if (ov) ov.innerHTML = videoPostDetailHTML(post);
            } else {
              openComments(id);
            }
          });
