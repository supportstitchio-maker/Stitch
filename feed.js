        ...
        function videoPostDetailHTML(post){
          ...
          return `
            <div class="relative w-full h-full bg-black overflow-hidden" id="video-post-${post.id}">
              ...
              <div class="absolute z-20 flex flex-col items-center gap-5" style="right:12px;bottom:calc(env(safe-area-inset-bottom, 12px) + 100px);">
              ...
              <div class="absolute z-20" style="left:16px;right:88px;bottom:calc(env(safe-area-inset-bottom, 12px) + 20px);">
                <div class="flex items-center gap-2 ${captionHtml ? 'mb-1.5' : ''} cursor-pointer" onclick="event.stopPropagation(); openPersonProfileForPost(${post.id})">
                  <div class="w-8 h-8 rounded-full ${post.avatarBg} overflow-hidden flex items-center justify-center flex-shrink-0">${(post.mine && profileData.photo) ? `<img src="${profileData.photo}" class="w-full h-full object-cover">` : (!post.mine && post.photo) ? `<img src="${post.photo}" class="w-full h-full object-cover">` : Icon(post.avatarIcon,'w-4 h-4 text-white')}</div>
                  <span class="text-white font-semibold text-sm" style="text-shadow:0 1px 3px rgba(0,0,0,0.6);">${escapeHtml(post.name)}</span>
                  ${post.verified ? verifiedBadge() : ''}
                </div>
                ${captionHtml ? `<div class="text-white text-[13px] leading-snug" style="text-shadow:0 1px 3px rgba(0,0,0,0.6);">${captionHtml}</div>` : ''}
              </div>
            </div>`;
        }
