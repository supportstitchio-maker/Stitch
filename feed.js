        ...
        function Icon(type, cls){
          const paths = {
            ...
            /* Plain WhatsApp-style read-receipt ticks used on sent chat
               bubbles (chat.js convoLogHTML): a single tick for "sent",
               and this double tick (colored via the class passed in --
               gray for delivered, blue once the recipient has actually
               opened the conversation) for "seen". */
            tick: `<path d="M4.5 12.75l5 5L20 6.75" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>`,
            tickDouble: `<path d="M0.75 12.75l5 5L16.5 6.75" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/><path d="M7 12.75l5 5L23.25 6.75" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>`,
              ...
              if (mediaFile) {
                ...
                if (publicUrl) {
                  post.mediaHtml = mediaType === 'video'
                    ? `<video src="${publicUrl}" controls class="w-full h-auto bg-black"></video>`
                    : `<img src="${publicUrl}" class="w-full h-auto">`;
                }
              }
        ...
        function feedPost(post){
          ...
          return `
            <div class="bg-white overflow-hidden -mx-5 border-b border-gray-100" id="post-${post.id}">
              ...
              <div class="px-3.5 pt-1 pb-3 text-[10px] uppercase text-gray-400" id="share-count-${post.id}">${(post.views || 0) > 0 ? `${formatCount(post.views)} views &middot; ` : ''}${formatPostTimeAgo(post)}</div>
            </div>`;
        }
