        // ---------------- HOME / FEED ---------------- remotePostsLoaded (declared with loadRemotePosts further down) flips true once that first fetch...
        ...
        // ---- Pull to refresh (Home feed) ---- Dragging down from the very top of the feed re-fetches posts and glimpses, same data as a tab-switch refresh,...
        ...
        function renderFeed() {
          // A full innerHTML rebuild (below) always resets #screen's scrollTop to 0 -- normally fine right after a tab switch (scroll is already 0 then), but...
          ...
          if (screenEl) {
            // Right after the innerHTML swap above, the fresh post images/ videos haven't finished loading yet, so #screen's scrollable content is briefly...
        ...
        // Updates the Home feed in place instead of the full innerHTML rebuild renderFeed() does -- reuses each post card's existing DOM node (moving it into...
        ...
        // Wires up the drag gesture on #screen: only "arms" when Home is the active tab and the feed is already scrolled all the way to the top (scrollTop 0),...
        let ptrPulling = false;
        let ptrStartY = 0;
        let ptrDistance = 0;
        // Small deadzone before a pull is allowed to "arm" -- guards against a touch's natural jitter near the start of a gesture (a few px of downward noise...
        ...
        function setupPullToRefresh(screenEl){
          ...
          screenEl.addEventListener('touchmove', (e) => {
            ...
            // Below the deadzone -- whether slightly down or up -- treat as "not pulling yet" rather than committing the indicator, and keep tracking from the same...
            if (rawDy <= PTR_DEADZONE) { ptrDistance = 0; setPullIndicatorHeight(0, false); return; }
            const dy = rawDy - PTR_DEADZONE;
            // Half-rate drag (like iOS rubber-banding) so it takes a deliberate pull to trigger, not a light nudge.
        ...
        // Runs the same data refresh switchTab(0) already does when returning to Home (loadRemotePosts + loadPostInteractions + loadRemoteGlimpses), then...
        async function startHomeFeedRefresh(){
          ...
          if (typeof currentTab === 'undefined' || currentTab !== 0) return;
          // Let the indicator slide back up gently first -- collapsing it to 0 with its own transition -- instead of just calling renderFeed() straight away,...
          setPullIndicatorHeight(0, true);
          setTimeout(() => {
        ...
        // ---------------- STORIES ---------------- Each story can hold more than one glimpse: `items` is a list of { caption, icon?, iconClass?
        let stories = [
        ...
        function Icon(type, cls){
          const paths = {
            ...
            // "Photo" tile icon: framed picture containing a sun and a mountain range, matching the classic image-placeholder glyph -- used for the Pictures tab on...
            ...
            // "Video" tile icon: filmstrip with a play triangle in the center window -- used for the Videos tab on someone's profile.
            ...
            // Solid/outline thumbs-up pair used for the feed "like" button (replacing the old heart icon) -- same glyph in both cases, solid fill when liked...
            ...
            // Plain (non-circled) checkmarks used for chat read-receipt ticks -- single for "sent", double (overlapping) for "delivered"/"read" -- see...
            ...
            // Post-type badge glyphs (see post.tagClass in feed.js) -- videoClip: a filmstrip with sprocket holes down each side and a play triangle on its screen,...
        ...
        function storyBubble(s){
          if (s.mine) {
            ...
            // Tapping your own bubble/photo opens the actual glimpse (like tapping anyone else's) once you have one posted -- it used to always open the...
        ...
        // A person's glimpses each load for the same fixed duration.
        const STORY_ITEM_DURATION_MS = 4500;
        let currentStoryId = null;
        let currentItemIndex = 0;
        let storyTimer = null;
        let storyTypingTimeout = null;
        // Real microphone speech-to-text, backed by the Web Speech API (which itself requests actual mic access from the browser).
        ...
        function startMicRecording(opts){
          ...
          if (!SR) {
            // No mic speech-recognition support in this browser: say so instead of silently inserting fake text.
            setListening(true);
        ...
        // Tracks how far into the current item's duration we are (ms) and when the current running segment started, so a press-and-hold or typing pause can...
        ...
        // Once a person's glimpse ring has been marked viewed (gray, not blue) it should also drop to the back of the story strip -- same "seen, out of...
        ...
        function openStoryOverlay(s){
          ...
          startStoryTimer(s);
          // Record a real view against this specific item -- s.items only exists (with real backend row ids) for other accounts' remote glimpses; single-item...
          if (!s.mine && s.items && s.items[currentItemIndex] && s.items[currentItemIndex].id != null) {
            glimpseViewRemote(s.items[currentItemIndex].id);
          }
        }

        // Called whenever a new item/story loads: resets progress to 0 and starts the bar running from scratch.
        function startStoryTimer(s){
          storyElapsedMs = 0;
          resumeStoryTimer(s);
        }

        // Resumes the bar from wherever storyElapsedMs left off (used after a press-and-hold or a typing pause ends).
        ...
        // Freezes the bar exactly where it is; used while the user is pressing and holding the glimpse, or typing in the reply box.
        ...
        // Resumes the currently open story/item after a pause (as opposed to resumeStoryTimer(s), which needs the story object directly).
        ...
        // Press-and-hold-to-pause: holding a finger/mouse down on the glimpse freezes the loading bar; lifting it quickly (a genuine tap) advances to the next...
        ...
        function toggleStoryVoiceInput(){
          ...
          // Use the real microphone via the Web Speech API instead of a canned reply; the send icon just needs its own accent classes (bg-[NAVY] rather than the...
        ...
        function handleStoryReplyFileSelect(event){
          ...
          // Give visible feedback right on the glimpse screen; otherwise tapping the attach icon looks like it did nothing.
          const input = document.getElementById('story-reply-input');
          if (input) {
        ...
        // Best-effort: records that the current account viewed a specific glimpse item (own glimpses are skipped, same convention as postViewRemote, and each...
        ...
        // Pulls every other account's still-live (< 24h old) glimpses into the story strip, grouped one bubble per person with their items in posting order --...
        async function loadRemoteGlimpses(){
          ...
          try {
            ...
            // Home re-runs this fetch every time the tab is reopened (see the comment above), which used to rebuild every bubble from scratch with viewed: false --...
            ...
            // Also fold in whatever's persisted from a previous session/login (see persistGlimpseViewed) so a glimpse this account already watched doesn't come...
        ...
        let myGlimpses = [];
        // Mirrors the "viewed" flag other people's story rings use -- true once you've actually opened one of your own glimpses in the full-screen viewer...
        let myGlimpsesViewed = false;
        let openMyGlimpseMenuId = null;

        // Persists whether *your own* glimpse ring has been opened (see myGlimpsesViewed above) so it doesn't come back showing the blue/gradient "unwatched"...
        ...
        // Called once myGlimpses has been restored from saved account state (see applyUserState in core.js) to decide whether the ring should start out gray...
        ...
        // Glimpses auto-expire this long after being posted (24 hours), matching the "disappear after 24 hours" copy shown in the list.
        const GLIMPSE_LIFETIME_MS = 24 * 60 * 60 * 1000;

        // Which other-account glimpses (keyed by the author's user id, matching stories[].id for a remote story) *this* signed-in account has already opened,...
        function glimpsesViewedStorageKey(){
          const uid = (typeof _cachedAuthUser !== 'undefined' && _cachedAuthUser) ? _cachedAuthUser.id : null;
          return uid ? `glimpses-viewed-v1:${uid}` : null;
        }

        // Returns { [authorUserId]: { newestId, ts } }, dropping any entries older than a glimpse's 24-hour lifetime (that author's glimpse from back then is...
        function loadPersistedViewedGlimpses(){
          try {
            ...
            Object.keys(map).forEach(id => {
              // Older entries were stored as a bare timestamp number; normalize those to the new { newestId, ts } shape so a stale-format entry doesn't throw off...
        ...
        // Marks one author's glimpse(s) as viewed for next login, up through whatever their newest item id is right now.
        ...
        // Drops any glimpse whose 24-hour window has passed.
        ...
        // ---------------- COMPOSE GLIMPSE (pen icon) ---------------- Glimpse compose background now just follows the app's own light/dark theme instead of...
        ...
        function myGlimpseRow(g){
          // The dropdown menu lives in its own container (#glimpse-menu-<id>) instead of being part of the same template string that's regenerated wholesale on...
          return `
        ...
        function toggleMyGlimpseMenu(id){
          ...
          // Patch just the menu container(s) that actually changed -- the one being opened, and the one being closed if a different row's menu was already open...
        ...
        // Best-effort: pulls the real list of who has viewed this glimpse (see glimpse_views, written by glimpseViewRemote whenever someone else opens it) and...
        async function loadGlimpseViewers(g){
          ...
          try {
            ...
            // Patch just the eye-button/viewers-panel container instead of the whole viewer screen -- a full ov.innerHTML rebuild here would tear down and reload...
        ...
        // Whether the "Viewed by" list is expanded over the glimpse (tapping the eye icon toggles this, WhatsApp-status style) -- views aren't shown by...
        ...
        function toggleMyGlimpseViewers(id){
          ...
          const g = myGlimpses.find(x => x.id === id);
          if (!g) return;
          // Only re-render the eye-button/viewers-panel container, same reasoning as loadGlimpseViewers above -- toggling "Viewed by" open and closed used to...
          const panelEl = document.getElementById('glimpse-viewers-panel-' + id);
          if (panelEl) panelEl.innerHTML = glimpseViewersPanelHTML(g);
        }

        // Shows your own glimpse plus who has viewed it, each with a reply button; unlike other people's glimpses, your own glimpse screen has nothing to...
        function myGlimpseViewerHTML(g){
          // Text-only glimpses now only ever use white or black as their own background (see glimpseComposeBg) -- keep the caption and header readable against...
          ...
          return `
            <div class="flex flex-col h-full relative" style="color:${fg};${g.mediaUrl ? 'background:#000;' : g.bgStyle}">
              ...
              ` : ''}
              <div id="glimpse-viewers-panel-${g.id}">${glimpseViewersPanelHTML(g)}</div>
            </div>`;
        }

        // The eye-count button plus (when expanded) the "Viewed by" sheet over the glimpse.
        ...
        // Opens (or creates) a chat with a glimpse viewer so you can reply directly to them, the same way tapping any inbox conversation does.
        ...
        // Cycles the typeface used for the glimpse text (not its size; the size stays fixed at GLIMPSE_COMPOSE_FONT_SIZE).
        ...
        // Glimpse videos are capped at this length.
        const GLIMPSE_MAX_SECONDS = 60;

        // Shared <video> attributes for every place a glimpse's own video renders (compose preview, thumbnail row, my/other viewers, story viewer): loops...
        ...
        function handleGlimpseMediaSelected(e){
          ...
          // A blob: URL (just a lightweight reference to the file, not the file's actual bytes) instead of a data: URL here on purpose -- reading a large/long...
          ...
          // Figure out the clip's length in the background so the caption screen can flag (and, on submit, actually perform) a split once it's known -- doesn't...
          ...
          probe.onloadedmetadata = function(){
            ...
            // The user may have already cancelled or picked something else by the time metadata loads -- only apply this to the still-current preview.
        ...
        // ---------------- COMPOSE GLIMPSE (media caption step) ---------------- Shown right after a photo/video is picked, before it's posted -- lets the user...
        ...
        // Re-renders just the caption screen in place (used once a picked video's duration finishes loading, so the "will post as N glimpses" notice can appear...
            ...
            <div class="flex flex-col h-full bg-black text-white relative">
              <div class="absolute inset-0 overflow-hidden">
                ...
                <div class="absolute inset-0 flex items-center justify-center">
                  ${glimpseMediaPreviewType === 'video'
                    // Only the blurred background copy above stays muted (it's a decorative duplicate playing in lockstep -- unmuting it too would just double up the...
        ...
        // Re-renders the home story strip in place (if it's currently on screen) -- used so the ring around your own bubble appears or disappears right away...
        ...
        // Stateful post data so like/comment/repost/share persist while browsing connected:true -> poster is in your network (no Connect button, just meta...
        let feedPosts = [];

        // Tombstone of post ids this account has deleted locally but whose remote delete (postDeleteRemote) hasn't been confirmed yet.
        let deletedPostIds = new Set();

        // The number shown as "Posts" on the profile (and the Job Plan stats panel) -- computed live from feedPosts rather than a manually incremented counter.
        function myPostsCount(){
          return feedPosts.filter(p => p.mine).length;
        }

        // ================== BACKEND-READY DATA LAYER (Posts & Glimpses) ================== Every Post and Glimpse ("My glimpse" / status) read-or-write goes...
        ...
        // Removes a post/glimpse's media file from the shared Storage bucket.
        ...
        // Deletes every file behind a multi-media post in one go -- same as deletePostMediaFromStorage but for the mediaPaths array a post with more than one...
        ...
        // Plain tap-to-play/pause video for feed posts -- deliberately NOT using the native `controls` bar, since that bar comes with a seek/scrub strip along...
        ...
        // Toggles play/pause on a feed video wrapper built by simplePostVideoHtml above, and shows/hides the centered play icon to match -- the only controls...
        function toggleFeedVideoPlay(wrapId){
          ...
          if (video.paused) {
            ...
            video.pause();
            // Remembers this was a deliberate pause so the scroll- triggered autoplay observer (setupFeedVideoAutoplay) below doesn't immediately resume it again...
            video.dataset.userPaused = '1';
            if (btn) btn.style.opacity = '1';
          }
        }

        // ---------------- Scroll-triggered feed video autoplay ---------------- A post's video starts playing on its own once enough of it is visible on...
          ...
          feedVideoObserver = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
              ...
              if (ratio >= 0.6) {
                ...
                // Leaving view clears any manual pause -- scrolling back to it later autoplays fresh, the same as landing on it the first time.
                delete video.dataset.userPaused;
              }
              // Between 0.15 and 0.6: deliberately left alone -- whatever state the video was already in stays, which is what actually stops the flicker at the...
            });
          }, { threshold: [0, 0.15, 0.6] });
          videos.forEach(v => feedVideoObserver.observe(v));
        }

        // Mutes/unmutes a feed video without also toggling play/pause (the click handler on the corner button stops propagation so the tap doesn't fall through...
        ...
        // Builds a post's mediaHtml from already-uploaded {url, type} items pointing at public Storage URLs: a single item renders as a plain img/video...
        ...
        // One grid tile: a fixed-position cell (its placement/span comes from `areaStyle`, set by the caller below) showing the item cropped to fill the cell...
        ...
        // A grid tile's onclick above needs the id of the post it belongs to, but postMediaHtmlFromUploaded/postMediaGridHtml only ever receive the raw items...
        function postGridOwnerId(tileEl){
          const card = tileEl.closest('[id^="post-"]');
          return card ? Number(card.id.replace('post-', '')) : null;
        }

        // The "1 big + up to 3 stacked" grid itself: 2 items sit side by side; 3 or more put one large tile on the left (spanning the full height) with the...
        function postMediaGridHtml(items){
          ...
          // Only the first 4 items get an actual grid tile -- but the gallery viewer (postMediaItemsList) finds its list of photos to swipe through by parsing...
        ...
        // Pulls every photo/video out of a post's rendered mediaHtml, whichever shape it's in -- a plain single img/video tag, or the grid's several tiles --...
        ...
        // Best-effort remote insert so a new post actually reaches other accounts instead of only living in this browser's own state blob.
        ...
        // Pulls every account's posts into the shared feed.
        async function loadRemotePosts(){
          ...
          try {
            ...
            if (error || !data) return false;
            // Drop any post already loaded here (this account's cached feedPosts, restored as-is from saved state) that no longer exists in POSTS_TABLE -- e.g.
            ...
            // Look up each author's current profile photo so avatars in the feed stay in sync with whatever they've set on their profile, instead of freezing...
            ...
            // Refresh the cached avatar on every already-known post from another account so a photo change reaches posts that were loaded before it happened.
            feedPosts.forEach(p => {
            ...
            // The feed favors what you're actually likely to engage with -- see rankFeedPosts below -- rather than a fixed chronological or fully random order.
            rankFeedPosts();
            return newRows.length > 0 || postsWereRemoved;
          } catch (e) { remotePostsLoaded = true; return false; }
        }

        // ---------------- FEED RANKING (Suggested for you) ---------------- Builds a simple per-author affinity score from this account's own past activity on...
        ...
        // Reorders feedPosts in place to favor: posts from people you're connected to, posts from authors you've previously liked/commented/saved/reposted, and...
        ...
        // Fisher-Yates shuffle. The feed itself now uses rankFeedPosts above instead of a flat shuffle, but this stays available for anything else in this file...
        ...
        // Best-effort writes -- mirror postInsertRemote/postDeleteRemote above: local optimistic state already updated by PostsAPI by the time these run, so a...
        ...
        // ---- Live "someone posted" / "someone commented" notifications ---- Wires the Notifications bell tab (addNotif, in overlays.js) up to real activity...
        ...
        async function subscribeToFeedActivity(){
          ...
          feedActivityChannel = sb.channel('feed-activity:' + myId)
            ...
            // DELETE was previously missing here entirely: deletePost() correctly removed the row from Supabase (postDeleteRemote), so the post really was gone,...
        ...
        // Removes a post that was deleted on another account from this account's own local feed the moment it happens, instead of it lingering until the next...
        function handleRemotePostDelete(payload){
          ...
          // Targeted DOM removal instead of a full renderFeed()/ renderProfile() rebuild -- same reasoning as patchFeedInPlace above: a post disappearing...
        ...
        function handleRemotePostInsert(payload, myId){
          ...
          // Tag notice: fires whenever this account is one of the post's taggedUsers, regardless of network status -- being tagged by someone you're not yet...
          ...
          // Also drop it straight into the feed itself if it's currently open, same as the rest of the app's "live" realtime updates.
          if (typeof currentTab !== 'undefined' && currentTab === 0 && typeof feedPosts !== 'undefined' && !feedPosts.some(p => String(p.id) === String(row.data.id))) {
            ...
            // patchFeedInPlace (not a full renderFeed()) so every other post already on screen keeps its DOM node -- and any already-loaded/playing <video> inside...
        ...
        // Looks up a display name for a user_id from public_profiles -- shared by the like/repost handlers below, neither of which carries a name on the row...
        ...
        // Fires for every account the instant anyone shares a post -- two separate jobs, same as the like/repost handlers above: 1.
        ...
        // Best-effort: records one share event for this post -- called once per actual share action (sent to a contact, "Copy link", "Add to Glimpse",...
        ...
        // Pulls the real like/repost/save/comment state for every post currently in the feed and overwrites the local counts with it -- called right after...
        async function loadPostInteractions(){
          ...
          try {
            ...
            // Look up each commenter's current profile photo, the same way loadRemotePosts does for post authors, so a comment's avatar reflects whoever actually...
            ...
            // Real engagement numbers for this batch just came in -- re-rank now so the order reflects them, not just whatever was cached from the last time this...
            rankFeedPosts();
        ...
        const PostsAPI = {
          ...
          create(data){
            // mediaFiles/mediaTypes (arrays, one entry per picked photo/ video) are the current path from the composer -- mediaFile/ mediaType (singular) are still...
            ...
            return (async () => {
              ...
              // Locally-picked images/videos only exist as blob:/data: URLs in this browser -- upload the real files to shared Storage first and point mediaHtml at...
              if (files.length) {
          ...
          remove(id){
            return mockRequest(() => {
              ...
              queueSaveUserState();
              // Only delete the underlying Storage file when this is one of MY posts.
              ...
              postDeleteRemote(id).then(ok => {
                if (ok) {
                  // Confirmed gone remotely -- the tombstone has done its job, so drop it rather than let the set grow forever.
                  deletedPostIds.delete(String(id));
                  queueSaveUserState();
                }
                // On failure the tombstone stays, so the next loadRemotePosts still won't resurrect the not-actually-deleted-yet post.
              });
              return { id };
            });
          },
          // Used when blocking an account: clears every post from that author out of the feed in one go.
          removeByAuthor(name){
            return mockRequest(() => {
          ...
          toggleRepost(id){
            return mockRequest(() => {
              ...
              if (post.reposted) {
                // Publish the repost as an actual post on your own profile/ feed -- same as Instagram/LinkedIn's "Repost": everyone who follows you now sees it in...
          ...
          // Records one share event -- called once per actual share action (sent to a contact, "Copy link", "Add to Glimpse", "More apps"), same insert-only...
          share(id){
            return mockRequest(() => {
              const post = feedPosts.find(p => p.id === id);
              if (!post) return null;
        ...
        const GlimpsesAPI = {
          ...
          create(data){
            ...
            // createdAt anchors the 24h expiry independently of `id`, so expiry logic keeps working even if id generation ever changes to something other than...
            const glimpse = { id: Date.now(), createdAt: Date.now(), timeLabel, type: 'text', viewers: [], ...rest };
            // Goes into myGlimpses (with its local blob:/data: URL) and resolves right away instead of waiting on the Storage upload first -- that upload,...
        ...
        // Backstop for the 24h expiry: catches a glimpse aging out while the "My glimpse" list happens to be sitting open (openMyGlimpses/ refreshMyGlimpses...
        setInterval(() => {
        ...
        // Same backend-ready pattern as PostsAPI/GlimpsesAPI above: a real backend later just means swapping the body of these two methods for a fetch() call,...
        const NetworkAPI = {
          listBlocked(){
        ...
        function toggleSavePost(id){
          PostsAPI.toggleSave(id).then(post => {
            if (!post) return;
            // Updates whichever save/bookmark button is actually visible in place (see forEachById above) instead of a full renderFeed() -- that rebuild only ever...
            ...
            // The Saved screen is a genuinely different list of posts (unsaving one removes it from that list entirely, not just a style change), so it still needs...
            if (typeof renderSavedItemsOverlay === 'function') renderSavedItemsOverlay();
          });
        }

        // Refreshes every on-screen copy of one specific post's card (its dots menu, delete, "Not interested", Report, ...) without rebuilding the whole feed...
        function refreshFeedPostCard(id){
          ...
          // Whichever list is actually visible just had its video(s) recreated by the replace above -- hand them back to the scroll-triggered autoplay observer,...
        ...
        // The backdrop + dropdown markup for a post's three-dot menu, shared between feedPost's initial render and the direct DOM patching in...
        ...
        // Drops just the backdrop + dropdown nodes for a post's menu, wherever it's currently showing (there can be zero, one, or two copies -- see...
        ...
        // Opening/closing used to call refreshFeedPostCard(), which rebuilds the post's *entire* card -- including its image/video in .feed-media -- just to...
        function togglePostMenu(id){
          ...
          // Watch whichever container is actually scrollable around this post right now -- #screen's Home feed, or the profile's own #post-feed-list inside the...
          const overlayEl = document.getElementById('overlay');
          const overlayVisible = overlayEl && !overlayEl.classList.contains('hidden');
        ...
        // Closing the post menu because the user scrolled the feed (rather than tapped) shouldn't trigger a full renderFeed() -- rebuilding the whole post list...
        ...
        function deletePost(id){
          PostsAPI.remove(id).then(() => {
            openPostMenuId = null;
            // The "Posts" stat on the profile is computed live from feedPosts (see myPostsCount above), so removing the post from feedPosts above is all that's...
            refreshFeedPostCard(id);
            // Previously only ran renderProfile() (a header-only patch, see patchProfileHeaderInPlace in profile.js) and only when Profile happened to be the...
            if (typeof refreshProfilePostsUI === 'function') refreshProfilePostsUI();
          });
        }

        // "Not interested" just quietly drops this one post out of the feed -- unlike blocking, it doesn't touch the author's account or their other posts.
        function notInterestedPost(id){
          openPostMenuId = null;
          PostsAPI.remove(id).then(() => refreshFeedPostCard(id));
        }

        // Reports are real, persistent state (not just an alert): once a post is reported, its id is stored so the menu item flips to "Reported" and won't fire...
        ...
        function reportPost(id){
          ...
          queueSaveUserState();
          openPostMenuId = null;
          // Just closing the dropdown here (not refreshFeedPostCard) -- the menu is going away either way, so there's no need to rebuild the whole card (and its...
          removePostMenuDom(id);
        ...
        // "Cancel Request" tapped on a post's pill -- cancels by author, so it also clears connectRequestSent on every other post from the same person still...
        ...
        // Tapping a post's avatar opens the same read-only profile card (see openPersonProfile in overlays.js) used everywhere else -- lets you see who they...
        ...
        // Relative post timestamp ("Now", "5m", "3h", "a day ago", "5 days ago", then a plain date past a week) computed live from the post's id -- every...
        ...
        // Renders a post's caption with any tagged people's "@username" mentions highlighted in blue (and tappable, opening their profile) at the exact spot...
        ...
        function feedPost(post){
          // Real network status (see isUserInMyNetwork in chat.js), not the static post.connected flag the original poster's own client happened to save -- that...
        ...
        // Routes a tap on a feed card's media area (the .feed-media wrapper, whatever it contains) to opening the full post -- every tap opens it, whether it...
        function handleFeedMediaTap(evt, postId){
          openPostDetail(postId, evt);
        }

        // Opens a full post detail window. A post whose one piece of media is a photo or a video (i.e.
        let reelSwipeEnabled = false;
        function openPostDetail(id, evt){
          const post = findPost(id);
          if (!post) return;
          // The tap that opens this bubbles up through the in-feed video's own tap-to-play area (toggleFeedVideoPlay), which starts that video playing with sound...
          ...
          // The scroll-triggered autoplay observer (setupFeedVideoAutoplay) watching whichever list this post was opened from is still live at this point -- just...
          if (feedVideoObserver) { feedVideoObserver.disconnect(); feedVideoObserver = null; }
          ...
          // Optimistic +1 so the author sees the count move without waiting on the round trip; postViewRemote no-ops (and doesn't re-count) if this is your own...
        ...
        // Full-screen, edge-to-edge player for a single-media post (photo or video) -- the media fills the whole screen (a video is tap to play/pause, same as...
        function reelPostQueue(){
          return postListForSource(postFeedSource).filter(p => p.mediaHtml && !/post-media-grid/.test(p.mediaHtml));
        }

        // Swipes to the next/previous post in whichever list this reel view was opened from (see reelSwipeEnabled above) -- direction 1 = next (older), -1 =...
        function navigateReelPost(currentId, direction){
          ...
          const ov = document.getElementById('overlay');
          if (!ov) return;
          // Explicitly pause whatever's currently playing before swapping it out -- innerHTML below tears the old <video> out of the document either way, but...
        ...
        // Touch tracking for the full-screen swipe above.
        ...
        // The backdrop+foreground pair behind a full-bleed photo/video: a photo that doesn't share the screen's aspect ratio used to just letterbox against a...
        ...
        // ---------------- Multi-photo post gallery (grid tap-to-open) ---------------- The full-bleed, swipeable viewer a grid tile (see postMediaGridTile)...
        ...
        // Touch tracking for the gallery's left/right swipe between this post's own photos -- same swipe-threshold approach as vpdTouchStart/vpdTouchEnd above,...
        ...
        function galleryTouchEnd(e, postId, index){
          ...
          const dy = t.clientY - startY;
          const SWIPE_THRESHOLD = 60;
          if (Math.abs(dx) < SWIPE_THRESHOLD || Math.abs(dy) > Math.abs(dx)) return; // not a deliberate horizontal swipe
          navigateGalleryItem(postId, index, dx < 0 ? 1 : -1); // swipe left -> next, swipe right -> previous
        }
        ...
        function navigateGalleryItem(postId, index, direction){
          ...
          if (nextIndex < 0 || nextIndex >= items.length) return; // no more photos that way
          const ov = document.getElementById('overlay');
          if (!ov) return;
          const outgoingVideo = ov.querySelector('video');
          if (outgoingVideo && !outgoingVideo.paused) outgoingVideo.pause();
        ...
        // ---------------- Filtered post feed (profile grids -> scrollable feed) ---------------- Tapping a tile in the Posts/Reposts grid on your own Profile,...
        let postFeedSource = 'mine';
        let postFeedStartId = null;

        // `source` is one of 'mine' | 'reposts' | 'saved', 'author:<id>' for another person's full post list, or one of the three grids their profile actually...
        ...
        function postFeedTitle(source){
          ...
          if (typeof source === 'string' && (source.indexOf('author-videos:') === 0 || source.indexOf('author-pictures:') === 0 || source.indexOf('author-reposts:') === 0 || source.indexOf('author:') === 0)) {
            ...
            // Prefer whichever profile is currently loaded (has the most up-to-date name); fall back to any post by that author.
        ...
        // Opens the scrollable feed at `postId`, remembering whichever screen it was tapped from (the Profile tab itself, the Saved overlay, or someone else's...
        ...
        // Jumps straight to the tapped post with no scroll animation -- same "land exactly where you tapped" behavior as opening a photo from an Instagram...
        ...
        // Compact counter formatting (1.2K, 67.4K, 1,171) to match the like/comment/repost/share numbers shown inline next to each icon.
        ...
        // A post can legitimately be on screen twice at once -- its card in the Home feed / a profile grid's scrollable list sitting in #screen, underneath the...
        ...
        function toggleLike(id){
          PostsAPI.toggleLike(id).then(post => {
            ...
            forEachById('like-btn-'+id, btn => {
              // This same id exists on two very differently laid-out buttons -- the plain in-feed card icon row, and the full-screen detail/reel player's stacked...
              const isReelDetail = btn.closest('#video-post-' + id) != null;
              btn.className = isReelDetail
        ...
        function toggleRepost(id){
          PostsAPI.toggleRepost(id).then(post => {
            ...
            // Reposting/un-reposting adds or removes an actual post from the feed (see PostsAPI.toggleRepost), so the feed list itself needs a re-render, not just...
        ...
        function commentRow(c){
          // c.photo is set from the real commenter's profile (see loadPostInteractions' commentPhotoById lookup) for anyone, not just you.
        ...
        function addComment(id){
          ...
          PostsAPI.addComment(id, text).then(post => {
            ...
            // Refresh whichever view is currently open in the overlay: the full post detail page (opened by tapping a photo/ carousel post) keeps its media/caption...
            const ov = document.getElementById('overlay');
        ...
        // Contacts tapped in the share sheet below (openShare/shareHTML) before hitting the send button -- reset every time the sheet opens so a leftover...
        ...
        // Everyone the user already has a thread with (primary inbox, requests, and collabs), deduped by conversation id; this is the "network" list shown in...
        ...
        // Laid out the same way as the Tag People page (tagPickerListHTML in overlays.js) -- a single-column list of full-width rows, avatar + name/handle on...
        ...
        // Tapping a contact only selects/deselects them (a round checkbox, same look as the "New collaboration" contact picker) -- nothing is actually sent...
        ...
        // Flips one contact's selection state in-place (no full re-render of the list, so scroll position and the search filter stay put) and refreshes the...
        ...
        // Floating send button, same fixed-corner placement already used for the "Next" button in the new-collaboration contact picker (see newCollabSelectHTML...
        ...
        // Each external-share option is an icon button with its label written underneath (link/copy, add to glimpse, more apps) -- previously the label only...
        ...
        // Fires when the send FAB is tapped: delivers the post to every selected contact for real (see deliverSharedMessage -- same sendMessageRemote path a...
        ...
        // Shared delivery path for both "Share post" and "Share glimpse": pushes the message into the local thread immediately (so it shows up right away for...
        function deliverSharedMessage(convoId, text){
          ...
          // Neither branch matched: every member is a local-only mock contact with no backend user, so the message just stays sent on this device, same as...
        }

        // Refreshes the inline share count next to the send icon on the feed card, same pattern as the like/repost/comment counters.
        ...
        // Pulls the actual photo/video out of a post's rendered mediaHtml so it can be reused as glimpse media -- glimpses only ever hold pictures and videos,...
        ...
        // Same video-vs-picture check as extractPostMedia, but for anywhere that just needs the media *kind* without pulling the URL back out -- used to split...
        ...
        // Wired to the "Add to Glimpse" option in the share sheet: takes the post's own photo/video and posts it straight to the current user's glimpse, same...
        ...
        // ---------------- SHARE GLIMPSE ---------------- "Share glimpse" in the My Glimpse row menu -- same contacts-grid sheet as sharing a post...
        ...
        // Same list layout as shareHTML above / the Tag People page -- full-width rows instead of a grid of circles.
        ...
        // Tapping a contact only selects/deselects them -- sending happens when the send FAB is tapped (see toggleShareGlimpseContact /...
        ...
        // Fires when the send FAB is tapped: actually delivers the glimpse to every selected contact via deliverSharedMessage (same real sendMessageRemote path...
