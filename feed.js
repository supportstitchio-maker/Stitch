        ...
        function shareGlimpseHTML(g){
          ...
          return `
            <div class="flex-shrink-0 w-full" style="padding-top:20px;">
              <div class="max-w-2xl mx-auto px-5 pb-3 flex items-center justify-between">
                <div class="font-semibold text-lg font-display">Share glimpse</div>
            ...
            <div class="flex-1 overflow-y-auto">
              <div class="max-w-2xl mx-auto px-5">
                <div id="share-glimpse-grid-${g.id}" class="grid gap-x-2 gap-y-5" style="grid-template-columns:repeat(3,minmax(0,1fr));">
        ...
        function shareGlimpseContactCell(glimpseId, c){
          return `
            <button id="share-glimpse-cell-${glimpseId}-${c.id}" data-name="${escapeHtml(c.name.toLowerCase())}" onclick="sendGlimpseToContact(${glimpseId}, '${c.id}')" class="flex flex-col items-center gap-1.5 text-center">
              <div class="relative">
                <div class="w-14 h-14 ${c.avatarBg} rounded-full flex items-center justify-center text-gray-600 overflow-hidden">${avatarInnerHTML(c,'w-6 h-6')}</div>
                <div id="share-glimpse-check-${glimpseId}-${c.id}" class="hidden absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full bg-[${ROYAL}] border-2 border-white flex items-center justify-center text-white">${Icon('check','w-3 h-3')}</div>
              </div>
              <div id="share-glimpse-label-${glimpseId}-${c.id}" class="text-xs font-medium text-gray-700 truncate w-14">${escapeHtml(c.name.split(' ')[0])}</div>
            </button>`;
        }
