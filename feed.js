// ---- Feed render + pull-to-refresh ----
        ...
        async function loadRemotePostsImpl(){
          ...
          try {
            ...
            if (authorIds.length) {
              const { data: profiles, error: profilesError } = await sb.from(PUBLIC_PROFILES_TABLE).select('user_id, photo').in('user_id', authorIds);
              if (profilesError) {
            ...
            newRows.forEach(row => {
              ...
              feedPosts.push({ ...row.data, id: row.data.id ?? id, mine: isMine, authorId: row.created_by, connected: isMine ? true : !!row.data.connected, photo: isMine ? row.data.photo : (photoFetchFailed ? null : (photoById[row.created_by] || null)) });
              knownIds.add(id);
            });
        ...
        function shareHTML(post){
          ...
          return `
            ...
            <div class="flex-shrink-0 w-full flex items-start justify-center no-scrollbar border-t border-gray-100" style="gap:36px;padding:14px 20px calc(env(safe-area-inset-bottom, 12px) + 14px) 20px;background:#fafafa;overflow-x:auto;">
        ...
        function shareActionHTML(postId){
          ...
          if (count > 0) {
            return `
              <div class="flex flex-col items-center gap-1 flex-shrink-0" style="width:48px;">
                <button onclick="sendPostToSelectedContacts(${postId})" title="Send" class="rounded-full flex items-center justify-center flex-shrink-0 relative" style="width:30px;height:30px;background:linear-gradient(135deg,${ROYAL},${NAVY});color:#fff;">
                  ${Icon('send','w-[13px] h-[13px]')}
          ...
          return shareExternalOption('send','More apps', `sharePostExternally(${postId})`, `linear-gradient(135deg,${ROYAL},${NAVY})`, '#fff');
        }
        ...
        function shareExternalOption(icon,label,onclick,bg,fg){
          ...
          return `
            <div class="flex flex-col items-center gap-1 flex-shrink-0" style="width:48px;">
              <button onclick="${onclick}" title="${label}" class="rounded-full flex items-center justify-center flex-shrink-0" style="width:30px;height:30px;background:${bg};color:${fg};">${Icon(icon,'w-[13px] h-[13px]')}</button>
        ...
        function shareGlimpseActionHTML(glimpseId){
          ...
          if (count > 0) {
            return `
              <div class="flex flex-col items-center gap-1 flex-shrink-0" style="width:48px;">
                <button onclick="sendGlimpseToSelectedContacts(${glimpseId})" title="Send" class="rounded-full flex items-center justify-center flex-shrink-0 relative" style="width:30px;height:30px;background:linear-gradient(135deg,${ROYAL},${NAVY});color:#fff;">
                  ${Icon('send','w-[13px] h-[13px]')}
                  <span class="absolute bg-white text-[9px] font-bold rounded-full flex items-center justify-center" style="top:-3px;right:-3px;width:15px;height:15px;color:${ROYAL};box-shadow:0 0 0 1.5px ${ROYAL};">${count}</span>
                </button>
                <div class="text-[10px] text-gray-600 font-medium text-center leading-tight">Send</div>
          ...
          return shareExternalOption('send','More apps', `shareGlimpseExternally(${glimpseId})`, `linear-gradient(135deg,${ROYAL},${NAVY})`, '#fff');
        }
