          ...
          if (s.mine) {
            ...
            return `
              <div class="flex flex-col items-center gap-1.5 flex-shrink-0 w-16">
                <div class="relative">
                  <button onclick="openMyGlimpses()" class="w-16 h-16 rounded-full overflow-hidden flex items-center justify-center" style="padding:${hasGlimpse ? '2.5px' : '0'};background:${hasGlimpse ? NAVY : 'transparent'};">
                    <div class="w-full h-full rounded-full overflow-hidden flex items-center justify-center" style="color:${NAVY};background:rgba(10,37,64,0.14);">${profileData.photo ? `<img src="${profileData.photo}" class="w-full h-full object-cover">` : silhouetteIcon(s.icon,'w-7 h-7')}</div>
                  </button>
                  <button onclick="event.stopPropagation(); openMyGlimpses()" title="Add to your glimpse" class="absolute z-10 rounded-full border-[2px] border-white flex items-center justify-center shadow" style="top:0;left:0;width:1.1rem;height:1.1rem;color:${NAVY};background:rgba(65,105,225,0.08);">${Icon('plus','w-2 h-2')}</button>
                </div>
                <button onclick="openMyGlimpses()" class="text-xs font-semibold text-gray-800 truncate w-16 text-center">${escapeHtml(s.name)}</button>
              </div>`;
          }
          const ring = s.viewed ? 'bg-gray-300' : `bg-[${NAVY}]`;
          return `
            <button onclick="viewStory('${s.id}')" class="flex flex-col items-center gap-1.5 flex-shrink-0 w-16">
              <div class="w-16 h-16 rounded-full p-[2.5px] ${ring}">
                <div class="w-full h-full rounded-full ${s.bg} flex items-center justify-center border-2 border-white overflow-hidden">${s.photo ? `<img src="${s.photo}" class="w-full h-full object-cover">` : silhouetteIcon(s.icon, 'w-7 h-7 ' + (s.iconClass||''))}</div>
              </div>
              <div class="text-xs font-semibold text-gray-800 truncate w-16 text-center">${escapeHtml(s.name)}</div>
            </button>`;
        }
          ...
          return `
            <div class="flex flex-col h-full bg-white relative">
              <div class="flex items-center gap-4 px-4 pb-4 border-b border-gray-100 flex-shrink-0 bg-white" style="padding-top:20px;">
                ...
                <div class="w-9 h-9 rounded-full overflow-hidden flex items-center justify-center flex-shrink-0" style="padding:${myGlimpses.length > 0 ? '2px' : '0'};background:${myGlimpses.length > 0 ? NAVY : 'transparent'};">
                  <div class="w-full h-full rounded-full overflow-hidden flex items-center justify-center" style="color:${NAVY};background:rgba(10,37,64,0.14);">${profileData.photo ? `<img src="${profileData.photo}" class="w-full h-full object-cover">` : silhouetteIcon('user','w-4 h-4')}</div>
                </div>
              ...
              <div class="absolute flex flex-col items-end gap-4" style="right:1.25rem; bottom:50px;">
                <button onclick="composeGlimpseText()" class="w-12 h-12 rounded-2xl bg-gray-100 shadow flex items-center justify-center" style="color:dodgerblue;">${Icon('edit','w-5 h-5')}</button>
                <button onclick="composeGlimpseMedia()" class="w-12 h-12 rounded-2xl bg-gray-100 shadow flex items-center justify-center" style="color:dodgerblue;">${Icon('camera','w-5 h-5')}</button>
              </div>
            </div>`;
        }
        ...
        const PostsAPI = {
          ...
          create(data){
            ...
            return (async () => {
              ...
              if (mediaFile) {
                const uploaded = await uploadPostMediaToStorage(mediaFile, post.id);
                if (uploaded) {
                  post.mediaPath = uploaded.path;
                  post.mediaHtml = mediaType === 'video'
                    ? `<video src="${uploaded.url}" controls class="w-full h-auto bg-black" onerror="this.onerror=null;this.replaceWith(Object.assign(document.createElement('div'),{className:'w-full py-10 flex items-center justify-center bg-gray-100 text-gray-400 text-xs italic',textContent:'Video no longer available'}))"></video>`
                    : `<img src="${uploaded.url}" class="w-full h-auto" onerror="this.onerror=null;this.replaceWith(Object.assign(document.createElement('div'),{className:'w-full py-10 flex items-center justify-center bg-gray-100 text-gray-400 text-xs italic',textContent:'Image no longer available'}))">`;
                }
              }
          ...
          remove(id){
            return mockRequest(() => {
              ...
              if (removed && removed.mine && removed.mediaPath) deletePostMediaFromStorage(removed.mediaPath);
        ...
        function commentRow(c){
          ...
          const avatarPhoto = isMine ? profileData.photo : (c.photo || null);
          return `
