        ...
        function setupPullToRefresh(screenEl){
          ...
          screenEl.addEventListener('touchmove', (e) => {
            ...
            if (dy <= 0) { ptrDistance = 0; setPullIndicatorHeight(0, false); return; }
            ...
            return `
              <div class="flex flex-col items-center gap-1.5 flex-shrink-0 w-16">
                <div class="relative">
                  ...
                  <button onclick="event.stopPropagation(); openMyGlimpses()" title="Add to your glimpse" class="absolute z-10 rounded-full border-[3px] border-white flex items-center justify-center shadow" style="top:-2px;left:-2px;width:1.5rem;height:1.5rem;color:${NAVY};background:rgba(65,105,225,0.08);">${Icon('plus','w-3 h-3')}</button>
                </div>
        ...
        async function loadRemotePosts(){
          ...
          try {
            ...
            // The feed is meant to feel fresh and different every time it
            // loads (initial open, tab switch back to Home, or a manual
            // pull-to-refresh) rather than a fixed newest-first order, so
            // shuffle the display order right after assembling/sorting it.
            // Nothing else depends on feedPosts' array order (profile post
            // counts, lookups, etc. all use .filter/.find), so this is
            // safe to randomize in place.
            shuffleArray(feedPosts);
          } catch (e) { remotePostsLoaded = true; }
        }

        // Fisher-Yates shuffle, used to randomize the feed's post order on
        // every refresh (see loadRemotePosts above and
        // startHomeFeedRefresh below). Kept local to this file (other
        // files in this app -- study.js, quizzesAndGames.js -- each keep
        // their own copy of the same helper rather than relying on
