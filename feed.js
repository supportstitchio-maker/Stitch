// ---- Feed render + pull-to-refresh ----
            ...
            <div class="relative w-full h-full bg-black overflow-hidden" id="gallery-post-${post.id}" ontouchstart="galleryTouchStart(event)" ontouchend="galleryTouchEnd(event, ${post.id}, ${index})">
              ...
              <div class="absolute z-20" style="left:16px;right:16px;bottom:calc(env(safe-area-inset-bottom, 12px) + 20px);">
                <div class="flex items-center gap-2 ${captionHtml ? 'mb-1.5' : ''} cursor-pointer" onclick="event.stopPropagation(); openPersonProfileForPost(${post.id})">
                  ...
                  <span class="text-white font-semibold text-sm rounded-full" style="background:rgba(120,120,120,0.45);padding:2px 10px;">${escapeHtml(post.mine ? (profileData.name || post.name) : post.name)}</span>
                  ${post.verified ? verifiedBadge() : ''}
                </div>
        ...
        function shareHTML(post){
          ...
          return `
            ...
            <div class="flex-shrink-0 w-full flex items-start justify-start no-scrollbar border-t border-gray-100" style="gap:28px;padding:14px 20px calc(env(safe-area-inset-bottom, 12px) + 14px) 20px;background:#fafafa;overflow-x:auto;">
              <div id="share-action-${post.id}">${shareActionHTML(post.id)}</div>
              ${shareExternalOption('send','Share via', `sharePostExternally(${post.id})`, `linear-gradient(135deg,${ROYAL},${NAVY})`, '#fff')}
        ...
        function shareActionHTML(postId){
          ...
          if (count > 0) {
            return `
              <div class="flex flex-col items-center gap-1 flex-shrink-0" style="width:48px;">
                <button onclick="sendPostToSelectedContacts(${postId})" title="Send" class="rounded-full flex items-center justify-center flex-shrink-0 relative" style="width:22px;height:22px;background:linear-gradient(135deg,${ROYAL},${NAVY});color:#fff;">
                  ${Icon('send','w-[10px] h-[10px]')}
                  <span class="absolute bg-white text-[9px] font-bold rounded-full flex items-center justify-center" style="top:-3px;right:-3px;width:15px;height:15px;color:${ROYAL};box-shadow:0 0 0 1.5px ${ROYAL};">${count}</span>
                </button>
                <div class="text-[10px] text-gray-600 font-medium text-center leading-tight">Send</div>
              </div>`;
          }
          return '';
        }
        ...
        function shareExternalOption(icon,label,onclick,bg,fg){
          ...
          return `
            <div class="flex flex-col items-center gap-1 flex-shrink-0" style="width:48px;">
              <button onclick="${onclick}" title="${label}" class="rounded-full flex items-center justify-center flex-shrink-0" style="width:22px;height:22px;background:${bg};color:${fg};">${Icon(icon,'w-[10px] h-[10px]')}</button>
              <div class="text-[10px] text-gray-600 font-medium text-center leading-tight">${escapeHtml(label)}</div>
            </div>`;
        }

        // Brand-colored share targets (WhatsApp / Messages) shown alongside
        // the generic "Share via" (opens the OS share sheet) and "Copy link"
        // options, matching a native share panel's row of quick-share icons.
        const WHATSAPP_ICON_SVG = `<path fill="#fff" d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.45 1.32 4.95L2 22l5.29-1.39a9.87 9.87 0 004.75 1.21h.01c5.46 0 9.9-4.45 9.9-9.91 0-2.65-1.03-5.14-2.9-7.01A9.82 9.82 0 0012.04 2m0 1.8a8.1 8.1 0 018.1 8.1c0 4.47-3.63 8.1-8.1 8.1a8.06 8.06 0 01-4.13-1.13l-.3-.17-3.14.82.84-3.06-.19-.31a8.08 8.08 0 01-1.24-4.31 8.1 8.1 0 018.1-8.1m-4.44 4.61c-.18 0-.47.07-.71.34-.24.27-.94.92-.94 2.24s.96 2.6 1.1 2.78c.13.18 1.88 2.87 4.55 4.02.64.27 1.14.44 1.53.56.64.2 1.23.17 1.69.1.52-.08 1.6-.65 1.82-1.28.23-.63.23-1.17.16-1.28-.07-.11-.25-.18-.53-.31-.27-.14-1.6-.79-1.85-.88-.25-.09-.43-.14-.61.14-.18.27-.7.88-.86 1.06-.16.18-.31.2-.59.07-.27-.14-1.15-.42-2.19-1.35-.81-.72-1.36-1.61-1.52-1.88-.16-.27-.02-.42.12-.55.13-.13.27-.32.41-.48.13-.16.18-.27.27-.46.09-.18.05-.34-.02-.48-.07-.14-.61-1.47-.85-2.01-.22-.53-.44-.46-.61-.46z"/>`;
        const MESSAGES_ICON_SVG = `<path fill="#fff" d="M12 2C6.48 2 2 5.94 2 10.8c0 2.64 1.34 5 3.44 6.62-.15 1.09-.53 2.6-1.44 3.98 0 0 2.18-.24 4.24-1.62.85.24 1.79.37 2.76.37 5.52 0 10-3.94 10-8.8S17.52 2 12 2z"/>`;

        function shareBrandOption(label, onclick, bg, svgInner){
          return `
            <div class="flex flex-col items-center gap-1 flex-shrink-0" style="width:48px;">
              <button onclick="${onclick}" title="${label}" class="rounded-full flex items-center justify-center flex-shrink-0" style="width:22px;height:22px;background:${bg};">
                <svg viewBox="0 0 24 24" style="width:11px;height:11px;">${svgInner}</svg>
              </button>
              <div class="text-[10px] text-gray-600 font-medium text-center leading-tight">${escapeHtml(label)}</div>
            </div>`;
        }
        ...
        function shareWhatsAppOption(onclick){
          return shareBrandOption('WhatsApp', onclick, '#25D366', WHATSAPP_ICON_SVG);
        }

        function shareMessagesOption(onclick){
          return shareBrandOption('Messages', onclick, 'linear-gradient(135deg,#3b82f6,#1d4ed8)', MESSAGES_ICON_SVG);
        }
        ...
        function shareGlimpseHTML(g){
          ...
          return `
            ...
            <div class="flex-shrink-0 w-full flex items-start justify-start no-scrollbar border-t border-gray-100" style="gap:28px;padding:14px 20px calc(env(safe-area-inset-bottom, 12px) + 14px) 20px;background:#fafafa;overflow-x:auto;">
              ...
              ${shareExternalOption('send','Share via', `shareGlimpseExternally(${g.id})`, `linear-gradient(135deg,${ROYAL},${NAVY})`, '#fff')}
        ...
        function shareGlimpseActionHTML(glimpseId){
          ...
          if (count > 0) {
            return `
              <div class="flex flex-col items-center gap-1 flex-shrink-0" style="width:48px;">
                <button onclick="sendGlimpseToSelectedContacts(${glimpseId})" title="Send" class="rounded-full flex items-center justify-center flex-shrink-0 relative" style="width:22px;height:22px;background:linear-gradient(135deg,${ROYAL},${NAVY});color:#fff;">
                  ${Icon('send','w-[10px] h-[10px]')}
                  <span class="absolute bg-white text-[9px] font-bold rounded-full flex items-center justify-center" style="top:-3px;right:-3px;width:15px;height:15px;color:${ROYAL};box-shadow:0 0 0 1.5px ${ROYAL};">${count}</span>
                </button>
                <div class="text-[10px] text-gray-600 font-medium text-center leading-tight">Send</div>
