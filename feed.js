          ...
          return `
            <div class="relative w-full h-full bg-black overflow-hidden" id="video-post-${post.id}" ontouchstart="vpdTouchStart(event)" ontouchend="vpdTouchEnd(event, ${post.id})">
              ...
              <div class="absolute z-20 flex flex-col items-center gap-4" style="right:12px;bottom:calc(env(safe-area-inset-bottom, 12px) + 100px);">
                <button onclick="event.stopPropagation(); toggleLike(${post.id})" id="like-btn-${post.id}" class="flex flex-col items-center gap-1 ${post.liked ? 'text-red-500' : 'text-white'}">
                  <span id="like-icon-${post.id}" class="rounded-full flex items-center justify-center" style="width:2.75rem;height:2.75rem;background:rgba(120,120,120,0.45);">${Icon(post.liked ? 'heart' : 'heartOutline','w-6 h-6')}</span>
                  <span class="text-[11px] font-semibold text-white" id="like-inline-count-${post.id}">${formatCount(post.likes)}</span>
                </button>
                <button onclick="event.stopPropagation(); openComments(${post.id})" id="comment-btn-${post.id}" class="flex flex-col items-center gap-1 text-white">
                  ...
                  <span class="text-[11px] font-semibold text-white" id="comment-inline-count-${post.id}">${formatCount(post.comments)}</span>
                </button>
                <button onclick="event.stopPropagation(); toggleRepost(${post.id})" id="repost-btn-${post.id}" class="flex flex-col items-center gap-1 ${post.reposted ? 'text-emerald-600' : 'text-white'}">
                  ...
                  <span class="text-[11px] font-semibold text-white" id="repost-inline-count-${post.id}">${formatCount(post.reposts)}</span>
                </button>
                <button onclick="event.stopPropagation(); openShare(${post.id})" id="share-btn-${post.id}" class="flex flex-col items-center gap-1 text-white">
                  ...
                  <span class="text-[11px] font-semibold text-white" id="share-inline-count-${post.id}">${formatCount(post.shares)}</span>
                </button>
                <button onclick="event.stopPropagation(); toggleSavePost(${post.id})" class="flex flex-col items-center ${post.saved ? `text-[${ROYAL}]` : 'text-white'}">
                  <span class="rounded-full flex items-center justify-center" style="width:2.75rem;height:2.75rem;background:rgba(120,120,120,0.45);">${Icon('bookmark','w-6 h-6')}</span>
                </button>
              </div>
        ...
        function toggleLike(id){
          PostsAPI.toggleLike(id).then(post => {
            ...
            if (iconEl) iconEl.innerHTML = Icon(post.liked ? 'heart' : 'heartOutline', 'w-6 h-6');
            const likeCountEl = document.getElementById('like-count-'+id);
            if (likeCountEl) likeCountEl.textContent = post.likes.toLocaleString() + ' likes';
            const btn = document.getElementById('like-btn-'+id);
            if (btn) {
              // This same id exists on two very differently laid-out
              // buttons -- the plain in-feed card icon row, and the
              // full-screen video player's stacked icon+count column
              // (see videoPostDetailHTML) -- so which layout classes
              // belong here depends on which one is actually open, not
              // just the liked color.
              const isVideoDetail = !!document.getElementById('video-post-' + id);
              btn.className = isVideoDetail
                ? `flex flex-col items-center gap-1 ${post.liked ? 'text-red-500' : 'text-white'}`
                : `p-1 -ml-1 flex items-center gap-1 ${post.liked ? 'text-red-500' : ''}`;
            }
