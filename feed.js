          ...
          return `
            <button onclick="viewStory(${s.id})" class="flex flex-col items-center gap-1.5 flex-shrink-0 w-20">
              <div class="w-20 h-20 rounded-full p-[2.5px] ${ring}">
                <div class="w-full h-full rounded-full ${s.bg} flex items-center justify-center border-2 border-white">${silhouetteIcon(s.icon, 'w-9 h-9 ' + (s.iconClass||''))}</div>
              </div>
            ...
            <div class="flex flex-col h-full bg-gradient-to-b from-[${NAVY}] to-[${ROYAL}] text-white" style="padding-top:20px;">
              <div class="flex items-center justify-between px-4 flex-shrink-0">
                <div class="flex items-center gap-2">
                  <div class="w-8 h-8 rounded-full border-2 border-white/60 flex items-center justify-center overflow-hidden" style="background:transparent;">${(s.mine && profileData.photo) ? `<img src="${profileData.photo}" class="w-full h-full object-cover">` : silhouetteIcon(icon, 'w-4 h-4 ' + (iconClass||''))}</div>
        ...
        async function loadRemotePosts(){
          ...
          try {
            ...
            data.forEach(row => {
              ...
              feedPosts.push({ ...row.data, id: row.data.id ?? id, mine: isMine, authorId: row.created_by, connected: isMine ? true : !!row.data.connected });
              knownIds.add(id);
            });
        ...
        async function loadPostInteractions(){
          ...
          try {
            ...
            const [likesRes, repostsRes, savesRes, commentsRes] = await Promise.all([
            ...
            const likesByPost = {}, repostsByPost = {}, commentsByPost = {};
          ...
          create(data){
            ...
            return (async () => {
              const post = {
                ...
                body: '', mediaHtml: null,
                likes: 0, liked: false, reposts: 0, reposted: false, shares: 0, comments: 0, commentsList: [], saved: false,
                ...rest,
              };
              ...
              if (mediaFile) {
                ...
                if (publicUrl) {
                  post.mediaHtml = mediaType === 'video'
                    ? `<video src="${publicUrl}" controls class="w-full h-auto rounded-2xl mb-4 bg-black"></video>`
                    : `<img src="${publicUrl}" class="w-full h-auto rounded-2xl mb-4">`;
                }
              }
        ...
        const GlimpsesAPI = {
          ...
          create(data){
            return mockRequest(() => {
              ...
              const glimpse = { id: Date.now(), createdAt: Date.now(), timeLabel, type: 'text', viewers: [], ...data };
        ...
        function deletePost(id){
          ...
          PostsAPI.remove(id).then(() => {
            openPostMenuId = null;
            // Deleting a post also removes it from Reposts/Saved on the
            // profile (those tabs just filter feedPosts live, so removing
            // it here is enough), but the numeric "Posts" stat in the
            // profile header is tracked separately in profileData.posts
            // and only ever incremented on create -- decrement it here so
            // the count stays accurate.
            if (post && post.mine && profileData.posts > 0) {
              profileData.posts -= 1;
              queueSaveUserState();
            }
            renderFeed();
        ...
        function reportPost(id){
          ...
          openPostMenuId = null;
          renderFeed();
          alert('Thanks for letting us know. This post has been reported and our team will review it.');
        }
          ...
          return `
            <div class="bg-white overflow-hidden -mx-5 border-b border-gray-100" id="post-${post.id}">
              <div class="px-4 py-2 flex gap-2.5 items-center relative">
                <div class="w-7 h-7 rounded-full flex-shrink-0 p-[2px] ${isStranger ? 'bg-gradient-to-tr from-pink-500 via-red-500 to-yellow-400' : 'bg-gray-200'}">
                  <div class="w-full h-full ${post.avatarBg} rounded-full overflow-hidden flex items-center justify-center border-2 border-white cursor-pointer" onclick="event.stopPropagation(); openPersonProfileForPost(${post.id})">${post.mine && profileData.photo ? `<img src="${profileData.photo}" class="w-full h-full object-cover">` : Icon(post.avatarIcon,'w-3.5 h-3.5 text-gray-600')}</div>
                </div>
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-[11px] text-gray-500 truncate">${isStranger ? 'Suggested for you' : (post.timeAgo || post.meta)}</div>
                </div>
                ${showConnect ? `
              ...
              <div class="px-3.5 pt-1 pb-3 text-[10px] uppercase text-gray-400" id="share-count-${post.id}">${post.timeAgo || post.meta.split('·').pop().trim()}</div>
            </div>`;
        }
        ...
        function toggleRepost(id){
          PostsAPI.toggleRepost(id).then(post => {
            if (!post) return;
            document.getElementById('repost-btn-'+id).className = `p-1 ml-2 flex items-center gap-1 ${post.reposted ? 'text-emerald-600' : ''}`;
