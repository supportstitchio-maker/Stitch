// ---- Feed render + pull-to-refresh ----
        ...
        // Set the moment the bare shimmer skeleton (feedSkeletonHTML) is painted into #screen with
        // nothing cached to show instead; see the minimum-visible-time guard at the top of the
        let feedSkeletonShownAt = null;
        // How long the skeleton stays up at minimum once it's shown, so a fast connection (or an
        // already-warm Supabase session) doesn't make it flash by too quickly to register as a
        const FEED_SKELETON_MIN_VISIBLE_MS = 450;
        ...
        // Single source of truth for "does this post belong in the home feed right now" -- keeps
        // renderFeed() and patchFeedInPlace() from drifting apart. A stranger's video only
        ...
        function renderFeed() {
          ...
          if (typeof remotePostsLoaded !== 'undefined' && !remotePostsLoaded) {
            // A slow first connection used to just sit on a bare skeleton with nothing to look at. If
            // this account has a saved snapshot from a previous successful load, show that instead --
            ...
            if (cached) {
              ...
              screenEl.innerHTML = `
                <div class="px-5 pb-5" style="padding-top:var(--top-safe-pad);">
                  ${feedCacheBannerHTML()}
                  <div class="space-y-[5px]" id="feed-list">
            ...
            feedSkeletonShownAt = Date.now();
            return;
          }
          // The bare skeleton (not a cached snapshot -- that path returns above and is unaffected)
          // was just shown very recently. Wait out the rest of a minimum visible window before
          if (feedSkeletonShownAt) {
            ...
            if (remaining > 0) {
              setTimeout(renderFeed, remaining);
              return;
            }
          }
          feedShowingCachedOnly = false;
          screenEl.innerHTML = `
            <div class="px-5 pb-5" style="padding-top:var(--top-safe-pad);">
              ...
              <div class="space-y-[5px]" id="feed-list">
                ${feedVisiblePosts().map(feedPost).join('')}
              </div>
            </div>`;
          if (screenEl) {
        ...
        function patchFeedInPlace(){
          // Still showing the cached snapshot (no successful live fetch yet) -- defer to
          // renderFeed(), which knows how to correctly redisplay either state. Patching the cached
          ...
          // Uses the same feedVisiblePosts() the initial render uses (see above) so this patch path
          // can't drift out of sync with it -- a post mid-upload has no mediaHtml yet, but it should
        ...
        function setupPullToRefresh(screenEl){
          ...
          // Desktop (mouse) support: mirrors the touch handlers above so click-and-drag with a mouse
          // also triggers pull-to-refresh, since desktop browsers don't fire touch events.
          ...
          // Desktop trackpad/scroll-wheel support: most desktop users scroll with a wheel rather
          // than click-and-drag, so mirror the same gesture there -- once the feed is already
        ...
        function IconBold(type, cls){
          const boldPaths = {
            ...
            check: `<path d="M4.5 12.5l4.75 4.75L19.5 6.5" fill="none" stroke="currentColor" stroke-width="3.4" stroke-linecap="round" stroke-linejoin="round"/>`,
        ...
        function storyBubble(s){
          if (s.mine) {
            // Self-heal on every render (tab switches, feed refreshes, etc.), not just on
            // sign-in -- a glimpse that quietly crosses the 24h mark while the app stays open
            // should drop out of the ring right away instead of waiting for the next login.
            if (typeof purgeExpiredGlimpses === 'function') purgeExpiredGlimpses();
              ...
              <div class="flex flex-col items-center gap-1.5 flex-shrink-0 w-20">
                <div class="relative">
                  <button onclick="${openMine}" class="w-20 h-20 rounded-full overflow-hidden flex items-center justify-center" style="padding:3.5px;background:${mineRingBg};">
                    <div class="w-full h-full rounded-full overflow-hidden flex items-center justify-center border-2 border-white" style="color:${NAVY};background:rgba(10,37,64,0.14);">${profileData.photo ? `<img src="${profileData.photo}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="w-full h-full items-center justify-center" style="display:none;">${silhouetteIcon(s.icon,'w-9 h-9')}</div>` : silhouetteIcon(s.icon,'w-9 h-9')}</div>
                  </button>
          ...
          // A broken/expired profile-photo URL used to just leave the whole circle showing the
          // browser's default broken-image icon on a transparent background -- which reads as a
          // solid black bubble against the dark story strip. Fall back to the silhouette icon,
          // same as the "mine" bubble above already does.
          return `
            <button onclick="viewStory('${s.id}')" class="flex flex-col items-center gap-1.5 flex-shrink-0 w-20">
              <div class="w-20 h-20 rounded-full" style="${ringStyle}">
                <div class="w-full h-full rounded-full ${s.bg} flex items-center justify-center border-2 border-white overflow-hidden">${s.photo ? `<img src="${s.photo}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="w-full h-full items-center justify-center" style="display:none;">${silhouetteIcon(s.icon, 'w-9 h-9 ' + (s.iconClass||''))}</div>` : silhouetteIcon(s.icon, 'w-9 h-9 ' + (s.iconClass||''))}</div>
              </div>
        ...
        function reorderStoriesByViewed(){
          ...
          // Admin glimpse always sits first among "others".
          const adminIdx = others.findIndex(s => s.otherUserId === ADMIN_USER_ID);
          if (adminIdx > 0) others.unshift(others.splice(adminIdx, 1)[0]);
          stories = [...mineList, ...others];
        }

        function viewStory(id){
          // stories can be replaced wholesale between the tap firing and this running (a
          // background loadRemoteGlimpses() refresh, an expiry purge, etc.) -- tapping a bubble
          // for a story that no longer exists in the current array used to throw here (reading
          // .viewed off undefined) and silently abort with nothing on screen: no overlay, no
          // error, just a tap that appeared to do nothing. Bail out quietly instead.
        ...
        function openStoryOverlay(s){
          if (!s) return;
          // Opening a glimpse (story) sits on top of the home feed -- pause any feed post video
          // that's autoplaying underneath so it doesn't keep playing (and making sound) in the
          ...
          try {
            ...
            // A bad render here (missing field on an old/odd glimpse item, etc.) used to leave
            // the overlay showing (it was already un-hidden below) with nothing painted into
            // it -- since the overlay itself sits on a dark backdrop, that reads as the glimpse
            // "turning black" with no way out except force-closing the app. Fail safely closed
            // instead: never show the overlay for content that didn't actually render.
            console.error('Glimpse failed to render:', e);
        ...
        function storyViewerHTML(s, itemIndex){
          ...
          // A text-only glimpse carries its own background (item.bgStyle, chosen at compose
          // time) -- this view never applied it, so it fell back to the parent's plain black
          // and anyone but the author saw what looked like a black screen with faint text.
          const isLightGlimpse = !item.mediaUrl && item.bgStyle && item.bgStyle.indexOf('#ffffff') !== -1;
          const fg = isLightGlimpse ? '#0a2540' : '#ffffff';
          const bgIconWrap = isLightGlimpse ? 'rgba(10,37,64,0.10)' : 'rgba(255,255,255,0.2)';
          // A text glimpse with no bgStyle at all (an older/odd row missing that field) used to
          // fall all the way back to plain black -- indistinguishable from a broken media load,
          // and with no caption visible against it, it read as a totally blank/black glimpse.
          // Fall back to the same navy/royal gradient used for the unviewed ring instead, so
          // there's always something visible behind the caption.
            ...
            <div class="flex flex-col h-full" style="padding-top:var(--top-safe-pad);color:${fg};${item.mediaUrl ? 'background:#000;' : (item.bgStyle || ('background:' + noBgFallback + ';'))}">
              ...
              <div class="flex-1 relative">
                ...
                <div class="absolute inset-0 flex items-center justify-center px-8 text-center pointer-events-none">
                  <div>
                    <div class="w-20 h-20 mx-auto mb-4 rounded-full flex items-center justify-center" style="background:${bgIconWrap};">${silhouetteIcon(icon, 'w-10 h-10 ' + (iconClass||''))}</div>
                    <div class="text-lg font-medium" style="color:${fg};">${escapeHtml(item.caption)}</div>
                  </div>
                </div>
                `}
        ...
        async function loadRemoteGlimpses(){
          ...
          try {
            ...
            // The in-memory/localStorage checks above only cover "already viewed earlier this
            // browser session/on this device". Logging back in (or opening Stitch on a second
            // device) used to lose that entirely and show the ring as unviewed/blue again even
            // though this account has already watched that exact glimpse -- glimpse_views is the
            // durable, server-side record of that, so check it too as the source of truth.
            ...
            const remoteStories = Object.keys(byAuthor).map(userId => {
              ...
              const viewed = (!!inMemory && inMemory.viewed && inMemory.newestId >= newestId)
                ...
                || myViewedGlimpseIds.has(String(newestId));
              return {
                id: userId,
        ...
        function glimpsesViewedStorageKey(uid){
          const id = uid || ((typeof _cachedAuthUser !== 'undefined' && _cachedAuthUser) ? _cachedAuthUser.id : null);
          return id ? `glimpses-viewed-v1:${id}` : null;
        }

        function loadPersistedViewedGlimpses(uid){
          try {
            const key = glimpsesViewedStorageKey(uid);
            if (!key) return {};
        ...
        function persistGlimpseViewed(story, uid){
          try {
            const key = glimpsesViewedStorageKey(uid);
            if (!key) return;
            ...
            const map = loadPersistedViewedGlimpses(uid);
        ...
        function myGlimpseViewerHTML(g){
          ...
          // Same fallback as storyViewerHTML: never fall through to plain black with nothing to
          // look at when a text glimpse is missing its bgStyle.
          const noBgFallback = `linear-gradient(135deg, ${NAVY}, ${ROYAL})`;
          return `
            <div class="flex flex-col h-full relative" style="color:${fg};${g.mediaUrl ? 'background:#000;' : (g.bgStyle || ('background:' + noBgFallback + ';'))}">
        ...
        function glimpseViewersPanelHTML(g){
          ...
          // Pinned to the real bottom of the screen via absolute positioning instead of a flex
          // margin-top:auto -- the wrapping #glimpse-viewers-panel div around this (see
          ...
          return `
              <div class="absolute left-4 flex items-center" style="bottom:calc(env(safe-area-inset-bottom, 16px) + ${hasCaptionBar ? '4.5rem' : '16px'});pointer-events:auto;">
                <button onclick="toggleMyGlimpseViewers(${g.id})" class="flex items-center gap-2 rounded-full px-4 py-2.5 text-white" style="background:rgba(0,0,0,0.45);color:#ffffff;">
        ...
        // failed load used to just leave that whole screen solid black with zero feedback,
        function glimpseMediaLoadError(el){
          const wrap = el.parentElement;
          if (!wrap) return;
          const isVideo = el.tagName === 'VIDEO';
          const baseUrl = el.dataset.baseSrc || el.currentSrc || el.src;
          wrap.classList.add('glimpse-media-failed');
          // The other-person story viewer (storyViewerHTML) layers full- screen invisible "tap
          // left/right to skip" buttons on top of the media in plain DOM order, since they normally
          wrap.style.zIndex = '10';
          wrap.style.pointerEvents = 'auto';
          const offline = feedIsOffline();
          const errMsg = offline ? 'No internet' : "Couldn't load this glimpse -- check your connection";
          wrap.innerHTML = `
            <div class="px-8 text-center">
              <div class="text-sm mb-3 glimpse-retry-msg" style="color:#fff;opacity:0.85;">${errMsg}</div>
              <button onclick="event.stopPropagation(); glimpseMediaRetryTap(this)" data-kind="${isVideo ? 'video' : 'image'}" data-src="${baseUrl}" data-class="${el.className}" class="text-sm font-semibold underline" style="color:#fff;">Tap to retry</button>
            </div>`;
        }

        // Same "probe silently first, only touch the DOM once we know it worked" approach as
        // feedMediaAttemptReload/feedMediaCommitLiveMedia -- tapping retry used to clear the
        function glimpseMediaRetryTap(btn){
          const wrap = btn.closest('.px-8') && btn.closest('.px-8').parentElement;
          if (!wrap) return;
          const kind = btn.dataset.kind;
          const src = btn.dataset.src;
          const cls = btn.dataset.class;
          const isVideo = kind === 'video';
          const label = wrap.querySelector('.glimpse-retry-msg');
          if (feedIsOffline()) {
            if (label) label.textContent = 'Still no internet';
            setTimeout(() => { if (label && label.isConnected) label.textContent = 'No internet'; }, 900);
            return;
          }
          btn.style.visibility = 'hidden';
          if (label) label.textContent = 'Retrying\u2026';
          const sep = src.indexOf('?') === -1 ? '?' : '&';
          const probeUrl = src + sep + '_retry=' + Date.now();
          const probe = document.createElement(isVideo ? 'video' : 'img');
          const commit = () => {
            if (!wrap.isConnected) return;
            probe.className = cls;
            probe.draggable = false;
            probe.setAttribute('oncontextmenu', 'return false');
            probe.style.webkitTouchCallout = 'none';
            probe.style.webkitUserSelect = 'none';
            probe.style.userSelect = 'none';
            if (isVideo) { probe.autoplay = true; probe.playsInline = true; probe.loop = true; probe.muted = false; }
            probe.dataset.baseSrc = src;
            probe.onerror = () => glimpseMediaLoadError(probe);
            wrap.style.pointerEvents = 'none';
            wrap.style.zIndex = '';
            wrap.classList.remove('glimpse-media-failed');
            wrap.innerHTML = '';
          ...
          const revert = () => {
            ...
            if (label) label.textContent = feedIsOffline() ? 'No internet' : "Couldn't load this glimpse -- check your connection";
          };
          if (isVideo) {
            probe.preload = 'metadata';
            probe.muted = true;
            probe.onloadedmetadata = commit;
            probe.onerror = revert;
          } else {
            probe.onload = commit;
            probe.onerror = revert;
          }
        ...
        // Excludes repost cards: those already have their own "Reposts" tab on the profile, so
        // counting them here too would inflate "Posts" with items the person never actually
        ...
        // Grabs a real frame out of a video file client-side (via an off-screen <video>+<canvas>)
        // so it can be uploaded as a poster image alongside the video. A poster shows instantly
        ...
        // ---- Feed post video playback (mute/autoplay) ---- Videos autoplay with sound on by
        // default as they scroll into view (see setupFeedVideoAutoplay below) and pause once
        ...
        // Client-side cache of a real decoded frame for videos that have no stored posterUrl
        // (older posts uploaded before generateVideoPosterFile existed, or remote/demo posts with
        const feedVideoFrameCache = {};

        // Draws whatever frame the <video> currently has decoded onto an off-screen canvas and
        // reads it back out as a jpeg data URL -- a genuine frame from that exact video, not a
        function captureFeedVideoFrame(video){
          try {
        ...
        function rememberFeedVideoFrame(video){
          const src = video.dataset.baseSrc || video.currentSrc || video.src;
          if (!src || feedVideoFrameCache[src]) return;
          const frame = captureFeedVideoFrame(video);
          if (frame) {
            feedVideoFrameCache[src] = frame;
            if (!video.getAttribute('poster')) video.poster = frame;
          }
        }
        ...
        function armFeedVideoReveal(video){
          ...
          // without immediately stalling again -- unlike 'timeupdate', which can fire the instant
          video.addEventListener('canplay', reveal, { once: true });
          video.addEventListener('loadeddata', () => { setTimeout(reveal, 700); }, { once: true });
          // Safety net: a <video poster="..."> only shows that poster once the poster image itself
          // has actually finished downloading -- until then the browser paints the raw video canvas
        ...
        function attemptFeedVideoPlay(video, btn){
          ...
          if (playPromise && typeof playPromise.catch === 'function') {
            playPromise.catch((err) => {
              // Leaving the feed, opening an overlay, or scrolling a post out of view (see
        ...
        // A failed video/image load almost always means "the network hiccuped just now" rather
        // than "this file was actually deleted" -- but the old error handlers immediately declared
        document.addEventListener('load', function(e){
          const el = e.target;
          if (el && el.tagName === 'IMG' && el.closest && el.closest('.feed-media')) {
            el.classList.add('feed-img-loaded');
          }
        }, true);

        // Catches images that had already finished loading (browser cache, or just genuinely done)
        // by the moment a render happens to run -- 'load' only ever fires once, so a post that
        function syncFeedImageReadyState(root){
          (root || document).querySelectorAll('.feed-media > img').forEach(img => {
            if (img.complete && img.naturalWidth > 0) img.classList.add('feed-img-loaded');
          });
        }

        // True as soon as we're sure there's no connection at all -- in that case there's no point
        // burning through the timed background-retry attempts one by one (each is guaranteed to
        // fail); we go straight to the "No internet" placeholder and let the 'online' listener /
        // feedMediaSweepFailed poll pick it back up the moment connectivity actually returns.
        function feedIsOffline(){
        ...
        function feedMediaAutoRetry(el, kind){
          ...
          const placeholder = feedMediaEnterPlaceholder(el, kind, baseUrl);
          if (feedIsOffline()) { feedMediaFinalizePlaceholder(placeholder, kind, baseUrl); return; }
          feedMediaScheduleBackgroundRetry(placeholder, kind, baseUrl, attempt);
        }

        // Silently tries loading baseUrl exactly once, off-screen -- nothing in the visible DOM is
        // touched until we already know whether it worked. onSuccess receives the already-loaded
        function feedMediaAttemptReload(kind, baseUrl, onSuccess, onFail){
          const sep = baseUrl.indexOf('?') === -1 ? '?' : '&';
          const probeUrl = baseUrl + sep + '_retry=' + Date.now();
          const probe = kind === 'video' ? document.createElement('video') : new Image();
          if (kind === 'video') {
            probe.preload = 'metadata';
            probe.muted = true;
            probe.onloadedmetadata = () => onSuccess(probe);
        ...
        function feedMediaScheduleBackgroundRetry(placeholder, kind, baseUrl, attempt){
          if (attempt > 4 || feedIsOffline()) {
            feedMediaFinalizePlaceholder(placeholder, kind, baseUrl);
            return;
          }
          setTimeout(() => {
            // If the person already tapped "retry" themselves (or navigated away and this post got re-
            // rendered), this placeholder node is no longer attached -- stop.
            if (!placeholder.isConnected) return;
            if (feedIsOffline()) { feedMediaFinalizePlaceholder(placeholder, kind, baseUrl); return; }
            feedMediaAttemptReload(
              kind, baseUrl,
              (loadedEl) => feedMediaCommitLiveMedia(placeholder, kind, loadedEl),
              () => feedMediaScheduleBackgroundRetry(placeholder, kind, baseUrl, attempt + 1)
            );
          }, attempt * 1200);
        }

        // Drops an *already-loaded* element in place of the placeholder -- this is the only place
        // that ever touches the visible DOM for a successful retry, whether that retry was
        function feedMediaCommitLiveMedia(placeholder, kind, loadedEl){
          if (!placeholder.isConnected) return;
          const isVideo = kind === 'video';
          ...
          loadedEl.onerror = () => feedMediaAutoRetry(loadedEl, kind);
          placeholder.replaceWith(loadedEl);
        }

        function feedMediaFinalizePlaceholder(placeholder, kind, baseUrl){
          if (!placeholder.isConnected) return;
          const offline = feedIsOffline();
          // Offline: stay on the neutral gray skeleton look (no "broken" styling, no italics --
          // this isn't a dead file, it's just waiting on a connection) and say so plainly. Online
          // but still failing after retries is the only time we call it a real load failure.
          placeholder.classList.remove('skel-shimmer');
          if (offline) {
            placeholder.classList.add('flex', 'flex-col', 'items-center', 'justify-center', 'gap-1.5', 'bg-gray-100', 'text-gray-400', 'text-xs', 'text-center', 'px-2', 'feed-media-failed');
          } else {
            placeholder.classList.add('flex', 'flex-col', 'items-center', 'justify-center', 'gap-1.5', 'bg-gray-100', 'text-gray-400', 'text-xs', 'italic', 'text-center', 'px-2', 'feed-media-failed');
          }
          // Tagged + tracked in dataset so a reconnect (see the 'online' listener near
          // feedMediaAutoRetry) can find every failed post on screen and quietly retry it in the
          placeholder.dataset.retryKind = kind;
          placeholder.dataset.baseSrc = baseUrl;
          const msg = offline ? 'No internet' : "Couldn't load this " + (kind === 'video' ? 'video' : 'image') + ' -- check your connection';
          placeholder.innerHTML = `<span>${msg}</span><button type="button" class="not-italic font-semibold" style="pointer-events:auto;color:${NAVY};">Tap to retry</button>`;
          const btn = placeholder.querySelector('button');
          // stopPropagation matters here: .feed-media (the ancestor of this placeholder) has its own
          // onclick that opens the full-screen post detail view (handleFeedMediaTap) -- without
          btn.addEventListener('click', (e) => {
            e.stopPropagation();
            feedMediaRetryTapped(placeholder, kind, baseUrl);
          });
        }

        // The manual "Tap to retry" path used to jump straight to feedMediaSwapInLiveMedia, which
        // tore the message down and dropped in a brand-new, empty <img>/<video> *before*
        function feedMediaRetryTapped(placeholder, kind, baseUrl){
          if (!placeholder.isConnected) return;
          if (feedIsOffline()) {
            const span = placeholder.querySelector('span');
            if (span) span.textContent = 'Still no internet';
            setTimeout(() => { if (placeholder.isConnected) feedMediaFinalizePlaceholder(placeholder, kind, baseUrl); }, 900);
            return;
          }
          const btn = placeholder.querySelector('button');
          const span = placeholder.querySelector('span');
        ...
        // Reconnect handling: a failed image/video doesn't get another automatic attempt once it's
        // shown "Couldn't load -- tap to retry" -- it just waits for a tap. But the most common
        function feedMediaSweepFailed(){
          document.querySelectorAll('.feed-media-failed').forEach(placeholder => {
            if (!placeholder.isConnected) return;
            feedMediaAttemptReload(
              placeholder.dataset.retryKind, placeholder.dataset.baseSrc,
              (loadedEl) => feedMediaCommitLiveMedia(placeholder, placeholder.dataset.retryKind, loadedEl),
              () => {} // still down -- leave the existing message as-is, no need to re-render it
            );
          });
          document.querySelectorAll('.glimpse-media-failed').forEach(wrap => {
            if (!wrap.isConnected) return;
            const btn = wrap.querySelector('button[data-src]');
            if (btn) glimpseMediaRetryTap(btn);
          });
        }
        window.addEventListener('online', feedMediaSweepFailed);
        // The browser's 'online' event is unreliable in a lot of mobile WebViews/PWAs --
        // connectivity can come back without it ever firing, which is exactly what made a post
        setInterval(feedMediaSweepFailed, 7000);
        document.addEventListener('visibilitychange', () => {
          if (document.visibilityState === 'visible') feedMediaSweepFailed();
        });

        function feedMediaEnterPlaceholder(el, kind, baseUrl){
          const isVideo = kind === 'video';
          const host = isVideo ? (el.closest('.feed-video-wrap') || el) : el;
          const mediaWrap = host.closest('.feed-media');
          // Nothing in this app records a post's real media width/height at upload time, and .feed-
          // media has no reserved aspect-ratio of its own -- it's a flex box that simply sizes
          const containerWidth = (mediaWrap || host).getBoundingClientRect().width || host.getBoundingClientRect().width;
          let aspect = 4 / 5;
          const posterImg = isVideo && host.querySelector ? host.querySelector('img[src]') : null;
          if (posterImg && posterImg.naturalWidth > 0 && posterImg.naturalHeight > 0) {
            aspect = posterImg.naturalWidth / posterImg.naturalHeight;
          } else if (!isVideo && el.naturalWidth > 0 && el.naturalHeight > 0) {
            aspect = el.naturalWidth / el.naturalHeight;
          } else if (isVideo && el.videoWidth > 0 && el.videoHeight > 0) {
            aspect = el.videoWidth / el.videoHeight;
          }
          const placeholderHeight = containerWidth > 0 ? Math.round(containerWidth / aspect) : 0;
          const isAbsolute = el.classList.contains('absolute');
          // While a background retry is still in flight, show the same neutral shimmer used
          // elsewhere for "still loading" (skel- shimmer) rather than error text -- most flaky-
          const fillCls = isAbsolute
            ? 'absolute inset-0 skel-shimmer'
            : 'skel-shimmer';
          const originalClass = el.className;
          const placeholder = document.createElement('div');
          placeholder.className = fillCls;
          placeholder.dataset.originalClass = originalClass;
          if (!isAbsolute && containerWidth > 0 && placeholderHeight > 0) {
            placeholder.style.width = containerWidth + 'px';
            placeholder.style.height = placeholderHeight + 'px';
          } else if (isAbsolute && containerWidth > 0 && placeholderHeight > 0) {
            // An absolutely-positioned placeholder doesn't contribute to its own parent's height
            // (that's what "absolute" means) -- and, per the comment above, that parent has no
            const pinTarget = mediaWrap || host.parentElement || host;
            if (pinTarget) pinTarget.style.height = placeholderHeight + 'px';
          }
        ...
        function simplePostVideoHtml(url, errorTarget, posterUrl){
          ...
          // Feed videos are only fetched with preload="metadata" (to avoid burning data on posts
          // nobody scrolls to), so the <video> element has nothing decoded to paint yet and renders
          const effectivePoster = posterUrl || feedVideoFrameCache[url] || '';
        ...
        // but that also means nothing real starts downloading until playback actually kicks off
        ...
        // Grid-tile videos never autoplay (they're static previews until tapped open), so there's
        // no playback to key off of the way armFeedVideoReveal does. Instead, nudge currentTime
        function armFeedVideoThumbnail(video){
          ...
          // Safety net so the skeleton can never get stuck forever -- see the matching comment in
          // armFeedVideoReveal for why this is needed even when a posterUrl is present.
          setTimeout(reveal, 5000);
        }

        function postMediaGridTile(it, index, areaStyle, extraCount){
          // Same black-canvas issue as simplePostVideoHtml, same fix: the skeleton is always
          // rendered (even when a posterUrl exists, because that poster is still a real network
          const effectivePoster = it.posterUrl || feedVideoFrameCache[it.url] || '';
          const posterAttr = effectivePoster ? ` poster="${effectivePoster}"` : '';
        ...
        async function postInsertRemote(post){
          ...
          if (!sb) return false;
          try {
            // Use the shared getCachedAuthUser() helper (same one every other "who am I" check in the
            // app uses) instead of a fresh sb.auth.getUser() call. A bare sb.auth.getUser() here could
        ...
        // ---- Feed cache: show something instantly on a slow/first connection instead of a bare
        // skeleton, using whatever was last successfully loaded for THIS account. ----
        const FEED_CACHE_PREFIX = 'stitchFeedCache:';
        const FEED_CACHE_MAX_POSTS = 10;
        // True only while the feed on screen is the cached snapshot (not live data yet). Every
        // write action checks this and refuses to run rather than let someone
        ...
        function saveFeedCacheSnapshot(userId){
          try {
            ...
            // Only real, finished posts -- never an in-progress upload (meaningless after a reload)
            // and never anything whose media markup embeds actual file bytes as a data: URL (a fresh
        ...
        // Belt-and-suspenders sweep for sign-out on a shared/public device: even if a user id
        // somehow isn't available at the exact moment of sign-out, this guarantees no account's
        ...
        // ---- Initial post-login media preload ---- Right after signing in, the skeleton used to
        // come down the moment post *metadata* (captions, like counts, author names) had loaded --
        const FEED_INITIAL_MEDIA_PRELOAD_COUNT = 4;
        const FEED_INITIAL_MEDIA_PRELOAD_TIMEOUT_MS = 2500;
        // Only meant to gate the very first paint of a session -- reset to false alongside
        // remotePostsLoaded on every fresh login (see authEnterApp in games.js) so a new session
        let feedInitialMediaPreloadDone = false;

        function preloadOneFeedMedia(url, isVideo){
          return new Promise(resolve => {
            try {
        ...
        // Waits for the first FEED_INITIAL_MEDIA_PRELOAD_COUNT visible posts' media to load (or
        // fail -- either way, that post is done being "unknown"), bounded by a hard timeout so one
        function preloadInitialFeedMedia(){
          if (feedInitialMediaPreloadDone) return Promise.resolve();
          const posts = feedVisiblePosts().slice(0, FEED_INITIAL_MEDIA_PRELOAD_COUNT);
          if (!posts.length) { feedInitialMediaPreloadDone = true; return Promise.resolve(); }
          const jobs = [];
          posts.forEach(post => {
            if (!post.mediaHtml) return;
            const matches = post.mediaHtml.match(/<(img|video)[^>]*\ssrc="([^"]+)"/gi) || [];
            // A grid post can have several tiles -- cap per-post so one
            // multi-item post can't dominate the whole preload budget.
            matches.slice(0, 4).forEach(tag => {
              const m = tag.match(/<(img|video)[^>]*\ssrc="([^"]+)"/i);
              if (m) jobs.push(preloadOneFeedMedia(m[2], m[1].toLowerCase() === 'video'));
            });
          });
          if (!jobs.length) { feedInitialMediaPreloadDone = true; return Promise.resolve(); }
          const timeout = new Promise(resolve => setTimeout(resolve, FEED_INITIAL_MEDIA_PRELOAD_TIMEOUT_MS));
          return Promise.race([Promise.all(jobs), timeout]).then(() => { feedInitialMediaPreloadDone = true; });
        }
        ...
        // Guards against loadRemotePosts() running twice at once (e.g. the boot sequence in
        // authEnterApp() and the initial switchTab(0) both call it within moments of each other).
        let loadRemotePostsInFlight = null;
        // `prefetch`, if given, is an in-flight promise from fetchRemotePostsRaw() that was
        // already kicked off earlier (see authEnterApp in games.js) -- letting the network round-
        function loadRemotePosts(prefetch){
          if (loadRemotePostsInFlight) return loadRemotePostsInFlight;
          loadRemotePostsInFlight = loadRemotePostsImpl(prefetch).finally(() => { loadRemotePostsInFlight = null; });
          return loadRemotePostsInFlight;
        }

        // Fires the posts table request by itself, with no dependency on feedPosts/auth state, so
        // it can be started as early as possible (e.g. right alongside ensureUserStateLoaded on
        function fetchRemotePostsRaw(){
          const sb = getSupabaseClient();
          if (!sb) return Promise.resolve({ rawData: null, error: null, noClient: true });
          return sb.from(POSTS_TABLE)
            .select('id, data, created_by, created_at')
            .order('created_at', { ascending: false })
            .then(({ data, error }) => ({ rawData: data, error }))
            .catch(error => ({ rawData: null, error }));
        }
        // Collapses accidental duplicate rows in the shared posts table -- e.g. a real double-tap
        // on "Post" that got two separate inserts through (each tap uploads its media to its own
        ...
        async function loadRemotePostsImpl(prefetch){
          const sb = getSupabaseClient();
          if (!sb) { remotePostsLoaded = true; return false; }
          try {
            // See the matching note in postInsertRemote: use the shared cached-auth-user helper here
            // too, so "who am I" resolves to the exact same id every insert and every reload compare
            const me = await getCachedAuthUser();
            // Use the already-in-flight request if one was handed in (see
            // fetchRemotePostsRaw/authEnterApp) so this call doesn't wait for -- or redundantly re-
            const { rawData, error } = prefetch ? (await prefetch) : (await fetchRemotePostsRaw());
            ...
            feedPosts = feedPosts.filter(p => {
              ...
              if (p.mine) {
                ...
                // A "mine" post is normally kept unconditionally so it doesn't flicker out of the feed
                // while its own insert is still in flight. But a post older than a couple minutes that
            ...
            // A post can be pruned here (deleted elsewhere, or a ghost that never actually made it to
            // the server) while the person is sitting on some other tab entirely, or while
            ...
            // FIX: row.id comes straight off the Supabase row (often a number/bigint), while
            // knownIds is built from String(p.id). Comparing them unconverted meant this `.has()`
            // check almost never matched an already-loaded post, so loadRemotePostsImpl() re-pushed
            // the same row into feedPosts on every 30s poll (see startFeedPolling) -- which is why
            // posts (especially other people's) kept showing up twice/duplicated in the feed.
            const newRows = data.filter(row => !knownIds.has(String(row.id)) && !deletedPostIds.has(String(row.id)));
            ...
            let photoById = {};
            let nameById = {};
            // FIX: the profiles query's `error` was never checked. On a transient network hiccup (this
            // whole function is polled every 30s -- see startFeedPolling -- so it's a matter of
            let photoFetchFailed = false;
            if (authorIds.length) {
              // `name` is fetched here alongside `photo` for the exact same reason: without it, a post
              // only ever showed whatever name its author had at the moment they posted. Renaming your
            ...
            // Catch up on tags made while this device was offline/closed: the realtime handler below
            // only fires for someone who was already connected at the exact moment they were tagged,
            ...
            // Safety net: collapse any duplicate ids that slipped in (belt-and-braces alongside the
            // knownIds fix above), keeping the first occurrence of each id.
            {
        ...
        // loadRemotePostsImpl/loadPostInteractions below), not just when a post is actually new.
        function rankFeedPosts(){
          ...
          const scored = feedPosts.map(post => {
            let score = 0;
            // Admin posts always float to the top, outweighing every factor below.
            if (post.authorId === ADMIN_USER_ID) score += 1000000;
        ...
        // ---- Feed polling fallback ---- The realtime channel above is the fast path for a new
        // post showing up right away, but it isn't the only path in to feedPosts -- same idea as
        ...
        function handleRemotePostDelete(payload){
          ...
          // This can fire while Home/Profile are cached-but-off-screen (the person is on a different
          // tab, or the delete came from another device/tab) -- invalidate both cached DOM snapshots
        ...
        // ---- "Still posting" placeholder (shown until upload is fully done) ---- Mirrors the
        // real grid shapes from postMediaGridHtml so the layout never jumps once the real media
        ...
        const PostsAPI = {
          ...
          create(data){
            ...
            // A post with media stays in an explicit "uploading" state -- rendered via
            // uploadingPostMediaHtml above, shaped to match the real grid it'll become -- until the
            ...
            const background = (async () => {
              ...
              // postInsertRemote used to be fire-and-forget here: if the insert failed silently (flaky
              // network, an RLS hiccup), submitPost() still reported success and the post stuck around
            ...
            // Exposed so callers (submitPost's postSubmitInFlight guard) can wait for the real
            // upload+insert to finish before letting the person submit again -- see the note on
            ...
            return mockRequest(() => {
              ...
              postDeleteRemote(id).then(ok => {
                if (ok) {
                  ...
                  queueSaveUserState();
                  // Only clear the media out of storage once the row itself is confirmed gone -- doing this
                  // unconditionally used to mean a failed delete could still strand a restored post with its
                ...
                // The delete never actually reached the server (RLS rejection, dropped connection, etc).
                // Leaving the local state saying "gone" while the server still has the row is exactly what
                deletedPostIds.delete(String(id));
        ...
        const GlimpsesAPI = {
          ...
          create(data){
            ...
            queueSaveUserState();
            // rest.mediaUrl (when present) is only the local blob: preview created for the
            // composer -- it's shown instantly in this tab, but it's never valid for anyone
            // else, or even for this tab after a reload, so it must never be the URL we save
            // remotely. We always replace it with the real uploaded URL before persisting, and
            // if the upload or the remote save fails we drop the glimpse and tell the person,
            // instead of silently leaving a permanently broken glimpse (same pattern as
            // PostsAPI.create above).
            if (mediaFile) {
              uploadPostMediaToStorage(mediaFile, glimpse.id).then(async uploaded => {
                if (!uploaded) {
                  myGlimpses = myGlimpses.filter(g => g.id !== glimpse.id);
                  queueSaveUserState();
                  refreshMyGlimpses();
                  refreshStoryStrip();
                  if (typeof openAppAlertModal === 'function') openAppAlertModal("Your glimpse couldn't be uploaded. Please check your connection and try again.");
                  return;
                }
                ...
                refreshMyGlimpses();
                refreshStoryStrip();
                const inserted = await glimpseInsertRemote(glimpse);
                if (!inserted) {
                  myGlimpses = myGlimpses.filter(g => g.id !== glimpse.id);
                  queueSaveUserState();
                  refreshMyGlimpses();
                  refreshStoryStrip();
                  if (typeof openAppAlertModal === 'function') openAppAlertModal("Your glimpse couldn't be saved. Please check your connection and try again.");
                }
              }).catch(err => console.warn('Glimpse background sync failed:', err));
            } else {
              glimpseInsertRemote(glimpse).then(inserted => {
                if (!inserted) {
                  myGlimpses = myGlimpses.filter(g => g.id !== glimpse.id);
                  queueSaveUserState();
                  refreshMyGlimpses();
        ...
        // Shared guard for every write-action entry point below. Cached posts can already be stale
        // (liked/deleted/edited elsewhere) by the time a slow connection catches up, so
        ...
        // ---- Hashtags: turn "#word" into a tappable link to that hashtag's own feed (see
        // openHashtagFeed / postListForSource below), and let trending hashtags be computed
        ...
        function isVideoMediaHtml(mediaHtml){
          return !!mediaHtml && /feed-video-wrap/.test(mediaHtml) && !/post-media-grid/.test(mediaHtml);
        }

        function isPostAuthorStranger(post){
          return !post.mine && !!post.authorId && !isUserInMyNetwork(post.authorId);
        }

        // A stranger's video only gets the "Suggested for you" treatment (and only shows up in the
        // feed at all) when it lines up with this account's own search/activity signals -- see
        function shouldSuggestStrangerVideo(post){
          return typeof postMatchesContentInterest === 'function' ? postMatchesContentInterest(post) : true;
        }

        // Video posts used to render this header as a "floating" overlay -- absolutely positioned
        // on top of the video with a dark top-to-transparent gradient behind it so white text
        function feedPostHeaderHtml(post, isStranger, showConnect, showRequestSent){
          const nameCls = 'font-semibold text-[13px] truncate';
          const timeCls = 'text-[11px] text-gray-500 truncate';
          const dotsCls = 'w-7 h-7 flex items-center justify-center text-gray-400 flex-shrink-0';
          const connectCls = 'text-[11px] font-semibold px-3 py-1 rounded-full flex-shrink-0';
          const connectStyle = `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};`;
          const rowCls = 'px-4 py-2 flex gap-2.5 items-center relative';
          return `
              <div id="post-header-${post.id}" class="${rowCls}">
                <div class="w-7 h-7 rounded-full ${post.avatarBg} overflow-hidden flex items-center justify-center flex-shrink-0 cursor-pointer" onclick="event.stopPropagation(); openPersonProfileForPost(${post.id})">${(() => {
                  // FIX: this used to be a bare <img src="..."> with no onerror fallback, so a
                  // profile photo that fails to load (e.g. no internet, or the image host is
                  // unreachable) fell back to the browser's own broken-image icon instead of the
                  // app's placeholder avatar. Mirrors the onerror/sibling-fallback pattern already
                  // used elsewhere (see storyBubble / profile.js) so it degrades the same way.
                  const photo = post.mine ? profileData.photo : (!post.mine ? post.photo : null);
                  if (!photo) return Icon(post.avatarIcon,'w-3.5 h-3.5 text-gray-600');
                  return `<img src="${photo}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';"><div class="w-full h-full items-center justify-center" style="display:none;">${Icon(post.avatarIcon,'w-3.5 h-3.5 text-gray-600')}</div>`;
                })()}</div>
                <div class="flex-1 min-w-0 cursor-pointer" onclick="event.stopPropagation(); openPersonProfileForPost(${post.id})">
                  ...
                  <div class="${timeCls}">${formatPostTimeAgo(post)}</div>
                </div>
                ${showConnect ? `
                  <button onclick="event.stopPropagation(); connectWithPoster(${post.id})" class="${connectCls}" style="${connectStyle}">Connect</button>
                ` : ''}
                ${showRequestSent ? `
                  <button onclick="event.stopPropagation(); cancelConnectWithPoster(${post.id})" class="text-[11px] font-semibold px-3 py-1 rounded-full flex-shrink-0 bg-gray-100 text-gray-600">Cancel Request</button>
                ` : ''}
        ...
        function feedPost(post){
          const isStranger = isPostAuthorStranger(post);
          const showConnect = isStranger && !post.connectRequestSent;
          const showRequestSent = isStranger && post.connectRequestSent;
          // NOTE: multi-media grids use the "post-media-grid" class (see postMediaGridHtml), not
          // "post-media-carousel". This was checking for a class name that's never actually
          const isVideoPost = isVideoMediaHtml(post.mediaHtml);
          ...
          return `
            <div class="bg-white overflow-hidden -mx-5 border-b border-gray-100" id="post-${post.id}">
              ...
              ${feedPostHeaderHtml(post, isStranger, showConnect, showRequestSent)}

              <div class="feed-media relative" ${post.uploading ? '' : `onclick="handleFeedMediaTap(event, ${post.id})"`} style="cursor:${post.uploading ? 'default' : 'pointer'};">${mediaAreaHtml}</div>
        ...
        function framedMediaLayerHtml(media, extraImgAttrs, anchorTop){
          if (!media) return '';
          // anchorTop pins object-position to the top instead of the default center. Used when this
          // media sits behind the comments sheet, which covers the bottom ~58-92% of the screen --
        ...
        // Left/right arrow keys move between posts (reel view) or between images within the same
        // post (gallery view), mirroring the horizontal swipe gestures above -- desktop users get
        ...
        function postListForSource(source){
          ...
          if (typeof source === 'string' && source.indexOf('single:') === 0) {
            const id = source.slice('single:'.length);
            const p = feedPosts.find(pp => String(pp.id) === id);
            return p ? [p] : [];
          }
        ...
        function postFeedTitle(source){
          ...
          if (typeof source === 'string' && source.indexOf('single:') === 0) return 'Post';
          if (typeof source === 'string' && source.indexOf('hashtag:') === 0) {
        ...
        function toggleLike(id){
          ...
          PostsAPI.toggleLike(id).then(post => {
            if (!post) return;
            // Liking something is a strong activity signal for which
            // strangers' videos are worth surfacing as "Suggested for you".
            if (post.liked) {
              if (post.authorId && typeof recordContentInterestAuthor === 'function') recordContentInterestAuthor(post.authorId);
              if (typeof recordContentInterestTerms === 'function' && typeof extractHashtags === 'function') {
                extractHashtags(post.body).forEach(tag => recordContentInterestTerms(tag));
              }
            }
        ...
        function openComments(id){
          ...
          if (typeof pauseAllOverlayMedia === 'function') pauseAllOverlayMedia();
          const ov = document.getElementById('overlay');
          // Same "remember what was underneath" pattern as the Share sheet (see
          // openShare/closeShareSheet below): lets closing this sheet return to whatever overlay --
        ...
        // Comments now open as a draggable bottom sheet sitting over the post's own media (mirrors
        // the reel/gallery full-screen viewer) instead of navigating to a separate "Comments"
        ...
        function openShare(id){
          ...
          if (typeof pauseAllOverlayMedia === 'function') pauseAllOverlayMedia();
          const ov = document.getElementById('overlay');
          // If another overlay -- e.g. a single post's expanded "postFeed" view -- is already open
          // underneath, remember its markup so closing this sheet (X button or hardware/browser
          shareSheetReturnHTML = (!ov.classList.contains('hidden')) ? ov.innerHTML : null;
          ov.classList.remove('hidden');
          ov.style.top = OVERLAY_TOP;
          ov.style.paddingTop = '';
          ov.style.bottom = '0';
          // Share now presents as a bottom sheet (like LinkedIn's "Send as message") instead of a
          // full page: #overlay becomes a dim backdrop that the sheet itself sits at the bottom of,
        ...
        function closeShareSheet(fromPopState){
          ...
          // Undo the dim-backdrop background from openShare so whatever shows next (a returned-to
          // overlay, or nothing) isn't left looking tinted.
          ov.style.background = '';
          if (returnHTML !== null) {
            if (typeof pauseAllOverlayMedia === 'function') pauseAllOverlayMedia();
            ov.innerHTML = returnHTML;
        ...
        // Wraps shareHTML's content in a bottom-anchored sheet: a drag handle, rounded top
        // corners, capped height so it never covers the whole screen, and a tap-outside-to-close
        ...
        function shareHTML(post){
          ...
          return `
            ...
            <div class="flex-shrink-0 w-full border-t border-gray-100" style="background:#fafafa;">
              <div class="share-sheet-actions-row flex items-start no-scrollbar" style="gap:22px;padding:16px 20px calc(env(safe-area-inset-bottom, 12px) + 16px) 20px;overflow-x:auto;-webkit-overflow-scrolling:touch;">
                ...
                ${shareGoogleDriveOption(`shareViaDrive(${post.id})`)}
                ${shareOutlookOption(`shareViaOutlook(${post.id})`)}
                ${shareGmailOption(`shareViaGmail(${post.id})`)}
                ${shareBluetoothOption(`shareViaBluetooth(${post.id})`)}
              </div>
            </div>`;
        }
        ...
        // Renders the trailing action button in the share sheet: "More apps" (opens the OS share
        // sheet) when nothing's selected, or "Send" (delivers directly to the picked contacts,
        ...
        // Brand logos (WhatsApp / Messages) shown alongside the generic "Share" (opens the OS
        // share sheet) and "Copy link" options, matching a native share panel's row of quick-share
...
const GOOGLE_DRIVE_LOGO_URI = "data:image/png;base64,";
        const OUTLOOK_LOGO_URI = "data:image/png;base64,";
        const GMAIL_LOGO_URI = "data:image/png;base64,";
        const BLUETOOTH_LOGO_URI = "data:image/png;base64,";
        ...
        function shareGoogleDriveOption(onclick){
          return shareBrandOption('Drive', onclick, GOOGLE_DRIVE_LOGO_URI);
        }

        function shareOutlookOption(onclick){
          return shareBrandOption('Outlook', onclick, OUTLOOK_LOGO_URI);
        }

        function shareGmailOption(onclick){
          return shareBrandOption('Gmail', onclick, GMAIL_LOGO_URI);
        }

        function shareBluetoothOption(onclick){
          return shareBrandOption('Bluetooth', onclick, BLUETOOTH_LOGO_URI);
        }
        ...
        // Pulls a thumbnail straight out of a post's already-rendered media HTML (same lightweight
        // regex approach postMediaKind above uses) instead of re-deriving it from raw upload data,
        function postShareThumbnailUrl(post){
          if (!post || !post.mediaHtml) return null;
          const tagPattern = /<(img|video)\b[^>]*>/gi;
          let match;
          while ((match = tagPattern.exec(post.mediaHtml))) {
            const tag = match[0];
            if (/^<img/i.test(tag)) {
              const src = tag.match(/\bsrc="([^"]+)"/i);
              if (src) return src[1];
            } else {
              // Video: only its poster frame counts as a thumbnail. A video with no poster yet (e.g. an
              // older post from before poster generation existed) is skipped rather than falling back to
              const poster = tag.match(/\bposter="([^"]+)"/i);
        ...
        // Shape of a "shared post" chat attachment. This rides in the same `attachments` JSON
        // column messages already use for photos/files -- no backend schema change needed -- so it
        function buildPostShareAttachment(post){
          return {
            type: 'stitch/post',
            postId: post.id,
            authorName: post.name || 'Stitch member',
            caption: (post.body || '').trim(),
            thumbnail: postShareThumbnailUrl(post),
          };
        }
        ...
        function sendPostToSelectedContacts(postId){
          ...
          const post = findPost(postId);
          if (!post) { closeShareSheet(); return; }
          const caption = (post.body || '').trim();
          // The caption now travels inside the shared-post card itself, so the plain-text bubble
          // only needs to exist as an inbox preview / fallback for clients that don't render the
          const previewText = caption ? `Shared a post: "${caption.slice(0,60)}"` : 'Shared a post';
          const attachment = buildPostShareAttachment(post);
          const ids = Array.from(shareSelected);
          ids.forEach(contactId => deliverSharedMessage(contactId, '', { attachments: [attachment], previewText }));
          PostsAPI.share(postId).then(() => bumpShareCountDisplay(postId));
          closeShareSheet();
          openAppAlertModal(ids.length === 1 ? 'Sent' : `Sent to ${ids.length} people`);
        }

        function deliverSharedMessage(convoId, text, opts){
          opts = opts || {};
          const attachments = opts.attachments;
          const previewText = opts.previewText != null ? opts.previewText : text;
        ...
        // Tapping a shared-post card in a chat -- opens straight to that post using the same
        // single-post "post feed" overlay the rest of the app already uses for deep views, so the
        function openSharedPostFromChat(postId){
          if (postId == null) return;
          const post = findPost(postId);
          if (!post) { openAppAlertModal("This post isn't available anymore."); return; }
          openPostFeedFrom('single:' + postId, postId);
        }
        ...
        // ---- Real link-sharing for posts: build a permalink-style URL, then either copy it or
        // hand off to the OS share sheet via the Web Share API (same pattern used for class
        ...
        function sharePostExternally(id){
          const post = findPost(id);
          const link = postShareLink(id);
          // Closing our own sheet used to happen before navigator.share() was even called, so there
          // was a brief flash of the bare feed underneath before the OS share sheet slid up on top
          if (navigator.share) {
        ...
        function shareViaGmail(id){
          const post = findPost(id);
          const link = postShareLink(id);
          const authorName = (post && post.name) ? post.name : 'this post';
          const body = `Check out ${authorName}'s post on Stitch: ${link}`;
          PostsAPI.share(id).then(() => bumpShareCountDisplay(id));
          window.open(`https://mail.google.com/mail/?view=cm&fs=1&su=${encodeURIComponent('Check this out on Stitch')}&body=${encodeURIComponent(body)}`, '_blank');
          closeShareSheet();
        }

        function shareViaOutlook(id){
          const post = findPost(id);
          const link = postShareLink(id);
          const authorName = (post && post.name) ? post.name : 'this post';
          const body = `Check out ${authorName}'s post on Stitch: ${link}`;
          PostsAPI.share(id).then(() => bumpShareCountDisplay(id));
          window.open(`https://outlook.office.com/mail/deeplink/compose?subject=${encodeURIComponent('Check this out on Stitch')}&body=${encodeURIComponent(body)}`, '_blank');
          closeShareSheet();
        }

        // Google Drive has no web share-target for a plain link the way WhatsApp/SMS/Gmail do, so
        // the closest useful action is copying the link and pointing the person at Drive to paste
        function shareViaDrive(id){
          const link = postShareLink(id);
          PostsAPI.share(id).then(() => bumpShareCountDisplay(id));
          const afterCopy = () => { window.open('https://drive.google.com/drive/my-drive', '_blank'); closeShareSheet(); };
          if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(link).then(() => { openAppAlertModal('Link copied -- paste it in Drive'); afterCopy(); });
          } else {
            openAppAlertModal('Link: ' + link);
            afterCopy();
          }
        ...
        // Browsers can't push text/files over Bluetooth directly (no web API for that), so
        // Bluetooth routes through the OS share sheet, same as the "Share" icon -- Bluetooth is
        function shareViaBluetooth(id){
          sharePostExternally(id);
        }
          ...
          return `
            ...
            <div class="flex-shrink-0 w-full border-t border-gray-100" style="background:#fafafa;">
              <div class="share-sheet-actions-row flex items-start no-scrollbar" style="gap:22px;padding:16px 20px calc(env(safe-area-inset-bottom, 12px) + 16px) 20px;overflow-x:auto;-webkit-overflow-scrolling:touch;">
                ...
                ${shareGoogleDriveOption(`shareGlimpseViaDrive(${g.id})`)}
        ...
        function shareGlimpseViaGmail(id){
          const link = glimpseShareLink(id);
          const body = `Check out this Glimpse on Stitch: ${link}`;
          window.open(`https://mail.google.com/mail/?view=cm&fs=1&su=${encodeURIComponent('Check this out on Stitch')}&body=${encodeURIComponent(body)}`, '_blank');
          closeOverlay();
        }

        function shareGlimpseViaOutlook(id){
          const link = glimpseShareLink(id);
          const body = `Check out this Glimpse on Stitch: ${link}`;
          window.open(`https://outlook.office.com/mail/deeplink/compose?subject=${encodeURIComponent('Check this out on Stitch')}&body=${encodeURIComponent(body)}`, '_blank');
          closeOverlay();
        ...
        function shareGlimpseViaDrive(id){
          const link = glimpseShareLink(id);
          const afterCopy = () => { window.open('https://drive.google.com/drive/my-drive', '_blank'); closeOverlay(); };
          if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(link).then(() => { openAppAlertModal('Link copied -- paste it in Drive'); afterCopy(); });
          } else {
            openAppAlertModal('Link: ' + link);
        ...
        function shareGlimpseViaBluetooth(id){
          shareGlimpseExternally(id);
        }
