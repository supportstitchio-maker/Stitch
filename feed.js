        ...
        function viewStory(id){
          ...
          s.viewed = true;
          if (!s.mine) persistGlimpseViewed(s.id);
        ...
        function goToAdjacentStory(dir){
          ...
          if (!nextStory.mine) persistGlimpseViewed(nextStory.id);
          reorderStoriesByViewed();
          const bubble = document.getElementById('story-strip');
          if (bubble) bubble.innerHTML = stories.map(storyBubble).join('');
        ...
        async function loadRemoteGlimpses(){
          ...
          try {
            ...
            Object.keys(persistedViewed).forEach(id => {
              if (!previouslyViewed[id]) previouslyViewed[id] = true;
            });
        ...
        // Returns { [authorUserId]: viewedAtMs }, dropping any entries
        // older than a glimpse's 24-hour lifetime (that author's glimpse
        // from back then is gone anyway, and a new one from them should
        // show as unviewed like normal).
        function loadPersistedViewedGlimpses(){
          try {
            ...
            Object.keys(map).forEach(id => {
              if (!(map[id] > cutoff)) { delete map[id]; changed = true; }
            });
        ...
        // Marks one author's glimpse(s) as viewed for next login. Called
        // right alongside the existing in-memory `s.viewed = true` (see
        // viewStory and goToAdjacentStory).
        function persistGlimpseViewed(authorId){
          try {
            const key = glimpsesViewedStorageKey();
            if (!key) return;
          ...
          return `
            <div class="flex-shrink-0 w-full" style="padding-top:20px;">
              <div class="max-w-2xl mx-auto px-5 pb-3 flex items-center justify-between">
                <div class="font-semibold text-lg font-display" style="color:${NAVY};">Share</div>
        ...
        function shareContactCell(postId, c){
          return `
            <button id="share-cell-${postId}-${c.id}" data-name="${escapeHtml(c.name.toLowerCase())}" onclick="sendPostToContact(${postId}, '${c.id}')" class="w-full flex items-center gap-3 px-2 py-2.5 rounded-xl text-left">
              ...
              <div id="share-check-${postId}-${c.id}" class="hidden flex-shrink-0" style="color:${ROYAL};">${Icon('check','w-4 h-4')}</div>
              <div id="share-label-${postId}-${c.id}" class="text-xs font-medium flex-shrink-0" style="color:${ROYAL};"></div>
            </button>`;
        }
          ...
          return `
            <div class="flex-shrink-0 w-full" style="padding-top:20px;">
              <div class="max-w-2xl mx-auto px-5 pb-3 flex items-center justify-between">
                ...
                <button onclick="openMyGlimpses()">${Icon('close','w-5 h-5 text-gray-500')}</button>
              </div>
              <div class="max-w-2xl mx-auto px-5 pb-4">
                <div class="relative">
                  <div class="text-gray-400" style="position:absolute;left:14px;top:50%;transform:translateY(-50%);">${Icon('search','w-4 h-4')}</div>
        ...
        function shareGlimpseContactCell(glimpseId, c){
          return `
            <button id="share-glimpse-cell-${glimpseId}-${c.id}" data-name="${escapeHtml(c.name.toLowerCase())}" onclick="sendGlimpseToContact(${glimpseId}, '${c.id}')" class="w-full flex items-center gap-3 px-2 py-2.5 rounded-xl text-left">
              <div class="w-9 h-9 ${c.avatarBg} rounded-full flex items-center justify-center text-gray-600 overflow-hidden flex-shrink-0">${avatarInnerHTML(c,'w-4 h-4')}</div>
              <div class="flex-1 min-w-0">
                <div class="text-sm font-semibold truncate">${escapeHtml(c.name)}</div>
              </div>
              <div id="share-glimpse-check-${glimpseId}-${c.id}" class="hidden flex-shrink-0" style="color:${ROYAL};">${Icon('check','w-4 h-4')}</div>
              <div id="share-glimpse-label-${glimpseId}-${c.id}" class="text-xs font-medium flex-shrink-0" style="color:${ROYAL};"></div>
            </button>`;
        }
        ...
        function sendGlimpseToContact(glimpseId, contactId){
          const g = myGlimpses.find(x => x.id === glimpseId);
          const check = document.getElementById(`share-glimpse-check-${glimpseId}-${contactId}`);
          const label = document.getElementById(`share-glimpse-label-${glimpseId}-${contactId}`);
          if (check && !check.classList.contains('hidden')) return; // already sent
          const convo = findConvoById(contactId);
          if (convo && g) {
            conversationMessages[convo.id] = conversationMessages[convo.id] || [];
            const desc = g.caption ? `"${g.caption.slice(0,60)}"` : (g.mediaType === 'video' ? 'a video' : g.mediaType === 'image' ? 'a photo' : 'a glimpse');
            conversationMessages[convo.id].push({ from: 'me', text: `Shared a glimpse: ${desc}` });
            convo.preview = 'You shared a glimpse';
            convo.time = 'now';
            convo.unread = false;
            bumpConvoToTop(convo);
          }
