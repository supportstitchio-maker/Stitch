            ...
            <div class="flex flex-col h-full bg-gradient-to-b from-[${NAVY}] to-[${ROYAL}] text-white" style="padding-top:20px;">
              <div class="flex items-center justify-between px-4 flex-shrink-0">
                <div class="flex items-center gap-2">
                  <div class="w-8 h-8 rounded-full border-2 border-white/60 flex items-center justify-center" style="background:transparent;">${silhouetteIcon(icon, 'w-4 h-4 ' + (iconClass||''))}</div>
        ...
        // below. Likes/reposts/saves/comments still only update the local
        // copy of the post (not the shared row), matching how the rest of
        // the app works today -- making those live too would need
        // per-user like/comment tables of their own, a separate change.
        //
        ...
        async function loadRemotePosts(){
          ...
          try {
            ...
            data.forEach(row => {
              ...
              feedPosts.push({ ...row.data, id: row.data.id ?? id, mine: isMine, connected: isMine ? true : !!row.data.connected });
              knownIds.add(id);
            });
        ...
        const NetworkAPI = {
          ...
          block(name, icon, avatarBg){
            return mockRequest(() => {
              if (!blockedAccounts.some(b => b.name === name)) {
                blockedAccounts.push({ name, icon: icon || 'user', avatarBg: avatarBg || 'bg-gray-200' });
              }
              queueSaveUserState();
        ...
        function blockAccount(name, icon, avatarBg){
          if (!name) return;
          NetworkAPI.block(name, icon, avatarBg);
        ...
        function feedPost(post){
          const showConnect = !post.mine && post.connected === false;
          return `
            <div class="bg-white overflow-hidden -mx-5 border-b border-gray-100" id="post-${post.id}">
              <div class="px-4 py-2 flex gap-2.5 items-center relative">
                <div class="w-7 h-7 rounded-full flex-shrink-0 p-[2px] ${showConnect ? 'bg-gradient-to-tr from-pink-500 via-red-500 to-yellow-400' : 'bg-gray-200'}">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-[11px] text-gray-500 truncate">${showConnect ? 'Suggested for you' : (post.timeAgo || post.meta)}</div>
                </div>
                ${showConnect ? `
