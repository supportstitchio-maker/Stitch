          ...
          return `
            <div class="flex-shrink-0 w-full" style="padding-top:20px;">
              <div class="max-w-2xl mx-auto px-5 pb-3 flex items-center justify-between">
                ...
                <button onclick="closeOverlay()">${IconBold('close','w-5 h-5 text-[' + ROYAL + ']')}</button>
              </div>
            ...
            <div class="flex-shrink-0 w-full flex justify-center" style="padding-top:14px;padding-bottom:calc(env(safe-area-inset-bottom, 12px) + 16px);">
              <div class="flex items-start gap-6 rounded-3xl py-3 px-5 shadow-md" style="background:#f2f2f2;">
        ...
        function shareContactCell(postId, c){
          ...
          return `
            <button id="share-cell-${postId}-${c.id}" data-name="${escapeHtml(c.name.toLowerCase())}" onclick="toggleShareContact(${postId}, '${c.id}')" class="w-full flex items-center gap-3 px-2 py-2.5 rounded-xl text-left">
              ...
              <div id="share-check-${postId}-${c.id}" class="w-5 h-5 rounded-full border-2 ${selected ? `bg-gradient-to-br from-[${ROYAL}] to-[${NAVY}] border-[${ROYAL}]` : 'border-gray-300'} flex items-center justify-center flex-shrink-0">${selected ? Icon('check','w-3.5 h-3.5 text-white') : ''}</div>
            </button>`;
        }
        ...
        function toggleShareContact(postId, contactId){
          ...
          if (box) {
            ...
            box.className = `w-5 h-5 rounded-full border-2 ${selected ? `bg-gradient-to-br from-[${ROYAL}] to-[${NAVY}] border-[${ROYAL}]` : 'border-gray-300'} flex items-center justify-center flex-shrink-0`;
          ...
          return `
            <div class="flex-shrink-0 w-full" style="padding-top:20px;">
              <div class="max-w-2xl mx-auto px-5 pb-3 flex items-center justify-between">
                ...
                <button onclick="openMyGlimpses()">${IconBold('close','w-5 h-5 text-[' + ROYAL + ']')}</button>
              </div>
              <div class="max-w-2xl mx-auto px-5 pb-4">
                <div class="relative">
                  <div class="text-gray-400" style="position:absolute;left:14px;top:50%;transform:translateY(-50%);">${Icon('search','w-4 h-4')}</div>
        ...
        function shareGlimpseContactCell(glimpseId, c){
          ...
          return `
            <button id="share-glimpse-cell-${glimpseId}-${c.id}" data-name="${escapeHtml(c.name.toLowerCase())}" onclick="toggleShareGlimpseContact(${glimpseId}, '${c.id}')" class="w-full flex items-center gap-3 px-2 py-2.5 rounded-xl text-left">
              ...
              <div id="share-glimpse-check-${glimpseId}-${c.id}" class="w-5 h-5 rounded-full border-2 ${selected ? `bg-gradient-to-br from-[${ROYAL}] to-[${NAVY}] border-[${ROYAL}]` : 'border-gray-300'} flex items-center justify-center flex-shrink-0">${selected ? Icon('check','w-3.5 h-3.5 text-white') : ''}</div>
            </button>`;
        }
