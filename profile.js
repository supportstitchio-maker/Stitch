        ...
        // Full rebuild vs. in-place patch: see patchProfileHeaderInPlace
        // below for why this only does the full innerHTML rebuild once.
        function renderProfile(){
          const screenEl = document.getElementById('screen');
          // The Profile screen is already showing (its grid is in the
          // DOM) -- patch just the header fields that could have changed
          // instead of rebuilding everything. A full rebuild recreates
          // the Posts/Reposts/Saved grid from scratch, which reloads
          // every video thumbnail in it -- that's what made the grid
          // visibly flash/blink on every realtime update (a connection
          // accepted, network count changing, etc.) while Profile was
          // open, even though nothing in the grid itself had changed.
        ...
        // Patches every header field renderProfile()'s full markup below
        // sets, without touching #profile-tabs or #profile-tab-content --
        // keeping the Posts/Reposts/Saved grid (and any video thumbnails
        // in it) untouched. Falls back to a full renderProfile() rebuild
        // if any expected element is missing, so this never silently
        ...
        function patchProfileHeaderInPlace(){
          ...
          usernameEl.innerHTML = `${escapeHtml(profileData.username)}${appPrefs.accountPrivacy === 'Private' ? `<span title="Private account" class="text-gray-400">${Icon('lock','w-3.5 h-3.5')}</span>` : ''}`;
        ...
        // The profile photo (or fallback placeholder) button, its own
        // function since it needs to be regenerated wholesale both by the
        // full profileScreenHTML() markup and by patchProfileHeaderInPlace's
        // outerHTML swap above.
        ...
        function profileScreenHTML(){
          return `
            <div class="px-5 pb-3 flex items-center justify-between" style="padding-top:20px;">
              ...
              <span id="profile-username-el" class="font-bold text-base font-display grad-text flex items-center gap-1.5">${escapeHtml(profileData.username)}${appPrefs.accountPrivacy === 'Private' ? `<span title="Private account" class="text-gray-400">${Icon('lock','w-3.5 h-3.5')}</span>` : ''}</span>
        ...
        // A small photo/video glyph sits bottom-right of every media tile
        // (mirroring the top-right stacked-photos badge below) so the two
        // types are distinguishable at a glance, the way Instagram grids
        // mark video posts.
        ...
        function profileGridItem(post, source){
          // A multi-photo/video post's mediaHtml is a swipeable carousel
          // (see postMediaCarouselHtml/postMediaHtmlFromUploaded in
          // overlays.js/feed.js) -- inside a tiny square grid tile that
          // would just mean an unwanted extra scroll gesture, so the tile
          // only ever shows the first pane, with a small stacked-photos
          // badge (Instagram-style) hinting there's more to swipe through
          // once the tile is opened.
          ...
          if (isMulti) {
            ...
            // A single video post's mediaHtml is the full interactive
            // feed player (simplePostVideoHtml in feed.js) -- tap-to-
            // play/pause, a mute button, its own onclick handler. Reusing
            // that as-is inside a grid tile meant tapping the thumbnail
            // both started the video playing in place *and* (since the
            // click bubbles up) opened the full post, instead of just
            // opening it. The grid only ever needs a static, silent
            // first-frame preview, so a plain non-interactive <video> is
            // built from the same source URL instead -- all taps fall
            // through to the tile's own onclick below.
          ...
          return `<div onclick="openPostFeedFrom('${source||'mine'}', ${post.id})" class="relative w-full aspect-square overflow-hidden" style="border-radius:0;cursor:pointer;">${inner}${isMulti ? `<div class="absolute top-1.5 right-1.5 text-white" style="filter:drop-shadow(0 1px 2px rgba(0,0,0,0.5));">${Icon('copy','w-4 h-4')}</div>` : ''}${typeBadge}</div>`;
        }

        // feedPosts itself gets re-sorted by rankFeedPosts() (see feed.js)
        // every time the Home feed loads/refreshes -- that ranking mixes in
        // a little randomness on purpose so the feed doesn't look frozen,
        // but the Profile grids were just rendering feedPosts.filter(...)
        // directly, so a Home refresh would silently reshuffle a person's
        // own post grid too. Sorting by `id` (each post's Date.now()
        // creation timestamp -- see PostsAPI.create) newest-first here
        // keeps Profile strictly chronological regardless of whatever
        // order feedPosts happens to be in underneath.
        ...
        // Bio uses a textarea that grows taller as you type so the full
        // text stays visible instead of scrolling inside a fixed box, but
        // now sits inside the same rounded/bordered pill as every other
        // field on this form (name, username, etc.) for a consistent look.
        ...
        // Cleans up any leftover scroll listeners from a previous open of
        // the gender/pronouns dropdowns before (re)arming them. Without
        // this, a listener registered while a menu was open stuck around
        // even after the menu was closed by tapping an option or the
        // backdrop (attachMenuScrollCloser's own listener only ever
        // removes itself when a scroll actually fires it) -- so scrolling
        // the sheet later for any unrelated reason could fire that stale
        // listener and flip the menu open again on its own, with no tap
        // involved, and the same leak could stack up multiple listeners
        // the more times the menu was opened and closed.
        ...
        // Snapshot of profileData taken the moment Edit Profile opens, and
        // a flag for a pending (not-yet-saved) profile picture deletion.
        // Every field in the form -- including the photo -- is staged
        // straight into profileData for live preview purposes as the
        // person edits, but none of it is persisted (queueSaveUserState/
        // syncPublicProfile) or shown anywhere else in the app until
        // "Save Changes" is actually tapped (saveEditProfile). If they
        // instead back out via Cancel, the back arrow, or the hardware
        // back button, discardEditProfileChanges() below rolls profileData
        // back to this snapshot so nothing staged -- a typed name, a
        // picked photo, a preset avatar -- ever silently takes effect.
        ...
        // Leaving the editor WITHOUT saving -- Cancel, the back arrow, or
        // the hardware back button all route here. Rolls every staged
        // field (name, username, bio, photo, pronouns, gender, links)
        // back to how it was before this edit session started, then
        // closes the modal. Nothing staged-but-unsaved was ever persisted
        // or broadcast in the first place, so restoring profileData here
        // is enough -- there's nothing to revert on the server.
        ...
        function confirmPhotoCrop(){
          ...
          // Staged only -- this becomes the real profile picture just
          // like a typed name or username does: it previews right away
          // inside the Edit Profile form (via profileData.photo, which
          // that form reads from), but nothing is persisted or shown
          // anywhere else in the app -- nav icon, feed, other people's
          // view of you -- until "Save Changes" is actually tapped
          // (saveEditProfile). Backing out instead (Cancel/back arrow)
          // rolls this back via discardEditProfileChanges().
          const previewDataUrl = canvas.toDataURL('image/jpeg', 0.9);
          profileData.photo = previewDataUrl;
          closePhotoCropModal();
          renderEditProfileForm();
          // Upload the real bytes in the background now so a small
          // Storage URL is already available by the time Save Changes
          // commits this -- previously the base64 dataURL itself
          // (80-150KB of text) was what got kept in profileData.photo
          // permanently and pushed to Supabase as-is, which could
          // silently fail to save and left other people seeing an old
          // photo. Still just staged locally until Save; if the person
          // saves before this resolves, the swap-in below finishes the
          // job (and syncs) itself.
          canvas.toBlob(blob => {
            if (!blob) return;
            uploadProfilePhotoToStorage(blob).then(remoteUrl => {
              ...
              if (stillStaging) {
                ...
                // Save Changes already committed this photo before the
                // upload finished; push the real Storage URL out now.
        ...
        function deleteProfilePicture(){
          ...
          openAppConfirmModal('Delete your profile picture?', '', 'Delete', function(){
            // Staged only, same as everything else in this form -- the
            // photo disappears from the Edit Profile preview right away,
            // but it isn't actually removed anywhere else, and the file
            // isn't deleted from Storage, until Save Changes is tapped.
        ...
        function selectEditProfileAvatar(src){
          // Staged like every other field on this form -- previews in
          // Edit Profile immediately, but only becomes real (persisted,
          // and visible elsewhere in the app) once Save Changes is
          // tapped; see confirmPhotoCrop's comment above for why.
        ...
        // Shared by both the username and full-name inputs: keeps the red
        // border + message under a field in sync with whatever error
        // string is currently set for it, without needing a full
        // re-render on every keystroke (the message div is created/
        // removed/updated in place).
        ...
        // "Stitch" is reserved and, as someone types it into either the
        // username or full name field, the field should flag it live --
        // not just at save time -- so the person sees right away that the
        // name is unavailable rather than only finding out after tapping
        // Save.
        ...
        function onEditUsernameInput(){
          ...
          // Not just flagged red anymore -- the instant the field's
          // content IS "Stitch" (in any casing/spacing/punctuation
          // variant), it's wiped right back out. That means "Stitch"
          // can never actually sit in the username field waiting to be
          // saved; the person has to type something else. The message
          // still shows briefly so it's clear why the field just emptied.
        ...
        // Same live-reserved-word check as onEditUsernameInput above --
        // "Stitch" is wiped out of the full name field the instant it's
        // typed, rather than just being flagged and left sitting there --
        // plus clears any stale "that name isn't available" (taken) error
        // as the person edits it, so the red border/message doesn't just
        // sit there stale once they've actually started fixing it.
        ...
        async function saveEditProfile(){
          ...
          // This is the moment everything staged in this edit session --
          // name, username, bio, and any photo change -- actually takes
          // effect: persisted locally, synced to other people's view of
          // this account, and cleared off the "revert on cancel" snapshot.
          if (editProfilePendingPhotoDelete) {
            // Only now does the old file actually get removed from
            // Storage -- if the person had backed out instead, nothing
            // about the old photo would need to be touched.
        ...
        function autoSaveEditProfileDraft(){
          ...
          // Same commit as tapping "Save Changes" -- any staged photo
          // change (upload, delete, preset pick) takes effect here too,
          // since profileData.photo already holds whatever was staged.
          if (editProfilePendingPhotoDelete) {
            deleteProfilePhotoFromStorage();
            editProfilePendingPhotoDelete = false;
          }
