        ...
        function profileGridItem(post, source){
          ...
          if (isMulti) {
            ...
            inner = firstTag ? firstTag[0] : post.mediaHtml;
          } else {
            inner = post.mediaHtml || `<div class="w-full h-full bg-gray-100 flex items-center justify-center p-2"><div class="text-[9px] leading-tight text-gray-500 text-center overflow-hidden" style="display:-webkit-box;-webkit-line-clamp:4;-webkit-box-orient:vertical;">${post.body||''}</div></div>`;
          }
          return `<div onclick="openPostFeedFrom('${source||'mine'}', ${post.id})" class="relative w-full aspect-square overflow-hidden" style="border-radius:0;cursor:pointer;">${inner}${isMulti ? `<div class="absolute top-1.5 right-1.5 text-white" style="filter:drop-shadow(0 1px 2px rgba(0,0,0,0.5));">${Icon('copy','w-4 h-4')}</div>` : ''}</div>`;
        }
          ...
          return `
            <div class="flex-1 flex flex-col overflow-y-auto" style="background:#ffffff;">
              ...
              <div class="flex-1 flex flex-col items-center px-8" style="padding-top:48px;">
                ...
                <div class="flex flex-col items-center" style="margin-bottom:50px;">
                  <div class="font-bold text-lg font-display mb-3 tracking-wide text-[${NAVY}]">@${escapeHtml(profileData.username.toUpperCase())}</div>
                  <div class="flex items-start justify-between rounded-3xl pt-2.5 pb-3 px-4 bg-white shadow-xl" style="width:215px;">
