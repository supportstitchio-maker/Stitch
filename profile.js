        ...
        // Best-effort: mirrors this account's public-facing fields so it can actually show up in other people's Discover list.
        ...
        let profileData = {
          ...
          photo: null,
          link: '',
          // Social/other-page links shown as tappable pills under the bio, both on this account's own Profile tab and on the read-only personProfileHTML card...
          links: []
        };

        // A name and username are required before someone can post, comment, apply, or otherwise create anything -- including when the account came from...
        ...
        // "Switch account" is a real, lightweight multi-account switcher: each account keeps its own identity (name/username/photo/bio), the rest of the app's...
        const PRONOUN_OPTIONS = ['He/Him', 'She/Her', 'They/Them', 'Prefer not to say'];

        // Full rebuild vs. in-place patch: see patchProfileHeaderInPlace below for why this only does the full innerHTML rebuild once.
        function renderProfile(){
          const screenEl = document.getElementById('screen');
          // The Profile screen is already showing (its grid is in the DOM) -- patch just the header fields that could have changed instead of rebuilding...
        ...
        // Patches every header field renderProfile()'s full markup below sets, without touching #profile-tabs or #profile-tab-content -- keeping the...
        ...
        // The profile photo (or fallback placeholder) button, its own function since it needs to be regenerated wholesale both by the full profileScreenHTML()...
        ...
        // Posts / Reposts / Saved tab row on the profile page.
        ...
        // Small square thumbnail grid, Instagram-profile style.
        function profileGridItem(post, source){
          // A multi-photo/video post's mediaHtml is a grid of tiles (see postMediaHtmlFromUploaded in feed.js) -- inside a tiny square grid tile that would just...
          const isMulti = !!(post.mediaHtml && /post-media-grid/.test(post.mediaHtml));
          let inner;
          let mediaType = null; // 'video' | 'photo' | null (text-only post)
          if (isMulti) {
            ...
            // A single video post's mediaHtml is the full interactive feed player (simplePostVideoHtml in feed.js) -- tap-to- play/pause, a mute button, its own...
        ...
        // feedPosts itself gets re-sorted by rankFeedPosts() (see feed.js) every time the Home feed loads/refreshes -- that ranking mixes in a little...
        ...
        // ---------------- Desktop right panel ---------------- Telegram-style info column fixed to the right edge (desktop only).
        ...
        function openRightPanel(mode, syncNav){
          // The right panel is a Telegram-style info column for the rest of the app -- but an *open* conversation on desktop already uses that same screen real...
          ...
          rightPanelMode = mode;
          // Both 'profile' (Discover friends folded in underneath) and 'discover' show the same real people list -- load it fresh each time either opens, same as...
          ...
          // syncNav defaults to true for direct profile-panel shortcuts (Profile/Settings/Notifications); it's passed as false when a tab (e.g.
          if (syncNav !== false) {
        ...
        // ---- Right panel retract/restore ---- Distinct from closeRightPanel: collapsing keeps rightPanelMode (and everything already rendered in...
        ...
        // ---- Right panel resize (drag) + collapse/expand ---- The panel's width is a single CSS variable (--rp-width) so dragging the handle, toggling...
        ...
        function renderRightPanelBody(){
          ...
          // The File mode supplies its own header (filename + close button) inside lecturePreviewHTML, so the panel's generic header hides rather than stacking a...
        ...
        // Condensed profile card (avatar, name, stats, bio) plus Discover friends folded in underneath -- tapping Profile on desktop doesn't need the full...
        ...
        // Classroom notebook: quick freeform notes scoped to study sessions.
        let notebookNotes = [];
        let notebookDraft = '';
        let notebookExpandedIds = new Set();

        // Shared body for the Class Pad -- notes first, then (while a lecture is live) the Resources list right underneath, so there's a single "Class Pad"...
        ...
        // Each saved note shows collapsed (a one-line preview) until tapped, then expands in place to show the full text -- tapping again collapses it back...
        ...
        // Refreshes every Class Pad surface currently in the DOM (the desktop right panel, and/or the mobile lecture slide-in panel) rather than assuming only...
        ...
        // Job Plan: same idea as the Classroom notebook, but scoped to the Jobs tab -- quick tips/notes on postings, applications, or interview prep.
        let jobPlanNotes = [];
        let jobPlanDraft = '';
        ...
        // Best-effort persistence for both note lists above: saved after every add/delete, loaded once on startup before the first render (see window.onload)...
        ...
        // Called by feed.js right after a post is created or deleted, so the profile's "Posts" count and its thumbnail grid update the moment it happens --...
        ...
        function profileTabSwitch(tab){
          // Tapping the tab that's already active used to still tear down and rebuild the whole grid -- reloading every video thumbnail in it for no reason,...
        ...
        // ---------------- EDIT PROFILE ---------------- Each field's label sits above its own outlined box, rather than being merged inside it; matching the...
        ...
        // Bio uses a textarea that grows taller as you type so the full text stays visible instead of scrolling inside a fixed box, but now sits inside the...
        ...
        // ---------------- PROFILE LINKS ---------------- A short list of outside pages/socials (Instagram, a portfolio, WhatsApp channel, whatever) shown as...

        // Always stored/rendered with an explicit scheme so a saved link is safe to drop straight into an href and always opens instead of being treated as a...
        ...
        // What actually shows on the pill: the bare host + path, no scheme/www, so "https://www.instagram.com/mister_festus/" reads as a clean...
        ...
        // Read-only pill strip -- used on this account's own Profile tab, the desktop right-panel profile card, and personProfileHTML for other users.
        ...
        // Editable pill strip shown inside the Edit Profile sheet: same look as profileLinksHTML but each pill carries its own remove button, plus a trailing...
        ...
        // Cleans up any leftover scroll listeners from a previous open of the gender/pronouns dropdowns before (re)arming them.
        ...
        function profileLinksAddRowHTML(){
          return `
            <div class="flex items-center gap-2 mb-3">
              ...
              <button type="button" onclick="addProfileLink()" class="flex-shrink-0 px-3 py-2.5 rounded-2xl font-semibold text-sm text-white" style="background:${ROYAL};">Add</button>
            </div>
        ...
        // Re-renders the Edit Profile form from profileData.
        ...
        // Real availability check against public_profiles -- usernames are stored lowercase-insensitive there (see syncPublicProfile), so this does a...
        async function isUsernameTaken(username){
          ...
          try {
            const user = await getCachedAuthUser();
            // ilike's pattern syntax treats "%" and "_" as wildcards -- and usernames are allowed to contain literal underscores (see USERNAME_INVALID_RE) -- so...
        ...
        // "Stitch" is the app/brand name and is reserved -- nobody may set it as their username OR their full name (in any casing/spacing/ punctuation variant:...
        ...
        // Snapshot of profileData taken the moment Edit Profile opens, and a flag for a pending (not-yet-saved) profile picture deletion.
        ...
        // Leaving the editor WITHOUT saving -- Cancel, the back arrow, or the hardware back button all route here.
        ...
        // Tapping the avatar in Edit Profile no longer opens the upload/ choose-avatar controls; it just shows the current photo full-screen.
        ...
        // ---- Crop/resize step, shown between picking a file and it actually becoming the profile picture.
        ...
        // The slider goes 0-100 for a simple, unitless "more zoom" feel rather than asking the person to think in zoom-ratio units -- 0 is Cropper's default...
        ...
        // Refreshes every place a profile picture is currently shown on screen -- whichever of these happens to be open/active right now.
        ...
        function confirmPhotoCrop(){
          ...
          // Staged only -- this becomes the real profile picture just like a typed name or username does: it previews right away inside the Edit Profile form...
          const previewDataUrl = canvas.toDataURL('image/jpeg', 0.9);
          profileData.photo = previewDataUrl;
          closePhotoCropModal();
          renderEditProfileForm();
          // Upload the real bytes in the background now so a small Storage URL is already available by the time Save Changes commits this -- previously the...
          canvas.toBlob(blob => {
            if (!blob) return;
            uploadProfilePhotoToStorage(blob).then(remoteUrl => {
              // If the person picked a different photo (or deleted it) while this upload was still in flight, don't clobber whatever they've since chosen with this...
              ...
              if (stillStaging) {
                // Still mid-edit and unsaved -- just refresh the preview.
                renderEditProfileForm();
              } else {
                // Save Changes already committed this photo before the upload finished; push the real Storage URL out now.
        ...
        function deleteProfilePicture(){
          ...
          openAppConfirmModal('Delete your profile picture?', '', 'Delete', function(){
            // Staged only, same as everything else in this form -- the photo disappears from the Edit Profile preview right away, but it isn't actually removed...
        ...
        function selectEditProfileAvatar(src){
          // Staged like every other field on this form -- previews in Edit Profile immediately, but only becomes real (persisted, and visible elsewhere in the...
        ...
        // Usernames may only contain letters, numbers, a period, or an underscore -- strips anything else out as it's typed (rather than just rejecting on...
        const USERNAME_ALLOWED_RE = /[^A-Za-z0-9._]/g;
        const USERNAME_INVALID_RE = /[^A-Za-z0-9._]/;

        // Shared by both the username and full-name inputs: keeps the red border + message under a field in sync with whatever error string is currently set...
        ...
        // "Stitch" is reserved and, as someone types it into either the username or full name field, the field should flag it live -- not just at save time --...
        ...
        function onEditUsernameInput(){
          ...
          // Not just flagged red anymore -- the instant the field's content IS "Stitch" (in any casing/spacing/punctuation variant), it's wiped right back out.
        ...
        // Same live-reserved-word check as onEditUsernameInput above -- "Stitch" is wiped out of the full name field the instant it's typed, rather than just...
        ...
        // Async because isUsernameTaken now does a real Supabase lookup; called from a plain onclick (fire-and-forget), same convention as the other async...
        ...
        async function saveEditProfile(){
          ...
          // "Stitch" is reserved -- it's the app's own name, not something any member can present themselves as.
          editNameError = '';
          editUsernameError = '';
          let hasError = false;
          ...
          // This is the moment everything staged in this edit session -- name, username, bio, and any photo change -- actually takes effect: persisted locally,...
          if (editProfilePendingPhotoDelete) {
            // Only now does the old file actually get removed from Storage -- if the person had backed out instead, nothing about the old photo would need to be...
        ...
        // If the user taps a nav tab while Edit Profile is open, quietly save whatever they've typed so far into profileData before leaving, so nothing is lost...
        function autoSaveEditProfileDraft(){
          ...
          const usernameEl = document.getElementById('edit-username');
          const bioEl = document.getElementById('edit-bio');
          // Don't silently save the reserved "Stitch" name on the way out any more than a taken username below -- just keep whatever name was already valid.
          if (nameEl && !isReservedStitchIdentity(nameEl.value.trim())) {
            profileData.name = nameEl.value.trim() || profileData.name;
          }
          // Don't silently save a reserved username on the way out either; just keep whatever username was already valid.
          ...
          // Same commit as tapping "Save Changes" -- any staged photo change (upload, delete, preset pick) takes effect here too, since profileData.photo already...
          if (editProfilePendingPhotoDelete) {
            deleteProfilePhotoFromStorage();
            editProfilePendingPhotoDelete = false;
          }
        ...
        // ---------------- REFERRALS ---------------- Settings > Referrals: this account's own code + shareable link, live points balance (1 referral = 1 point...
