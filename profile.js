const PUBLIC_PROFILES_TABLE = 'public_profiles';
        ...
        let profileData = {
          ...
          gender: '',
          photo: null,
          // True once the person has explicitly removed their photo (see deleteProfilePicture
          // below). Google sign-in used to re-fill an empty photo from the Google account picture on
          photoCleared: false,
          link: '',
          links: []
        };
        ...
        // reference, so anything that captured the reference stays in sync). Used when logging
        ...
        // localStorage so it's available INSTANTLY the next time this account logs in -- even if
        ...
        function saveProfileCacheSnapshot(userId){
          try {
            ...
            // -- don't overwrite a good earlier snapshot with an empty one.
            ...
            const snapshot = {
              ...
              bio: profileData.bio || '',
              // A photo mid-upload can briefly be a data: URL embedding the actual file bytes -- never
              // cache that (it's meaningless after a reload and would bloat localStorage); the real
        ...
        // clearAllFeedCaches in feed.js -- no account's cached name/photo should survive into
        ...
        function patchProfileHeaderInPlace(){
          ...
          // account, different posts, a save/repost toggle, etc). This is also what keeps a stale
        ...
        function profilePhotoButtonHTML(){
          // If a photo URL is set but fails to load (e.g. it was never actually finished
          // uploading, or points at something that's since been removed), fall back to the
          // plain silhouette placeholder instead of leaving the browser's own broken-image
          // glyph on screen -- same onerror pattern as avatarMediaHTML in chat.js.
        ...
        function profileScreenHTML(asOverlay){
          ...
          <div id="profile-screen-root">
            ...
            <div class="p-5">
              ...
              <div style="font-size:10px;color:#f00;word-break:break-all;margin-top:8px;">TEMP DEBUG — uid: ${typeof currentUserId !== 'undefined' ? currentUserId : '(undefined)'} | role: ${typeof currentUserRole !== 'undefined' ? currentUserRole : '(undefined)'} | isAdmin(): ${typeof isCurrentUserAdmin === 'function' ? isCurrentUserAdmin() : '(fn missing)'}</div>
            </div>
        ...
        // fully leaves whatever tab the person was on -- most jarringly when they were inside a
        ...
        function profileGridItem(post, source){
          // A post still being uploaded has no mediaHtml yet -- show the same shimmering placeholder
          // the feed uses instead of falling through to the "no media" text tile, which used to
          if (post.uploading) {
        ...
        function editProfileHTML(){
          const p = profileData;
          // Same broken-image fallback as profilePhotoButtonHTML above: an image that fails to
          // load falls back to the plain silhouette instead of showing a broken-image icon.
        ...
        async function discardEditProfileChanges(fromPopState){
          ...
          // Whether the photo actually changed while this modal was open -- picking/cropping a new
          // photo (confirmPhotoCrop) already uploads it to storage right away, and deleting one
        ...
        function refreshProfilePhotoEverywhere(){
          ...
          // previously only reached whichever tab was live above -- Home/Profile stay cached-but-
        ...
        // directly, instead of re-rendering the whole edit-profile form (renderEditProfileForm). A
        ...
        async function saveEditProfile(){
          ...
          // upload, checking username availability) -- lock the button and show a spinner for that
