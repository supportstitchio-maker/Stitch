        ...
        function profileGridItem(post, source){
          // A multi-photo/video post's mediaHtml is a grid of tiles (see
          // postMediaHtmlFromUploaded in feed.js) -- inside a tiny square
          // grid tile that would just mean an unwanted extra scroll
          // gesture, so the tile only ever shows the first pane, with a
          // small stacked-photos badge (Instagram-style) hinting there's
          // more to see once the tile is opened.
        ...
        // Called by feed.js right after a post is created or deleted, so
        // the profile's "Posts" count and its thumbnail grid update the
        // moment it happens -- instead of only catching up the next time
        // renderProfile() happens to run a full rebuild (e.g. leaving and
        // re-opening the Profile tab). Safe to call from anywhere: each
        // element lookup is a no-op if Profile isn't currently on screen.
