        ...
        function renderProfile(){
          document.getElementById('screen').innerHTML = `
            ...
            <div class="p-5">
              ...
              ${profileData.link ? `<a href="${escapeHtml(profileData.link)}" target="_blank" rel="noopener" class="text-sm font-semibold mb-4 block truncate" style="color:${ROYAL}">${Icon('link','w-3.5 h-3.5 inline-block mr-1 align-[-2px]')}${escapeHtml(profileData.link)}</a>` : ''}
        ...
        async function saveUserNotes(){
          try {
            if (window.storage) await window.storage.set('userNotes', JSON.stringify({ notebookNotes, jobPlanNotes }), false);
          } catch (e) { /* not persisted this time, in-memory state still works */ }
        }

        async function loadUserNotes(){
          try {
            if (!window.storage) return;
            const res = await window.storage.get('userNotes', false);
            if (!res || !res.value) return;
            const data = JSON.parse(res.value);
            if (Array.isArray(data.notebookNotes)) notebookNotes = data.notebookNotes;
            if (Array.isArray(data.jobPlanNotes)) jobPlanNotes = data.jobPlanNotes;
        ...
        function promptProfileLink(){
          const current = profileData.link || '';
          const input = (prompt('Add a link to your profile (leave blank to remove):', current) || '').trim();
          profileData.link = input;
        ...
        function editProfileHTML(){
          ...
          return `
            ...
            <button onclick="promptProfileLink()" class="w-full flex items-center justify-between py-3 border-b border-gray-100 mb-5 text-left">
              ${p.link ? `<span class="text-[15px] truncate" style="color:${ROYAL}">${escapeHtml(p.link)}</span>` : `<span class="text-[15px]">Add link</span>`}
              ${p.link ? Icon('edit','w-4 h-4 text-gray-400 flex-shrink-0') : Icon('plus','w-4 h-4 text-gray-400 flex-shrink-0')}
            </button>
