const PUBLIC_PROFILES_TABLE = 'public_profiles';
        ...
        // ---- Profile cache: last-known-good name/username/photo for this
        // account, kept in localStorage so it's available INSTANTLY the
        // next time this account logs in -- even if the connection is too
        // poor to reach the server in time. Without this, profileData sat
        // at its blank defaults (see above) for however long the real
        // account-data fetch took, which is what made the name/photo
        // appear to "disappear" on a slow reconnect: the nav icon and the
        // Profile tab briefly painted with nothing before the real fetch
        // landed. This is a nicety only -- it never replaces the live
        // fetch, just covers the gap while it's in flight or retrying.
        ...
        function saveProfileCacheSnapshot(userId){
          try {
            ...
            // Nothing worth caching yet (e.g. a brand-new account that
            // hasn't finished profile setup) -- don't overwrite a good
            // earlier snapshot with an empty one.
            ...
            const snapshot = {
              ...
              bio: profileData.bio || '',
              // A photo mid-upload can briefly be a data: URL embedding the
              // actual file bytes -- never cache that (it's meaningless
              // after a reload and would bloat localStorage); the real
              // storage URL replaces it moments later and gets cached then.
        ...
        // Loads this account's cached snapshot straight into the live
        // profileData object (same reference, so anything holding onto it
        // stays in sync), resetting to defaults first so nothing from a
        // previous account can linger. Returns true if a cached snapshot
        ...
        // matching clearAllFeedCaches in feed.js -- no account's cached
        // name/photo should survive into whoever logs in next in this browser.
        ...
        function refreshProfilePhotoEverywhere(){
          ...
          // A name/photo edit made while sitting on some other tab (e.g.
          // Classroom, Messaging, Jobs) previously only reached whichever
          // tab was live above -- Home/Profile stay cached-but-detached
          // DOM (see cachedHomeFeedNode/cachedProfileScreenNode in
          // core.js) while you're elsewhere, and switchTab() reattaches
          // that stale cached node as-is instead of re-rendering, so the
          // old name kept showing on your own posts until something else
          // happened to invalidate the cache. Dropping both caches here
          // guarantees Home and Profile rebuild fresh -- with the new
          // name/photo -- the next time either is actually visited.
        ...
        // Toggles the Save Changes button's spinner/disabled state by
        // mutating that one button directly, instead of re-rendering the
        // whole edit-profile form (renderEditProfileForm). A full
        // re-render reads name/username/bio straight from profileData
        // (see editProfileHTML), which isn't updated with what's been
        // typed until the save actually succeeds -- doing that while a
        // save is still in flight would wipe out whatever the person
        // had typed but not yet submitted. Touching just the button
        ...
        async function saveEditProfile(){
          ...
          // Everything past this point can involve a network round-trip
          // (an in-progress photo upload, checking username availability)
          // -- lock the button and show a spinner for that entire
          // stretch, not just part of it, so a slow connection can't get
          // tapped two or three times and fire this save repeatedly.
