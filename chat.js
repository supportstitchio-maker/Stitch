        ...
        function incomingCallHTML(){
          ...
          return `
            <div class="flex-1 flex flex-col text-white" style="padding-top:20px;background:linear-gradient(160deg, ${NAVY}, ${ROYAL});">
              ...
              <div class="flex-shrink-0 flex items-center justify-center" style="gap:0.5px;padding-bottom:calc(env(safe-area-inset-bottom, 0px) + 10px);">
                <button onclick="declineIncomingCall()" title="Decline" class="rounded-full flex items-center justify-center shadow-lg" style="width:60px;height:60px;background:#ef4444;">${Icon('phoneHangup','w-6 h-6')}</button>
                <button onclick="acceptIncomingCall()" title="Accept" class="rounded-full flex items-center justify-center shadow-lg" style="width:60px;height:60px;background:#10b981;">${Icon(isVideo ? 'video' : 'phoneOutline','w-6 h-6')}</button>
              </div>
            </div>`;
        }
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto p-5">
              <div class="flex flex-col items-center text-center mb-6">
                <div class="w-24 h-24 ${meta.avatarBg} rounded-full flex items-center justify-center text-gray-600 mb-3 overflow-hidden" onclick="openPersonProfileForConvo('${activeConvoId}')">${avatarInnerHTML(meta,'w-10 h-10')}</div>
        ...
        function callStageHTML(){
          ...
          if (callState.connected) {
            ...
            // Video calls: split the stage into two equal panels, one per
            // side of the call (their video/avatar on top, your own
            // self-view below) instead of a small corner picture-in-picture
            // -- so both participants get an equally-sized "side", the way
            // WhatsApp shows a proper tile for each person on the call.
            if (isVideo) {
              return `
                <div class="w-full h-full flex flex-col gap-3">
                  <div class="flex-1 min-h-0 rounded-2xl overflow-hidden bg-black relative">
                    <video id="call-remote-video" autoplay playsinline class="w-full h-full object-cover ${showRemoteVideo ? '' : 'hidden'}"></video>
                    ${!showRemoteVideo ? `<div class="w-full h-full flex items-center justify-center ${meta.avatarBg} overflow-hidden">${avatarInnerHTML(meta,'w-16 h-16')}</div>` : ''}
                    <div class="absolute bottom-2 left-2 text-[10px] font-bold uppercase tracking-wide text-white/80 bg-black/40 rounded-full px-2 py-1 truncate max-w-[80%]">${escapeHtml(meta.name)}</div>
                  </div>
                  <div class="flex-1 min-h-0 rounded-2xl overflow-hidden bg-black relative">
                    ...
                    ${!showLocalVideo ? `<div class="w-full h-full flex items-center justify-center bg-gray-800 text-white/60 text-xs font-semibold">${callState.mediaError ? escapeHtml(callState.mediaError) : 'Camera off'}</div>` : ''}
        ...
        function callHTML(){
          ...
          return `
            <div id="call-screen" class="flex-1 flex flex-col text-gray-800" style="padding-top:20px;background:#ffffff;overflow-y:auto;">
              ...
              <div class="flex-shrink-0 pt-4 flex justify-center" style="padding-bottom:15px;">
