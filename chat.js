        ...
        function convoLogHTML(){
          ...
          return msgs.map(m => {
            ...
            const files = (m.attachments || []).filter(f => !images.includes(f));
            ...
            const voiceHTML = m.voice ? convoVoiceNoteHTML(m.voice, mine) : '';
        ...
        // Non-image attachments (pdf, slides, etc.) still show as a
        // filename chip inside the normal text bubble -- there's no
        // useful inline preview for those, unlike photos.
