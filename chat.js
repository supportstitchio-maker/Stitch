        ...
        function handleIncomingCallRing(payload){
          ...
          // Log it in the in-app Notifications bell tab as it rings, then
          // update the same entry to reflect how it was resolved (missed /
          // declined / answered) -- see acceptIncomingCall/declineIncomingCall
          // below. This is separate from the real push notification
          // already sent by sendCallRing (reaches a closed tab/app);
          // this only covers what's shown while signed in.
          if (typeof addNotif === 'function') {
            incomingCallInfo.notifId = addNotif({
              type: 'call',
              source: 'calls',
              icon: payload.type === 'video' ? 'video' : 'phoneOutline',
              iconBg: 'bg-purple-50',
              iconClass: 'text-purple-600',
              name: payload.callerName || 'Someone',
              message: payload.type === 'video' ? 'Incoming video call' : 'Incoming voice call',
        ...
        function acceptIncomingCall(){
          ...
          const { convoId, type, notifId } = incomingCallInfo;
          if (typeof updateNotifOutcome === 'function') updateNotifOutcome(notifId, 'Call answered', true);
        ...
        function declineIncomingCall(isTimeout){
          ...
          if (typeof updateNotifOutcome === 'function') updateNotifOutcome(incomingCallInfo.notifId, isTimeout ? 'Missed call' : 'Call declined', false);
          incomingCallInfo = null;
          closeOverlay();
        }
        ...
        function callStageHTML(){
          ...
          return showLocalVideo
            ...
            : `<div class="w-56 h-56 ${meta.avatarBg} rounded-full flex items-center justify-center text-gray-600 overflow-hidden">${avatarInnerHTML(meta,'w-24 h-24')}</div>`;
        }
