        ...
        function sendConvoVoiceNote(dataUrl, durationSecs, blob){
          if (!activeConvoId) return;
          conversationMessages[activeConvoId].push({ from: 'me', voice: { src: dataUrl, duration: durationSecs }, time: Date.now() });
          const log = document.getElementById('convo-log');
          if (log) { log.innerHTML = convoLogHTML(); log.scrollTop = log.scrollHeight; }
          queueSaveUserState();
        ...
        function sendConvoMessage(){
          ...
          conversationMessages[activeConvoId].push({ from: 'me', text, attachments, time: Date.now() });
          ...
          if (meta && meta.otherUserId) {
          ...
          } else if (meta && meta.icon === 'users' && Array.isArray(meta.members) && text) {
            ...
            meta.members.forEach(m => {
              ...
              sendMessageRemote(convoIdAtSend, m.otherUserId, text);
            });
          }
