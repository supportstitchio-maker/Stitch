        ...
        // Opened/closed via direct DOM insertion rather than a full renderInboxTab() re-render; re-rendering restarts the header's slide/fade transition on the...
        ...
        function openInboxFilterMenuDom(){
          ...
          // Close the dropdown if the user scrolls the inbox list while it's open, same as tapping the backdrop would.
        ...
        // Conversation data, split by filter, so items can be pinned, marked read, blocked, or deleted from the shared actions menu.
        const primaryConvos = [];
        const requestConvos = [];
        const collabConvos = [];

        // Shared avatar renderer for messaging: whenever a contact/convo has a real profile photo (set via Edit Profile -> profileData.photo, synced to the...
        ...
        // Batch version of the same idea as refreshConvoAvatarFromProfile, but for a whole list of contacts/members at once instead of one conversation at a...
        ...
        // Refreshes every contact shown in My Contacts (both the "My Network" and "General Chats" sections -- see myContactsHTML) in one batch call.
        ...
        // convoMeta[id].photo/.username are whatever was cached the last time this function ran for that contact (originally, off an accepted connection...
        ...
        // "My Contacts" screen: splits contacts into the people who are part of the user's professional network vs. everyone else the user just...
        ...
        // Search bar on the Select Contact screen: tapping the search icon expands it into a text input that filters the contact list below live, by name.
        ...
        function newCollabSelectHTML(){
          ...
          return `
            ...
            ${count ? `
              <button onclick="proceedToNewCollabName()" class="flex items-center justify-center gap-2 text-white rounded-full px-6 py-3 text-sm font-semibold shadow-lg" style="position:fixed;right:1.25rem;bottom:1.25rem;z-index:50;background:rgba(30,144,255,0.5);">
                Next
              </button>
            ` : ''}`;
        }
        ...
        function newCollabNameHTML(){
          ...
          return `
            ...
            <button onclick="createNewCollaboration()" class="flex items-center justify-center gap-2 text-white rounded-full px-6 py-3 text-sm font-semibold shadow-lg" style="position:fixed;right:1.25rem;bottom:1.25rem;z-index:50;background:rgba(30,144,255,0.5);">
              Create
            </button>`;
        }
        ...
        async function createNewCollaboration(){
          ...
          renderInboxTab();
          openConversation(id);
          // Syncs the collaboration + its members to Supabase so everyone added actually sees it appear in their own inbox (see...
          ...
          if (createdRemotely && convoMeta[id]) {
            ...
            // collabCreateRemote failed silently (offline, or -- most commonly -- the collaborations/collaboration_members tables aren't set up in Supabase yet,...
        ...
        // ---- Collaboration members: view / add / remove, still usable after the collaboration has been created (previously the only way to set members was at...
        ...
        function addCollabMembersHTML(){
          ...
          return `
            ...
            ${count ? `
              <button onclick="confirmAddCollabMembers()" class="flex items-center justify-center gap-2 text-white rounded-full px-6 py-3 text-sm font-semibold shadow-lg" style="position:fixed;right:1.25rem;bottom:1.25rem;z-index:50;background:rgba(30,144,255,0.5);">
                Add
              </button>
            ` : ''}`;
        }
        ...
        // ---- Leave / delete a collaboration. Leaving just removes your own membership (and it stays visible to everyone else); the creator can also delete it...
        ...
        function handleCollabPhotoSelected(e){
          ...
          reader.onload = function(ev){
            const img = new Image();
            img.onload = function(){
              // Downscale to a modest square thumbnail before storing -- matches the resolution profile photos are saved at, keeps the data URL small since it's...
              const size = 320;
        ...
        // ---- Rename a collaboration after it's already been created -- previously the name could only ever be set once, at creation time (see...
        ...
        function renderInboxTab(){
          // Remember where the user was scrolled to before we rebuild the screen's contents, so an in-place action (accepting/declining a request, selecting a...
          ...
          // Close the actions dropdown if the user scrolls the list while it's open, same as tapping the backdrop would.
          if (inboxMenuOpen) {
            const screenEl = document.getElementById('screen');
            if (screenEl) {
              const anchor = screenEl.scrollTop;
        ...
        // The number shown as "My Network" on the profile (and the Job Plan stats panel).
        ...
        async function refreshNetworkCount(retryDelayMs){
          ...
          try {
            ...
            // A newer refreshNetworkCount() call was kicked off while this one was still in flight -- its answer is more current, so drop this stale one instead of...
            ...
            // Re-paint whichever visible screen is actually showing the number right now -- profile tab, or the Career Analytics overlay (see careerAnalyticsHTML...
        ...
        // Whether a given real (Supabase) user id is someone you're actually connected to -- used by the feed to decide whether a post's author gets a...
        ...
        // Applies pin / mark as read / block / delete to whichever conversations are currently selected in the Inbox.
        ...
        function inboxContent(){
          // When the contact search bar is active, search across every contact (all inbox tabs), not just whichever filter tab is currently selected, so it...
        ...
        // Messaging, chat, and calls should surface someone's @username rather than their real full name -- falls back to name for group collaborations (which...
        ...
        // A connect request card: shows the sender, an optional attached message (some requests come with a note, some don't), and Accept / Reject actions.
        ...
        // Accepting moves the request into Primary as a normal conversation (with whatever message they attached, if any, as the opening line); rejecting just...
        function acceptRequest(id){
          ...
          renderInboxTab();
          if (typeof refreshMessagingBadges === 'function') refreshMessagingBadges();
          // Instant feedback while the real count re-fetches in the background (see refreshNetworkCount) -- the .then() below is what actually corrects it once...
        ...
        // Pulls this account's pending incoming connection requests (rows in connection_requests where to_user = me) so a request sent via Discover's "Connect"...
        ...
        async function loadIncomingConnectionRequests(){
          ...
          try {
            ...
            // Requests that were sitting here as pending but aren't in this pending set anymore -- the sender cancelled it (see cancelMyConnectionRequestTo in...
            ...
            for (let i = requestConvos.length - 1; i >= 0; i--) {
              ...
              if (c.connectionRequestId && !stillPendingIds.has(c.connectionRequestId)) {
                ...
                if (typeof deleteNotif === 'function') deleteNotif(c.id);
                removedAny = true;
              }
            }
            ...
            newRows.forEach(row => {
              ...
              // row.id is already the id connectToDiscoverPerson generated (e.g.
              ...
              // Surface it in the in-app Notifications bell tab too, not just the Messaging -> Requests tab.
            ...
            renderInboxTab();
            // Public accounts don't gate who joins their network -- if this account is already set to Public, don't leave these sitting in Requests waiting for a...
        ...
        // Mirror of loadMyAcceptedOutgoingRequests, but for the OTHER direction: requests THIS account received and already accepted (to_user = me, status =...
        ...
        // Realtime top-up for everything connection-related: instead of only re-checking when the person happens to switch to Messaging or reopen the app,...
        ...
        // Mirror of loadIncomingConnectionRequests, but for requests THIS account sent that the other person has since accepted.
        async function loadMyAcceptedOutgoingRequests(){
          ...
          try {
            ...
            newRows.forEach(row => {
              ...
              // Surface it in the Notifications bell tab too -- reuses convoId (== the connection_requests row id) as the notification id so a later re-poll of this...
              if (typeof addNotif === 'function') {
                addNotif({
                  id: 'accepted-' + convoId,
        ...
        // Compact time label for the Inbox / requests list, and for the "time" field on every conversation row elsewhere.
        ...
        // Voice-note audio blobs upload here so the recipient can actually fetch and play them -- same pattern as...
        ...
        // Image/file attachments upload here, same idea as uploadConvoVoiceToStorage above -- previously attachments were never given anywhere to live...
        ...
        // PostgREST returns this shape (code PGRST204, "Could not find the 'X' column of 'Y' in the schema cache") when a request references a column that...
        ...
        // Returns the new row's id (so the sender can track its read receipt -- see convoReadTicksHTML/handleMessageReadUpdate) or null if it didn't sync.
        async function sendMessageRemote(convoId, otherUserId, text, voiceUrl, voiceDuration, attachments, localId){
          ...
          try {
            ...
            // If the live `messages` table is missing an optional column this row includes (most commonly local_id, on a project where that migration hasn't been...
            let data, error;
            ...
            // Real push so the recipient hears about the message even if they aren't currently sitting in the app (postgres_changes Realtime in...
        ...
        // Replaces whatever placeholder/local history a real conversation had with its actual message history from Supabase -- called when opening a...
        async function loadConversationHistory(convoId){
          ...
          try {
            ...
            // Same schema-drift situation as sendMessageRemote above: if the live table is missing a column this SELECT asks for (again, usually local_id), fall...
        ...
        // Tells Supabase the messages the other person sent us in this conversation have now been seen, so their copy's ticks can turn from "delivered" to...
        async function markConvoMessagesRead(convoId, otherUserId){
          const sb = getSupabaseClient();
          const myId = await getCurrentUserId();
          if (!sb || !myId || !otherUserId) return;
          try {
            // Voice notes are deliberately left out of this bulk update -- see markVoiceMessageAsPlayed below for why they're only marked read once actually...
        ...
        // Voice notes used to get swept up in markConvoMessagesRead just like text and images -- the moment the conversation was open (or already open when the...
        ...
        // Re-derives each 1:1 conversation's unread state from the real `messages` table on app entry, rather than trusting whatever unread/read flags happened...
        ...
        // One Realtime subscription per signed-in session, listening for any message row addressed to this account.
        ...
        async function subscribeToIncomingMessages(){
          ...
          messagesChannel = sb.channel('messages:' + myId)
            ...
            // A message *we* sent just got marked read on the other end (see markConvoMessagesRead) -- flip its ticks blue live instead of only finding out next...
            ...
            // Someone deleted a message "for everyone" on their end (see deleteActionMessageForEveryone) -- this is what actually makes it disappear live on ours...
        ...
        // ---- Polling fallback for incoming messages ---- subscribeToIncomingMessages above is the primary way new messages arrive live, but it depends on...
        ...
        // Called once at app entry (see games.js authEnterApp), same spot as subscribeToIncomingMessages.
        ...
        // Small shared helper: several places (a message getting its id back from Supabase, a read receipt flipping, a new message arriving) all need...
        ...
        // Best-effort writes, same fallback philosophy as postLikeRemote/sendMessageRemote above: local state is already updated by the caller by the time...
        async function collabCreateRemote(id, name, photo, memberContacts){
          ...
          try {
            ...
            // upsert (not insert): re-adding someone already in collaboration_members -- entirely plausible, e.g.
            ...
            // No sendPushTo here: added members find out live, in-app, via the collab-members Realtime INSERT listener below (see subscribeToCollabMembership)...
            return true;
        ...
        async function collabAddMembersRemote(id, name, newContacts){
          ...
          try {
            ...
            if (rows.length) {
              // Same reasoning as collabCreateRemote above -- upsert with ignoreDuplicates so one already-a-member row in the batch can't 409 the rest of it.
            ...
            // Same as collabCreateRemote: rely on the live Realtime INSERT listener to notify the new members in-app instead of a hosting-platform push.
            return true;
        ...
        // Same idea as collabUpdatePhotoRemote, for renaming a collaboration after it's already been created -- also updates the member row backend keeps for...
        ...
        // Pulls every collaboration this account is a member of -- including ones someone else created and added them to -- and merges them into...
        async function loadMyCollaborations(){
          ...
          try {
            ...
            // Member photos in collab_members are a snapshot taken when each person was added to the group and otherwise never touched again -- someone who changed...
        ...
        // Removes a collaboration from local state (all convo arrays, its meta, its messages) and, when this wasn't something we just did ourselves a moment...
        ...
        // One Realtime subscription per signed-in session, covering the full live lifecycle of collaborations this account belongs to: being added as a member...
        ...
        async function subscribeToCollabMembership(){
          ...
          collabMembersChannel = sb.channel('collab-members:' + myId)
            ...
            // Someone removed just this account from a collaboration (the creator kicked us, or this cascaded from the whole collaboration being deleted -- see the...
            ...
            // The collaboration itself got deleted (by its creator, from any of their devices) -- take it away for every member live, not just next time...
        ...
        // ---- Incoming call ringing ---- Calls themselves were already real peer-to-peer WebRTC (see the VOICE/VIDEO CALL section below) -- but previously the...
        ...
        // ---- Incoming call ringtone ---- handleIncomingCallRing below only popped up the Accept/Decline sheet -- silently, with no actual sound, so a call...
        ...
        // One "brring" burst: two tones together (440Hz + 480Hz, the classic North American ring frequency pair) for ~1.8s.
        ...
        function startIncomingCallRingtone(){
          ...
          // Repeat the burst+vibrate pattern every 4s, like a phone ringing in cycles, until stopIncomingCallRingtone() is called.
        ...
        // A one-shot broadcast to someone else's inbox channel (ringing them, or replying to a ring they sent) -- opens a channel just long enough to deliver...
        ...
        async function sendCallRing(convoId, type, otherUserId){
          ...
          // A group ring also carries the collaboration's name, so the person being rung sees "calling into 'Study group'" rather than a plain 1:1-looking ring...
          const meta = convoMeta[convoId];
          ...
          // Realtime broadcast only reaches the other side if they're actively signed in with the app open right now (see subscribeToIncomingCalls) -- a real...
          sendPushTo(otherUserId, {
        ...
        // Logs a call outcome (missed / declined / duration) as a system line inside the conversation itself -- same place a real chat app shows "Missed call"...
        ...
        // Pushes a "Missed call" entry into the Notifications bell tab (addNotif, in overlays.js) -- separate from logCallEvent above, which only writes the...
        ...
        // fromUserId matters for group calls specifically: the caller rang several people individually (see startCall's group branch), so when a response comes...
        ...
        // The caller's side of hanging up before we ever picked up (see endCall's cancel broadcast) -- separate from 'response' because 'response' is us...
        async function sendCallCancel(convoId, toUserId){
          const myId = await getCurrentUserId();
          broadcastToUserChannel(toUserId, 'cancel', { convoId, fromUserId: myId });
        }

        // Someone is calling us. If we're already on a call (or already ringing from a different call), respond busy automatically instead of showing a second...
        ...
        // A reply to a call WE placed (the other side declined, was busy, or never picked up).
        function handleCallResponse(payload){
          ...
          if (payload.response === 'declined' || payload.response === 'busy' || payload.response === 'missed') {
            // Group call: one member declining/being busy/not answering doesn't mean the call is over -- others we rang might still pick up.
        ...
        // The caller hung up (or gave up on ringing us in) before we ever accepted or declined -- stop the ringtone/vibration and dismiss the incoming-call...
        ...
        // fromPopState is threaded through to closeOverlay so the hardware/browser back button (see overlayBackAction in overlays.js) can decline a ring the...
        ...
        function declineIncomingCall(isTimeout, fromPopState){
          ...
          // Only ringing out unanswered counts as "missed" for the bell notification -- actively hitting Decline already tells the person via the call screen, so...
          if (isTimeout) addMissedCallNotif({ convoId, type, fromUserId, callerName, groupName });
          incomingCallInfo = null;
          closeOverlay(fromPopState);
        }

        // "Remind Me" on the incoming-call screen: declines the call (logged as missed, same as letting it ring out) and schedules an in-app reminder...
        ...
        // "Message" on the incoming-call screen: declines the call and drops straight into the conversation so you can reply with a quick text instead of...
        ...
        // A message just arrived from someone else in real time.
        function handleIncomingMessage(row){
          if (!row || !row.convo_id) return;
          const convoId = row.convo_id;
          if (!conversationMessages[convoId]) conversationMessages[convoId] = [];
          // Realtime (subscribeToIncomingMessages) and the polling fallback (pollForNewMessages, below) can both independently observe the same row -- skip...
          ...
          // We're already looking at this conversation, so it counts as read the instant it lands -- ack it back to the sender so their ticks turn blue without...
          if (activeConvoId === convoId && row.sender_id) markConvoMessagesRead(convoId, row.sender_id);

          // Bump this conversation to the top of its list (splice it out, unshift it back) the same way a real chat app surfaces whoever just messaged you,...
          const found = findConvoAndArray(convoId);
          let c = null;
          if (found) {
          ...
          if (activeConvoId === convoId) {
            ...
            // Only notify here (in-app bell tab) when the chat wasn't already open above -- someone actively reading the conversation doesn't need a notification...
            if (typeof addNotif === 'function') {
              const meta = convoMeta[convoId] || {};
              ...
              addNotif({
                ...
                name: senderName,
                // Don't surface the actual message text here -- keep it generic (who it's from, not what it says) and let opening the conversation be how you actually...
        ...
        function openConversation(id){
          ...
          // Reading it clears the unread state -- the blue dot in the inbox row (see convoRow's showUnread) and the Messaging badge count.
          const found = findConvoAndArray(id);
          ...
          // Reading the conversation -- from Messaging, a notification tap, wherever -- is what clears any matching row in the Notifications bell tab too,...
          if (typeof markNotifsReadForConvo === 'function') markNotifsReadForConvo(id);
          openOverlay('conversation');
          // Open straight to the bottom of the log -- the most recent messages -- instead of leaving it scrolled to wherever the fresh render happens to start...
          requestAnimationFrame(() => {
            const log = document.getElementById('convo-log');
            if (log) log.scrollTop = log.scrollHeight;
          });
          initConvoJumpToLatestButton();
          // In desktop split view the chat list (#screen) stays visible beside the open conversation, so re-paint just its rows to move the "active chat"...
          const inboxList = document.getElementById('inbox-list');
          if (inboxList) inboxList.innerHTML = inboxContent();
          if (typeof refreshMessagingBadges === 'function') refreshMessagingBadges();

          // Real (backend-linked) conversations: swap in the actual message history from Supabase once it arrives, so switching between chats doesn't leave...
          const meta = convoMeta[id];
          if (meta && meta.otherUserId) {
            loadConversationHistory(id).then(() => {
              // Opening the conversation is what "reading" it means here -- mark whatever they'd sent us as read so their ticks turn blue (see...
        ...
        // Jumps the open conversation's message log straight to the bottom (the most recent message) -- used by the floating down-arrow button below and by the...
        ...
        // Shows a floating down-arrow over the message log once the user scrolls up away from the bottom, so they can jump straight back to the most recent...
        ...
        function toggleConvoOptionsMenu(){
          ...
          // Close the menu if the user scrolls the chat log while it's open, same as tapping the backdrop would.
          if (opening && log) {
        ...
        // ---- Chat contact profile (the "View profile" screen reachable from the conversation's three-dot menu).
        ...
        // Deletes the whole chat (not just its messages) and returns to the inbox, unlike "Clear chat" which empties the thread but leaves the conversation...
        ...
        // Pulls every image/file attachment and every link shared in the thread so the "Media, links and docs" screen has real content instead of a static...
        ...
        function convoProfileHTML(){
          ...
          // Every collab thread the contact could plausibly share with the user, aside from a 1:1 chat with them directly.
        ...
        // Conversation reports are real, persistent state (not just an alert): once reported, the id is stored so the menu item flips to "Reported" and won't...
        let reportedConvos = new Set();
        // Message ids this account chose "Delete for me" on -- filtered out of loadConversationHistory's results below so a message deleted locally doesn't...
          ...
          return `
            ...
            <div id="convo-composer-wrap" class="flex-shrink-0 px-3 pt-2 convo-composer-anim" style="padding-bottom:20px;">
              ...
              <div class="flex items-center gap-2 rounded-3xl px-2 py-1.5" style="background:#ffffff;border:1.5px solid rgba(10,37,64,0.10);box-shadow:0 8px 24px rgba(10,37,64,0.10);">
                ...
                <textarea id="convo-input" placeholder="Message" rows="1" enterkeyhint="enter" oninput="autoGrowConvoInput(this)" onfocus="handleConvoInputFocus()" onblur="handleConvoInputBlur()" class="flex-1 min-w-0 bg-transparent text-sm resize-none leading-snug self-center" style="max-height:120px;overflow-y:auto;"></textarea>
        ...
        // Image attachments show an actual thumbnail (once its data URL has loaded -- see handleConvoFileSelect); other file types keep the filename-chip look,...
        ...
        // Previously this only kept {name, type} -- never the actual file bytes -- so a picked image had nothing to preview, nothing to show full-size on tap,...
        ...
        // ---- DM voice notes: record real audio via getUserMedia + MediaRecorder and send it as a playable message bubble.
        ...
        // Pending recording captured but not yet sent -- set by the mic/ pause button (stopConvoVoiceNote(false)) once recording stops, and cleared once...
        ...
        async function toggleConvoVoiceNote(){
          ...
          // The permission prompt is async; bail if the user navigated away from this conversation while it was open.
        ...
        // Three visual states for the mic button: idle (mic icon), actively recording (red pulsing stop icon), and paused with a clip ready to send (a trash...
        ...
        // Stopping the recorder (via the mic/pause button) no longer sends immediately -- it stashes the clip as convoPendingVoice so the send button can send...
        ...
        // The paper-plane send button: sends whatever's ready to go -- a pending recorded voice note if there is one (stopping an in-progress recording first...
        ...
        function sendConvoVoiceNote(dataUrl, durationSecs, blob){
          ...
          if (meta && meta.otherUserId && blob) {
            // Upload first, then insert the message row -- the recipient has nothing to fetch until the clip actually exists in Storage, so this can't be fired in...
            uploadConvoVoiceToStorage(blob, convoIdAtSend).then(url => {
              if (!url) {
                ...
                // Visible, not just console-only: silently failing here is exactly what made voice notes look "not live" -- it played fine for the sender (local data...
        ...
        // A centered "Today" / "Yesterday" / "Monday" / "Jan 5" pill, inserted once before the first message of each new calendar day -- kept on its own row,...
        ...
        // Small per-bubble time (e.g. "11:11 PM"), shown once under each message instead of next to the date pill.
        ...
        function convoLogHTML(){
          ...
          return msgs.map(m => {
            ...
            // Images and voice notes are "media" -- they get their own clean surface instead of sitting inside the heavy colored (mine) / gray (theirs) text...
            ...
            // Voice notes render as their own standalone chip (see convoVoiceNoteHTML) rather than being nested inside the colored/gray text bubble below --...
        ...
        // ---- Long-press a message: "Delete for me" / "Delete for everyone" / Cancel ---- Reuses the same startPress/endPress/movePress hold-to-trigger...
        ...
        function deleteActionMessageForMe(){
          ...
          if (idx === -1) return;
          // Remembers the id (if it already has one) so a later reopen of this chat -- which re-fetches the full thread from Supabase -- doesn't just bring the...
        ...
        function deleteActionMessageForEveryone(){
          ...
          refreshConvoLogIfOpen(convoId);
          queueSaveUserState();
          // Best-effort remote delete so it disappears for the other side(s) too, live -- not just here.
          if (removed) {
            const sb = getSupabaseClient();
            if (sb) {
        ...
        // A voice-note bubble: a real <audio> element (playable, since the clip is embedded as a data URL) plus a mic icon and duration label so it reads...
        ...
        // Actual tappable image thumbnails -- rendered outside/above the colored text bubble (not inside it) so a photo shows as itself, not as a photo sitting...
        ...
        // Non-image attachments (pdf, docs, zip...) still show as a filename chip inside the normal text bubble -- there's no useful inline preview for those,...
        ...
        // Video attachments get an actual inline <video> player, same idea as convoImageAttachmentsHTML for photos -- previously any video fell into the plain...
        ...
        // Read-receipt ticks, WhatsApp-style: a single gray check for a message that's only sent locally (no real recipient to confirm delivery -- e.g.
        ...
        // Fullscreen tap-to-view for a chat image (see convoImageAttachmentsHTML).
        ...
        // Keeps the inbox list's preview line in sync with what's actually been said, instead of getting stuck on the placeholder shown right after connecting...
        function updateConvoPreview(convoId, previewText){
          // Bump it to the top of its list too, same as an incoming message does (see handleIncomingMessage) -- sending a message in an old conversation should...
          const found = findConvoAndArray(convoId);
          if (found) {
            const [c] = found.arr.splice(found.idx, 1);
        ...
        // The message box used to be a plain single-line <input>, whose Enter key was wired straight to sendConvoMessage() -- so there was no way to type a...
        let convoInputFocused = false;

        function syncConvoKeyboardInset(){
          const ov = document.getElementById('overlay');
          if (!ov || ov.classList.contains('hidden')) return;
          if (typeof currentOverlayKind !== 'undefined' && currentOverlayKind !== 'conversation') return;
          ...
          // Gap between the composer and the keyboard: 10px once the keyboard is up, 20px the rest of the time (matching the breathing room every other closed...
        ...
        // Tapping into the message box: pin the composer above the keyboard immediately rather than waiting on the visualViewport 'resize' event, which can lag...
        function handleConvoInputFocus(){
          convoInputFocused = true;
          syncConvoKeyboardInset();
        }

        // Leaving the message box: only settle back down once the keyboard has actually finished closing (the viewport can still briefly report itself as...
        ...
        function sendConvoMessage(){
          ...
          // pendingConvoAttachments entries carry a raw `file` (for uploading) alongside the display fields -- keep that out of the persisted message object...
          ...
          const meta = convoMeta[convoIdAtSend];
          (async () => {
            // Attachments upload first (same reasoning as voice notes -- see sendConvoVoiceNote): the recipient has nothing to fetch until the file actually exists...
            let uploaded = [];
            ...
            if (meta && meta.otherUserId) {
              // Real connection: actually deliver it via Supabase so the other account receives it (see subscribeToIncomingMessages).
              ...
              // Collaboration (group): fan out one row per member (besides ourselves) into the same `messages` table 1:1 chats use, all sharing this convo_id -- each...
            ...
            // No known backend user(s) on the other end (e.g.
          })();
        }

        // ---------------- VOICE / VIDEO CALL ---------------- Real peer-to-peer calling over WebRTC.
        ...
        // ---- Multi-party mesh ---- A 1:1 call only ever has one entry here; a collaboration (group) call has one entry per other member who's actually...
        let callPeers = {};
        ...
        const RTC_ICE_SERVERS = {
          iceServers: [
            ...
            // Free public TURN relay -- lets calls connect across networks STUN alone can't punch through (mobile data, hotel/corporate wifi, symmetric NATs).
        ...
        async function startCall(id, type, answering){
          ...
          if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
            try {
              ...
              // The user may have hit "end call" or switched calls while the permission prompt was open; if so, just release the stream.
          ...
          if (sb) {
            ...
            // Only actually ring anyone when we're the one placing the call -- answering an incoming ring (see acceptIncomingCall) joins the exact same...
            if (!answering) {
              const meta = convoMeta[id];
              const isGroup = meta && meta.icon === 'users';
              if (isGroup) {
                // Ring every other member who has a real backend account linked (contacts added before any connection was ever accepted have no otherUserId, so there's...
                ...
                if (ringTargets.length) {
                  ...
                  // Give up after 2 minutes if literally nobody has joined yet -- same no-answer convention as the 1:1 case below, just judged against "has anyone at...
                ...
                // Tracked the same way a group ring's targets are (see above) purely so endCall knows who's still ringing and needs a cancel if we hang up before they...
                callState.pendingRingTargets = new Set([meta.otherUserId]);
                sendCallRing(id, type, meta.otherUserId);
                // Give up after 2 minutes of ringing with no answer, same as a normal phone call.
                const ringingConvoId = id;
                callAnswerTimeout = setTimeout(() => {
                  if (callState.convoId !== ringingConvoId || callState.connected) return;
                ...
                // No known backend user on the other end of this convo (it wasn't created from an accepted connection request) -- there's nobody real to ring.
            ...
            // No Supabase configured yet: fall back to the old self-view-only demo so the screen still does *something* sensible instead of just hanging on...
        ...
        // ---- Minimize / resume a live call ---- Tapping back on the call screen used to run straight into closeOverlay(), which unconditionally tears down...
        ...
        // Re-renders the call screen without touching the live call itself -- used by resumeCall above, and by anything (like the "add to call" picker) that...
        ...
        function updateMinimizedCallBanner(){
          ...
          // Shows the live call duration, or a short status while still connecting, in the horizontal top pill.
        ...
        // ---- WebRTC + Supabase Realtime signaling ---- Every other peer id in the call room gets its own RTCPeerConnection (see callPeers above); this...
        ...
        // Decides, once both peer ids in a pair are known, which side sends the SDP offer for that pair -- both browsers compute this the same way...
        ...
        async function handleCallSignal(payload){
          ...
          // Signals with a `to` are addressed to one specific peer (a real 1:1 SDP/ICE exchange happening within a many-peer room) -- everyone else in the room...
        ...
        // One specific participant left (they hung up, or their tab closed on the way out) -- in a group call that's not the same as the whole call being over...
        ...
        // Same idea, per remote participant. A 1:1 call (and the waiting-room/pre-connect view) uses the plain #call-remote-video/#call-remote-audio ids from...
        function attachCallRemoteVideo(){
          ...
          // The call screen itself isn't even mounted while minimized (see minimizeCall) -- keep whatever's playing through the background <audio> element in...
          if (callMinimized) {
            const bgAudio = document.getElementById('call-bg-audio');
            if (bgAudio) {
        ...
        // Enables/disables the actual audio & video tracks to match callState.muted / callState.camOff, so mute and camera-off really stop your mic/camera -...
        ...
        function endCall(remoteInitiated){
          ...
          stopCallLocalStream();
          if (!remoteInitiated) {
            // Anyone still in callState.pendingRingTargets hasn't answered yet -- they're sitting on their own incoming-call sheet with no idea we just hung up...
            if (callState.pendingRingTargets) {
              Array.from(callState.pendingRingTargets).forEach(uid => sendCallCancel(callState.convoId, uid));
            }
            teardownCallSignaling();
          } else {
            // Every other peer already left (see handlePeerHangup) -- just release whatever's left without re-broadcasting our own hangup back out at nobody.
            Object.values(callPeers).forEach(entry => {
              if (entry.pc) { entry.pc.onicecandidate = null; entry.pc.ontrack = null; entry.pc.close(); }
            });
            callPeers = {};
          ...
          // If the call was ended from the minimized banner (or by the other side while minimized), stay wherever the person actually was instead of yanking them...
        ...
        function toggleCallControl(key){
          ...
          applyCallTrackStates();
          if (key === 'camOff') {
            // Only camera on/off actually changes what's displayed (video vs. avatar), so only the stage needs to be swapped - the header, other buttons, and...
            const stage = document.getElementById('call-stage-wrap');
            if (stage) stage.innerHTML = callStageHTML();
            attachCallLocalVideo();
            attachCallRemoteVideo();
          }
          // Mute/speaker/camera buttons only ever change their own class and icon; previously every one of these taps replaced the entire #call-screen via...
        ...
        // ---- Add someone to a live call ---- Replaces the old three-dot "participants" button: tapping it opens a picker of the person's contacts (the same...
        ...
        function ringIntoCall(otherUserId){
          ...
          // If the call hasn't connected to anyone yet, track this new ring the same way startCall's initial ring-out does (see handleCallResponse) -- so this...
          if (!callState.connected) {
        ...
        // The video/avatar "stage" pulled into its own function, and the control buttons given stable ids, so toggling mute/speaker/ camera only updates the...
        function callStageHTML(){
          ...
          if (callState.connected) {
            ...
            // Video calls, WhatsApp-style: the other person's video fills the whole stage, and your own camera sits as a small floating tile in the corner -- not...
            if (isVideo) {
              return `
            ...
            // Voice call, connected: just the other person's avatar filling the stage -- there's no video from either side to split.
            return `
        ...
        // Group (collaboration) call stage: a simple responsive grid, one tile per person actually connected, plus your own.
        ...
        function callHTML(){
          ...
          return `
            <div id="call-screen" class="flex-1 flex flex-col text-gray-800" style="padding-top:var(--top-safe-pad);background:#ffffff;overflow-y:auto;">
              ...
              <div id="call-controls-bar" class="flex-shrink-0 pt-4 flex justify-center" style="padding-bottom:20px;">
