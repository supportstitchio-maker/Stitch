        ...
        function convoLogHTML(){
          ...
          return msgs.map(m => {
            ...
            const bubbleHTML = hasBubbleContent ? (
              mine
                ? `<div class="text-white rounded-2xl px-4 py-2.5 text-sm" style="background:linear-gradient(135deg, rgba(65,105,225,0.55), rgba(65,105,225,0.35));">${convoFileAttachmentsHTML(files, mine)}${escapeHtml(m.text)}</div>`
                : `<div class="bg-gray-100 rounded-2xl px-4 py-2.5 text-sm text-gray-700">${convoFileAttachmentsHTML(files, mine)}${escapeHtml(m.text)}</div>`
            ) : '';
