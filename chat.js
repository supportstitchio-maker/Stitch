const inboxFilters = [['general','General',0],['collaborations','Collaborations',0],['requests','Requests',3]];
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto px-5 py-5">
              <div class="flex items-center gap-4 mb-6">
                ...
                <input id="new-collab-name-input" type="text" placeholder="Collaboration name" value="${newCollabName.replace(/"/g,'&quot;')}" class="flex-1 min-w-0 border-b-2 border-[${ROYAL}] py-2 text-[15px]" style="outline:none;">
              </div>
        ...
        async function createNewCollaboration(){
          ...
          if (!newCollabPhoto) {
            openAppAlertModal('Please add a profile picture for the collaboration.');
            return;
          }
          const contacts = newCollabContactSource().filter(c => newCollabSelected.has(c.id));
          ...
          convoMeta[id] = { icon:'users', avatarBg:'bg-emerald-100', name: finalName, photo: finalPhoto, preview:'You created this collaboration', members, createdBy: 'me', membersCanAdd: false };
          ...
          renderInboxTab();
          openConversation(id);
          const createdRemotely = await collabCreateRemote(id, finalName, finalPhoto, contacts);
        ...
        // A round, colour-filled badge for the state icon -- matching how Telegram (and most chat
        // apps) show file attachments: a solid circle with a plain down-arrow before it's
        function convoDocIconHTML(state){
          const badge = (inner, bg) => `<div class="w-full h-full rounded-full flex items-center justify-center text-white" style="background:${bg};">${inner}</div>`;
          if (state && (state.status === 'downloading' || state.status === 'opening')) {
            const pct = Math.max(0, Math.min(100, Math.round((state.progress || 0) * 100)));
            const deg = pct * 3.6;
            return `
              <div class="relative w-full h-full rounded-full flex items-center justify-center" style="background:${ROYAL};">
                <div style="position:absolute;inset:0;border-radius:9999px;background:conic-gradient(rgba(255,255,255,0.95) ${deg}deg, rgba(255,255,255,0.25) ${deg}deg);-webkit-mask:radial-gradient(farthest-side, transparent calc(100% - 2.5px), #000 calc(100% - 2.5px));mask:radial-gradient(farthest-side, transparent calc(100% - 2.5px), #000 calc(100% - 2.5px));"></div>
              </div>`;
          }
          if (state && state.status === 'done') return badge(Icon('file','w-4.5 h-4.5'), ROYAL);
          if (state && state.status === 'error') return badge(Icon('close','w-4.5 h-4.5'), '#ef4444');
          return badge(Icon('download','w-4.5 h-4.5'), ROYAL);
        }
                ...
                if (!f.url) {
                  return `
                    <div class="flex items-center gap-2.5 ${mine ? 'bg-white/15' : 'bg-black/5'} rounded-xl px-2.5 py-1.5">
                      <div class="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-white" style="background:${ROYAL};">${Icon(convoFileIcon(f.type),'w-4.5 h-4.5')}</div><span class="text-xs truncate">${escapeHtml(f.name)}</span>
                    </div>`;
                }
                const id = convoDocChipId(f.url);
                const state = convoDocDownloadState[f.url] || { status: 'idle' };
                // Main tap opens the file inside Stitch (openConvoDocViewer) instead of going straight to
                // convoDownloadDocument -- see that function's comment for why the OS share sheet
                return `
                  <div class="flex items-center gap-2 ${mine ? 'bg-white/15' : 'bg-black/5'} rounded-xl px-2.5 py-1.5">
                    <button type="button" id="${id}" onclick="openConvoDocViewer('${encodeURIComponent(f.url)}','${encodeURIComponent(f.name || '')}')" class="flex items-center gap-2.5 text-left flex-1 min-w-0">
                      <div id="${id}-icon" class="w-10 h-10 flex items-center justify-center flex-shrink-0">${convoDocIconHTML(state)}</div>
        ...
        // Previously this was only attempted on phones/tablets, on the assumption that desktop's
        // Web Share just opens the OS "Share" charm rather than an app-opener. But that's not
