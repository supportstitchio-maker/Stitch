const inboxFilters = [['general','General',0],['collaborations','Collaborations',0],['requests','Requests',3]];
        ...
        function myContactsHTML(){
          ...
          // De-dupe by the actual person (otherUserId), not by conversation
          // id: if the same person ends up with two separate conversation
          // rows both flagged network:true (e.g. a leftover request convo
          // plus its accepted-primary convo), they'd otherwise show up as
          // two separate rows here even though it's one connection. Fall
          // back to the conversation id for the rare row with no
          // otherUserId on record, so it still renders instead of vanishing.
          const seen = new Set();
        ...
        // Whether *I* currently have an outgoing connection request pending
        // to this person -- kept as a live check against sentRequestConvos
        // (rather than a flag baked into whatever list item happened to be
        // built) so "Connect"/"Cancel Request"/"Connected" reads correctly
        // no matter where that person shows up (their own profile, someone
        // else's network list, discover, etc).
        function isConnectionRequestPendingTo(userId){
          if (!userId) return false;
          return sentRequestConvos.some(c => c.otherUserId === userId);
        }

        // Keeps any currently-open "other person's network" list (and its
        // cache) lined up with reality whenever a connection is accepted,
        // sent, cancelled, or declined -- without this, a network list
        // fetched earlier in the session keeps showing stale Connect/
        // Connected/Cancel Request state for members whose status changed
        // after that fetch.
        ...
        function convoDisplayName(c){
          if (!c) return '';
          // Was `@${c.username}` -- the header/name label is meant to be a
          // plain display name, same as the contact list row right above
          // this (contactListRow, which already shows c.username with no
          // "@"). This just brings the chat header, incoming-call screen,
          // and person profile header in line with that.
        ...
        // LinkedIn-style: name/preview on the left, and two round icon
        // buttons on the right -- an outlined gray X to ignore and an
        // outlined royal-blue check to accept -- instead of the old
        // stacked text pill buttons.
        ...
        // LinkedIn-style: a single outlined "Pending" pill on the right
        // (rather than a filled amber label + separate Cancel button) --
        // tapping it asks to confirm withdrawing the invitation, same as
        // tapping "Pending" on a sent LinkedIn invite.
        ...
        function cancelSentRequest(id){
          ...
          // FIX: this used to remove the row and save state immediately,
          // with no check on whether the backend cancel actually
          // succeeded -- so if it silently failed (see the RLS/RPC note in
          // cancelMyConnectionRequestTo), the UI showed it as cancelled
          // for good, then the request quietly came back as "Pending"
          // again on the next refresh. Now the row only stays removed
          // (and only gets saved) once the cancel is confirmed; on
          // failure it's put back and marked pending again.
        ...
        async function subscribeToConnectionUpdates(){
          ...
          const onConnectionChange = () => {
            Promise.all([
              ...
              // Run after those settle so convoArrays/sentRequestConvos are
              // already up to date when we re-derive connected/requestSent
              // for any open network list.
        ...
        async function reconcileUnreadMessages(){
          ...
          if (!myId) return;
          try {
            // Previously this only asked "is there ANY message ever sent to me
            // in this convo that's still read:false on the server?" -- which
            // means a single message whose "mark as read" write never landed
            // (e.g. it lost the race with a network drop, or got rejected)
            // left that convo flagged unread forever, even after I'd since
            // replied. A convo's badge should reflect whether the most
            // recent message is an unread one addressed to me, not whether
            // some earlier message is still stuck read:false. So instead we
            // walk messages newest-first and, per convo, use whichever
            // happened last: if I sent it, the convo is read (I replied); if
            // it was sent to me, its own read flag decides.
        ...
        async function reconcileConvoOrder(){
          ...
          try {
            ...
            latestByConvo.forEach((row, convoId) => {
              ...
              // Trust the server's own "latest row per convo" query as
              // ground truth rather than gating on a locally-cached
              // _lastMsgAt marker -- that marker was only ever written by
              // this same function, so a preview updated locally elsewhere
              // (e.g. right after sending, via updateConvoPreview) never
              // advanced it, which could make a genuinely newer message
              // look "not newer" than a stale cached value and get
              // skipped. Only bail out if there's nothing to change.
        ...
        async function subscribeToIncomingMessages(){
          ...
          messagesChannel = sb.channel('messages:' + myId)
            ...
            // "Delete for everyone" can be triggered by either side of a conversation, so this
            // user's client needs to hear about a DELETE whether they were the sender or the
            // recipient of the row that got removed -- a single filter can only match one column,
            // so we listen on both.
        ...
        function messageActionSheetHTML(){
          // "Delete for everyone" only makes sense (and is only ever allowed by the DB's
          // delete policy) for a message the current user actually sent -- offering it on
          // messages someone else sent you just fails silently at the database and makes
          // it look like deletion is broken.
        ...
        // Same delete-for-me/delete-for-everyone actions as the mobile
        // long-press sheet above, but as a small anchored dropdown next to
        // the "..." button instead of a bottom sheet, since long-press
        // isn't a discoverable gesture with a mouse.
        ...
        function deleteActionMessageForEveryone(){
          ...
          const idx = msgs.findIndex(m => m.time === time);
          if (idx === -1) return;
          // Only the sender of a message is allowed to delete it for everyone -- the DB's
          // delete policy should enforce this too, but check here as well so we don't remove
          // it locally only to have the remote delete silently rejected.
              ...
              if (query) {
                query.then(({ error, count }) => {
                  if (error) {
                    ...
                    // put the message back locally so this device doesn't disagree with the DB,
                    // and let the person know it didn't actually delete for everyone.
        ...
        // A shared post's caption now travels with it in one tappable card
        // (thumbnail + author + caption) instead of a bare text line, so
        // the recipient can go straight to the post itself.
              ...
              ${items.map(sp => `
                <button type="button" onclick="openSharedPostFromChat(${JSON.stringify(sp.postId)})" class="block text-left overflow-hidden bg-white" style="max-width:230px;width:100%;border-radius:16px;border:1px solid rgba(0,0,0,0.08);">
                  ...
                  <div style="padding:8px 12px 10px;">
                    ...
                    ${sp.caption ? `<div class="text-[12.5px] text-gray-700 mt-0.5" style="display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;">${escapeHtml(sp.caption)}</div>` : ''}
        ...
        // Persistent cache of downloaded documents' actual bytes (not just
        // the "done" status), so reopening one doesn't need a fresh
        // network fetch. That matters because navigator.share() only
        // works within a short window after the tap that triggered it --
        // an awaited network round-trip first can eat that window, which
        // silently makes share() fail and fall back to just re-downloading
        // the file instead of handing it to an app. An IndexedDB read is
        // fast enough to stay inside that window.
        ...
        // Previously this was only attempted on phones/tablets, on the
        // assumption that desktop's Web Share just opens the OS "Share"
        // charm rather than an app-opener. But that's not universally true
        // (Windows' share pane does list apps like Word/Acrobat that can
        // open the file), and gating on device type meant desktop users
        // never even got a chance -- they always fell straight through to
        // a forced download. canShareFiles() above is already a real
        // feature-detection check (navigator.share + canShare with this
        // exact file), so trust that instead of second-guessing it by
        // device class.
        ...
        async function convoDownloadDocument(encodedUrl, encodedName){
          ...
          if (current && current.status === 'done') {
            ...
            // Try the fast local cache first -- an IndexedDB read is quick
            // enough that navigator.share() below still lands inside the
            // "was just tapped" window browsers require, unlike a network
            // fetch, which can lose that window and silently fall back to
            // a plain download instead of the share sheet.
            ...
            // Not in the local cache (cleared, or downloaded before this
            // feature existed) -- fall back to fetching it, though on a
            // slow connection this can lose the share gesture and end up
            // re-downloading instead of opening directly.
          ...
          try {
            ...
            // Keep the real Content-Type from the response on the chunked
            // path -- new Blob(chunks) with no `type` produces a blob that
            // reports application/octet-stream no matter what the file
            // actually is. That broke two things downstream: the share
            // sheet often won't match an app to an untyped file, and
            // isBrowserPreviewableMime() would never recognize even a
            // plain PDF opened through this path.
            ...
            convoUpdateDocChipDOM(url);
            // Route the freshly-downloaded file through the same
            // open/share logic as a repeat tap, instead of unconditionally
            // forcing a save-to-disk. Previously this first tap always
            // ended here with a forced <a download> click no matter what
            // -- so even on a phone with Word installed, the very first
            // tap never offered to open it, only ever saved it, and you
            // had to tap a second time to get the share sheet.
            convoOpenDocumentBlob(blob, name);
            return;
          } catch (err) {
