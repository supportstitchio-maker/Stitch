        ...
        // Who's actually on a collaboration call right now vs. who's
        // still being rung -- only relevant for group calls, since a
        // 1:1 call has nobody else to list.
        function callParticipantsSummary(){
          const meta = convoMeta[callState.convoId];
          if (!meta) return 'Just you';
          const others = (meta.members || []).filter(m => !m.mine);
          if (!others.length) return 'Just you';
          return others.map(m => {
            const entry = Object.values(callPeers).find(e => e.userId === m.otherUserId);
            const status = entry && entry.connected
              ? 'On the call'
              : (callState.pendingRingTargets && callState.pendingRingTargets.has(m.otherUserId) ? 'Ringing…' : 'Not joined');
            return `${m.name} — ${status}`;
          }).join('\n');
        }

        function showCallParticipants(){
          alert(callParticipantsSummary());
        }
        ...
        // callParticipantsSummary/showCallParticipants (the participants
        // button in callHTML) for who's still ringing.
        function groupCallStageHTML(meta, isVideo, showLocalVideo){
          ...
          const localTile = `
            <div class="relative rounded-xl overflow-hidden bg-black flex items-center justify-center" style="aspect-ratio:1;">
              ${showLocalVideo
                ? `<video id="call-local-video" autoplay playsinline muted class="w-full h-full object-cover" style="transform:scaleX(-1);"></video>`
              ...
              <div class="absolute bottom-1.5 left-1.5 text-[10px] font-bold uppercase tracking-wide text-white/85 bg-black/40 rounded-full px-2 py-0.5">You</div>
            </div>`;
          ...
          const remoteTiles = connected.map(e => {
            ...
            return `
              <div class="relative rounded-xl overflow-hidden bg-black flex items-center justify-center" style="aspect-ratio:1;">
                ...
                <div class="absolute bottom-1.5 left-1.5 text-[10px] font-bold uppercase tracking-wide text-white/85 bg-black/40 rounded-full px-2 py-0.5 truncate max-w-[80%]">${escapeHtml(m.name)}</div>
              </div>`;
          }).join('');
            ...
            <div id="call-screen" class="flex-1 flex flex-col text-gray-800" style="padding-top:20px;background:#ffffff;overflow-y:auto;">
              ...
              <div class="flex-shrink-0 pt-4 flex justify-center" style="padding-bottom:20px;">
                <div class="flex items-center gap-2 rounded-full py-1.5 px-2 shadow-md" style="background:#f2f2f2;">
                  ${isGroup ? `<button onclick="showCallParticipants()" title="Participants" class="w-9 h-9 rounded-full bg-white flex items-center justify-center text-gray-700 shadow-sm">${IconBold('dots','w-4 h-4')}</button>` : ''}
