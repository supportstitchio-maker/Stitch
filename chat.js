          ...
          return `
            <div class="flex-1 flex flex-col text-white" style="padding-top:20px;background:linear-gradient(160deg, ${NAVY}, ${ROYAL});">
              <div class="flex-1 flex flex-col items-center justify-center px-6">
                ...
                <div class="text-sm text-white/70">Incoming ${isVideo ? 'video' : 'voice'} call${info.groupName ? ` in "${escapeHtml(info.groupName)}"` : ''}…</div>
              </div>
        ...
        function minimizeCall(fromPopState){
          ...
          // Hide the suggestion pill while a minimized call bubble is on
          // screen -- both float in the same bottom corner area, and the
          // call bubble is the thing that actually needs to stay reachable
          // and unobstructed here.
          const suggestionPill = document.getElementById('suggestion-pill');
          if (suggestionPill) {
            suggestionPill.style.display = 'none';
            if (typeof suggestionPillOpen !== 'undefined' && suggestionPillOpen) toggleSuggestionPill();
          }
        }
        ...
        function resumeCall(){
          ...
          // Restore the suggestion pill to whatever visibility the current
          // tab normally gives it (hidden on Classroom, visible elsewhere)
          // now that the call bubble it was hidden for is gone.
          const suggestionPill = document.getElementById('suggestion-pill');
          if (suggestionPill) suggestionPill.style.display = (typeof currentTab !== 'undefined' && currentTab === 2) ? 'none' : '';
          returnToCallScreen();
        }
        ...
        function updateMinimizedCallBanner(){
          ...
          // No name label anymore -- the bubble is small and icon-first
          // (like WhatsApp's floating call chip), so it just shows the
          // live call duration, or a short status while still connecting.
        ...
        // ---- Drag-to-reposition for the minimized call bubble (mirrors
        // WhatsApp's floating call chip: pick it up and move it anywhere
        // on screen; a plain tap without moving re-opens the call) ----
        let mcDrag = null;
        let mcWasDragged = false;

        function mcPointerDown(e){
          // Let the hang-up badge handle its own click untouched --
          // don't start a drag/tap gesture that would also try to
          // resume the call the instant the finger lifts off it.
          if (e.target.closest && e.target.closest('.call-min-hangup')) return;
          const banner = document.getElementById('call-minimized-banner');
          if (!banner) return;
          ...
          try { e.currentTarget.setPointerCapture(e.pointerId); } catch (err) { /* not supported -- drag still works via document-level pointermove */ }
        }

        function mcPointerMove(e){
          if (!mcDrag || e.pointerId !== mcDrag.pointerId) return;
          const dx = e.clientX - mcDrag.startX;
          const dy = e.clientY - mcDrag.startY;
          // Small, deliberate threshold so a slightly-shaky tap (very
          // common on a phone screen) still registers as "resume the
          // call" instead of silently doing nothing because it read as
          // an accidental drag.
          if (!mcWasDragged && (Math.abs(dx) > 6 || Math.abs(dy) > 6)) mcWasDragged = true;
          if (!mcWasDragged) return;
          const banner = document.getElementById('call-minimized-banner');
          if (!banner) return;
          const margin = 8;
          const maxRight = window.innerWidth - banner.offsetWidth - margin;
          const maxBottom = window.innerHeight - banner.offsetHeight - margin;
          banner.style.right = Math.min(maxRight, Math.max(margin, mcDrag.origRight - dx)) + 'px';
          banner.style.bottom = Math.min(maxBottom, Math.max(margin, mcDrag.origBottom - dy)) + 'px';
        }

        function mcPointerUp(e){
          const wasHangup = e.target.closest && e.target.closest('.call-min-hangup');
          const dragged = mcWasDragged;
          mcDrag = null;
          mcWasDragged = false;
          if (wasHangup || dragged) return;
          resumeCall();
        }
