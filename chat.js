          ...
          return `
            <div class="flex-1 flex flex-col text-white" style="padding-top:20px;background:linear-gradient(160deg, ${NAVY}, ${ROYAL});">
              ...
              <div class="flex-shrink-0 flex items-center justify-center" style="gap:0.5px;padding-bottom:calc(env(safe-area-inset-bottom, 0px) + 10px);">
                <button onclick="declineIncomingCall()" title="Decline" class="rounded-full flex items-center justify-center shadow-lg" style="width:76px;height:76px;background:#ef4444;">${Icon('phoneHangup','w-7 h-7')}</button>
                <button onclick="acceptIncomingCall()" title="Accept" class="rounded-full flex items-center justify-center shadow-lg" style="width:76px;height:76px;background:#10b981;">${Icon(isVideo ? 'video' : 'phoneOutline','w-7 h-7')}</button>
              </div>
            </div>`;
        }
