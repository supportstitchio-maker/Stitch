        ...
        async function loadIncomingConnectionRequests(){
          ...
          try {
            ...
            // Requests that were sitting here as pending but aren't in
            // this pending set anymore -- the sender cancelled it (see
            // cancelMyConnectionRequestTo in overlays.js), or it was acted
            // on from another device -- so drop them locally too, instead
            // of leaving a cancelled request looking live in Requests/
            // Notifications until this account happens to act on it.
        ...
        // Pushes a "Missed call" entry into the Notifications bell tab
        // (addNotif, in overlays.js) -- separate from logCallEvent above,
        // which only writes the "Missed call" system line inside the
        // conversation itself. Only called for calls that actually went
        // unanswered (rang out, or the caller hung up before we picked
        // up) -- a call we actively declined isn't a *missed* call, so
        // declineIncomingCall's manual-decline path and messageIncomingCall
        // don't call this.
        // Tapping the notification jumps straight into that conversation,
        // same as tapping a message notification does (see notifTap in
        // overlays.js).
        ...
        // The caller's side of hanging up before we ever picked up (see
        // endCall's cancel broadcast) -- separate from 'response' because
        // 'response' is us telling *them* our decision (declined/busy/
        // missed) on a call they placed, while 'cancel' is them telling
        // *us* they've called it off on a call we're still ringing for.
        ...
        // The caller hung up (or gave up on ringing us in) before we ever
        // accepted or declined -- stop the ringtone/vibration and dismiss
        // the incoming-call sheet the same way declining does, but
        // without sending a 'response' back: there's no longer a call on
        // the other end to respond to. Logged as missed, same as letting
        // it ring out, since from our side that's exactly what happened.
        ...
        function declineIncomingCall(isTimeout, fromPopState){
          ...
          // Only ringing out unanswered counts as "missed" for the bell
          // notification -- actively hitting Decline already tells the
          // person via the call screen, so it doesn't also need a
          // Notifications entry.
        ...
        function openConversation(id){
          ...
          // Open straight to the bottom of the log -- the most recent
          // messages -- instead of leaving it scrolled to wherever the
          // fresh render happens to start (the top). Runs after layout
          // settles so scrollHeight reflects the just-inserted content.
        ...
        // Jumps the open conversation's message log straight to the
        // bottom (the most recent message) -- used by the floating
        // down-arrow button below and by the "New messages" case.
        ...
        // Shows a floating down-arrow over the message log once the user
        // scrolls up away from the bottom, so they can jump straight back
        // to the most recent message instead of scrolling all the way
        // down by hand. Hides again once they're back near the bottom.
        // Re-run every time the conversation overlay is (re)opened, since
        // openOverlay('conversation') rebuilds #convo-log from scratch.
        ...
        // Also wired into the same modalBackStack every other modal uses
        // (see pushModalBackHandler in overlays.js): previously this had
        // only the top-right X, no back arrow, and didn't push any
        // history, so the phone/browser back button skipped straight past
        // it and left the conversation (or the app) instead of just
        // closing the viewer.
        ...
        // Same idea as Stitch Bot's syncAIClassKeyboardInset (see
        // courses.js): mobile browsers resize the *visual* viewport (not
        // the layout viewport) when the keyboard opens, so this tracks
        // that inset and pins the composer bar right above the keyboard
        // instead of leaving it stranded behind it. The overlay's own
        // `bottom` transition (set in openOverlay for kind==='conversation')
        // is what turns these updates into a smooth slide rather than an
        // instant jump.
        ...
        function syncConvoKeyboardInset(){
          ...
          // Gap between the composer and the keyboard: 10px once the
          // keyboard is up, 20px the rest of the time (matching the
          // breathing room every other closed composer keeps above the
        ...
        // Tapping into the message box: pin the composer above the
        // keyboard immediately rather than waiting on the visualViewport
        // 'resize' event, which can lag behind the keyboard's own slide-up
        ...
        // Leaving the message box: only settle back down once the
        // keyboard has actually finished closing (the viewport can still
        // briefly report itself as covered right as blur fires), so give
        // it a beat before re-checking.
          ...
          if (sb) {
            ...
            if (!answering) {
              ...
              if (isGroup) {
                ...
                // Tracked the same way a group ring's targets are (see
                // above) purely so endCall knows who's still ringing and
                // needs a cancel if we hang up before they answer (see
                // endCall's cancel broadcast) -- handleCallResponse
                // already tolerates a single-entry set fine, since
                // deleting the one entry just brings it to 0 same as if
        ...
        function endCall(remoteInitiated){
          ...
          stopCallLocalStream();
          if (!remoteInitiated) {
            // Anyone still in callState.pendingRingTargets hasn't
            // answered yet -- they're sitting on their own incoming-call
            // sheet with no idea we just hung up (the WebRTC hangup
            // signal below only reaches people who already joined the
            // call:<convoId> room, which ringing-but-not-yet-accepted
            // people never did). Tell them directly so their screen
            // dismisses immediately instead of ringing the full 45s out
            // to a "missed call" on their end.
            if (callState.pendingRingTargets) {
