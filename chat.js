        ...
        function acceptRequest(id){
          ...
          primaryConvos.unshift({ ...c, preview: c.preview || 'You are now connected', unread: true, read: false });
          queueSaveUserState();
          renderInboxTab();
        ...
        // Messages used to only ever be pushed into this browser's own
        // in-memory conversationMessages map, with a fake setTimeout'd
        // "them" reply standing in for the other person -- nothing was
        // ever actually sent anywhere, so the other account never saw it.
        // This adds a real `messages` table plus a Realtime subscription:
        // sending writes a row addressed to the other account's real user
        // id, and every signed-in account listens for rows addressed to
        // itself so a message shows up on the recipient's screen right
        ...
        // other end (e.g. a collaboration you made up a name for) keep
        // using the old simulated quick-reply so they still feel alive --
        // see sendConvoMessage below.
        //
        // Note: only message text is synced for now, not file/voice-note
        // attachments -- those still only show up in your own browser
        // (same as before). Making those live too would need a Storage
        // bucket for chat attachments, a separate change.
        //
        ...
        async function sendMessageRemote(convoId, otherUserId, text){
          const sb = getSupabaseClient();
        ...
        async function loadConversationHistory(convoId){
          ...
          try {
            ...
            conversationMessages[convoId] = data.map(r => ({ from: r.sender_id === myId ? 'me' : 'them', text: r.text }));
        ...
        // visible next time they open Messaging.
        function handleIncomingMessage(row){
          ...
          conversationMessages[convoId].push({ from: 'them', text: row.text || '' });
          queueSaveUserState();
          if (activeConvoId === convoId) {
            ...
            if (c) { c.unread = true; c.read = false; c.preview = row.text || c.preview; c.time = 'now'; }
        ...
        const defaultConvoThreads = {};

        function openConversation(id){
          activeConvoId = id;
          if (!conversationMessages[id]) {
            if (defaultConvoThreads[id]) {
              conversationMessages[id] = defaultConvoThreads[id].slice();
            } else {
              const meta = convoMeta[id] || {};
              conversationMessages[id] = [{ from: 'them', text: meta.preview || 'Hi!' }];
            }
          }
        ...
        function handleConvoVoiceNoteStop(){
          ...
          reader.onload = () => sendConvoVoiceNote(reader.result, durationSecs);
        ...
        function sendConvoVoiceNote(dataUrl, durationSecs){
          if (!activeConvoId) return;
          ...
          setTimeout(() => {
            if (activeConvoId !== convoIdAtSend) return;
            conversationMessages[convoIdAtSend].push({ from: 'them', text: convoQuickReply() });
            const log2 = document.getElementById('convo-log');
            if (log2) { log2.innerHTML = convoLogHTML(); log2.scrollTop = log2.scrollHeight; }
            queueSaveUserState();
          }, 1000);
        }
        ...
        function sendConvoMessage(){
          ...
          if (meta && meta.otherUserId) {
            ...
            // instead of faking a reply nobody sent.
            if (text) sendMessageRemote(convoIdAtSend, meta.otherUserId, text);
            return;
          }

          // No known backend user on the other end (a locally-created
          // collaboration, or a leftover demo thread) -- keep the
          // simulated quick reply so those threads still feel alive.
          setTimeout(() => {
            if (activeConvoId !== convoIdAtSend) return;
            conversationMessages[convoIdAtSend].push({ from: 'them', text: convoQuickReply() });
            const log2 = document.getElementById('convo-log');
        ...
        function convoQuickReply(){
          const replies = ["Sounds good!", "Got it, thanks 🙏", "Sure, let me check and get back to you.", "👍", "Okay, noted!", "Perfect, see you then."];
        ...
        let callState = { convoId: null, type: 'audio', connected: false, seconds: 0, muted: false, speaker: false, camOff: false, mediaError: null };
        ...
        async function startCall(id, type){
          if (!id) return;
          clearCallTimers();
          stopCallLocalStream();
          teardownCallSignaling();
          callState = { convoId: id, type, connected: false, seconds: 0, muted: false, speaker: false, camOff: false, mediaError: null };
          openOverlay('call');
        ...
        function endCall(remoteInitiated){
          ...
          callState = { convoId: null, type: 'audio', connected: false, seconds: 0, muted: false, speaker: false, camOff: false, mediaError: null };
        ...
        function callHTML(){
          const meta = convoMeta[callState.convoId] || { icon: 'user', avatarBg: 'bg-gray-100', name: 'Unknown' };
          const isVideo = callState.type === 'video';
          const statusText = callState.connected ? `<span id="call-timer">${formatCallTime(callState.seconds)}</span>` : (isVideo ? 'Video calling…' : 'Calling…');
          return `
