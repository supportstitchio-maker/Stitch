const inboxFilters = [['general','General',0],['collaborations','Collaborations',0],['requests','Requests',3]];
        ...
        // ---- Contacts list + avatar rendering ---- Renders a photo if one is set, but falls back
        // to the plain silhouette placeholder (same as a contact with no photo at all) if that
        function avatarMediaHTML(photo, icon, iconSizeClass){
          if (!photo) return Icon(icon || 'user', iconSizeClass);
          return `<img src="${photo}" class="w-full h-full object-cover" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';">
        ...
        async function startLastActiveHeartbeat(){
          ...
          lastActiveHeartbeatTimer = setInterval(beat, 45000);
            ...
            <div class="flex-1 overflow-y-auto px-5 py-5">
              ...
              <div class="flex items-center gap-4 mb-6">
                <div class="relative flex-shrink-0" onclick="triggerNewCollabPhotoUpload()">
                  <div class="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center text-gray-400 overflow-hidden">${newCollabPhoto ? `<img src="${newCollabPhoto}" class="w-full h-full object-cover">` : Icon('users','w-6 h-6')}</div>
        ...
        async function createNewCollaboration(){
          ...
          // Photo is optional and can be skipped -- unlike the name, it's never required to create the group.
          const contacts = newCollabContactSource().filter(c => newCollabSelected.has(c.id));
          ...
          if (createdRemotely && convoMeta[id]) {
            ...
            // myMember above) -- backfill it with the real id now that we have one, so the "Creator"
            const mineEntry = (convoMeta[id].members || []).find(m => m.mine);
            if (mineEntry && myRealId) mineEntry.otherUserId = myRealId;
            queueSaveUserState();
            // Push each invited person so they're notified even if their app is closed -- the realtime
            // listener above covers them while they're online, this covers them while they're not.
        ...
        async function confirmAddCollabMembers(){
          ...
          collabAddMembersRemote(id, meta.name, newContacts).then(ok => {
            ...
            // members themselves -- both as a push (in case their app is closed) and an in-app
        ...
        // Whoever opens this link gets asked to join or cancel (see
        // guestCollabJoinHTML) rather than being added silently -- mirrors
        // checkPendingLectureLinkJoin's guest-prompt pattern in courses.js.
        ...
        function guestCollabJoinHTML(){
          ...
          return `
            <div id="guest-collab-join-screen" class="flex-1 flex flex-col items-center justify-center px-8 text-center" style="padding-top:var(--top-safe-pad);">
              <div class="w-16 h-16 rounded-full flex items-center justify-center mb-5 overflow-hidden" style="background:rgba(30,144,255,0.1);color:${NAVY};">${guestCollabPreview && guestCollabPreview.photo ? `<img src="${guestCollabPreview.photo}" class="w-full h-full object-cover">` : Icon('users','w-7 h-7')}</div>
          ...
          return `
            <div id="inbox-titlebar-row" class="relative flex items-center justify-between">
              ...
              ` : `
                ...
                <span id="inbox-username-el" class="font-bold text-base font-display grad-text">${escapeHtml(profileData.username)}</span>
        ...
        // "General" should only count unread DMs -- collaborations get their own badge, so folding
        // collabConvos into this too (as the old code did via unreadMessageCount()) double-counted
        ...
        // used to only refresh #inbox-list -- the filter chips above it kept showing the old
        ...
        // Whether *I* currently have an outgoing connection request pending to this person -- kept
        // as a live check against sentRequestConvos (rather than a flag baked into whatever list
        ...
        // reality whenever a connection is accepted, sent, cancelled, or declined -- without this,
        ...
        function inboxContent(){
          ...
          if (inboxFilter === 'requests') {
            ...
            if (!incoming.length && !sent.length) {
              return `<div class="bg-white p-8 text-center text-gray-400 text-sm">${emptyLabel('No requests.')}</div>`;
            }
        ...
        function convoDisplayName(c){
          if (!c) return '';
          // Was `@${c.username}` -- the header/name label is meant to be a plain display name, same
          // as the contact list row right above this (contactListRow, which already shows c.username
        ...
        // two round icon buttons on the right -- an outlined gray X to ignore and an outlined
        function requestRow(c){
          ...
          return `
            <div class="flex items-center gap-5 px-5 py-4 bg-white">
              <div class="${avatarBg} rounded-full flex items-center justify-center text-gray-600 flex-shrink-0 overflow-hidden cursor-pointer" style="width:52px;height:52px;" onclick="openPersonProfileForConvo('${id}')">${avatarInner}</div>
              <div class="flex-1 min-w-0 cursor-pointer" onclick="openPersonProfileForConvo('${id}')">
                <div class="font-semibold text-sm text-gray-800 truncate">${name}</div>
                <div class="text-xs text-gray-500 mt-0.5 truncate">${preview || 'Wants to connect with you'}</div>
                <div class="text-[11px] text-gray-400 mt-0.5">${time}</div>
              </div>
              <div class="flex items-center gap-2 flex-shrink-0">
                <button onclick="event.stopPropagation(); rejectRequest('${id}')" aria-label="Ignore" class="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 bg-white" style="border:1.5px solid #d1d5db;color:#6b7280;">${IconBold('close','w-4 h-4')}</button>
                <button onclick="event.stopPropagation(); acceptRequest('${id}')" aria-label="Accept" class="w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0 bg-white" style="border:1.5px solid ${ROYAL};color:${ROYAL};">${IconBold('check','w-4 h-4')}</button>
              </div>
            </div>`;
        }

        // ---- Requests I've sent (pending, cancellable) ---- LinkedIn-style: a single outlined
        // "Pending" pill on the right (rather than a filled amber label + separate Cancel button)
        function sentRequestRow(c){
          const { id, icon, avatarBg, name, preview, time, photo } = c;
          const avatarInner = avatarMediaHTML(photo, icon, 'w-6 h-6');
          return `
            <div class="flex items-center gap-5 px-5 py-4 bg-white">
              <div class="flex-shrink-0 rounded-full ${avatarBg} flex items-center justify-center text-gray-600 overflow-hidden cursor-pointer" style="width:52px;height:52px;" onclick="openPersonProfileForConvo('${id}')">${avatarInner}</div>
              <div class="flex-1 min-w-0 cursor-pointer" onclick="openPersonProfileForConvo('${id}')">
                <div class="font-semibold text-sm text-gray-800 truncate">${name}</div>
                <div class="text-xs text-gray-500 mt-0.5 truncate">${preview || 'Invitation sent'}</div>
                <div class="text-[11px] text-gray-400 mt-0.5">${time}</div>
              </div>
              <button onclick="event.stopPropagation(); confirmWithdrawSentRequest('${id}','${escapeForJsAttr(name)}')" class="text-xs font-semibold px-4 py-1.5 rounded-full flex-shrink-0 bg-white" style="border:1.5px solid #d1d5db;color:#6b7280;">Pending</button>
            </div>`;
        }
        ...
        function cancelSentRequest(id){
          ...
          // the backend cancel actually succeeded -- so if it silently failed (see the RLS/RPC note
        ...
        async function loadIncomingConnectionRequests(){
          ...
          try {
            ...
            // manually accepted (see acceptRequest) -- nothing is auto-accepted.
        ...
        async function loadMyAcceptedOutgoingRequests(){
          ...
          try {
            ...
            newRows.forEach(row => {
              ...
              // The request is no longer pending -- drop the "Cancel Request" pill wherever it's
              // shown (their posts in the feed, Discover, their profile) and refresh the feed so
              // it doesn't keep showing the stale pending state now that we're connected.
        ...
        async function loadMyPendingOutgoingRequests(){
          const sb = getSupabaseClient();
          if (!sb) return;
          try {
            const me = await getCachedAuthUser();
            ...
            if (removedAny) {
              queueSaveUserState();
              renderInboxTab();
              // A request that's no longer pending was either accepted or declined -- either way
              // the "Cancel Request" pill on their posts/profile is stale now, so clear it and
              // let the feed re-render with the up-to-date state.
        ...
        // opts.excludeUserIds) that something changed -- someone was added, removed, or left --
        ...
        async function loadConversationHistory(convoId){
          ...
          try {
            ...
            // state is independent) -- a plain "every row for this convo_id" query also pulls in every
            ...
            conversationMessages[convoId] = data
              ...
              // member -- collapse those back into a single line here instead of showing (and counting)
              .filter((m, idx, arr) => {
        ...
        // Same as markConvoMessagesRead but for a group/collaboration conversation, where messages
        // arrive from several different senders rather than one fixed otherUserId -- so this just
        ...
        async function reconcileUnreadMessages(){
          ...
          try {
            ...
            // still read:false on the server?" -- which means a single message whose "mark as read"
            ...
            (data || []).forEach(row => {
              if (decided.has(row.convo_id)) return;
              decided.add(row.convo_id);
              if (row.sender_id === myId) {
                // I sent the most recent message in this convo -- read.
              } else if (row.recipient_id === myId && !row.read) {
            ...
            convoArrays().flat().forEach(c => {
              ...
              if (shouldBeUnread && (!c.unread || c.read)) {
                ...
                // The locally-restored snapshot can lag behind the server -- e.g. the app closed before
        ...
        // was signed out. Conversations normally bump themselves to the top the moment a message
        function convoAttachmentPreviewText(attachments){
          const first = attachments && attachments[0];
          if (first && first.type === 'stitch/post') return first.caption ? `Shared a post: "${first.caption.slice(0,60)}"` : 'Shared a post';
        ...
        async function reconcileConvoOrder(){
          ...
          try {
            ...
            latestByConvo.forEach((row, convoId) => {
              ...
              // on a locally-cached _lastMsgAt marker -- that marker was only ever written by this same
              if (!rowTime || rowTime === c._lastMsgAt) return;
              ...
              const previewText = isSysEvent ? row.text.slice(SYS_MSG_PREFIX.length)
                ...
                : (Array.isArray(row.attachments) && row.attachments.length) ? convoAttachmentPreviewText(row.attachments)
                : (row.text || 'Attachment');
        ...
        function handleIncomingMessageDelete(row){
          ...
          // REPLICA IDENTITY FULL, so convo_id can be missing here -- fall back to searching every
        ...
        // controlling whether non-creator members are allowed to add new people. If that column
        ...
        async function subscribeToCollabMembership(){
          ...
          collabMembersChannel = sb.channel('collab-members:' + myId)
            ...
            // Keeps name/photo/creator/permission changes live for every member -- without this,
            // someone else renaming the group, the creator toggling "let members add people", or
            ...
            // fire for *my own* membership changing), these aren't filtered -- so every other member's
        ...
        // e.g. tapping "Message" on someone's profile from Discover, or from a notification/feed
        ...
        function closeConversationOverlay(fromPopState){
          ...
          // "Message" on someone's Discover profile). Passing fromPopState through here used to let
        ...
        function openConversation(id){
          ...
          if (meta && (meta.otherUserId || isGroupConvo)) {
            ...
            // -- the bug was this call being gated on meta.otherUserId, which only 1:1 DMs have.
        ...
        function convoLastSeenText(meta){
          ...
          // priority even in a collaboration -- this used to be checked *after* the group-name early
        ...
        function sendConvoVoiceNote(dataUrl, durationSecs, blob){
          ...
          uploadConvoVoiceToStorage(blob, convoIdAtSend).then(url => {
            ...
            // previously this branch didn't exist at all, so a voice note in a collaboration only ever
        ...
        function convoLogHTML(){
          ...
          return msgs.map(m => {
            ...
            const sharedPostsHTML = convoSharedPostAttachmentsHTML(sharedPosts);
        ...
        function messageActionSheetHTML(){
          ...
          // policy) for a message the current user actually sent -- offering it on messages someone
          const msgs = (messageActionConvoId && conversationMessages[messageActionConvoId]) || [];
          const msg = msgs.find(m => m.time === messageActionTime);
          const canDeleteForEveryone = !!msg && msg.from === 'me';
          return `
        ...
        // ---- Desktop message-options dropdown ("..." button) ---- Same delete-for-me/delete-for-
        // everyone actions as the mobile long-press sheet above, but as a small anchored dropdown
        ...
        function toggleDesktopMessageMenu(key, evt){
          ...
          const menuWidth = 190;
          const menuHeightEstimate = (canDeleteForEveryone ? 100 : 60) + 40;
        ...
        function deleteActionMessageForEveryone(){
          ...
          const idx = msgs.findIndex(m => m.time === time);
          if (idx === -1) return;
          // Only the sender of a message is allowed to delete it for everyone -- the DB's delete
          // policy should enforce this too, but check here as well so we don't remove it locally
          if (msgs[idx].from !== 'me') return;
              ...
              if (query) {
                query.then(({ error, count }) => {
                  if (error) {
                    ...
                    // The remote delete didn't go through (RLS rejection, network error, etc.) -- put the
                    // message back locally so this device doesn't disagree with the DB, and let the person
            ...
            <div class="px-5 pb-3 flex-shrink-0 border-b border-gray-100" style="padding-top:var(--top-safe-pad);">
              <div class="flex items-center gap-4">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  ${msg ? `<div class="text-xs text-gray-400 truncate">${forwardMessagePreviewText(msg)}</div>` : ''}
                </div>
              </div>
            </div>
            <div class="flex-1 overflow-y-auto px-5">
        ...
        async function forwardMessageTo(targetConvoId){
          ...
          // that only plays back on the sender's own device, so don't forward it until it's a real URL.
        ...
        function convoSharedPostAttachmentsHTML(items){
          if (!items || !items.length) return '';
          return `
            <div class="flex flex-col gap-1.5">
              ${items.map(sp => `
                <button type="button" onclick="openSharedPostFromChat(${JSON.stringify(sp.postId)})" class="block text-left overflow-hidden bg-white" style="max-width:230px;width:100%;border-radius:16px;border:1px solid rgba(0,0,0,0.08);">
                  ${sp.thumbnail ? `<img src="${sp.thumbnail}" alt="" style="width:100%;height:140px;object-fit:cover;display:block;">` : ''}
                  <div style="padding:8px 12px 10px;">
                    <div class="text-[11px] font-semibold" style="color:${ROYAL};">${escapeHtml(sp.authorName || 'Stitch member')}'s post</div>
            ...
            <div class="flex flex-col gap-1.5">
              ${images.map(f => {
                ...
                return `<div class="relative" style="max-width:220px;width:100%;">
                  <img src="${src}" alt="${escapeHtml(name)}" onclick="openConvoImageViewer('${encodeURIComponent(src)}')" class="cursor-pointer" style="max-width:220px;max-height:280px;width:100%;border-radius:16px;object-fit:cover;display:block;">
                  <button type="button" onclick="event.stopPropagation();convoDownloadDocument('${encodeURIComponent(src)}','${encodeURIComponent(name)}')" aria-label="Download photo" class="convo-media-action-btn" style="top:8px;right:8px;"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12"/><path d="M7 10l5 5 5-5"/><path d="M5 21h14"/></svg></button>
                </div>`;
              }).join('')}
            </div>`;
        }
        ...
        // progress download back to its idle icon. Which documents have already been downloaded
        ...
        // reopening one doesn't need a fresh network fetch. That matters because navigator.share()
        ...
        function convoDocIconHTML(state, mine){
          ...
          if (state && (state.status === 'downloading' || state.status === 'opening')) {
            // Spins in place until the download/open resolves -- simpler and more honest than a
            // progress ring when the server doesn't send a content-length to compute % from.
              ...
              ${files.map(f => {
                ...
                return `
                  <div class="flex items-center gap-2 ${mine ? 'bg-white/15' : 'bg-black/5'} rounded-xl px-2.5 py-1.5">
                    <button type="button" id="${id}-icon" data-mine="${mine ? '1' : '0'}" onclick="event.stopPropagation(); convoDownloadDocument('${encodeURIComponent(f.url)}','${encodeURIComponent(f.name || '')}')" class="w-10 h-10 flex items-center justify-center flex-shrink-0" aria-label="Download">${convoDocIconHTML(state, mine)}</button>
                    <button type="button" id="${id}" onclick="openConvoDocViewer('${encodeURIComponent(f.url)}','${encodeURIComponent(f.name || '')}')" class="flex flex-col items-start text-left flex-1 min-w-0">
        ...
        // instead of navigating to it in a new tab -- which for non-previewable types like .docx
        ...
        // on desktop, tapping a document should just open it in the browser (or download it),
        // not hand it off to an OS-level share dialog.
        function isMobileDevice(){
          if (navigator.userAgentData && typeof navigator.userAgentData.mobile === 'boolean') return navigator.userAgentData.mobile;
          return /Android|iPhone|iPad|iPod/i.test(navigator.userAgent || '');
        }

        // Browsers can only render a handful of types inline -- PDFs, images, plain text. Opening
        // anything else (docx, pptx, xlsx...) as a blob in a new tab just dumps the file's raw
        ...
        async function convoOpenDocumentBlob(blob, name){
          ...
          // "open" surface otherwise. On desktop it should just open in the browser (or download),
          // not hand the file off to an OS share dialog -- so only try Web Share on mobile.
          if (isMobileDevice() && canShareFiles(file)) {
            try {
              ...
              if (err && err.name === 'NotAllowedError') {
                ...
                // a user tap ("transient activation"). A slow network fetch in convoDownloadDocument above
          ...
          if (isBrowserPreviewableMime(type)) {
            ...
            // Not something the browser can render. Navigating straight to a blob: URL of an
            // unrecognized binary type (previous version of this code) does NOT reliably download it
        ...
        // First tap: fetch + save to disk (with progress). Any tap after that -- this session or a
        // later one, since the "done" status above is persisted -- hands the file straight to the
        async function convoDownloadDocument(encodedUrl, encodedName){
          ...
          if (current && current.status === 'done') {
            ...
            // Try the fast local cache first -- an IndexedDB read is quick enough that
            // navigator.share() below still lands inside the "was just tapped" window browsers
            ...
            // Not in the local cache (cleared, or downloaded before this feature existed) -- fall back
            // to fetching it, though on a slow connection this can lose the share gesture and end up
          ...
          try {
            ...
            // Keep the real Content-Type from the response on the chunked path -- new Blob(chunks)
            // with no `type` produces a blob that reports application/octet-stream no matter what the
            ...
            // instead of unconditionally forcing a save-to-disk. Previously this first tap always
            convoOpenDocumentBlob(blob, name);
            return;
          } catch (err) {
        ...
        // ---- In-app document viewer (docx / pdf / txt / csv) ---- Tapping a document chip
        // previously always went through convoDownloadDocument() first, which tries
          ...
          return `
            <div class="flex flex-col gap-1.5">
              ${videos.map(f => {
                ...
                return `<div class="relative" style="max-width:220px;width:100%;">
                  <video preload="metadata" muted playsinline disablepictureinpicture controlslist="nodownload noremoteplayback nofullscreen noplaybackrate" oncontextmenu="return false;" src="${src}" onclick="openConvoVideoViewer('${encodeURIComponent(src)}')" class="cursor-pointer" style="max-width:220px;max-height:280px;width:100%;border-radius:16px;display:block;object-fit:cover;"></video>
                  <div onclick="openConvoVideoViewer('${encodeURIComponent(src)}')" class="absolute" style="top:0;left:0;right:0;bottom:0;display:flex;align-items:center;justify-content:center;pointer-events:none;">
                  ...
                  <button type="button" onclick="event.stopPropagation();convoDownloadDocument('${encodeURIComponent(src)}','${encodeURIComponent(name)}')" aria-label="Download video" class="convo-media-action-btn" style="top:8px;right:8px;"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3v12"/><path d="M7 10l5 5 5-5"/><path d="M5 21h14"/></svg></button>
                </div>`;
              }).join('')}
            </div>`;
        }
        ...
        function openConvoVideoViewer(encodedSrc){
          ...
          modal.innerHTML = `
            ...
            <video src="${src}" controls autoplay playsinline disablepictureinpicture controlslist="nodownload noremoteplayback noplaybackrate" onclick="event.stopPropagation();" style="max-width:96vw;max-height:96vh;width:auto;height:auto;object-fit:contain;"></video>`;
          document.body.appendChild(modal);
        ...
        function sendConvoMessage(){
          ...
          // -- videos in particular -- have something to render from (convoVideoAttachmentsHTML
        ...
        // callBannerPos remembers the card's dropped size/position (in px) once the person has
        // dragged it at least once during the current call, so re-minimizing later in the same
        // call keeps it where they put it; a fresh call always starts back at the default top
        // bar (cleared in endCall()). callBannerDrag holds bookkeeping for the drag in progress,
        // and is null whenever no drag is happening.
        ...
        // Re-applies a remembered dropped position/size to the banner -- called whenever the
        // banner is shown again (minimizeCall) so it reappears where it was left instead of
        // snapping back to the default top bar.
        ...
        // Nothing moves yet -- callBannerDragMove only kicks in once the pointer has actually
        // travelled past CALL_BANNER_DRAG_THRESHOLD, so a plain tap still just calls resumeCall()
        // via callBannerRowClick below.
        ...
        // Keeps a dropped banner on screen if the viewport changes size (e.g. rotating the
        // device) instead of letting it get stranded off-screen.
        ...
        // The name/timer row's click handler -- a real drag suppresses the click that a browser
        // fires right after pointerup on the same element, so dragging the card never also
        // re-opens the full call screen.
        ...
        function callStageHTML(){
          ...
          if (callState.connected) {
            ...
            // wide/tall viewports (e.g. a laptop browser window). A capped square keeps it looking
            // like a normal avatar card on any screen size, matching the pre-connect view below.
            return `
        ...
        // 'local' for the person's own tile, a callPeers key for someone else, or null for the grid.
        // Reset whenever a call starts/ends so it never carries over into the next one.
        ...
        function groupCallStageHTML(meta, isVideo, showLocalVideo){
          ...
          // Nobody else has joined yet -- show one big view (like a 1:1 call) instead of a
          // single small grid tile; it switches to the grid the moment someone connects.
          if (tileCount <= 1) {
            return `
