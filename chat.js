        ...
        function handleIncomingMessage(row){
          ...
          if (activeConvoId === convoId) {
            ...
            updateConvoPreview(convoId, row.text || 'Attachment');
          } else {
        ...
        function sendConvoVoiceNote(dataUrl, durationSecs){
          ...
          if (log) { log.innerHTML = convoLogHTML(); log.scrollTop = log.scrollHeight; }
          queueSaveUserState();
          updateConvoPreview(activeConvoId, '🎤 Voice message');
          // Note: voice notes aren't synced to the other account yet --
          // only message text is (see sendMessageRemote/sendConvoMessage
          // below). That'd need a Storage bucket for chat attachments,
