        ...
        // convoMeta[id].photo is whatever was cached at the time this
        // conversation was created (e.g. off an accepted connection
        // request, see loadIncomingConnectionRequests/loadMyAcceptedOutgoingRequests).
        // Contacts created before profile photos existed, or restored
        // from an older saved user_state, can be missing it entirely --
        // which left the chat header and call screen stuck on the plain
        // icon forever even for someone who has since set a real photo.
        // Rather than only trusting that cached copy, pull the *current*
        // photo straight from public_profiles the first time a
        // conversation/call for that contact is opened, patch convoMeta
        // (and the matching inbox-row entry, so it's cached correctly
        // from now on and doesn't need re-fetching every time), and
        // repaint whatever avatar is currently on screen.
        async function refreshConvoAvatarFromProfile(convoId){
          const meta = convoMeta[convoId];
          if (!meta || !meta.otherUserId || meta.photo) return;
          ...
          try {
            ...
            convoMeta[convoId].photo = row.photo;
            const entry = convoArrays().flat().find(c => c.id === convoId);
            if (entry) entry.photo = row.photo;
            queueSaveUserState();
        ...
        // Falls back to the old local count only for the instant before
        // the first fetch has resolved (e.g. very first paint).
        ...
        async function refreshNetworkCount(){
          ...
          } catch (e) { console.warn('Refreshing network count failed:', e); }
        }
        ...
        function convoRow(c){
          ...
          if (!convoMeta[id]) convoMeta[id] = { icon, avatarBg, name, preview, photo };
        ...
        async function loadIncomingConnectionRequests(){
          ...
          try {
            ...
            newRows.forEach(row => {
              ...
              convoMeta[convoId] = { icon: entry.icon, avatarBg: entry.avatarBg, name: entry.name, photo: entry.photo, preview: 'Wants to connect with you', otherUserId: row.from_user };
        ...
        async function loadMyAcceptedIncomingRequests(){
          ...
          try {
            ...
            newRows.forEach(row => {
              ...
              convoMeta[convoId] = { icon: entry.icon, avatarBg: entry.avatarBg, name: entry.name, photo: entry.photo, preview: entry.preview, otherUserId: row.from_user };
        ...
        async function loadMyAcceptedOutgoingRequests(){
          ...
          try {
            ...
            newRows.forEach(row => {
              ...
              primaryConvos.unshift(entry);
              convoMeta[convoId] = { icon: entry.icon, avatarBg: entry.avatarBg, name: entry.name, photo: entry.photo, preview: entry.preview, otherUserId: row.to_user };
        ...
        //   create policy "Recipients can mark messages as seen" on messages
        ...
        //   -- And a public bucket for file attachments (photos, videos,
        //   -- PDFs, slides) themselves, same idea as chat-voice-notes:
        //   --   create a Storage bucket named "chat-files", set Public.
        //
        ...
        // And if it already existed from before read receipts were added:
        ...
        //   create policy "Recipients can mark messages as seen" on messages
        //     for update using (auth.uid() = recipient_id) with check (auth.uid() = recipient_id);
        // And if it already existed from before file attachments synced:
        ...
        async function sendMessageRemote(convoId, otherUserId, text, voiceUrl, voiceDuration, attachments){
          ...
          try {
            ...
            sendPushTo(otherUserId, {
              ...
              body: text ? (text.length > 120 ? text.slice(0, 117) + '...' : text) : (attachments && attachments.length ? 'Sent an attachment' : 'Sent a voice message'),
              tag: 'msg-' + convoId,
        ...
        async function loadConversationHistory(convoId){
          ...
          try {
            ...
            const { data, error } = await sb.from(MESSAGES_TABLE)
              .select('sender_id,text,voice_url,voice_duration,created_at,seen,attachments')
              .eq('convo_id', convoId)
            ...
            conversationMessages[convoId] = data.map(r => ({
              ...
              ts: r.created_at,
              seen: r.sender_id === myId ? !!r.seen : undefined,
            }));
            // "Last seen" for the header: the most recent activity we
            // actually know about from the other person, i.e. the
            // timestamp of the last message *they* sent -- not a vague
            // "recently" placeholder (see convoLastSeenText).
            const theirLast = [...data].reverse().find(r => r.sender_id !== myId);
            if (theirLast && convoMeta[convoId]) convoMeta[convoId].lastSeenAt = theirLast.created_at;
            // Mark whatever they sent us as seen now that we've actually
            // loaded/opened the thread -- this is what flips the sender's
            // ticks from single-gray to double-blue on their end (see
            // markMessagesSeenRemote and the UPDATE listener in
            // subscribeToIncomingMessages).
            markMessagesSeenRemote(convoId);
          } catch (e) { /* best effort -- keep whatever local history already existed */ }
        }

        // Flags every message addressed to us in this conversation as
        // seen. This is what the other person's client picks up (via the
        // UPDATE postgres_changes listener in subscribeToIncomingMessages)
        // to flip their own sent-message ticks from a single gray check
        // to a double blue one -- the same "read receipt" behavior as
        // WhatsApp.
        async function markMessagesSeenRemote(convoId){
          const sb = getSupabaseClient();
          if (!sb) return;
          try {
            const myId = await getCurrentUserId();
            if (!myId) return;
            await sb.from(MESSAGES_TABLE)
              .update({ seen: true, seen_at: new Date().toISOString() })
              .eq('convo_id', convoId)
              .eq('recipient_id', myId)
              .eq('seen', false);
          } catch (e) { /* best effort -- ticks just won't update this round */ }
        }

        // Fired when Supabase tells us one of OUR sent messages just got
        // marked seen by the recipient. We don't track individual row ids
        // client-side for optimistically-sent messages, so rather than
        // matching a single row this just flips every 'me' message in
        // that thread to seen -- correct in practice, since the recipient
        // can only mark messages seen by having the whole thread open in
        // front of them (same "read up to here" behavior WhatsApp shows).
        function markLocalMessagesSeen(convoId){
          const msgs = conversationMessages[convoId];
          if (!msgs || !msgs.length) return;
          let changed = false;
          msgs.forEach(m => { if (m.from === 'me' && !m.seen) { m.seen = true; changed = true; } });
          if (changed && activeConvoId === convoId) {
        ...
        async function subscribeToIncomingMessages(){
          ...
          messagesChannel = sb.channel('messages:' + myId)
            ...
            // The other side just opened the thread and marked our
            // messages seen (see markMessagesSeenRemote) -- flip our
            // local ticks from sent to seen for that conversation.
            .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: MESSAGES_TABLE, filter: `sender_id=eq.${myId}` }, (payload) => {
              if (payload.new && payload.new.seen) markLocalMessagesSeen(payload.new.convo_id);
            })
            .subscribe();
        }
        ...
        async function collabCreateRemote(id, name, photo, memberContacts){
          ...
          try {
            ...
            const { error: mErr } = await sb.from(COLLAB_MEMBERS_TABLE).insert(rows);
            if (mErr) console.warn('Collaboration members did not sync to Supabase:', mErr);
            memberContacts.forEach(c => {
              if (!c.otherUserId) return;
              sendPushTo(c.otherUserId, { title: 'New collaboration', body: `You were added to "${name}"`, tag: 'collab-' + id, data: { kind: 'collabInvite', convoId: id } });
            });
            return true;
        ...
        async function collabAddMembersRemote(id, name, newContacts){
          ...
          try {
            ...
            if (rows.length) {
              const { error } = await sb.from(COLLAB_MEMBERS_TABLE).insert(rows);
          ...
          try {
            ...
            collabs.forEach(row => {
              ...
              if (!collabConvos.some(c => c.id === id)) {
                ...
                if (!conversationMessages[id]) conversationMessages[id] = [{ from: 'them', text: `You were added to "${displayName}".` }];
              } else {
                const entry = collabConvos.find(c => c.id === id);
        ...
        // One Realtime subscription per signed-in session, listening for
        // this account being added as a member to any collaboration --
        // mirrors subscribeToIncomingMessages above so being added to a
        // group shows up live, the same way an incoming message does.
        ...
        async function subscribeToCollabMembership(){
          ...
          collabMembersChannel = sb.channel('collab-members:' + myId)
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: COLLAB_MEMBERS_TABLE, filter: `user_id=eq.${myId}` }, async () => {
        ...
        function logCallEvent(convoId, text){
          ...
          conversationMessages[convoId].push({ from: 'system', text });
          queueSaveUserState();
        ...
        function incomingCallHTML(){
          ...
          return `
            <div class="flex-1 flex flex-col text-white" style="padding-top:20px;background:linear-gradient(160deg, ${NAVY}, ${ROYAL});">
              ...
              <div class="flex-shrink-0 flex items-center justify-center" style="gap:20px;padding-bottom:calc(env(safe-area-inset-bottom, 0px) + 20px);">
        ...
        function handleIncomingMessage(row){
          ...
          const attachments = row.attachments && row.attachments.length ? row.attachments : undefined;
          ...
          queueSaveUserState();
          const previewText = isVoice ? '🎤 Voice message'
            : attachments ? `📎 ${attachments.length > 1 ? attachments.length + ' attachments' : attachments[0].name}`
            : (row.text || 'Attachment');
          // They just messaged us, so this is the freshest "last seen"
          // evidence we have for them -- updates the chat header (see
          // convoLastSeenText) once we're looking at this thread.
          if (convoMeta[convoId]) convoMeta[convoId].lastSeenAt = row.created_at || new Date().toISOString();
          // If we're already sitting in this exact conversation when the
          // message lands, it counts as read right away -- tell the
          // sender so their ticks turn blue instead of waiting for us to
          // close and reopen the thread.
          if (activeConvoId === convoId) markMessagesSeenRemote(convoId);
          ...
          if (activeConvoId === convoId) {
            ...
            if (typeof addNotif === 'function') {
              ...
              addNotif({
                ...
                name: meta.name || (c && c.name) || 'New message',
                message: previewText,
                convoId,
        ...
        // Formats a message/activity timestamp the way real chat apps do:
        // just the time for today, "Yesterday" + time, or a full
        // day/date + time once it's further back -- instead of a vague,
        // permanently-stuck "recently".
        function formatMsgTime(ts){
          ...
          const dateStr = d.toLocaleDateString([], sameYear ? { month: 'short', day: 'numeric' } : { month: 'short', day: 'numeric', year: 'numeric' });
          return `${dateStr}, ${timeStr}`;
        }

        function convoLastSeenText(meta){
          if (meta.icon === 'users') return 'Group';
          if (!meta.lastSeenAt) return 'last seen recently';
          return `last seen ${formatMsgTime(meta.lastSeenAt)}`;
        }
        ...
        function conversationHTML(){
          ...
          return `
            ...
            <div class="p-3 border-t border-gray-100 flex-shrink-0 flex items-center gap-1.5">
              <input type="file" id="convo-file-input" accept="image/*,video/*,.pdf,.ppt,.pptx" multiple class="hidden" onchange="handleConvoFileSelect(event)">
            ...
            <div class="px-4 pt-3 flex gap-2 flex-wrap">
              ${pendingConvoAttachments.map((f,i) => `
                <div class="flex items-center gap-1.5 bg-gray-100 rounded-full pl-1 pr-2 py-1 text-xs text-gray-700">
                  <div class="w-6 h-6 bg-white rounded-full flex items-center justify-center text-[${NAVY}] flex-shrink-0 overflow-hidden">${f.dataUrl && f.type && f.type.startsWith('image/') ? `<img src="${f.dataUrl}" class="w-full h-full object-cover">` : Icon(convoFileIcon(f.type),'w-3.5 h-3.5')}</div>
        ...
        // Reads a File as a data URL so it can preview instantly in the
        // pending-attachment strip and in the sender's own chat bubble
        // (see sendConvoMessage) without waiting on the network -- the
        // same pattern already used for voice notes
        // (handleConvoVoiceNoteStop) and post media elsewhere in the app.
        ...
        function handleConvoFileSelect(event){
          ...
          files.forEach(f => {
            const okType = allowed.test(f.type) || /\.(pdf|pptx?|jpe?g|png|gif|webp|mp4|mov|webm|m4v)$/i.test(f.name);
            if (!okType) return;
            const entry = { name: f.name, type: f.type, file: f };
            pendingConvoAttachments.push(entry);
            // Images/videos get a real preview (not just a filename chip)
            // as soon as the browser finishes reading them -- re-render
            // both the pending strip and, once it's actually in the log,
            // the sent bubble itself.
            if (f.type && (f.type.startsWith('image/') || f.type.startsWith('video/'))) {
              readFileAsDataUrl(f).then(dataUrl => {
                ...
                // If this file was already sent before the read finished
                // (rare, but possible on a fast double-tap), the same
                // entry object is sitting inside that sent message's
                // attachments too -- refresh the log so the thumbnail
                // pops in instead of staying stuck on the filename/icon
                // fallback until the next unrelated re-render.
                const log = document.getElementById('convo-log');
                if (log && activeConvoId) log.innerHTML = convoLogHTML();
              }).catch(() => {});
            }
          });
          event.target.value = '';
        ...
        // Chat file attachments (photos, videos, PDFs, slides) upload
        // here so the recipient can actually fetch and view them --
        // same pattern as CHAT_VOICE_BUCKET/uploadConvoVoiceToStorage
        // above. Requires a public bucket named CHAT_FILES_BUCKET to
        // exist in the configured Supabase project; if upload fails or
        // Supabase isn't configured, the file still shows for the sender
        // (via the local data URL captured above) but never reaches the
        // other account -- same fallback behavior as voice notes.
        const CHAT_FILES_BUCKET = 'chat-files';

        async function uploadConvoFileToStorage(file, convoId){
          const sb = getSupabaseClient();
          if (!sb) return null;
          try {
            const safeName = file.name.replace(/[^a-zA-Z0-9._-]/g, '_');
            const path = `${convoId}/${Date.now()}-${safeName}`;
            const { error } = await sb.storage.from(CHAT_FILES_BUCKET).upload(path, file, { contentType: file.type || 'application/octet-stream' });
            if (error) { console.warn('Attachment upload failed:', error.message); return null; }
            const { data } = sb.storage.from(CHAT_FILES_BUCKET).getPublicUrl(path);
            return (data && data.publicUrl) || null;
          } catch (err) {
        ...
        function sendConvoVoiceNote(dataUrl, durationSecs, blob){
          if (!activeConvoId) return;
          conversationMessages[activeConvoId].push({ from: 'me', voice: { src: dataUrl, duration: durationSecs }, ts: new Date().toISOString(), seen: false });
          const log = document.getElementById('convo-log');
          if (log) { log.innerHTML = convoLogHTML(); log.scrollTop = log.scrollHeight; }
          queueSaveUserState();
        ...
        function convoLogHTML(){
          ...
          return msgs.map(m => {
            if (m.from === 'system') {
              return `<div class="flex justify-center"><div class="text-[11px] text-gray-400 bg-gray-50 rounded-full px-3 py-1">${escapeHtml(m.text)}</div></div>`;
            }
            const timeStr = formatMsgTime(m.ts);
            return m.from === 'them' ? `
            <div class="flex items-start gap-3">
              <div class="w-8 h-8 ${meta.avatarBg} rounded-2xl flex items-center justify-center text-gray-600 flex-shrink-0 overflow-hidden cursor-pointer" onclick="openPersonProfileForConvo('${activeConvoId}')">${avatarInnerHTML(meta,'w-4 h-4')}</div>
              <div class="flex flex-col items-start" style="max-width:75%;">
                <div class="bg-gray-100 rounded-2xl px-4 py-2.5 text-sm text-gray-700">${m.voice ? convoVoiceNoteHTML(m.voice, false) : m.text}</div>
                ${timeStr ? `<div class="text-[10px] text-gray-400 mt-1 ml-1">${timeStr}</div>` : ''}
              </div>
            </div>` : `
            <div class="flex items-start justify-end gap-3">
              <div class="flex flex-col items-end" style="max-width:75%;">
                <div class="text-white rounded-2xl px-4 py-2.5 text-sm" style="background:linear-gradient(135deg, rgba(65,105,225,0.55), rgba(65,105,225,0.35));">${m.voice ? convoVoiceNoteHTML(m.voice, true) : (convoAttachmentsHTML(m.attachments) + m.text)}</div>
                <div class="flex items-center gap-1 mt-1 mr-1">
                  ${timeStr ? `<span class="text-[10px] text-gray-400">${timeStr}</span>` : ''}
                  ${Icon(m.seen ? 'tickDouble' : 'tick', `w-3 h-3 ${m.seen ? 'text-blue-400' : 'text-gray-400'}`)}
                </div>
              </div>
            </div>`;
          }).join('');
        ...
        function convoVoiceNoteHTML(voice, mine){
          ...
          return `
            <div class="flex items-center gap-2" style="min-width:170px;">
              ...
              <audio id="${uid}" controls preload="metadata" src="${voice.src}" style="height:32px;max-width:180px;flex:1;" onerror="document.getElementById('${uid}').outerHTML='<span class=&quot;text-xs italic ${mine ? 'text-white/80' : 'text-gray-400'}&quot;>Couldn&#39;t load voice note</span>'"></audio>
              <span class="text-xs flex-shrink-0 ${mine ? 'text-white/80' : 'text-gray-500'}">${formatCallTime(voice.duration)}</span>
            </div>`;
        }

        // Renders attachments already sent/received in a bubble. Images
        // and videos show as real, tappable media (not just a filename +
        // icon chip like before, which made them impossible to actually
        // view) -- tapping opens the full-size file in a new tab. Other
        // file types (PDF, slides) keep the icon/filename row, which
        // already doubles as a working download link.
          ...
          return `
            <div class="flex flex-col gap-1.5 mb-1.5">
              ${attachments.map(f => {
                ...
                if (src && f.type && f.type.startsWith('image/')) {
                  return `<img src="${src}" alt="${escapeHtml(f.name)}" class="rounded-xl cursor-pointer" style="max-width:220px;max-height:220px;object-fit:cover;" onclick="window.open('${src}','_blank')">`;
                }
                if (src && f.type && f.type.startsWith('video/')) {
                  return `<video src="${src}" controls preload="metadata" class="rounded-xl" style="max-width:220px;max-height:220px;"></video>`;
                }
                return `
                <a href="${src || '#'}" target="_blank" rel="noopener" class="flex items-center gap-2 bg-white/15 rounded-xl px-2.5 py-1.5 ${src ? '' : 'pointer-events-none opacity-70'}">
        ...
        function sendConvoMessage(){
          ...
          conversationMessages[activeConvoId].push({ from: 'me', text, attachments, ts: new Date().toISOString(), seen: false });
          ...
          const meta = convoMeta[convoIdAtSend];
          const deliver = (remoteAttachments) => {
            if (meta && meta.otherUserId) {
              // Real connection: actually deliver it via Supabase so the
              // other account receives it (see subscribeToIncomingMessages).
              // Previously this only fired when there was text, so an
              // attachment-only message (a photo with no caption) never
              // reached the other person at all -- it just sat in the
              // sender's own browser looking sent.
              if (text || (remoteAttachments && remoteAttachments.length)) {
                sendMessageRemote(convoIdAtSend, meta.otherUserId, text, undefined, undefined, remoteAttachments);
              }
            } else if (meta && meta.icon === 'users' && Array.isArray(meta.members) && (text || (remoteAttachments && remoteAttachments.length))) {
              ...
              // way a direct message would arrive.
              meta.members.forEach(m => {
                if (m.mine || !m.otherUserId) return;
                sendMessageRemote(convoIdAtSend, m.otherUserId, text, undefined, undefined, remoteAttachments);
              });
            }
          ...
          if (!attachments.length) { deliver(); return; }

          // Photos/videos/files: upload each to Storage first (the
          // recipient has nothing to fetch until the file actually
          // exists there), then send the message row pointing at the
          // resulting public URLs. If Storage isn't configured or an
          // upload fails, that one file just stays visible to the sender
          // only (via its local data URL) -- same graceful fallback as
          // voice notes.
          Promise.all(attachments.map(f =>
            uploadConvoFileToStorage(f.file, convoIdAtSend).then(url => ({ name: f.name, type: f.type, url }))
          )).then(uploaded => {
            const okOnes = uploaded.filter(u => u.url);
            if (okOnes.length < uploaded.length) {
              flashConvoMicMessage(uploaded.length === 1 ? 'Attachment not sent (upload failed)' : 'Some attachments not sent (upload failed)');
            }
            deliver(okOnes);
          });
        }
            ...
            if (isVideo) {
              return `
                <div class="w-full h-full rounded-2xl overflow-hidden bg-black relative">
                  ...
                  <div class="absolute top-3 left-3 text-[11px] font-bold uppercase tracking-wide text-white/85 bg-black/40 rounded-full px-2.5 py-1 truncate max-w-[70%]">${escapeHtml(meta.name)}</div>
