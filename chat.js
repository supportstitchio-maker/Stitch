        ...
        function updateMinimizedCallBanner(){
          ...
          const label = document.getElementById('call-minimized-label');
          const timer = document.getElementById('call-minimized-timer');
          if (label && meta) label.textContent = convoDisplayName(meta);
          if (timer) timer.textContent = callState.statusOverride || (callState.connected ? formatCallTime(callState.seconds) : (callState.type === 'video' ? 'Video calling…' : 'Calling…'));
        }
