              ...
              <div class="divide-y divide-gray-100 pb-24">
                ${contacts.map(c => `
                  <button onclick="toggleNewCollabContact('${c.id}')" class="w-full flex items-center gap-3 py-3 text-left">
                    ...
                    <div class="w-5 h-5 rounded-full border-2 ${newCollabSelected.has(c.id) ? `bg-[${ROYAL}] border-[${ROYAL}]` : 'border-gray-300'} flex items-center justify-center flex-shrink-0">${newCollabSelected.has(c.id) ? Icon('check','w-3.5 h-3.5 text-white') : ''}</div>
                  </button>`).join('')}
              </div>
            </div>
            ${count ? `
        ...
        function convoLogHTML(){
          ...
          return msgs.map(m => {
            ...
            const bubbleHTML = hasBubbleContent ? (
              mine
                ? `<div class="text-white rounded-2xl rounded-br-md px-4 py-2.5 text-sm" style="background:linear-gradient(135deg, rgba(65,105,225,0.55), rgba(65,105,225,0.35));">${convoFileAttachmentsHTML(files, mine)}${escapeHtml(m.text)}</div>`
                : `<div class="bg-gray-100 rounded-2xl rounded-bl-md px-4 py-2.5 text-sm text-gray-700">${convoFileAttachmentsHTML(files, mine)}${escapeHtml(m.text)}</div>`
            ) : '';
