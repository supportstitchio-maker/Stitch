const inboxFilters = [['general','General',0],['collaborations','Collaborations',0],['requests','Requests',3]];
          ...
          return `
            <div class="flex flex-col gap-1.5">
              ${videos.map(f => {
                ...
                return `<div class="relative" style="max-width:220px;width:100%;">
                  <video controls preload="metadata" src="${src}" style="max-width:220px;max-height:280px;width:100%;border-radius:16px;display:block;"></video>
                  <button type="button" onclick="event.stopPropagation();openConvoVideoViewer('${encodeURIComponent(src)}')" aria-label="Expand video" class="convo-media-action-btn" style="top:8px;right:44px;"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M8 3H5a2 2 0 00-2 2v3"/><path d="M16 3h3a2 2 0 012 2v3"/><path d="M8 21H5a2 2 0 01-2-2v-3"/><path d="M16 21h3a2 2 0 002-2v-3"/></svg></button>
        ...
        function openConvoVideoViewer(encodedSrc){
          ...
          modal.innerHTML = `
            ...
            <video src="${src}" controls autoplay playsinline onclick="event.stopPropagation();" style="max-width:96vw;max-height:96vh;width:auto;height:auto;object-fit:contain;"></video>`;
          document.body.appendChild(modal);
