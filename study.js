        ...
        function studyContent(){
          ...
          if (studySub === 'exams') return `
            ...
            <div onclick="openPracticeTestsOverlay()" class="rounded-2xl p-6 mb-4 cursor-pointer" style="background-image:linear-gradient(135deg, rgba(30,144,255,0.07) 0%, rgba(65,105,225,0.07) 100%);background-color:#ffffff;">
              ...
              <div class="text-sm text-gray-500 mb-4">Exam-ready questions sourced from your uploaded resources.</div>
            ...
            <div class="rounded-2xl p-4 mb-4" style="background-image:linear-gradient(135deg, rgba(30,144,255,0.07) 0%, rgba(65,105,225,0.07) 100%);background-color:#ffffff;">
              ...
              <div class="challenge-arena-actions">
            ...
            ${(!isTrialActive() && practiceCompleted && flashcardsCompleted) ? `
              <div class="rounded-2xl p-5 text-center" style="background-image:linear-gradient(135deg, rgba(30,144,255,0.09) 0%, rgba(65,105,225,0.09) 100%);background-color:#ffffff;">
                <div class="text-sm font-semibold text-gray-600 mb-3">Finished everything in your practice set? Unlock a bigger pool of questions and flashcards from the same uploads.</div>
                <button onclick="openSubscriptionUpgradeModal()" class="w-full text-white font-semibold px-6 py-3 rounded-2xl" style="background:rgba(30,144,255,0.5);">${Icon('plus','w-4 h-4 inline-block mr-1 -mt-0.5')} Add More Questions &amp; Flashcards</button>
              </div>
            ` : ''}
          `;
        }
        ...
        function getAllResourcesDeck(){
          ...
          // Capped to the student's current flashcard plan limit (15
          // free / 50 Pro / 70 Pro Plus) across ALL uploaded resources
          // combined, not per resource -- see PLAN_LIMITS above.
        ...
        // The free tier's three pools are sized differently (15
        // questions / 15 flashcards / 10 Challenge Arena questions);
        ...
        const PLAN_LIMITS = {
          free: { questions: 15, flashcards: 15, challenge: 10 },
        ...
        const PLAN_PRICING = {
          ...
          proPlus: { amount: 25, currency: 'GHS', label: 'Pro Plus', cycle: 'Monthly', periodDays: 30 }
        };
        let userSubscription = { plan: 'free', expiresAt: null }; // plan: 'free' | 'pro' | 'proPlus'
        ...
        // trialEndsAt is a ms-epoch timestamp, set once via
        // ensureTrialInitialized (called from ensureUserStateLoaded in
        // core.js, right after login) and persisted like any other
        // per-account field -- see collectUserState/applyUserState.
        const TRIAL_DAYS = 7;
        let trialEndsAt = null;

        // Anchors the trial to the account's real Supabase signup date
        // (user.created_at) rather than "now", so calling this more than
        // once, or on an older account that's missing this field for
        // some other reason, never hands out a fresh trial it isn't
        // owed -- an account whose real signup was more than 7 days ago
        // simply comes back with an already-expired trialEndsAt.
        function ensureTrialInitialized(user){
          if (trialEndsAt !== null) return; // already set this session (fresh save, or restored from a saved row)
        ...
        function subscriptionPlanCardHTML(planKey){
          ...
          return `
            <div class="rounded-2xl p-5 mb-3 border" style="border-color:rgba(65,105,225,0.25);background:rgba(65,105,225,0.06);">
              <div class="flex items-center justify-between mb-1">
                <div class="text-lg font-bold" style="color:${NAVY};">${p.label}</div>
                <div class="text-lg font-bold" style="color:${NAVY};">${p.currency} ${p.amount}<span class="text-xs font-semibold text-gray-400">/${p.cycle.toLowerCase()}</span></div>
              </div>
              <div class="text-sm text-gray-500 mb-4">${planKey === 'pro'
                ? 'Bumps practice questions, flashcards, and Challenge Arena questions to 50 each across all your uploaded resources. Renews monthly; cancel anytime.'
                : 'Bumps practice questions, flashcards, and Challenge Arena questions to 70 each across all your uploaded resources. Renews monthly; cancel anytime.'}</div>
              <button onclick="submitSubscriptionPurchase('${planKey}')" ${active ? 'disabled' : ''} class="w-full text-white font-semibold py-3 rounded-2xl text-center" style="background:${active ? 'rgba(30,144,255,0.25)' : 'rgba(30,144,255,0.5)'};">${active ? 'Current Plan' : 'Subscribe · ' + p.currency + ' ' + p.amount}</button>
            </div>`;
        }
        ...
        function subscriptionUpgradeHTML(){
          return `
            ...
            <div class="text-xs text-gray-400 leading-relaxed mt-2">Stitch Pro raises your total question/flashcard/Challenge Arena pool to 50 each (all uploads combined); Stitch Pro Plus raises it to 70 each. Both renew monthly and can be cancelled anytime from Profile. When a plan expires without renewing, your account automatically returns to the free pool (15 practice questions, 15 flashcards, 10 Challenge Arena questions) -- your uploads and existing progress are never deleted. Payments are processed securely by Paystack; we never see or store your card details.</div>
          `;
        }
        ...
        async function submitSubscriptionPurchase(planKey){
          ...
          const handler = window.PaystackPop.setup({
            ...
            email,
            amount: Math.round(p.amount * 100), // GHS -> pesewas: Paystack expects the smallest currency unit
            currency: p.currency,
            ref: reference,
            ...
            // table for course-payment charges.
            metadata: { plan: planKey },
            callback: function(response){
              verifyPaystackPaymentAndActivate(planKey, response.reference);
            },
            onClose: function(){
              subscriptionPaymentBusy = false;
        ...
        async function verifyPaystackPaymentAndActivate(planKey, reference){
          try {
            ...
            // Same trust model as course enrollment (finalizeJobApplication
            // in jobs.js): the server only confirms the charge really
            // happened and for the right plan/amount -- activating the
            // plan itself, like enrolling in a course, is this client's
            // own per-account state, saved via queueSaveUserState().
            const p = PLAN_PRICING[planKey];
            userSubscription = { plan: planKey, expiresAt: Date.now() + p.periodDays * 24 * 60 * 60 * 1000 };
            queueSaveUserState();
            subscriptionPaymentBusy = false;
            closeChallengeModal();
            pushInAppNotification(p.label + ' activated', 'Your practice questions, flashcards, and Challenge Arena questions are now capped at ' + PLAN_LIMITS[planKey].questions + ' each, across all your uploads, until this plan renews or expires.');
        ...
        async function generateStudyMaterials(text, resourceName){
          ...
          const system = 'You generate study material for an education app from a single uploaded resource. ' +
            ...
            'Questions: write genuine exam-ready multiple-choice questions in the exact register of a real ' +
            'university past-questions paper -- short, precise stems like "Deductive theory begins with which ' +
            'element?" or "Validity refers to which of the following?", never padded with filler like "According ' +
            'to the text..." or "The passage states...". Each one should test real understanding of a concept, a ' +
            'named principle/theory/term, a formula/procedure, or the ability to classify an example under the ' +
            'right category -- not just "which term matches this definition". If the material is quantitative ' +
            '(statistics, math, accounting, finance), write self-contained worked-problem questions with real ' +
            'numbers and a computable correct answer (e.g. "A sample of n=25 has a mean of 40 and a standard ' +
            'deviation of 5. What is the standard error?"), never a raw, half-finished textbook problem prompt ' +
            'copied from the source with no numbers resolved. Vary the question style across the set instead of ' +
            'reusing one template for every question. Every option must be a short phrase, number, or single term ' +
            '(typically 1-5 words, matching how real exam answer choices read), never a full sentence restating ' +
            'the question. ' +
            'Exactly one option is correct; the other three must be plausible, closely-related distractors from the ' +
            'same conceptual family (e.g. sibling terms, adjacent theories, common mix-ups students actually make) ' +
            '-- never an obviously-wrong filler option. ' +
