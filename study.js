        ...
        async function generateStudyMaterials(text, resourceName){
          ...
          const system = 'You generate study material for an education app from a single uploaded resource. ' +
            ...
            'Questions: phrase each stem the way an exam or tutorial question would, and vary the phrasing across ' +
            'questions instead of reusing one template -- mix styles such as "What is...", "Which of the following best describes...", ' +
            '"Explain which term/concept fits...", "Discuss the statement below and select the correct term", and direct ' +
            'application or scenario-based questions. Do not phrase every question as "Which term matches this definition". ' +
        ...
        function generateStudyMaterialsFallback(text){
          ...
          if (flashcards.length < 4) {
            rawParagraphs.forEach((p, idx) => {
              ...
              // Capped at 3 Title-Case words (was 5): a longer run is far
              // more likely to be a whole slide heading/byline than a
              // clean, reusable study term.
              ...
              if (term && isUsableFallbackTerm(term) && !seen.has(key)) {
                ...
                const def = stripStrayQuotes(defSource.length > 220 ? defSource.slice(0, 220).trim() + '…' : defSource);
          ...
          //
          // Stems rotate through a set of exam-style phrasings instead of
          // repeating the same "Which term matches this definition"
          // template on every question -- reads more like an actual
          // tutorial/exam paper.
          const EXAM_STEM_TEMPLATES = [
            def => 'Which of the following best describes: "' + def + '"?',
