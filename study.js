        ...
        // Cleans up any leftover scroll listener from a previous open of
        // the classroom FAB menu before (re)arming it. Without this, a
        // listener registered while the menu was open stuck around even
        // after the menu was closed by tapping the FAB or the backdrop
        // (attachMenuScrollCloser's own listener only ever removes itself
        // when a scroll actually fires it) -- and since #screen is the
        // one scrollable element reused across every tab, that stale
        // listener could fire later on a totally different screen and
        // flip the classroom menu open again on its own, with no tap
        // involved. This is the same fix already used for the
        // gender/pronouns dropdowns -- see armProfileMenuScrollCloser in
        // profile.js.
        ...
        function toggleStudyFabMenu(){
          ...
          renderStudy();
          // Tapping the FAB replays its rotate morph (see
          // .plus-morph-icon in styles.css / morphPlusIcon in core.js) --
          // same single icon the whole time, it just settles on "+" when
          // the menu is closed and "x" while it's open.
        ...
        // Plain-white "feature card" treatment for the Study tab's Practice
        // Tests / Challenge Arena / Flashcards / upgrade panels, matching
        // the flat white/shadow-sm card language used everywhere else in
        // Study (studyCard/statCard) instead of the old bright gradient
        // banners. Each card gets a decorative illustration (image, an
        // asset path under assets/practice/) bled off the top-right
        // corner at low opacity so it reads as texture behind the text
        // rather than competing with it. onclick is optional since some
        // cards (Challenge Arena) hold several separate buttons instead
        // of being one big tap target.
        ...
        // hands back a matches list). Pro Plus, Pro Max and Platinum all
        // get both halves; Free/Pro have neither -- they still get the
        // local keyword-based fallback scorer in jobs.js, just no Claude call.
        const PLAN_FEATURES = {
        ...
        // One brief line per plan describing what it adds, kept out of
        // the pricing/limits tables above since it's UI copy, not config
        // other code reads.
        // <br> after every full stop so each sentence lands on its own
        // line in the card instead of wrapping as one dense paragraph.
        ...
        // Each subscription plan gets its own fixed color card, sampled
        // straight from that plan's own badge artwork (assets/*.png) so
        // the label/price/pill colors always match the badge sitting on
        // the card -- free's bright green button, pro's gunmetal/silver,
        // pro plus's navy, pro max's gold, platinum's chrome.
        ...
        // Paths fixed to match where these files actually live in
        // assets/ (an assets/subscription/ subfolder doesn't exist --
        // the old paths 404'd, which is why the badges never rendered).
        ...
        // mix-blend-mode:multiply drops that white square out against
        // the card's own white background -- white * white = white, so
        // it vanishes, while the darker/colored pixels stay put -- the
        // same "no visible box" look the Practice/Flashcards cards get
        // (see studyFeatureCard above). No drop-shadow here: a shadow
        // is cast from the image's full rectangle (alpha-less PNG), so
        // it would just redraw the box we're trying to remove.
        //
        // The base box is 10.5rem, but pro-max.png/platinum.png are
        // cropped much tighter around their gear icon than pro.png/
        // pro-plus.png (barely any white margin), so at the same box
        // size they visually read as noticeably bigger than the other
        // badges even though the box itself is identical -- scaled down
        // per-plan here so all four badges look the same size on the
        // page, not just the same bounding box.
        const planBadgeScale = { free: 1, pro: 1, proPlus: 1, proMax: 0.86, platinum: 0.85 };
        // Pro Max's and Platinum's badge art is cropped much tighter than
        // the others (see planBadgeScale note above), so the same -0.5rem
        // edge offset that reads fine on the other cards left those two
        // gear icons sitting flush against, or bleeding past, the card's
        // right edge. Insetting just those two keeps a visible margin
        // like the free/pro/pro-plus cards already have.
        ...
        // Free tier gets the same card treatment as the paid plans (own
        // badge + matching green text/pill) instead of a separate plain
        // "Current Plan" summary box -- this single card now IS that
        // "current plan" slot when free is active, sitting first in the
        // list rather than duplicated in two places. It isn't purchasable
        // so the pill is just a status pill, never a Subscribe action.
        ...
        function subscriptionSettingsHTML(){
          ...
          // "Current Plan" slot is back to the original plain blue
          // summary box -- just states the active plan's name and a
          // one-line description, no badge/colors/pill. The card grid
          // below (with badges + matching colors) lists every plan,
        ...
        // Single plan row inside Settings > Subscription -- same white
        // card + right-aligned badge treatment as subscriptionPlanCardHTML
        // above, plus an "Active" tag next to the current plan. The button
        // is a real, working Subscribe action (submitSubscriptionPurchase),
        // not just a link out to another screen, so a plan can actually be
        // bought or switched right here.
        ...
        // Same auth + rate-limit path as callClaude, but hits the
        // ai-proxy's separate `task: "image"` branch (OpenAI's
        // gpt-image-1 under the hood, not Claude -- see the Edge
        // Function). Returns a data-URL-ready base64 PNG string, or
        // throws the same way callClaude does so callers can reuse the
        // same error handling/fallback pattern.
