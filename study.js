        ...
        async function extractPptxText(file){
          ...
          for (const name of slideFiles) {
            ...
            text += runs.map(r => r.replace(/<a:t>|<\/a:t>/g, '')).join(' ') + '\n';
          }
          return text;
        }
        ...
        async function generateStudyMaterials(text, resourceName){
          ...
          const system = 'You generate study material for an education app from a single uploaded resource. ' +
            ...
            'Flashcard definitions must be written as present-tense, general factual statements (e.g. "is", "means", "describes") -- ' +
            'never in the past tense and never narrating what the source said or did (avoid phrasing like "was", "described", "stated", "occurred"), even if the source text itself uses past tense. ' +
            'Practice questions must read like real exam questions, not flashcard lookups: vary the stems across the set using formats such as ' +
            '"What is [term]?", "Which of the following best describes...", "Explain why...", "Discuss the role/effect of...", "Which of the answers below...". ' +
            'Do not phrase every question as "What term..." or "Which term matches this definition" -- only use a couple of those, and lead most with the concept/scenario itself. ' +
        ...
        function generateStudyMaterialsFallback(text){
          ...
          for (const s of sentences) {
            for (const re of defPatterns) {
              ...
              if (m) {
                ...
                if (term.split(' ').length <= 8 && def.length > 10 && !seen.has(key)) {
                  seen.add(key);
          ...
          // Not enough definition-style sentences found -- pad with plain
          // context cards from the first line of sizeable paragraphs, so
          // the student still has something to review.
          if (flashcards.length < 4) {
            const paragraphs = text.split(/\n+/).map(p => p.trim()).filter(p => p.length > 40);
            for (const p of paragraphs) {
              if (flashcards.length >= 6) break;
              const words = p.split(' ');
              const term = words.slice(0, 6).join(' ').replace(/[.,;:]$/, '');
              const def = p.length > 220 ? p.slice(0, 220).trim() + '…' : p;
              const key = term.toLowerCase();
              if (term.length > 5 && !seen.has(key)) {
                seen.add(key);
                flashcards.push({ term, def });
              }
            }
          ...
          // three other extracted terms are the options.
          // Exam-style stems, cycled across the set so questions don't all
          // read like a flat "term -> definition" lookup.
          const questionStems = [
            def => 'Which of the answers below best describes: "' + def.slice(0, 140) + '"?',
            def => 'What is being described here: "' + def.slice(0, 140) + '"?',
            def => 'Which term best explains the following: "' + def.slice(0, 140) + '"?',
            def => 'Which of the following matches this definition: "' + def.slice(0, 140) + '"?'
          ];
          const questions = [];
          if (flashcards.length >= 4) {
            flashcards.slice(0, 8).forEach((card, i) => {
              const distractors = shuffleArray(flashcards.filter((c, j) => j !== i).map(c => c.term)).slice(0, 3);
              const options = shuffleArray([card.term, ...distractors]);
              questions.push({
                text: questionStems[i % questionStems.length](card.def),
                options,
        ...
        function studyCardFaceHTML(card){
          ...
          return `
            <div class="flip-card-inner">
              <div>
                <div class="text-base font-semibold text-gray-800 leading-snug">${text}</div>
                ${source ? `<div class="text-xs font-semibold mt-2" style="color:${NAVY};">${source}</div>` : ''}
              </div>
            </div>`;
        }
        ...
        function flipStudyCard(){
          ...
          // Swap the face at the animation's midpoint (card edge-on),
          // same beat as the Practice window's flashcard unfold, then
          // let the rest of the .3s animation play out before clearing
          // the class so it can replay next flip.
          setTimeout(() => {
