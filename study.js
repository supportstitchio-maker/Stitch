        ...
        const PLAN_DESCRIPTIONS = {
          ...
          platinum: 'Unlimited questions, flashcards & Stitch Bot.<br>CV filter, matching & recommendations — for a year.'
        };
        ...
        const planBadgeImgStyle = 'position:absolute;right:-0.5rem;top:50%;transform:translateY(-50%);width:10.5rem;height:10.5rem;object-fit:contain;mix-blend-mode:multiply;pointer-events:none;user-select:none;-webkit-user-select:none;';
        ...
        function subscriptionPlanCardHTML(planKey){
          ...
          return `
            <div class="rounded-2xl p-6 mb-4 relative overflow-hidden bg-white" style="box-shadow:0 10px 30px rgba(10,37,64,0.16), 0 3px 8px rgba(10,37,64,0.10);">
              ${badge ? `<img src="${badge}" alt="" class="absolute" style="${planBadgeImgStyle}">` : ''}
        ...
        function freePlanCardHTML(){
          ...
          return `
            <div class="rounded-2xl p-6 mb-4 relative overflow-hidden bg-white" style="box-shadow:0 10px 30px rgba(10,37,64,0.16), 0 3px 8px rgba(10,37,64,0.10);">
              <img src="${planCardBadges.free}" alt="" class="absolute" style="${planBadgeImgStyle}">
              <div class="relative" style="max-width:58%;">
        ...
        function subscriptionSettingsHTML(){
          ...
          // "Current Plan" slot at the top always shows whichever plan
          // is actually active -- the free card only occupies that
          // space while free is the active plan; once a paid plan is
          // active, that plan's own card (own badge/colors) takes over
          // the slot instead, rather than the free card staying pinned
          // there. The list below it is every OTHER plan, for switching
          // -- free included (as the "downgrade" option) whenever a
          // paid plan is the one currently active up top.
          const otherPaidKeys = Object.keys(PLAN_PRICING).filter(k => k !== activePlan);
          return `
            <div class="flex-1 overflow-y-auto">
              ${overlayHeader('Subscription', '20px')}
              <div class="p-5">
                ${activePlan !== 'free' && userSubscription.expiresAt ? `
                  <div class="text-xs text-gray-400 text-center mb-4">Renews/expires ${new Date(userSubscription.expiresAt).toLocaleDateString()}</div>
                ` : ''}
                ${referralCreditToggleHTML()}
                <div class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-3">Current Plan</div>
                ${activePlan === 'free' ? freePlanCardHTML() : subscriptionSettingsPlanRow(activePlan)}
                <div class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 mt-2">${activePlan === 'free' ? 'All Plans' : 'Other Plans'}</div>
                ${activePlan !== 'free' ? freePlanCardHTML() : ''}
                ${otherPaidKeys.map(subscriptionSettingsPlanRow).join('')}
