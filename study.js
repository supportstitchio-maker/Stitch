            ...
            <div class="px-5 pb-5" style="padding-top:20px;">
              <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2 min-w-0">
                  ...
                  <button onclick="openLeaveClassModal()" title="Leave Classroom" class="w-9 h-9 rounded-full bg-red-50 text-red-500 flex items-center justify-center flex-shrink-0">${IconBold('back','w-4 h-4')}</button>` : ''}
                  <h1 class="text-3xl font-bold font-display grad-text truncate">Classroom</h1>
                </div>
                ${(classroomAreaTab === 'classes' && (studySub === 'resources' || studySub === 'exams')) ? `
                <button onclick="openLeaveClassModal()" class="flex items-center gap-1 text-red-500 font-bold text-xs flex-shrink-0">
                  ${IconBold('back','w-3.5 h-3.5')} Leave Classroom
                </button>` : ''}
              </div>
        ...
        function studyContent(){
          ...
          if (studySub === 'exams') return `
            ...
            <div class="rounded-2xl p-4 mb-4" style="background-image:linear-gradient(135deg, rgba(30,144,255,0.07) 0%, rgba(65,105,225,0.07) 100%);background-color:#ffffff;">
              ...
              <button onclick="openInviteFriendModal()" class="w-full flex items-center justify-center gap-2 text-white font-semibold px-6 py-2.5 rounded-2xl" style="background:rgba(30,144,255,0.5);">Invite a Friend</button>
            </div>
        ...
        function generateStudyMaterialsFallback(text){
          ...
          if (flashcards.length >= 4) {
            flashcards.slice(0, 8).forEach((card, i) => {
              ...
              questions.push({
                text: 'Which term matches this definition: "' + card.def.slice(0, 140) + '"?',
                options,
        ...
        function studyCardFaceHTML(card){
          const text = cardFlipped ? card.def : card.term;
          return `
            <div class="flip-card-inner">
              <div>
                <div class="text-xl font-bold text-gray-800">${text}</div>
                ${card.source ? `<div class="text-xs font-semibold mt-2" style="color:${NAVY};">${card.source}</div>` : ''}
              </div>
            </div>`;
        }
