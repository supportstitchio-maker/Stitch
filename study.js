        ...
        // tap target.
        function studyFeatureCard(colorIndex, motifIndex, icon, eyebrow, title, sub, actionsHtml, onclick, extraStyle){
          const [from, to] = classCardPalette[colorIndex % classCardPalette.length];
          const motif = classCardMotifs[motifIndex % classCardMotifs.length];
          return `
            <div ${onclick ? `onclick="${onclick}" class="rounded-2xl p-6 mb-4 relative overflow-hidden cursor-pointer"` : `class="rounded-2xl p-6 mb-4 relative overflow-hidden"`} style="background:linear-gradient(135deg,${from},${to});${extraStyle || ''}">
              <svg viewBox="0 0 300 100" preserveAspectRatio="none" class="absolute inset-0 w-full h-full" style="opacity:0.16;">${motif}</svg>
              <div class="relative">
                <div class="flex items-center gap-1.5 text-xs uppercase tracking-wide text-white/80 font-bold mb-2">${Icon(icon,'w-3.5 h-3.5')} ${eyebrow}</div>
                <div class="text-xl font-bold mb-2 text-white">${title}</div>
                <div class="text-sm text-white/80 mb-4">${sub}</div>
                ${actionsHtml}
              </div>
            </div>`;
        }
        ...
        function studyContent(){
          ...
          if (studySub === 'exams') return `
            ...
            ${studyFeatureCard(0, 0, 'doc', 'Practice Tests', 'Full mock tests, timed or self-paced', 'Exam-ready questions sourced from your uploaded resources.', `
              <div class="text-[${NAVY}] font-semibold px-6 py-3 rounded-2xl inline-block bg-white">Browse Mock Tests</div>
            `, 'openPracticeTestsOverlay()')}
            ${studyFeatureCard(4, 2, 'bolt', 'Live · Head-to-head', 'Challenge Arena', 'Real-time, competitive practice: go head-to-head with another student on the same question set.', `
              <div class="challenge-arena-actions relative">
                <button onclick="openChallengeSetupModal()" class="w-full flex items-center justify-center gap-2 text-[${NAVY}] font-semibold px-6 py-2.5 rounded-2xl mb-3 bg-white">Set Up a Challenge</button>
                <button onclick="openInviteFriendModal()" class="w-full flex items-center justify-center gap-2 text-[${NAVY}] font-semibold px-6 py-2.5 rounded-2xl mb-3 bg-white">Invite a Friend</button>
                <button onclick="openJoinChallengeCodeModal()" class="w-full flex items-center justify-center gap-2 font-semibold px-6 py-2.5 rounded-2xl border border-white/60 text-white">Join with a Code</button>
              </div>
            `)}
            ...
            ${studyFeatureCard(6, 4, 'bookmark', 'All Resources', 'Study All Flashcards', "Auto-generated from everything you've uploaded: no need to open each resource separately.", `
              <div class="text-[${NAVY}] font-semibold px-6 py-3 rounded-2xl inline-block bg-white">Start Studying</div>
            `, "openFlashDeck('all')", 'margin-bottom:50px;')}
        ...
        // No boxed "pill" wrapper here on purpose -- each plan's text sits
        // straight on the page background (just a hairline divider
        // between plans) instead of inside its own bordered/tinted card,
        // with a bit more room to breathe (bigger type, taller spacing)
        // than the old boxed version.
        function subscriptionPlanCardHTML(planKey){
          const p = PLAN_PRICING[planKey];
          const active = currentPlanName() === planKey;
          return `
            <div class="py-6 mb-1 border-b border-gray-100 last:border-b-0">
              <div class="flex items-center justify-between mb-2">
                <div class="text-2xl font-bold" style="color:${NAVY};">${p.label}</div>
                <div class="text-xl font-bold" style="color:${NAVY};">${p.currency} ${p.amount}<span class="text-sm font-semibold text-gray-400">/${p.cycle === 'Yearly' ? 'year' : 'month'}</span></div>
              </div>
              <div class="text-base text-gray-500 mb-5 leading-relaxed">${PLAN_DESCRIPTIONS[planKey] || ''}</div>
              <button onclick="submitSubscriptionPurchase('${planKey}')" ${active ? 'disabled' : ''} class="w-full text-white font-semibold py-3.5 rounded-2xl text-center text-base" style="background:${active ? 'rgba(30,144,255,0.25)' : 'rgba(30,144,255,0.5)'};">${active ? 'Current Plan' : 'Subscribe · ' + p.currency + ' ' + p.amount}</button>
            </div>`;
        }
        ...
        // Single plan row inside Settings > Subscription -- same "text
        // sits straight on the background, no boxed pill" treatment as
        // subscriptionPlanCardHTML, plus an "Active" tag next to the
        // current plan. The button is a real, working Subscribe action
        // (submitSubscriptionPurchase), not just a link out to another
        // screen, so a plan can actually be bought or switched right here.
            ...
            <div class="py-5 border-b border-gray-100 last:border-b-0">
              <div class="flex items-center justify-between mb-1">
                <div class="flex items-center gap-2">
                  ...
                  ${active ? `<span class="text-[10px] font-bold uppercase px-2 py-1 rounded-full text-white" style="background:${NAVY};">Active</span>` : ''}
                </div>
                <div class="text-base font-bold" style="color:${NAVY};">${p.currency} ${p.amount}<span class="text-xs font-semibold text-gray-400">/${p.cycle === 'Yearly' ? 'year' : 'month'}</span></div>
              </div>
              <div class="text-sm text-gray-500 mb-3">${PLAN_DESCRIPTIONS[planKey] || ''}</div>
              <button onclick="submitSubscriptionPurchase('${planKey}')" ${active ? 'disabled' : ''} class="w-full text-white font-semibold py-3 rounded-2xl text-center text-sm" style="background:${active ? 'rgba(30,144,255,0.25)' : 'rgba(30,144,255,0.5)'};">${active ? 'Current Plan' : 'Subscribe · ' + p.currency + ' ' + p.amount}</button>
            </div>`;
        }
