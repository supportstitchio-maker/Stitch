let studyFabMenuOpen = false;
        ...
        // No paid tiers, and no caps on practice questions, flashcards, or
        // Challenge Arena pool size either -- these are all unlimited for
        // everyone now. currentPlanLimit() is kept (rather than deleted)
        // so the handful of call sites that do things like
        // `.slice(0, currentPlanLimit('questions'))` keep working
        // unchanged; Infinity there is a no-op (Array.slice(0, Infinity)
        ...
        // ISO week key (year + week number) so the weekly Stitch Bot cap
        // resets on a consistent Mon-Sun boundary rather than a rolling
        // 7-day window.
        ...
        // NOTE ON SECURITY: these counters live in the same client-side
        // "user state" blob as everything else in this file (see
        // collectUserState/applyUserState in core.js) and are written back
        // to Supabase by queueSaveUserState() -- a signed-in user can open
        // devtools and edit these numbers (or call the Supabase client
        // directly) before saving, bypassing this check entirely. That's
        // true of every counter in that shared state object, not just this
        // one. This client-side check is what drives the UI (showing
        // "62/100 used today", disabling the input, etc.) but is NOT what
        // should actually stop abuse -- the real limit has to be enforced
        // server-side, in the ai-proxy Edge Function, against a usage
        // table the client can only read, never write. See the SQL/Edge
        // Function notes provided alongside this change for that part.
        const AI_PROMPT_LIMITS = { daily: 100, weekly: 500, monthly: 2000 };
        let aiPromptCount = 0;
        ...
        // Returns the number of prompts left, taking the *tightest* of the
        // three windows -- e.g. someone could have daily prompts left but
        // have already hit their weekly or monthly cap.
        ...
        // Tells the UI which specific window was hit, so the message can
        // say "daily"/"weekly"/"monthly" instead of a generic "limit reached".
