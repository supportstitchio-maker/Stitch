        ...
        function studyFeatureCard(colorIndex, motifIndex, icon, eyebrow, title, sub, actionsHtml, onclick, extraStyle, paper){
          ...
          const bgStyle = paper
            ? `background-color:#efe3c9;background-image:url("${paperCardTexture}"),linear-gradient(135deg,#f7edd4,#e7d9b4);background-blend-mode:multiply,normal;background-size:220px 220px,cover;`
          ...
          return `
            <div ${onclick ? `onclick="${onclick}" class="rounded-2xl p-6 mb-4 relative overflow-hidden cursor-pointer"` : `class="rounded-2xl p-6 mb-4 relative overflow-hidden"`} style="${bgStyle}${extraStyle || ''}">
              <svg viewBox="0 0 300 100" preserveAspectRatio="none" class="absolute inset-0 w-full h-full" style="opacity:0.16;${paper ? 'filter:sepia(1) saturate(1.6) brightness(0.5) hue-rotate(-10deg);' : ''}">${motif}</svg>
              <div class="relative">
                <div class="flex items-center gap-1.5 text-xs uppercase tracking-wide font-bold mb-2" style="color:${paper ? `rgba(61,43,26,0.75)` : 'rgba(255,255,255,0.8)'};">${Icon(icon,'w-3.5 h-3.5')} ${eyebrow}</div>
                <div class="text-xl font-bold mb-2" style="color:${paper ? PAPER_INK : '#ffffff'};">${title}</div>
                <div class="text-sm mb-4" style="color:${paper ? `rgba(61,43,26,0.72)` : 'rgba(255,255,255,0.8)'};">${sub}</div>
                ${actionsHtml}
              </div>
            </div>`;
        }
          ...
          if (studySub === 'exams') return `
            ...
            ${studyFeatureCard(4, 2, 'bolt', 'Live · Head-to-head', 'Challenge Arena', 'Real-time, competitive practice: go head-to-head with another student on the same question set.', `
              <div class="challenge-arena-actions relative">
                ...
                <button onclick="openJoinChallengeCodeModal()" class="w-full flex items-center justify-center gap-2 font-semibold px-6 py-2.5 rounded-2xl border" style="border-color:rgba(61,43,26,0.35);color:${PAPER_INK};">Join with a Code</button>
              </div>
            `, null, null, true)}
