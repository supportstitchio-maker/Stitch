        ...
        async function generateStudyMaterials(text, resourceName){
          ...
          const system = 'You generate study material for an education app from a single uploaded resource. ' +
            ...
            'named principle/theory/term, or the ability to classify an example under the right category -- not ' +
            'just "which term matches this definition". Vary the question style across the set instead of reusing ' +
            'one template for every question. Every option must be a short phrase or single term (typically 1-5 ' +
            'words, matching how real exam answer choices read), never a full sentence restating the question. ' +
            'Exactly one option is correct; the other four must be plausible, closely-related distractors from the ' +
            ...
            '{"flashcards":[{"term":"string","def":"string"}],"questions":[{"text":"string","options":["string","string","string","string","string"],"correct":0}]}. ' +
            'Produce 6 to 10 flashcards and 5 to 8 questions. Each question needs exactly 5 options, with "correct" as the 0-based index of the right one.';
          ...
          return {
            ...
            questions: (parsed.questions || []).filter(q => q.text && Array.isArray(q.options) && q.options.length === 5 && typeof q.correct === 'number')
          };
        }
        ...
        function generateStudyMaterialsFallback(text){
          ...
          if (flashcards.length >= 4) {
            flashcards.slice(0, 8).forEach((card, i) => {
              ...
              const distractors = shuffleArray([...new Set(pool)]).slice(0, 4);
          ...
          document.getElementById('flashModalContent').innerHTML = `
            <div class="flex items-start justify-between mb-4">
              <div class="flex items-start gap-3">
                <button onclick="closeFlashModal()" class="text-gray-400 mt-1" aria-label="Back">${Icon('back','w-5 h-5')}</button>
                <div>
