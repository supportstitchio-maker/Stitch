        ...
        // button wiring.
        function openSubscriptionUpgradeModal(reason){
          openChallengeModal(subscriptionUpgradeHTML(reason));
        }
        ...
        // Same modal, different opening copy depending on what sent the
        // student here (see openSubscriptionUpgradeModal's "reason"
        // param): the plan cards/pricing/footnote below are identical
        // either way, only the headline + one-line explanation changes,
        // so this stays a single modal/function instead of a second
        // near-duplicate one.
        const SUBSCRIPTION_UPGRADE_COPY = {
          practice: {
            title: 'Add More Questions &amp; Flashcards',
            body: "You've been through everything in your free practice set. Subscribe to unlock a bigger pool of practice questions, flashcards, and Challenge Arena questions generated from the same uploads."
          },
          stitchBot: {
            title: 'Unlock Unlimited Stitch Bot',
            body: "You've used up today's Stitch Bot credits. Subscribe to chat with Stitch Bot as much as you want, any time, no daily limit."
          }
        };
        function subscriptionUpgradeHTML(reason){
          const copy = SUBSCRIPTION_UPGRADE_COPY[reason] || SUBSCRIPTION_UPGRADE_COPY.practice;
          return `
            ...
            <div class="text-2xl font-bold text-gray-900 mb-2">${copy.title}</div>
            <div class="text-sm text-gray-500 mb-5">${copy.body}</div>
            ...
            <div class="rounded-2xl p-5 mb-4 relative overflow-hidden" style="background:linear-gradient(135deg,${from},${to});">
              ...
              <div class="relative">
                <div class="flex items-center justify-between mb-1">
                  ...
                  <div class="text-base font-bold text-white">${p.currency} ${p.amount}<span class="text-xs font-semibold text-white text-opacity-70">/${p.cycle === 'Yearly' ? 'year' : 'month'}</span></div>
                </div>
                <div class="text-sm text-white text-opacity-85 mb-3">${PLAN_DESCRIPTIONS[planKey] || ''}</div>
                <button onclick="submitSubscriptionPurchase('${planKey}')" ${active ? 'disabled' : ''} class="w-full font-semibold py-3 rounded-2xl text-center text-sm ${active ? 'bg-white bg-opacity-25 text-white' : 'bg-white'}" style="${active ? '' : `color:${from};`}">${active ? 'Current Plan' : 'Subscribe · ' + p.currency + ' ' + p.amount}</button>
              </div>
            </div>`;
        }
