        ...
        function studyContent(){
          ...
          if (studySub === 'exams') return `
            ...
            ${(!isTrialActive() && practiceCompleted && flashcardsCompleted) ? studyFeatureCard('plus', 'Upgrade', 'Add More Questions &amp; Flashcards', "Finished everything in your practice set? Unlock a bigger pool of questions and flashcards from the same uploads.", `
              <button onclick="openSubscriptionUpgradeModal()" class="w-full font-semibold px-6 py-3 rounded-full text-white flex items-center justify-center gap-1.5" style="background:linear-gradient(135deg, ${NAVY} 0%, ${ROYAL} 100%);box-shadow:0 4px 14px rgba(65,105,225,0.35);">${Icon('plus','w-4 h-4 inline-block mr-1 -mt-0.5')} Add More Questions &amp; Flashcards</button>
        ...
        let userSubscription = { plan: 'free', expiresAt: null }; 
        ...
        const planCardPalette = {
          free: ['#16a34a', '#0f7a34'],      
          pro: ['#5b6472', '#2b3038'],       
          proPlus: ['#28395c', '#101a2e'],   
          proMax: ['#eab308', '#92660a'],    
          platinum: ['#7d8793', '#3f454d']   
        };
