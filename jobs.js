const jobsTabs = [['all','All'],['opportunities','Opportunities'],['internships','Internships'],['courses','Courses'],['scholarships','Scholarships'],['others','Others']];
        ...
        function posterAppStageHTML(stage){
          ...
          if (stage === 'bizname') {
            return `<input type="text" id="poster-app-input" ${enterNext} oninput="updatePosterAppField('businessName', this.value); const e=document.getElementById('poster-app-stage-error'); if(e) e.style.display='none';" placeholder="e.g. Acme Design Studio" value="${escapeHtml(d.businessName || '')}" class="${fieldCls}">`;
          }
          ...
          if (stage === 'bizwebsite') {
            return `
              <input type="url" id="poster-app-input" ${enterNext} oninput="updatePosterAppField('businessWebsite', this.value)" placeholder="e.g. instagram.com/yourbusiness" value="${escapeHtml(d.businessWebsite || '')}" class="${fieldCls}">
              ${skipLink}`;
          }
          if (stage === 'bizdesc') {
            return `
              <textarea id="poster-app-input" maxlength="100" oninput="updatePosterAppField('businessInfo', this.value); document.getElementById('poster-app-bizdesc-counter').textContent = this.value.length + '/100';" placeholder="e.g. Internships and design roles for students." rows="3" class="${fieldCls} resize-none">${escapeHtml(d.businessInfo || '')}</textarea>
        ...
        function posterApplicationOverlayHTML(){
          ...
          return `
            ...
            <div class="flex-shrink-0 w-full px-5">
              <div class="max-w-2xl mx-auto">
        ...
        function careerStartFieldHTML(label, field, type, placeholder){
          ...
          return `
            <div class="mb-4">
              <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 block">${label}</label>
              <input type="${type || 'text'}" value="${escapeHtml(careerStartDraft[field])}" oninput="updateCareerStartField('${field}', this.value)" onkeydown="if(event.key==='Enter'){event.preventDefault();careerStartFieldEnter(this);}" enterkeyhint="next" placeholder="${placeholder || ''}" class="w-full bg-gray-100 rounded-2xl px-4 py-3 text-sm${err ? ' field-invalid' : ''}" style="outline:none;">
              ${err ? `<div class="field-error">${escapeHtml(err)}</div>` : ''}
            </div>`;
        }
