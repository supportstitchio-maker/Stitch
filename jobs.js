const jobsTabs = [['all','All'],['opportunities','Opportunities'],['internships','Internships'],['courses','Courses'],['scholarships','Scholarships'],['others','Others']];
        ...
        function careerMenuPostItemHTML(){
          if (isCurrentUserAdmin() || currentUserPosterStatus === 'approved') {
            return `<button onclick="closeCareerMenuThen(() => openPostOpportunity())" class="text-left px-3 py-2 text-base whitespace-nowrap text-gray-700 menu-item-pill">Post an Opportunity</button>`;
          }
          if (currentUserPosterStatus === 'pending') {
            return `<button onclick="closeCareerMenuThen(() => openAppAlertModal('An admin is reviewing your application to post -- you\\'ll get a notification once it\\'s decided.'))" class="text-left px-3 py-2 text-base whitespace-nowrap text-gray-400 menu-item-pill">Poster application pending</button>`;
          }
          return `<button onclick="closeCareerMenuThen(() => openPosterApplicationForm())" class="text-left px-3 py-2 text-base whitespace-nowrap text-gray-700 menu-item-pill">Apply to post</button>`;
        }

        // Shared with refreshPosterStatusUI() below, which swaps this back in live if the
        // dropdown happens to be open when a poster_approved/poster_denied notification lands.
        function careerMenuDropdownInnerHTML(){
          return `
            <button onclick="closeCareerMenuThen(() => jobsSubTab('saved'))" class="text-left px-3 py-2 text-base whitespace-nowrap text-gray-700 menu-item-pill">Saved</button>
            <button onclick="closeCareerMenuThen(() => openCareerStartForm())" class="text-left px-3 py-2 text-base whitespace-nowrap text-gray-700 menu-item-pill">Match with CV</button>
        ...
        function refreshPosterStatusUI(){
          const dropdown = document.getElementById('career-menu-dropdown');
          if (dropdown) {
            dropdown.innerHTML = careerMenuDropdownInnerHTML();
            if (typeof fitCareerMenuDropdownWidth === 'function') fitCareerMenuDropdownWidth();
          }
        ...
        // Synchronous cache of the signed-in user's id, refreshed by
        // loadCurrentUserRole() at boot -- render functions need it synchronously.
        let currentUserId = null;
        // ---- Regular-user "poster" permission state ---- Posting (everything except Course)
        // requires the poster application flow below to be approved by an admin. Mirrors
        ...
        let currentUserPosterBusinessDocumentUrl = ''; // e.g. registration certificate, uploaded during application
        let currentUserPosterIdDocumentUrl = '';       // any government-issued ID, uploaded during application
        let currentUserPosterPhotoUrl = '';            // passport-size verification photo (kept separate from the public profile photo)
        ...
        async function loadCurrentUserRole(){
          ...
          try {
            ...
            if (!error && profile) {
              ...
              currentUserPosterPhotoUrl = profile.poster_photo_url || '';
        ...
        // Admins can post any type; a regular user can post everything except
        // Course once their poster application is approved (see Admin Dashboard).
        ...
        function isJobOwnedByCurrentUser(job){
          return !!(job && job.createdByUserId && currentUserId && job.createdByUserId === currentUserId);
        }

        function canCurrentUserManageJob(job){
          return isCurrentUserAdmin() || isJobOwnedByCurrentUser(job);
        }
        ...
        // review. The in-app "Apply to post" entry point (openPosterApplicationForm, below) is
        // ONLY ever reached by someone who already said they want to post -- either by tapping
        // "Apply to post" in Career Space's menu, or by trying to post and getting routed here --
        // so that overlay skips the post/explore intent question entirely and goes straight to
        // the business questions. Onboarding (games.js) still asks the intent question first,
        // since at that point we genuinely don't know what the person wants yet.
        let posterAppDraft = {
          ...
          idDocumentFile: null, idDocumentName: '',
          photoBlob: null,
          consent: false,
        };
        function resetPosterAppDraft(){
          posterAppDraft = {
            ...
            // What's shown in the typed DD/MM/YYYY box; businessEstablished above stays the
            // 'YYYY-MM-DD' value that gets saved (see posterAppOnEstablishedInput).
            ...
            // Files/photo are never pre-filled from a past application -- a fresh upload
            // (or none, if the person doesn't want to change what's already on file) is
            // staged here each time the form opens.
            businessDocumentFile: null, businessDocumentName: '',
            idDocumentFile: null, idDocumentName: '',
            photoBlob: null,
            // Re-confirmed on every application (including a reapply), rather than carried
            // over from currentUserPosterConsentAccepted -- see posterAppStageError('consent').
            consent: false,
          };
        }
        ...
        // Shared by onboarding (games.js) and the in-app "Apply to post" form. The person types
        // the date in a fixed, visible format (DD/MM/YYYY) instead of using the native date
        // picker -- slashes are inserted as they type. The draft keeps two values: the text in
        // the box (businessEstablishedTyped) and the 'YYYY-MM-DD' string that's actually saved
        // (businessEstablished), which stays empty until the typed text is a real, valid date.
        ...
        function posterAppEstablishedInputHTML(inputId, onEnter){
          ...
          return `
            <input type="text" id="${inputId}" inputmode="numeric" autocomplete="off" autocorrect="off" spellcheck="false" maxlength="10" placeholder="DD/MM/YYYY" oninput="posterAppOnEstablishedInput(this)"${enterAttr} value="${escapeHtml(typed)}" class="w-full bg-gray-100 border border-gray-300 rounded-2xl px-4 py-3 text-sm outline-none">
            <div style="font-size:12px;color:#9ca3af;margin-top:8px;">Format: DD/MM/YYYY (for example 14/03/2019)</div>`;
        }

        // Quick client-side square center-crop, same approach used during onboarding (see
        // authHandlePhotoSelected in games.js) so the passport-style preview isn't stretched.
        function posterAppHandlePhotoSelected(input, previewImgId, placeholderId, addBtnId){
        ...
        function posterAppHandleBizDocSelected(input){
          ...
          posterAppDraft.businessDocumentName = file.name || 'Document selected';
        ...
        function posterAppHandleIdDocSelected(input){
          const file = input.files && input.files[0];
          if (!file) return;
          posterAppDraft.idDocumentFile = file;
          posterAppDraft.idDocumentName = file.name || 'Document selected';
          const label = document.getElementById('poster-app-iddoc-label');
          if (label) label.textContent = posterAppDraft.idDocumentName;
        }

        let posterAppSubmitInFlight = false;

        // Saves the application (pending for 'post', none for 'explore') and
        // notifies admins. Returns true/false so callers can react.
        async function submitPosterApplication(){
          ...
          try {
            ...
            // Uploads happen at submit time, once we know the account exists (mirrors how the
            // profile photo is handled during onboarding in games.js). A failed upload shouldn't
            // block the whole application -- fall back to whatever was already on file.
            let businessDocumentUrl = currentUserPosterBusinessDocumentUrl || '';
            let idDocumentUrl = currentUserPosterIdDocumentUrl || '';
            let photoUrl = currentUserPosterPhotoUrl || '';
            ...
            if (posterAppDraft.idDocumentFile) {
              const uploaded = await uploadPosterIdDocumentToStorage(posterAppDraft.idDocumentFile);
              if (uploaded) idDocumentUrl = uploaded;
            }
            if (posterAppDraft.photoBlob) {
              const uploaded = await uploadPosterPhotoToStorage(posterAppDraft.photoBlob);
            ...
            const { error } = await sb.from(PROFILES_TABLE).upsert({
              ...
              // agreement to the Terms/Privacy Policy, and to Stitch tracking the opportunities
              // this person posts and the decisions (approve/deny/remove) made on them.
            ...
            if (error) { console.warn('Poster application did not save:', error); return false; }
            ...
            currentUserPosterPhotoUrl = photoUrl;
            ...
            if (posterStatus === 'pending') {
              notifyAllAdminsRemote({
                ...
                message: `${(typeof profileData !== 'undefined' && profileData.name) || 'A user'} wants to post opportunities on Career Space. Review it on the Admin Dashboard.`,
              });
            }
            // Make sure this account's name/username/photo are in PUBLIC_PROFILES_TABLE before an
            // admin looks at the application -- without this, someone who reached this form without
            // having synced yet (e.g. a stale/older session) would show up as "Unnamed user" on the
            // Admin Dashboard even though their own profile has a name. See syncPublicProfile in profile.js.
        ...
        // Reached only from an explicit "I want to post" action (the Career Space menu's
        // "Apply to post" item, or trying to post without permission) -- so intent is locked
        // to 'post' here and the form skips straight to the business questions instead of
        // re-asking "post or explore/network", which the person just answered by tapping this.
        ...
        // Mirrors the onboarding flow in games.js (authPosterApp*): one question on screen at a
        // time, a progress bar, and a single Next button. Intent is always 'post' here, so
        // there's no post/explore question -- see openPosterApplicationForm above.
        const POSTER_APP_STAGES = ['role', 'bizname', 'bizestablished', 'bizcategory', 'bizwebsite', 'bizdesc', 'bizdoc', 'photo', 'iddoc', 'consent'];
        let posterAppStageIdx = 0;

        // Minimum words required in the "In your own words" business description --
        // shared by the in-app "Apply to post" form (jobs.js) and onboarding (games.js),
        // since both write to the same posterAppDraft.businessInfo.
        ...
        function posterAppStageTitle(stage){
          ...
          if (stage === 'photo') return 'Say cheese';
          if (stage === 'iddoc') return 'Last one, promise';
        ...
        function posterAppStageSub(stage){
          ...
          if (stage === 'bizdoc') return 'A business or registration document (optional)';
          if (stage === 'photo') return 'A passport-size photo, for verification';
          if (stage === 'iddoc') return 'Submit an ID (any government-issued ID)';
        ...
        function posterAppStageHint(stage){
          ...
          if (stage === 'bizdoc') return 'Helps admins verify faster -- a registration certificate, permit, or similar.';
          if (stage === 'photo') return 'A clear, recent photo of your face for verification. Kept private to admins.';
          if (stage === 'iddoc') return "National ID, passport, driver's license, etc.";
          return '';
        }
        ...
        function posterAppStageHTML(stage){
          ...
          const fieldCls = 'w-full bg-gray-100 border border-gray-300 rounded-2xl px-4 py-3 text-sm outline-none';
            ...
            return `
              <div class="grid grid-cols-2 gap-2">
                ${POSTER_ROLE_OPTIONS.map(([key, label]) => `
                  <button type="button" onclick="setPosterAppRole('${key}')" class="px-4 py-2.5 rounded-full text-xs font-semibold text-center ${d.role===key ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.role===key ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">${label}</button>
                `).join('')}
              </div>`;
          }
          if (stage === 'bizname') {
          ...
          if (stage === 'bizcategory') {
            return `
              <div class="grid grid-cols-2 gap-2">
                ${POSTER_CATEGORY_OPTIONS.map(([key, label]) => `
                  <button type="button" onclick="setPosterAppCategory('${key}')" class="px-4 py-2.5 rounded-full text-xs font-semibold text-center ${d.businessCategory===key ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.businessCategory===key ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">${label}</button>
                `).join('')}
              </div>`;
          }
          if (stage === 'bizwebsite') {
          ...
          if (stage === 'photo') {
            return `
              <div class="flex flex-col items-center gap-3">
                <div id="poster-app-photo-circle" role="button" tabindex="0" aria-label="Add a photo" onclick="document.getElementById('poster-app-photo-file-input').click()" style="width:112px;height:112px;border-radius:9999px;overflow:hidden;background:#f1f5f9;display:flex;align-items:center;justify-content:center;flex-shrink:0;cursor:pointer;-webkit-tap-highlight-color:transparent;">
                  <img id="poster-app-photo-preview-img" src="${currentUserPosterPhotoUrl ? escapeHtml(currentUserPosterPhotoUrl) : ''}" style="display:${currentUserPosterPhotoUrl ? 'block' : 'none'};width:100%;height:100%;object-fit:cover;">
                  <span id="poster-app-photo-placeholder-icon" style="display:${currentUserPosterPhotoUrl ? 'none' : 'flex'};color:#9ca3af;">${Icon('camera','w-8 h-8')}</span>
                </div>
                <input type="file" accept="image/*" id="poster-app-photo-file-input" class="hidden" onchange="posterAppHandlePhotoSelected(this,'poster-app-photo-preview-img','poster-app-photo-placeholder-icon','poster-app-photo-add-btn')">
                <button type="button" onclick="document.getElementById('poster-app-photo-file-input').click()" id="poster-app-photo-add-btn" class="px-5 py-2 rounded-full text-sm font-semibold" style="background:rgba(10,37,64,0.08);color:${NAVY};">${currentUserPosterPhotoUrl ? 'Change photo' : 'Add a photo'}</button>
              </div>`;
          }
          if (stage === 'iddoc') {
            return `
            <input type="file" accept="image/*,.pdf" id="poster-app-iddoc-file-input" class="hidden" onchange="posterAppHandleIdDocSelected(this)">
            <button type="button" onclick="document.getElementById('poster-app-iddoc-file-input').click()" class="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-2xl text-sm font-semibold bg-gray-100 text-gray-600 border border-gray-300 border-dashed">
              ${Icon('upload','w-4 h-4')}
              <span id="poster-app-iddoc-label">${escapeHtml(d.idDocumentName || (currentUserPosterIdDocumentUrl ? 'ID on file -- tap to replace' : 'Upload an ID'))}</span>
            </button>`;
          }
          // stage === 'consent'
        ...
        // Shared by both the in-app "Apply to post" form (openOverlay links to the
        // Terms/Privacy overlays, see overlays.js) and onboarding in games.js (which links
        // to the pre-login authShowTerms()/openFooterInfo('privacy') modals instead, since
        // the overlay system doesn't exist yet at that point in the flow).
        function posterAppConsentHTML(checkboxId, onChangeExpr, termsOnClick, privacyOnClick){
          const d = posterAppDraft;
          return `
            <label for="${checkboxId}" style="display:flex;align-items:flex-start;gap:10px;cursor:pointer;background:#f9fafb;border:1px solid #e5e7eb;border-radius:16px;padding:14px 16px;">
        ...
        // Just the current question's input. The role/category setters above swap this element
        // in place (outerHTML) so picking a chip doesn't re-render the whole page.
        function posterApplicationFormHTML(){
          return `<div id="poster-app-form" style="margin-top:22px;">${posterAppStageHTML(POSTER_APP_STAGES[posterAppStageIdx])}</div>`;
        }
        ...
        // Per-question checks -- the same rules the old single page enforced at submit time
        // (see posterApplicationMissingFieldMessage), just caught on the page where they apply.
        function posterAppStageError(stage){
          ...
          if (stage === 'photo' && !posterAppDraft.photoBlob && !currentUserPosterPhotoUrl) return "A passport-size photo is required to apply.";
          if (stage === 'iddoc' && !posterAppDraft.idDocumentFile && !currentUserPosterIdDocumentUrl) return "Please submit an ID to apply.";
          if (stage === 'consent' && !posterAppDraft.consent) return "Please agree to the Terms of Service and Privacy Policy to continue.";
          return null;
        }

        // ---- ID document verification ----
        // Before an uploaded file is accepted as someone's ID, ask the AI vision model to
        // confirm it actually looks like a government/school-issued photo ID -- not just any
        // file with the right extension. Shared by both the in-app "Apply to post" form
        // (posterAppNext, below) and the signup-time onboarding version (authPosterAppNext in
        // games.js), since both write to the same posterAppDraft.idDocumentFile.
        // Only images can be checked this way (no vision check for a submitted PDF); and if the
        // check itself fails to run (offline, AI proxy down, rate-limited), we fail OPEN --
        // a real applicant still goes to admin review, same as before this check existed --
        // rather than blocking someone from applying because a network call hiccuped.
        ...
        async function posterAppNext(){
          ...
          const stage = POSTER_APP_STAGES[posterAppStageIdx];
          ...
          if (posterAppStageIdx < POSTER_APP_STAGES.length - 1) {
            posterAppStageIdx++;
        ...
        // Submit handler for the in-app "Apply to post" overlay (skipped-at-signup
        // or reapplying); signup itself uses submitPosterApplication() directly.
        function posterApplicationFormHTML_submitBtnLabel(){
          return currentUserPosterStatus === 'denied' ? 'Reapply' : 'Submit application';
        }

        // Requires the photo and ID for a fresh application; if one's already on file from a
        // previous submission (currentUserPosterPhotoUrl / currentUserPosterIdDocumentUrl) the
        // person isn't forced to re-upload it just to update something else.
        function posterApplicationMissingFieldMessage(){
          ...
          if (!posterAppDraft.photoBlob && !currentUserPosterPhotoUrl) return "A passport-size photo is required to apply.";
        ...
        async function submitPosterApplicationOverlay(){
          ...
          const ok = await submitPosterApplication();
          if (!ok) {
            openAppAlertModal("Couldn't submit your application. Please check your connection and try again.");
          ...
          openAppAlertModal(
            posterAppDraft.intent === 'post'
              ? "Thanks! Your application is with an admin for review -- you'll get a notification once it's decided."
        ...
        function posterApplicationOverlayHTML(){
          ...
          const total = POSTER_APP_STAGES.length;
          const idx = Math.min(Math.max(posterAppStageIdx, 0), total - 1);
          const stage = POSTER_APP_STAGES[idx];
          ...
          return `
            ...
            <div id="poster-app-stage-scroll" class="flex-1 overflow-y-auto px-5" style="padding-top:26px;padding-bottom:20px;">
              <div class="max-w-2xl mx-auto">
                ${deniedBanner}
        ...
        // Adds/updates this applicant's own entry on the shared opportunity row so its poster can
        // see them on the Dashboard (jobDashboardHTML). Fire-and-forget: applying should never
        async function syncJobApplicant(job){
        ...
        // ---- Cross-user notifications (server-side) ---- notifData is local-only; this table +
        // realtime subscription (below) is how one user's action reaches another user's device.
        ...
        function deliverRemoteNotification(row){
          ...
          // Update local poster status the instant the decision notification lands, so
          // "Poster application pending" reflects it right away instead of waiting on a reload.
          if (row.type === 'poster_approved' || row.type === 'poster_denied') {
            currentUserPosterStatus = row.type === 'poster_approved' ? 'approved' : 'denied';
        ...
        // Runs at boot: pulls in notifications missed while offline, then
        // marks them delivered so they aren't re-imported next login.
        ...
        // ---- Consistent admin-picked date system ---- Deadlines are chosen through a native
        // <input type="date"> so every listing's deadlineDate is always a plain, unambiguous ISO
        ...
        // admin edits typed by hand). Handles ordinal suffixes ("30th", "1st", "22nd", "23rd") and
        ...
        // ---- Auto-delete opportunities once their deadline has passed ---- Courses are exempt:
        // their "deadline" field is a start date, not a closing date, so they're left alone here.
        ...
        // ---- Job/course listing data + application status helpers ---- Defensive, synchronous
        // safety net: allJobs()/allExploreCards() are what every screen actually renders from, so
        function allJobs(){
        ...
        function jobDetailMenuDropdownHTML(job){
          ...
          return `
            <div class="absolute bg-white rounded-2xl border border-gray-100 py-2 z-20 menu-dropdown-inset" style="right:1.25rem;top:3.5rem;width:12rem;box-shadow:0 10px 30px rgba(0,0,0,.14);">
              <button onclick="jobDetailMenuOpen=false; openJobDashboard('${job.id}')" class="w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-700 menu-item-pill">${Icon('trending','w-4 h-4')} Dashboard</button>
        ...
        // ---- Report a listing ---- User posts publish instantly with no pre-review; this is the
        // moderation safety valve, feeding the Admin Dashboard's Reports tab.
        ...
        function setReportOppReasonChip(chip){
          ...
          const el = document.getElementById('report-opp-textarea');
          if (el) el.value = chip;
        }

        function updateReportOppReason(value){
          reportOppReason = value;
        }
        ...
        async function submitOpportunityReport(){
          ...
          if (!reportOppReason.trim()) { openAppAlertModal('Please add a quick reason so admins know what to look at.'); return; }
          ...
          try {
            ...
            const { error } = await sb.from(OPPORTUNITY_REPORTS_TABLE).insert({
              ...
              reported_by: myId,
              reason: reportOppReason.trim(),
            });
            if (error) { openAppAlertModal("Couldn't send your report. Please try again."); return; }
            notifyAllAdminsRemote({
              type: 'opportunity_reported',
              title: 'Listing reported',
              message: 'A listing was just reported on Career Space. Review it on the Admin Dashboard.',
        ...
        function reportOpportunityHTML(){
          ...
          return `
            ${overlayHeader('Report listing', '20px')}
            <div class="flex-1 overflow-y-auto px-5" style="padding-bottom:50px;">
              ${job ? `<div class="text-sm text-gray-500 mb-4">Reporting "<span class="font-semibold text-gray-800">${escapeHtml(job.title)}</span>"</div>` : ''}
              <div class="flex gap-2 flex-wrap mb-4">
                ${REPORT_REASON_CHIPS.map(chip => `<button onclick="setReportOppReasonChip('${escapeForJsAttr(chip)}')" class="px-3 py-2 rounded-full text-xs font-semibold ${reportOppReason===chip ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${reportOppReason===chip ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">${escapeHtml(chip)}</button>`).join('')}
              </div>
              <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 block">Tell us more</label>
              <textarea id="report-opp-textarea" oninput="updateReportOppReason(this.value)" placeholder="What's wrong with this listing?" rows="4" class="w-full bg-gray-100 border border-gray-300 rounded-2xl px-4 py-3 text-sm outline-none resize-none mb-5">${escapeHtml(reportOppReason)}</textarea>
              <div class="text-center">
                <button id="report-opp-submit-btn" onclick="submitOpportunityReport()" class="pill-cta inline-flex items-center justify-center text-white font-semibold text-center rounded-full text-sm" style="background:linear-gradient(135deg, ${NAVY} 0%, ${ROYAL} 100%);box-shadow:0 4px 14px rgba(65,105,225,0.35);padding:0.5rem 1.1rem;">Submit report</button>
              </div>
            </div>`;
        }
        ...
        // Poster/admin moves an applicant through the pipeline and this
        // auto-notifies them via NOTIFICATIONS_TABLE (see subscribeToUserNotifications).
        async function posterUpdateApplicantStatus(jobId, applicantId, status){
          ...
          notifyUserRemote(applicantId, {
            ...
            message: `Your ${job.type === 'Course' ? 'enrollment' : 'application'} for "${job.title}" is now: ${meta.label}.`,
            jobId: job.id,
          });
        }

        function jobDashboardApplicantRowHTML(job, a, isLast){
          const isCourse = job.type === 'Course';
          const meta = JOB_STATUS_META[a.status || 'applied'];
          const menuOpen = jobDashboardApplicantMenuOpenId === a.id;
          return `
            <div class="py-3.5 ${isLast ? '' : 'border-b border-gray-100'}">
              <div class="flex items-center gap-3">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-xs text-gray-400 truncate">${a.username ? '@' + escapeHtml(a.username) + ' &middot; ' : ''}${escapeHtml(a.email || '')}</div>
                  <div class="text-[11px] text-gray-400 mt-0.5">${a.appliedAt ? fmtJobDate(a.appliedAt) : ''}</div>
                </div>
                ${!isCourse ? `<button onclick="toggleJobDashboardApplicantMenu('${a.id}')" class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full flex-shrink-0 ${meta.bg} ${meta.text}">${meta.label} ${menuOpen ? '&#9650;' : '&#9660;'}</button>` : ''}
              </div>
              ${(!isCourse && menuOpen) ? `
                <div class="grid grid-cols-2 gap-2 mt-3 pl-[52px]">
                  ${JOB_STATUS_ORDER.map(k => {
                    const active = (a.status || 'applied') === k;
                    const m = JOB_STATUS_META[k];
                    return `<button onclick="posterUpdateApplicantStatus('${job.id}','${a.id}','${k}')" class="px-3 py-2 rounded-xl text-xs font-semibold text-center" style="${active ? `background:${m.color};color:#ffffff;` : 'background:#f3f4f6;color:#6b7280;'}">${escapeHtml(m.label)}</button>`;
                  }).join('')}
                </div>` : ''}
            </div>`;
        }
        ...
        function jobDashboardHTML(){
          ...
          if (!job) return `${overlayHeader('Dashboard', '20px')}<div class="p-5 text-center text-gray-400 text-sm">This listing is no longer available.</div>`;
          const isCourse = job.type === 'Course';
          const people = (job.applicants || []).slice().sort((a, b) => (b.appliedAt || 0) - (a.appliedAt || 0));
          return `
            <div class="flex-1 flex flex-col overflow-hidden">
              ${overlayHeader('Dashboard', '20px')}
              <div class="flex-1 overflow-y-auto px-5 pb-8">
                ...
                <div class="text-xs text-gray-400 mb-4">${isCourse ? 'Enrollment' : 'Application'} activity for this listing${isCourse ? '' : ' · tap a status pill below to update it and notify the applicant'}</div>
                <div class="bg-white rounded-2xl p-4 shadow-sm text-center mb-5">
                  <div class="text-2xl font-bold" style="color:${NAVY};">${people.length}</div>
                  <div class="text-xs text-gray-400">${isCourse ? 'Enrolled' : 'Applicants'}</div>
                </div>
                <div class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">${isCourse ? 'Enrolled people' : 'Applicants'}</div>
                ${people.length ? people.map((a, i) => jobDashboardApplicantRowHTML(job, a, i === people.length - 1)).join('') : `
                  <div class="bg-gray-50 border border-dashed border-gray-300 rounded-3xl p-8 text-center text-gray-500 text-sm">
                    ...
                    No ${isCourse ? 'enrollments' : 'applications'} yet.
                  </div>`}
              </div>
            </div>`;
        }
        ...
        function finalizeJobApplication(id){
          ...
          queueSaveUserState();
          // Record this applicant on the opportunity itself -- this is what powers the poster's
          // Dashboard (jobDashboardHTML) so they can actually see who applied and when, instead of
          syncJobApplicant(job);
            ...
            <div class="flex-1 overflow-y-auto px-5" style="padding-bottom:50px;">
              ...
              <div class="flex items-center gap-4 mb-5">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  ${(job.createdByRole === 'user' && job.createdByUsername) ? `<div class="text-xs text-gray-400 mt-0.5">Posted by @${escapeHtml(job.createdByUsername)}</div>` : ''}
                </div>
                ${!canManage ? `<button onclick="openReportOpportunity('${job.id}')" title="Report this listing" class="flex-shrink-0 w-9 h-9 flex items-center justify-center text-gray-300">${Icon('flag','w-4 h-4')}</button>` : ''}
              </div>
              <div class="bg-white rounded-3xl p-5 mb-4 shadow-sm">
        ...
        function openJobApply(id){
          ...
          // -- the org's own site collects that. Tapping "Apply" should just mark it applied and
        ...
        // hardware/browser back button, wired through overlayBackAction) steps back one
        // question at a time instead of leaving the whole flow, and only exits once you're
        // already on the first question.
        function careerStartBack(fromPopState){
          if (careerStartStepIndex > 0) {
        ...
        function careerStartFieldHTML(label, field, type, placeholder){
          ...
          return `
            <div class="mb-4">
              <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 block">${label}</label>
              <input type="${type || 'text'}" autocomplete="off" value="${escapeHtml(careerStartDraft[field])}" oninput="updateCareerStartField('${field}', this.value)" onkeydown="if(event.key==='Enter'){event.preventDefault();careerStartFieldEnter(this);}" enterkeyhint="next" placeholder="${placeholder || ''}" class="w-full bg-gray-100 rounded-2xl px-4 py-3 text-sm${err ? ' field-invalid' : ''}" style="outline:none;">
              ${err ? `<div class="field-error">${escapeHtml(err)}</div>` : ''}
            </div>`;
        }
        // Enter/keyboard-"Go" on one of careerStartFieldHTML's inputs. A step can hold more
        // than one field (e.g. 'contact': full name, email, phone all on the same screen),
        // so this moves to the next field on that screen first and only calls
        // careerStartNext() -- the same thing the Next/Continue button does -- once it's on
        // the last field, instead of jumping straight to validating fields not filled in yet.
        ...
        // Title/sub/hint split out from the step body, same shape as posterAppStageTitle /
        // posterAppStageSub / posterAppStageHint for the "Apply to post" flow -- the wrapper
        // (careerStartFormHTML) renders these above the step body instead of each step
        ...
        function careerStartStepTitle(stepId){
          ...
          if (stepId === 'contact') return 'How can we reach you?';
        ...
        function careerStartStepSub(stepId){
          ...
          if (stepId === 'contact') return 'Full name, email, and an optional phone number.';
        ...
        function careerStartStepHint(stepId){
          ...
          if (stepId === 'contact') return "We'll email match updates to the address above.";
          if (stepId === 'resume') return 'A resume increases your chances of getting a response -- but you can still submit without one.';
          return '';
        }
        function careerStartStepBodyHTML(){
          ...
          if (stepId === 'resume') {
            return careerStartResumePickerHTML();
          }
          return '';
        }
        ...
        function careerStartFormHTML(){
          ...
          return `
            ...
            <div id="career-start-stage-scroll" class="flex-1 overflow-y-auto px-5" style="padding-top:26px;padding-bottom:20px;">
              <div class="max-w-2xl mx-auto">
        ...
        // before the first tap finishes (e.g. while waiting on the network sync). The flag is set
        ...
        async function submitNewOpportunity(){
          ...
          try {
            ...
            // Course stays admin-only even for an approved poster (defense in
            // depth -- the type picker already hides it for non-admins).
            ...
            // Ownership metadata for "Posted by @username", edit/delete
            // permissions, and the Admin Dashboard's user/admin split.
        ...
        // Pending poster applications + reports on live listings, fetched fresh on open.
        ...
        async function loadAdminDashboardData(){
          ...
          try {
            const [appsRes, reportsRes] = await Promise.all([
              sb.from(PROFILES_TABLE).select('user_id, poster_status, poster_intent, poster_role, poster_business_name, poster_business_info, poster_business_established, poster_business_category, poster_business_website, poster_business_document_url, poster_id_document_url, poster_photo_url, poster_applied_at').eq('poster_status', 'pending').order('poster_applied_at', { ascending: true }),
        ...
        function switchAdminDashboardTab(tab){
          ...
          refreshAdminDashboardDom();
          if (tab === 'history' && !adminHistoryLoaded && !adminHistoryLoading) loadAdminHistoryData();
        }
        ...
        async function loadAdminHistoryData(){
          ...
          try {
            const [appsRes, reportsRes] = await Promise.all([
              sb.from(PROFILES_TABLE).select('user_id, poster_status, poster_role, poster_business_name, poster_reviewed_at').in('poster_status', ['approved', 'denied']).order('poster_reviewed_at', { ascending: false }).limit(50),
            ...
            const appItems = apps.map(a => ({
              kind: 'application',
              decision: a.poster_status,
              when: a.poster_reviewed_at,
              person: peopleById[a.user_id] || null,
              businessName: a.poster_business_name,
              role: a.poster_role,
            }));
            ...
            adminHistoryItems = [...appItems, ...reportItems].sort((a, b) => new Date(b.when || 0) - new Date(a.when || 0));
        ...
        function adminHistoryCardHTML(item){
          ...
          if (item.kind === 'application') {
            badge = item.decision === 'approved'
              ? `<span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-600 flex-shrink-0">Approved</span>`
              : `<span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-rose-100 text-rose-600 flex-shrink-0">Denied</span>`;
            summary = `Poster application by ${who}${item.businessName ? ' · ' + escapeHtml(item.businessName) : ''}`;
          } else {
            badge = item.decision === 'listing_removed'
              ? `<span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-rose-100 text-rose-600 flex-shrink-0">Listing removed</span>`
              : `<span class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 flex-shrink-0">Dismissed</span>`;
            summary = `Report on ${item.job ? escapeHtml(item.job.title) : 'a listing'} by ${who}`;
          }
          return `
        ...
        function adminDashboardTabPillsHTML(){
          const tabs = [['applications', `Poster applications${adminPosterApplications.length ? ` (${adminPosterApplications.length})` : ''}`], ['reports', `Reports${adminOpenReports.length ? ` (${adminOpenReports.length})` : ''}`], ['history', 'History']];
          return `
            <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1 mb-4">
              ${tabs.map(([key, label]) => `
                <button onclick="switchAdminDashboardTab('${key}')" class="flex-shrink-0 px-4 py-2 rounded-full text-xs font-semibold ${adminDashboardTab===key ? '' : 'bg-white text-gray-500 border border-gray-200'}" style="${adminDashboardTab===key ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">${label}</button>
              `).join('')}
            </div>`;
        }

        // Guesses whether a poster-application attachment URL is an image (shown as an actual
        // thumbnail) or a document like a PDF (opened in the same in-app doc viewer messaging
        // uses -- see openConvoDocViewer/openConvoImageViewer in chat.js). Business/ID uploads
        // accept "image/*,.pdf" at upload time, so this covers both cases.
        ...
        // was actually submitted at a glance, and open it in place -- same in-app viewers used in
        // Messaging (chat.js) -- rather than a plain "Photo"/"ID" label that opens a new tab.
        function adminAttachmentPreviewHTML(url, label){
          ...
          if (kind === 'image') {
            return `
              <button type="button" onclick="openConvoImageViewer('${encodeURIComponent(url)}')" class="flex items-center gap-2 pl-1 pr-3 py-1 rounded-full bg-gray-100" style="max-width:160px;">
                <img src="${escapeHtml(url)}" class="w-8 h-8 rounded-full object-cover flex-shrink-0" alt="${escapeHtml(label)}">
                <span class="text-[11px] font-semibold text-gray-600 truncate">${escapeHtml(label)}</span>
              </button>`;
          }
          const fileName = `${label}${/\.pdf$/i.test(url) ? '.pdf' : ''}`;
          return `<button type="button" onclick="openConvoDocViewer('${encodeURIComponent(url)}','${encodeURIComponent(fileName)}')" class="flex items-center gap-1 px-3 py-1.5 rounded-full text-[11px] font-semibold bg-gray-100 text-gray-600">${Icon('doc','w-3.5 h-3.5')} ${escapeHtml(label)}</button>`;
        }
          ...
          return `
            <div class="bg-white rounded-3xl p-4 shadow-sm mb-3">
              <div class="flex items-center gap-3 mb-2">
                ...
                <div class="flex-1 min-w-0">
                  <div class="font-semibold text-sm truncate">${escapeHtml(p.name || 'Unnamed user')}</div>
              ...
              <div class="flex flex-wrap gap-2 mb-3">
                ${adminAttachmentPreviewHTML(app.poster_photo_url, 'Photo')}
                ${adminAttachmentPreviewHTML(app.poster_id_document_url, 'ID')}
                ${app.poster_business_document_url ? adminAttachmentPreviewHTML(app.poster_business_document_url, 'Business doc') : ''}
              </div>
              <div class="flex gap-2">
        ...
        function adminReportCardHTML(report){
          ...
          return `
            <div class="bg-white rounded-3xl p-4 shadow-sm mb-3">
              ...
              <div class="text-xs text-gray-600 bg-gray-50 rounded-2xl p-3 mb-3" style="white-space:pre-wrap;">${escapeHtml(report.reason || 'No reason given.')}</div>
              <div class="flex gap-2">
        ...
        function adminDashboardHTML(){
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto px-5" style="padding-bottom:50px;">
              ...
              ${adminDashboardTab === 'history'
                ? (adminHistoryLoading ? `<div class="text-center text-gray-400 text-sm py-8">Loading…</div>` : (
                    adminHistoryItems.length ? adminHistoryItems.map(adminHistoryCardHTML).join('') : `<div class="bg-gray-50 border border-dashed border-gray-300 rounded-3xl p-8 text-center text-gray-500 text-sm">No past activity yet.</div>`
                  ))
                : (adminDashboardLoading ? `<div class="text-center text-gray-400 text-sm py-8">Loading…</div>` : (
                    adminDashboardTab === 'applications'
                      ? (adminPosterApplications.length ? adminPosterApplications.map(adminApplicationCardHTML).join('') : `<div class="bg-gray-50 border border-dashed border-gray-300 rounded-3xl p-8 text-center text-gray-500 text-sm">No pending poster applications.</div>`)
                      : (adminOpenReports.length ? adminOpenReports.map(adminReportCardHTML).join('') : `<div class="bg-gray-50 border border-dashed border-gray-300 rounded-3xl p-8 text-center text-gray-500 text-sm">No open reports.</div>`)
                  ))
              }
            </div>`;
        }
