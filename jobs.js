            ...
            <div id="career-filterbar" class="sticky z-10 bg-gray-50 px-5 pb-3 border-b border-gray-100">
              <div class="flex gap-2 overflow-x-auto no-scrollbar pb-1">
                ${jobsTabs.map(([key,label]) => `
                  <button onclick="jobsSubTab('${key}')" class="career-tab-pill flex-shrink-0 px-4 py-2 rounded-full text-xs font-semibold ${jobsSub===key ? '' : 'bg-white text-gray-500 border border-gray-200'}" style="${jobsSub===key ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">${label}</button>
                `).join('')}
              </div>
            </div>
        ...
        function jobCard(job){
          ...
          return `
            <div onclick="${job.isCatalogCourse ? `openCourseDetail('${job.id}')` : `openJobOrClassroom('${job.id}')`}" class="career-job-card rounded-3xl p-4 text-white relative overflow-hidden mb-4 shadow-sm cursor-pointer" style="${cardBackground}min-height:96px;">
