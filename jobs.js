        ...
        // Shown on the (optional/skippable) industries step -- a bigger,
        // centered icon-grid of pills rather than the plain full-width
        // rows the other steps use, plus a standalone "any industry" row.
        const careerIndustryOptions = [
          { id: 'it', label: 'IT & Software', icon: 'monitor' },
          { id: 'healthcare', label: 'Healthcare', icon: 'heart' },
          { id: 'finance', label: 'Finance & Insurance', icon: 'coin' },
          { id: 'sales', label: 'Sales & BizDev', icon: 'briefcase' },
          { id: 'marketing', label: 'Marketing & PR', icon: 'bolt' },
          { id: 'retail', label: 'Retail & Wholesale', icon: 'gift' },
          { id: 'education', label: 'Education', icon: 'graduate' },
          { id: 'hr', label: 'HR & Recruiting', icon: 'users' },
          { id: 'hospitality', label: 'Hospitality & Tourism', icon: 'star' },
        ];
        ...
        const careerStartStepIds = ['interests', 'keyword', 'experience', 'education', 'industries', 'workStyle', 'contact', 'resume'];
        ...
        const careerStartOptionalSteps = ['industries'];
        let careerStartStepIndex = 0;
        let careerStartDraft = { interests: [], jobTitle: '', experienceLevel: '', education: '', industries: [], workStyle: '', fullName: '', email: '', phone: '', resumeFileName: '', resumeFile: null };
        ...
        function resetCareerStartDraft(){
          ...
          careerStartDraft = { interests: [], jobTitle: '', experienceLevel: '', education: '', industries: [], workStyle: '', fullName: '', email: '', phone: '', resumeFileName: '', resumeFile: null };
          careerStartErrors = { interests: '', jobTitle: '', experienceLevel: '', education: '', workStyle: '', fullName: '', email: '', phone: '', resume: '' };
        }
        // Called by #career-start-pill in the app shell. Opening the form is
        // the pill's whole purpose, so once that happens it's dismissed for
        // good (see markCareerStartPillDismissed/isCareerStartPillDismissed
        // in core.js) instead of popping back up on later visits.
        ...
        // "Any industry" is mutually exclusive with picking specific ones --
        // choosing it clears the rest, and choosing a specific industry
        // while "any" is active drops "any" in favor of the specific pick.
        function toggleCareerStartIndustry(id){
          if (id === 'any') {
            careerStartDraft.industries = careerStartDraft.industries.includes('any') ? [] : ['any'];
          } else {
            let list = careerStartDraft.industries.filter(x => x !== 'any');
            const idx = list.indexOf(id);
            if (idx === -1) list.push(id); else list.splice(idx, 1);
            careerStartDraft.industries = list;
          }
          rerenderCareerStart();
        }
        ...
        function careerStartOptionListHTML(options, selectedId, setterName, errorKey, withDesc){
          ...
          return `
            <div class="flex flex-col gap-2.5 mb-1">
              ${options.map(o => `
                <button onclick="${setterName}('${o.id}')" class="w-full px-4 py-3.5 rounded-2xl text-left text-sm font-medium" style="${selectedId === o.id ? `background:rgba(10,37,64,0.08);border:1.5px solid ${NAVY};color:${NAVY};` : 'background:#fff;border:1px solid #e5e7eb;color:#1f2937;'}">
                  ${o.label}
        ...
        function careerStartStepBodyHTML(){
          ...
          if (stepId === 'interests') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-5">What are you looking for?</h2>
              <div class="flex flex-col gap-2.5 mb-1">
                ${careerInterestOptions.map(o => `
                  <button onclick="toggleCareerStartInterest('${o.id}')" class="w-full px-4 py-3.5 rounded-2xl text-left text-sm font-medium" style="${d.interests.includes(o.id) ? `background:rgba(10,37,64,0.08);border:1.5px solid ${NAVY};color:${NAVY};` : 'background:#fff;border:1px solid #e5e7eb;color:#1f2937;'}">${o.label}</button>
                `).join('')}
              </div>
              ${careerStartErrors.interests ? `<div class="field-error">${escapeHtml(careerStartErrors.interests)}</div>` : ''}
              <div class="text-xs text-gray-400 mt-3">Pick as many as apply -- we'll widen your matches across all of them.</div>`;
          }
          if (stepId === 'keyword') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-5">What role, subject, or keyword are you searching for?</h2>
              ${careerStartFieldHTML('Keyword', 'jobTitle', 'text', 'e.g. Project Manager, Nursing, Climate advocacy')}`;
          }
          if (stepId === 'experience') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-5">Which experience level fits you best?</h2>
              ${careerStartOptionListHTML(careerExperienceLevels, d.experienceLevel, 'setCareerStartExperience', 'experienceLevel', true)}`;
          }
          if (stepId === 'education') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-5">What's your highest level of education?</h2>
              ${careerStartOptionListHTML(careerEducationLevels, d.education, 'setCareerStartEducation', 'education', false)}`;
          }
          if (stepId === 'industries') {
            const isAny = d.industries.includes('any');
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-5">What industries are you interested in?</h2>
              <div class="grid grid-cols-2 gap-3 max-w-sm mx-auto mb-3">
                ${careerIndustryOptions.map(o => {
                  const on = d.industries.includes(o.id);
                  return `
                  <button onclick="toggleCareerStartIndustry('${o.id}')" class="flex flex-col items-center justify-center text-center gap-2 px-3 py-5 rounded-2xl text-sm font-medium" style="${on ? `background:rgba(10,37,64,0.08);border:1.5px solid ${NAVY};color:${NAVY};` : 'background:#fff;border:1px solid #e5e7eb;color:#1f2937;'}">
                    <span style="color:${on ? NAVY : '#9ca3af'};">${Icon(o.icon, 'w-6 h-6')}</span>
                    <span>${o.label}</span>
                  </button>`;
                }).join('')}
              </div>
              <button onclick="toggleCareerStartIndustry('any')" class="w-full max-w-sm mx-auto flex items-center gap-3 px-4 py-3.5 rounded-2xl text-left text-sm font-medium mb-2" style="${isAny ? `background:rgba(10,37,64,0.08);border:1.5px solid ${NAVY};color:${NAVY};` : 'background:#fff;border:1px solid #e5e7eb;color:#1f2937;'}">
                <span class="w-4 h-4 rounded-full flex-shrink-0" style="border:1.5px solid ${isAny ? NAVY : '#d1d5db'};background:${isAny ? NAVY : 'transparent'};"></span>
                I'm open to any industry
              </button>
              <div class="text-xs text-gray-400 mt-3 text-center max-w-sm mx-auto">Optional -- pick as many as apply, or skip this step.</div>`;
          }
          if (stepId === 'workStyle') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-5">How would you like to work or study?</h2>
          ...
          if (stepId === 'contact') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-5">How can we reach you?</h2>
          ...
          if (stepId === 'resume') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-3">Get 3x more matches with your resume</h2>
              <p class="text-sm text-gray-500 mb-5">We'll match your experience and skills to the best-fit opportunities.</p>
        ...
        // (resume) step -- submits the whole quiz.
        function careerStartNext(){
          const stepId = careerStartStepIds[careerStartStepIndex];
          const d = careerStartDraft;
          careerStartErrors = { interests: '', jobTitle: '', experienceLevel: '', education: '', workStyle: '', fullName: '', email: '', phone: '', resume: '' };

          if (stepId === 'interests' && !d.interests.length) {
            careerStartErrors.interests = 'Pick at least one to continue.';
          }
          if (stepId === 'keyword' && !d.jobTitle.trim()) {
            careerStartErrors.jobTitle = "Tell us what you're searching for.";
          }
          if (stepId === 'experience' && !d.experienceLevel) {
            careerStartErrors.experienceLevel = 'Pick the option that fits you best.';
          }
          if (stepId === 'education' && !d.education) {
            careerStartErrors.education = 'Pick your highest level of education.';
          }
          if (stepId === 'workStyle' && !d.workStyle) {
            careerStartErrors.workStyle = "Pick how you'd like to work or study.";
          }
          if (stepId === 'contact') {
        ...
        async function submitCareerStartForm(){
          ...
          const industryLabels = d.industries.includes('any') ? 'Open to any industry' : careerIndustryOptions.filter(o => d.industries.includes(o.id)).map(o => o.label).join(', ');
          ...
          try {
            // Unlike the suggestion pill/Contact Us/Report an Issue forms
            // (plain JSON body), this one actually attaches the resume
            // file itself via multipart/form-data -- Formspree accepts
            // real file uploads this way, it just can't be done with a
            // JSON body or a mailto: link (see the mailto fallback below).
            ...
            if (industryLabels) fd.append('industries', industryLabels);
            ...
            console.error('Career start form send failed, falling back to mailto:', err);
            window.reportError(err, { form: 'career-start', jobTitle: d.jobTitle });
            // Mailto can't carry the actual file, same limitation noted on
            // the job-application flow above -- so this fallback path
            // spells out the resume's filename (if any) and asks the
            // student to attach it by hand before sending.
            const subject = encodeURIComponent('Stitch - Career Space: ' + d.fullName);
            const body = encodeURIComponent(
              `Full name: ${d.fullName}\nEmail: ${d.email}${d.phone.trim() ? `\nPhone: ${d.phone.trim()}` : ''}\nInterested in: ${interestLabels}${industryLabels ? `\nIndustries: ${industryLabels}` : ''}\nLooking for: ${d.jobTitle}\nExperience level: ${levelLabel}\nEducation: ${educationLabel}\nWork style: ${workStyleLabel}` +
              (d.resumeFileName ? `\n\nPlease attach your resume/CV (${d.resumeFileName}) to this email before sending -- it couldn't be sent automatically.` : '')
            );
          ...
          resetCareerStartDraft();
          closeOverlay();
          openAppAlertModal("Thanks! We've got your details -- we'll follow up at " + email + " with opportunities worth a look.");
        }
        function careerStartFormHTML(){
          ...
          // Optional steps (currently just industries) show "Skip" instead
          // of "Continue" while nothing's picked yet -- either way the
          // click goes through with no validation, since the step is
          // allowed to be empty.
          const isOptionalEmpty = careerStartOptionalSteps.includes(stepId) && stepId === 'industries' && !careerStartDraft.industries.length;
          const mainBtnLabel = isLastStep ? 'Submit' : (isOptionalEmpty ? 'Skip' : 'Continue');
          const mainBtnOnclick = isOptionalEmpty ? 'careerStartStep(1)' : 'careerStartNext()';
          return `
            <div class="flex-shrink-0 w-full" style="padding-top:20px;">
              <div class="max-w-2xl mx-auto px-5 pb-3 flex items-center gap-3">
                ...
                <div class="font-semibold text-base font-display grad-text flex-1">Match with CV</div>
                <div class="text-xs font-bold px-2.5 py-1 rounded-full flex-shrink-0" style="border:1px solid #e5e7eb;color:#6b7280;">${stepNum}/${total}</div>
              </div>
            ...
            <div class="flex-1 overflow-y-auto px-5 pt-5">
              ...
              <button id="career-start-submit-btn" onclick="${mainBtnOnclick}" class="w-full font-semibold text-sm py-3 rounded-2xl text-white mt-6" style="background:${NAVY};">${mainBtnLabel}</button>
              <div class="flex items-center justify-between gap-3 mt-3" style="margin-bottom:16px;">
