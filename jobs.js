        ...
        // Shared markup for the Career Space filter pills (All / Opportunities / Internships / Courses / Scholarships / Others) -- used by both the initial...
        ...
        // The pill row rebuilds as a fresh DOM node every time the sub-tab changes (see jobsSubTab and the two renderJobMarket() calls after posting/editing an...
        ...
        function renderJobMarket(){
          // Same fix as the Inbox: keep whatever scroll position the user was at instead of snapping back to the top every time an action (saving a job,...
        ...
        // The dropdown is opened/closed by directly inserting/removing DOM nodes rather than re-rendering the whole career screen; a full re-render restarts...
        ...
        function fitCareerMenuDropdownWidth(){
          ...
          if (!dropdown) return;
          // The dropdown has left/right padding (menu-dropdown-inset, for the pill highlight to sit inset from the edges) that has to be added back on top of the...
        ...
        // Best-effort remote insert so a posted opportunity is actually shared with other users instead of only living in this browser tab.
        async function postOpportunityInsertRemote(job){
          ...
          try {
            ...
            // upsert, not insert: this runs both when a brand-new listing is posted AND when an existing one is edited (see submitNewOpportunity above) -- insert()...
        ...
        // Same best-effort pattern in reverse: deletes are only actually enforced by a Postgres RLS policy on OPPORTUNITIES_TABLE (mirror the "insert" policy...
        ...
        // Career opportunities live in one data store (instead of being generated inline) so a listing's saved state persists and can be shown back in Settings...
        let jobsData = {
          opportunities: [],
          internships: [],
          courses: [],
        ...
        // Loads every posted opportunity/internship/course listing from Supabase into jobsData -- called once on sign-in (see authEnterApp in games.js), same...
        ...
        // ---- Bringing Study > Courses catalog courses into Explore ---- A "Course" opportunity posted from Explore (createLinkedCourse above) already gets a...
        ...
        // Every course shown under Explore's Courses tab (and All): any posted "Course" opportunity, plus every other catalog course that wasn't posted as one...
        ...
        // Everything Explore can show at once -- backs the "All" tab and search, so a catalog-only course is findable there too.
        ...
        // ---- Per-account application / enrollment tracking ---- Applying to an opportunity/internship, or enrolling in a course posted here, used to set...
        ...
        // ---- Employer reply tracking ---- Stitch can't read the employer's inbox, but once someone applies we let them log what happened next (still waiting,...
        ...
        // Same lookup as findJob, but also covers catalog-only courses (allExploreCards() includes exploreCourseCards(), which pulls in Study > Courses catalog...
        ...
        function deleteOpportunity(id){
          ...
          openAppConfirmModal(`Delete "${job.title}"?`, `This removes this ${isCourse ? 'course' : 'listing'} for everyone and can't be undone.`, 'Delete', function(){
            ...
            // The listing's linked catalog course (modules/quizzes/progress) has no purpose without it, so it goes too -- mirrors deleteCourse() in courses.js but...
        ...
        function jobCard(job){
          // Same gradient-banner-plus-motif treatment as a My Classes card (see myClassesListHTML in study.js), but always some shade of blue (blueCardPalette in...
        ...
        // Whether the three-dot admin menu (Edit / Delete) is open on the opportunity detail screen -- see jobDetailMenuDropdownHTML.
        ...
        // Dropdown for the opportunity detail three-dot menu: lets admins edit or delete the listing/course, matching the treatment used for the course detail...
        ...
        // Tapping an opportunity card normally opens the job/course detail overlay -- but once a Course has been enrolled in, it has become a real class, so...
        ...
        // Marks the listing as applied and, when the poster left a contact email, opens the user's mail app with a pre-filled subject line.
        function createLinkedCourse(job){
          const course = {
            ...
            // Back-reference so deleting the opportunity can also clean up this course, and so it's easy to tell a course originated from an opportunity listing...
        ...
        // Keeps the linked course's title/org/description/photo in step whenever the opportunity listing itself is edited -- never touches its...
        ...
        // Enrolls the learner in the course linked to this opportunity (idempotent -- re-enrolling is a no-op), mirroring the enrolledCourseIds push in...
        ...
        // Opens the course linked to a Course opportunity, lazily creating it first if this listing predates job.courseId existing (e.g.
        ...
        function finalizeJobApplication(id){
          ...
          if (job.applyMethod === 'website' && job.website) {
            // Their applicant tracking is on their own site -- Stitch's job here is just to get the person there with the details they typed still on hand (nothing...
            ...
            let bodyLines;
            if (isCourse) {
              // Enrolling isn't an application -- no cover letter, no "I'd like to apply" language.
              bodyLines = [
                'Hi,',
                '',
        ...
        function updateJobApplyField(field, value){
          ...
          // Clear any inline error for this field as soon as the person starts fixing it, without re-rendering the whole overlay (that would reset the input's...
        ...
        // ---------------- "Don't know where to start?" intake quiz ---------------- Reached from the Career Space floating pill (#career-start-pill in core.js...
        ...
        // Order the quiz screens step through, in order.
        const careerStartStepIds = ['interests', 'keyword', 'experience', 'education', 'workStyle', 'contact', 'resume'];
        // Steps that don't require an answer to move on -- their primary button reads "Skip" instead of "Continue" while empty, and the Previous/Next row...
        const careerStartOptionalSteps = ['interests', 'keyword', 'experience', 'education', 'workStyle'];
        // Whether the given (optional) step currently has no answer -- drives the Skip-vs-Continue label in careerStartFormHTML.
        ...
        // ---------------- Career Profile (persisted "Match with CV" result) ---------------- Everything submitted through the quiz below is saved locally so...
        ...
        // Called (chained after ensureUserStateLoaded -- see essentialLoads in games.js) once the account's synced state has already had a chance to populate...
        async function loadCareerStartProfile(){
          try {
            ...
            // Keep the local cache in step with whichever copy won, so a reload of this device (before the network round trip lands again) starts from the right...
            if (key && winner) { try { localStorage.setItem(key, JSON.stringify(winner)); } catch (e) { /* ignore */ } }
          } catch (e) { /* keep whatever applyUserState already set, if anything */ }
        }
        async function saveCareerStartProfile(){
          ...
          // Also queue it into the account's synced state blob (core.js) so it follows the signed-in user to their other devices -- see careerStartProfile in...
          if (typeof queueSaveUserState === 'function') queueSaveUserState();
        }
        // Wraps a File in a Promise<dataURL> -- same FileReader pattern used for cover-photo uploads elsewhere in this file, just promisified so it can run...
        ...
        // Best-effort text extraction for the AI/local matcher below -- reuses the same PDF/DOCX extractors Study Materials already loads...
        ...
        // Playful multi-phase "processing" animation shown for a few seconds right after a resume/CV file is picked, before the attached-file state appears --...
        ...
        // Called by #career-start-pill (and the "Match with CV" menu item) in the app shell.
        ...
        // Reopens the quiz pre-filled with whatever's already saved, for the "Edit answers" / "Replace resume" actions on the Career Profile page -- unlike...
        function startCareerStartQuiz(returnKind){
          ...
          // Called both fresh (no returnKind -- back exits straight out, same as before) and from the Career Profile page's "Edit" / "Get Started" actions...
        ...
        // On-screen back arrow and the hardware/browser back button (overlayBackAction.careerStart in overlays.js) both route here.
        ...
        function handleCareerStartResumeSelect(event){
          ...
          // Runs alongside the fake "processing" animation above -- by the time it finishes (or shortly after), the plain-text copy (what actually powers...
          ...
          if (typeof uploadResumeFileToStorage === 'function') {
            uploadResumeFileToStorage(file).then(url => {
              ...
              if (careerStartDraft.resumeFile === file) {
                ...
                // The quiz was already submitted (draft reset) by the time this resolved -- patch the real, already-saved profile instead so the sync still lands.
        ...
        // Two-option segmented toggle for how the student wants to hear about their matches -- 'email' sends real emails via sendEmailNotification, 'inApp'...
        ...
        // Single-select list of pill/card buttons, shared by the experience/education/work-style steps below -- each just passes its own option list, selected...
        ...
        // Validates the current step only, then either blocks with an inline error, advances to the next step, or -- on the final (resume) step -- submits the...
        ...
        // Jumps a step forward/back with no validation at all -- backs the Previous/Next nav row and the "Skip" button on optional steps (see...
        ...
        async function submitCareerStartForm(){
          ...
          // Best-effort admin notification only -- unlike before, nothing here blocks or redirects the student if it fails.
          try {
          ...
          // Save (or update) the Career Profile locally -- this is what actually persists across visits now, independent of whether the Formspree notification...
          ...
          // Only stop nudging once they've actually attached a resume/CV -- if they submitted without one, the pill (and "Match with CV" menu item) should keep...
          if (careerStartProfile.resumeFileName) markCareerStartPillDismissed();
          resetCareerStartDraft();
          // Show the "Matching your CV/Resume" animation while the AI screens every posted opportunity against the fresh answers/ resume, then land on the Career...
          runCareerMatchingAnimation();
        }

        // ---------------- "Matching your CV/Resume" loading page ---------------- Full-screen animated interstitial shown right after Submit, between the quiz...
        ...
        async function runCareerMatchingAnimation(){
          ...
          // refreshCareerMatches() does the real work (AI pass, falling back to local scoring) and saves the result onto careerStartProfile -- run it alongside...
          await Promise.all([refreshCareerMatches(), minWait]);
          clearCareerMatchingTimers();
          // Only auto-advance into the results if the student is still sitting on this loading screen -- if they backed out of it early, don't yank them into a...
        ...
        function careerStartFormHTML(){
          ...
          // Optional steps show "Skip" instead of "Continue" while nothing's filled in yet -- either way the click goes through with no validation, since the...
          const isOptionalEmpty = careerStartOptionalSteps.includes(stepId) && careerStartStepIsEmpty(stepId, careerStartDraft);
          // While the resume's fake "processing" animation is running, block Submit so the student can't fire the request mid-animation -- see...
        ...
        // ---------------- Career Profile page (resume + matched opportunities) ---------------- Reached from the Career Space pill/menu (openCareerStartForm),...
        ...
        // Routes a Career Space update to whichever channel the student picked on the "How can we reach you?" step (see careerContactMethodToggleHTML).
        ...
        async function refreshCareerMatches(){
          ...
          try {
            ...
            // Only notify on a re-check that turned up something genuinely new -- not on the very first match computation right after submitting the quiz, which...
            if (hadMatchesBefore) {
            ...
            // Pro Plus: no recommendations, but still gets Claude's take on the resume itself.
        ...
        // Swapping in a new resume from the profile page (rather than going through the whole quiz again) -- re-extracts text/base64 the same way...
        function handleCareerProfileResumeReplace(event){
          ...
          Promise.all([extractCareerStartResumeText(file), fileToDataUrl(file)]).then(([text, dataUrl]) => {
            ...
            careerStartProfile.resumeDataUrl = dataUrl; // instant local preview
            saveCareerStartProfile();
            markCareerStartPillDismissed();
            refreshCareerMatches();
          });
          // Swap that local preview for the real Storage URL once it's up, same as handleCareerStartResumeSelect above -- this is what actually makes the...
          if (typeof uploadResumeFileToStorage === 'function') {
            uploadResumeFileToStorage(file).then(url => {
        ...
        // Trash icon next to the attached CV on the Career Profile page -- clears just the resume (file/text/data-url), keeping the rest of the quiz answers...
        ...
        // ---------------- Opportunity screening / matching engine ---------------- Turns the quiz's raw ids (interests/experience/education/ workStyle) into...
        ...
        // Screens every currently posted listing (allExploreCards -- opportunities, internships, courses, scholarships, "others") against the candidate's saved...
        function planHasCvRecommendations(){
          return typeof currentPlanFeatures === 'function' && currentPlanFeatures().cvRecommendations;
        }
        function planHasCvAI(){
          return typeof currentPlanFeatures === 'function' && currentPlanFeatures().cvAi;
        }
        async function computeCareerMatches(profile){
          ...
          try {
            ...
            // AI ran fine but genuinely found nothing (e.g.
        ...
        // ---------------- Pro Plus: AI resume feedback (no recommendations) ---------------- Pro Plus unlocks Claude actually reading the resume itself --...
        ...
        async function computeCareerMatchesAI(profile, jobs){
          ...
          // Capped at 60 listings / ~500 chars of description each to keep the prompt a reasonable size -- Career Space listings are short by nature (see...
          ...
          if (parsed.priority && parsed.priority.id) {
            ...
            // Only trust it if the AI's pick is actually one of the matches we're about to show -- otherwise fall back to the top-scored match below rather than...
        ...
        // Local fallback: plain keyword-overlap scoring between the resume text (plus typed keyword) and each listing's title/org/ description, with bonus...
        ...
        function computeCareerMatchesLocal(profile, jobs){
          ...
          // Nothing scored at all (blank profile, or a genuinely poor fit across the board) -- still surface something rather than an empty page, just labeled...
          if (!scored.length) {
          ...
          // Local scorer has no real "reasoning" to lean on beyond the score itself, so the priority pick is simply the top-ranked match -- still worth calling...
        ...
        // Small score/reason strip shown under each matched jobCard -- jobCard itself is reused unmodified for the listing above it.
        ...
        // Highlighted callout the AI's single "act on this first" pick (see the "priority" field computeCareerMatchesAI/Local return) -- sits above the ranked...
        ...
        function careerMatchesHTML(){
          ...
          if (!p) {
            return `
              ...
              <div class="flex-1 overflow-y-auto px-5 pb-6">
                ...
                <button onclick="startCareerStartQuiz('careerMatches')" class="w-full font-semibold text-sm py-3 rounded-2xl text-white mt-4" style="background:${NAVY};">Get Started</button>
              </div>`;
          }
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto px-5 pb-6">
              ...
              ${(!planHasCvRecommendations()) ? `
              <div class="rounded-3xl p-4 mb-4 border" style="border-color:rgba(65,105,225,0.25);background:rgba(65,105,225,0.06);">
                <div class="text-sm font-semibold mb-1" style="color:${NAVY};">${planHasCvAI() ? 'Unlock AI opportunity recommendations' : 'Unlock AI resume reading & recommendations'}</div>
                <div class="text-xs text-gray-500 mb-3">${planHasCvAI()
                  ? 'You\'re on Pro Plus, so Stitch already reads your resume (see below). Upgrade to Pro Max to have it rank and recommend specific posted opportunities for you.'
                  : 'The matches below are a basic keyword match. Subscribe to Pro Plus or Pro Max to have Stitch actually read your resume, and Pro Max to get it ranking and recommending specific opportunities for you.'}</div>
                <button onclick="openOverlayFrom('careerMatches','subscriptionSettings')" class="text-xs font-semibold py-2 px-4 rounded-2xl text-white" style="background:${NAVY};">See plans</button>
              </div>` : ''}
              ${(planHasCvAI() && !planHasCvRecommendations() && p.resumeAIFeedback) ? `
              <div class="bg-white rounded-3xl p-4 mb-4 shadow-sm">
        ...
        function submitJobApplication(){
          ...
          // Paid courses go through Paystack first -- finalizeJobApplication (the actual enrollment) only runs after the payment has been verified server-side,...
        ...
        // Shared tail end of both the free-enrollment/application path and the paid-course path (after payment is verified) -- actually records the...
        function completeJobApplication(job){
          finalizeJobApplication(job.id);
          resetJobApplyDraft();
          // Enrolling in a Course should land the learner straight in the classroom they just joined -- same place tapping it from My Classes/the opportunity...
          if (job.type === 'Course') {
            goToJobCourse(job.id);
          } else {
        ...
        // ---------------- Paystack payment (paid course enrollment) ---------------- Public key is safe to ship client-side -- it can only be used to *open* a...
        ...
        // Course prices are stored as the free-text string an admin typed in (e.g.
        ...
        async function startPaystackPayment(job){
          ...
          // See teardownPaystackPopup() in overlays.js: wraps the shared history-pop + DOM-cleanup with this checkout's own "payment in progress" flag reset and...
          ...
          const handler = window.PaystackPop.setup({
            ...
            callback: function(response){
              ...
              // Paystack reports success client-side, but that alone is never trusted -- verifyPaystackAndEnroll re-checks the transaction against Paystack's own...
        ...
        // Set while editing an existing opportunity/course instead of creating a new one -- see openEditOpportunity and the newOppEditingId check inside...
        ...
        // Cover picture: stored as a base64 data URL directly on the listing (same pattern as a class's photo in courses.js), shown as a banner on the...
        ...
        // Opens the post-opportunity form fresh, for creating a brand new listing (the "Post an Opportunity" career menu item).
        ...
        // Loads an existing opportunity/course into the same draft/form used for creating one, so admins can tweak it in place.
        ...
        // How applicants get in touch: an org's own mailbox (mailto, handled inside Stitch's Apply form) or their own external site (opens in a new tab instead...
        ...
        async function submitNewOpportunity(){
          ...
          // Editing an existing listing: update it in place (possibly moving it between buckets if the type changed) instead of creating a new one, same pattern...
          if (newOppEditingId) {
            ...
            if (existing) {
              ...
              // A Course opportunity that somehow doesn't have a linked catalog course yet (e.g.
              if (isCourse) {
                if (!existing.courseId) existing.courseId = createLinkedCourse(existing);
                else await syncLinkedCourseMeta(existing);
              }
              // Awaited now (was previously fire-and-forget) so a sync failure -- e.g.
            ...
            // Fell through: the listing no longer exists (e.g.
            newOppEditingId = null;
          }

          const job = Object.assign({ id: 'opp' + Date.now(), saved:false, applied:false, mine: true }, fields);
          // Fixed at creation time (not re-rolled on edit, since fields above never carries these) so the card's color/motif stays put wherever it's shown --...
          ...
          bucket.unshift(job);
          // A Course opportunity gets a matching (initially empty) course in the Coursera-style catalog right away, linked via job.courseId -- the admin fleshes...
          if (isCourse) job.courseId = createLinkedCourse(job);
          // Best-effort: also persist it so other users see it too.
