const jobsTabs = [['all','All'],['opportunities','Opportunities'],['internships','Internships'],['courses','Courses'],['scholarships','Scholarships'],['others','Others']];
        ...
        function posterAppStageHint(stage){
          if (stage === 'bizdesc') return "This goes straight to an admin's review queue.";
        ...
        function posterAppStageHTML(stage){
          ...
          if (stage === 'bizdesc') {
            return `
              <textarea id="poster-app-input" autocomplete="off" maxlength="100" oninput="updatePosterAppField('businessInfo', this.value); document.getElementById('poster-app-bizdesc-counter').textContent = this.value.length + '/100';" placeholder="e.g. Internships and design roles for students." rows="3" class="${fieldCls} resize-none">${escapeHtml(d.businessInfo || '')}</textarea>
          ...
          return `
            <div class="bg-white rounded-3xl p-4 shadow-sm mb-3">
              ...
              <div class="flex flex-wrap gap-2 mb-3">
                ${app.poster_photo_url ? `<a href="${escapeHtml(app.poster_photo_url)}" target="_blank" rel="noopener" class="flex items-center gap-1 px-3 py-1.5 rounded-full text-[11px] font-semibold bg-gray-100 text-gray-600">${Icon('camera','w-3.5 h-3.5')} Photo</a>` : `<span class="flex items-center gap-1 px-3 py-1.5 rounded-full text-[11px] font-semibold bg-rose-50 text-rose-500">${Icon('camera','w-3.5 h-3.5')} No photo</span>`}
                ${app.poster_id_document_url ? `<a href="${escapeHtml(app.poster_id_document_url)}" target="_blank" rel="noopener" class="flex items-center gap-1 px-3 py-1.5 rounded-full text-[11px] font-semibold bg-gray-100 text-gray-600">${Icon('doc','w-3.5 h-3.5')} ID</a>` : `<span class="flex items-center gap-1 px-3 py-1.5 rounded-full text-[11px] font-semibold bg-rose-50 text-rose-500">${Icon('doc','w-3.5 h-3.5')} No ID</span>`}
                ${app.poster_business_document_url ? `<a href="${escapeHtml(app.poster_business_document_url)}" target="_blank" rel="noopener" class="flex items-center gap-1 px-3 py-1.5 rounded-full text-[11px] font-semibold bg-gray-100 text-gray-600">${Icon('doc','w-3.5 h-3.5')} Business doc</a>` : ''}
              </div>
              <div class="flex gap-2">
