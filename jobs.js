        ...
        function jobCard(job){
          ...
          const avatarInner = job.coverImage
            ...
            : Icon(job.icon,'w-5 h-5');
          return `
            <div onclick="openJobOrClassroom('${job.id}')" class="rounded-3xl p-4 text-white relative overflow-hidden mb-4 shadow-sm cursor-pointer" style="${blueCardBackgroundStyle(job, idx === -1 ? 0 : idx)}min-height:96px;">
              ...
              <div class="flex items-center gap-4 relative">
                <div class="w-11 h-11 bg-white/15 rounded-2xl flex items-center justify-center flex-shrink-0 overflow-hidden">${avatarInner}</div>
                ...
                <button onclick="event.stopPropagation(); toggleSaveJob('${job.id}')" class="flex-shrink-0 p-1 text-white/${job.saved ? '100' : '60'}">${Icon('bookmark','w-5 h-5')}</button>
              </div>
            </div>`;
        }
        ...
        async function submitNewOpportunity(){
          ...
          if (!isCourse) {
            if (applyViaWebsite && !d.website.trim()) {
              alert('Please add the website link where applicants should apply.');
              return;
            }
            if (!applyViaWebsite && !d.email.trim()) {
              alert('Please add an email to receive applications.');
              return;
            }
          }
          if (applyViaWebsite && d.website.trim() && !isValidUrl(d.website.trim())) {
            alert('Please enter a valid website link (e.g. careers.yourorg.com/apply).');
            return;
          }
