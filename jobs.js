        ...
        const careerStartOptionalSteps = ['interests', 'keyword', 'experience', 'education', 'industries', 'workStyle'];
        ...
        function careerStartStepIsEmpty(stepId, d){
          ...
          if (stepId === 'industries') return !d.industries.length;
        ...
        // in the app shell. The pill is meant to keep nudging students who
        // haven't attached a resume/CV yet, so opening the form no longer
        // dismisses it by itself -- only actually submitting the form with
        // a resume attached does that (see submitCareerStartForm below).
        ...
        function careerStartStepBodyHTML(){
          ...
          if (stepId === 'interests') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-7 text-center leading-snug">What are you looking for?</h2>
              <div class="flex flex-col gap-3 mb-1">
          ...
          if (stepId === 'keyword') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-7 text-center leading-snug">What role, subject, or keyword are you searching for?</h2>
              ${careerStartFieldHTML('Keyword', 'jobTitle', 'text', 'e.g. Project Manager, Nursing, Climate advocacy')}`;
          }
          if (stepId === 'experience') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-7 text-center leading-snug">Which experience level fits you best?</h2>
              ${careerStartOptionListHTML(careerExperienceLevels, d.experienceLevel, 'setCareerStartExperience', 'experienceLevel', true)}`;
          }
          if (stepId === 'education') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-7 text-center leading-snug">What's your highest level of education?</h2>
          ...
          if (stepId === 'industries') {
            ...
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-7 text-center leading-snug">What industries are you interested in?</h2>
              <div class="grid grid-cols-2 gap-3 max-w-sm mx-auto mb-3">
                ${careerIndustryOptions.map(o => {
                  const on = d.industries.includes(o.id);
                  return `
                  <button onclick="toggleCareerStartIndustry('${o.id}')" class="flex flex-col items-center justify-center text-center gap-2 px-3 py-4 rounded-2xl text-sm font-medium" style="${on ? `background:rgba(10,37,64,0.08);border:1.5px solid ${NAVY};color:${NAVY};` : 'background:#fff;border:1px solid #e5e7eb;color:#1f2937;'}">
                    <span style="color:${on ? NAVY : '#9ca3af'};">${Icon(o.icon, 'w-6 h-6')}</span>
                    <span>${o.label}</span>
                  </button>`;
                }).join('')}
              </div>
              <button onclick="toggleCareerStartIndustry('any')" class="w-full max-w-sm mx-auto flex items-center gap-3 px-4 py-4 rounded-2xl text-left text-sm font-medium mb-2" style="${isAny ? `background:rgba(10,37,64,0.08);border:1.5px solid ${NAVY};color:${NAVY};` : 'background:#fff;border:1px solid #e5e7eb;color:#1f2937;'}">
          ...
          if (stepId === 'workStyle') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-7 text-center leading-snug">How would you like to work or study?</h2>
          ...
          if (stepId === 'contact') {
            return `
              <h2 class="text-xl font-bold text-gray-800 mb-7 text-center leading-snug">How can we reach you?</h2>
        ...
        async function submitCareerStartForm(){
          ...
          if (d.resumeFile) markCareerStartPillDismissed();
          resetCareerStartDraft();
          closeOverlay();
            ...
            <div class="flex-1 overflow-y-auto px-5 pt-5">
              <div class="flex flex-col justify-between" style="min-height:100%;">
                ...
                <div class="flex-shrink-0 mt-8">
                  <button id="career-start-submit-btn" onclick="${mainBtnOnclick}" class="w-full font-semibold text-sm py-3 rounded-2xl text-white" style="background:${NAVY};">${mainBtnLabel}</button>
