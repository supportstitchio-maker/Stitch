        // ---------------- JOB MARKET ----------------
        const jobsTabs = [['all','All'],['opportunities','Opportunities'],['internships','Internships'],['courses','Courses'],['applied','Applied'],['saved','Saved']];
        ...
        function allExploreCards(){
          return [...jobsData.opportunities, ...jobsData.internships, ...exploreCourseCards()];
        }
        ...
        function jobCard(job){
          ...
          return `
            <div onclick="${job.isCatalogCourse ? `openCourseDetail('${job.id}')` : `openJobOrClassroom('${job.id}')`}" class="rounded-3xl p-4 text-white relative overflow-hidden mb-4 shadow-sm cursor-pointer" style="${cardBackground}min-height:96px;">
              ...
              <div class="flex items-center gap-4 relative">
                <div class="w-11 h-11 bg-white/15 rounded-2xl flex items-center justify-center flex-shrink-0 overflow-hidden">${Icon(job.icon,'w-5 h-5')}</div>
                <div class="flex-1 min-w-0">
                  <div class="flex items-center gap-1.5">
                    <div class="font-semibold text-sm truncate">${escapeHtml(job.title)}</div>
                    ${jobStatusBadge(job)}
                  </div>
                  <div class="text-xs text-white/80 truncate">${job.sub} · ${job.deadline}</div>
                </div>
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto px-5 pb-8">
              ...
              <div class="flex items-center gap-4 mb-5">
                <div class="w-14 h-14 bg-blue-50 rounded-2xl flex items-center justify-center text-gray-600 flex-shrink-0 overflow-hidden">${job.coverImage ? `<img src="${job.coverImage}" class="w-full h-full object-cover" alt="">` : Icon(job.icon,'w-7 h-7')}</div>
                <div class="flex-1 min-w-0">
