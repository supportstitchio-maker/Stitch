        ...
        // Same lookup as findJob, but also covers catalog-only courses
        // (allExploreCards() includes exploreCourseCards(), which pulls in
        // Study > Courses catalog entries that never got a jobsData
        // listing of their own -- findJob's allJobs() doesn't see those).
        // computeCareerMatches screens against allExploreCards(), so the
        // Career Profile page needs to resolve matches against that same
        // wider pool, or a matched catalog course would score fine and
        // then silently vanish (findJob(id) returning undefined) instead
        // of getting its card rendered like every other match.
        ...
        //
        // IMPORTANT: the storage key is scoped to the signed-in user's id
        // (see careerProfileStorageKey below). It used to be a single
        // fixed key ('career-profile-v1') shared by every account that
        // ever signed in on this browser -- which meant whoever uploaded
        // a CV last had it show up, and get used for "apply", on every
        // other account too. loadCareerStartProfile() is now async and
        // must run AFTER auth resolves (it's called from authEnterApp's
        // essentialLoads in games.js, not eagerly at script load), and
        // careerStartProfile is reset to null on sign-out (authSignOut in
        // games.js) so a second account signing in on the same tab never
        // sees the previous account's in-memory profile for the instant
        // before its own load finishes.
        ...
        // Called (chained after ensureUserStateLoaded -- see essentialLoads
        // in games.js) once the account's synced state has already had a
        // chance to populate careerStartProfile from the server (see
        // applyUserState in core.js). Reconciles that synced copy against
        // this device's own localStorage cache using each one's updatedAt,
        // so: a resume attached on another device shows up here even if
        // this device never touched it locally, but a resume just attached
        // on THIS device a moment ago doesn't get reverted by an older
        // synced snapshot that simply hasn't caught up yet.
        async function loadCareerStartProfile(){
          try {
            ...
            // Keep the local cache in step with whichever copy won, so a
            // reload of this device (before the network round trip lands
            // again) starts from the right one.
        ...
        async function saveCareerStartProfile(){
          ...
          // Also queue it into the account's synced state blob (core.js)
          // so it follows the signed-in user to their other devices --
          // see careerStartProfile in collectUserState/applyUserState.
        ...
        function startCareerStartQuiz(returnKind){
          ...
          // Called both fresh (no returnKind -- back exits straight out,
          // same as before) and from the Career Profile page's "Edit" /
          // "Get Started" actions (returnKind = 'careerMatches'), in which
          // case the on-screen and hardware back button should land back
          // on that Career Profile page instead of leaving the overlay
          // system entirely -- see careerStartBack below.
        ...
        // exits the quiz outright rather than stepping back one question
        // at a time -- that's handled separately by the "Previous" button
        // in the nav row below. Where it lands depends on how the quiz was
        // reached: opened fresh (career-start-pill, "Match with CV" menu
        // item) it closes straight out to Explore same as before; opened
        // from the Career Profile page ("Edit"/"Get Started" -- see
        // startCareerStartQuiz's returnKind param) it returns to that page
        // instead of dropping the student all the way back to Explore.
        ...
        function handleCareerStartResumeSelect(event){
          ...
          // time it finishes (or shortly after), the plain-text copy (what
          // actually powers AI/local opportunity matching, see
          // computeCareerMatches) is ready, and resumeDataUrl holds an
          // instant local preview (what lets "View/Download" work right
          // away on the Career Profile page). uploadResumeFileToStorage
          // (core.js) runs in parallel and, once it resolves, swaps that
          // local preview for the real Supabase Storage URL -- the only
          // form of this that's small enough to sync across devices (see
          // careerStartProfile in collectUserState, core.js). Guarded on
          // draft.resumeFile still matching this file, in case a different
          // file was picked again before this one finished uploading.
          ...
          if (typeof uploadResumeFileToStorage === 'function') {
            uploadResumeFileToStorage(file).then(url => {
              ...
              if (careerStartDraft.resumeFile === file) {
                ...
                // The quiz was already submitted (draft reset) by the time
                // this resolved -- patch the real, already-saved profile
                // instead so the sync still lands.
        ...
        // Two-option segmented toggle for how the student wants to hear
        // about their matches -- 'email' sends real emails via
        // sendEmailNotification, 'inApp' just drops a bell notification
        // via addNotif. See notifyCareerContact below for where this is
        // actually read.
        ...
        async function submitCareerStartForm(){
          ...
          resetCareerStartDraft();
          // Show the "Matching your CV/Resume" animation while the AI
          // screens every posted opportunity against the fresh answers/
          // resume, then land on the Career Profile page (resume + quiz
          // summary, with matched opportunities screened underneath).
          runCareerMatchingAnimation();
        }

        // ---------------- "Matching your CV/Resume" loading page ----------------
        // Full-screen animated interstitial shown right after Submit,
        // between the quiz and the Career Profile/matches page -- same
        // idea as the phase-cycling animation on the resume-attach step
        // (careerStartResumeUploadPhases) but as its own overlay page
        // rather than inline, since this is the actual AI matching pass
        // (refreshCareerMatches), not just a file upload. Runs for at
        // least 5 seconds (5 phases * 1s) even if the AI responds
        // faster, so the "the AI is working on this" moment always reads
        // clearly instead of flashing by; if the AI genuinely takes
        // longer, the animation keeps cycling (looping the last phase)
        // until it's actually done rather than cutting away early.
        ...
        async function runCareerMatchingAnimation(){
          ...
          // refreshCareerMatches() does the real work (AI pass, falling
          // back to local scoring) and saves the result onto
          // careerStartProfile -- run it alongside the minimum-length
          // animation rather than after it, so a fast AI response
          // doesn't shorten the animation and a slow one doesn't get cut
          // off before it's actually finished.
          await Promise.all([refreshCareerMatches(), minWait]);
          clearCareerMatchingTimers();
          // Only auto-advance into the results if the student is still
          // sitting on this loading screen -- if they backed out of it
          // early, don't yank them into a page they didn't ask for.
        ...
        // Routes a Career Space update to whichever channel the student
        // picked on the "How can we reach you?" step (see
        // careerContactMethodToggleHTML). 'email' skips the in-app bell
        // entirely and emails them directly via sendEmailNotification
        // (still delivered to their verified account address, same as
        // every other email Stitch sends -- see send-email-function.ts);
        // 'inApp' (the default) just drops a normal bell notification via
        // addNotif, which also respects the separate global "Email
        // notifications" Settings toggle on top of that if it's on.
        function notifyCareerContact(profile, { name, message, jobId, icon } = {}){
          ...
          if (profile.contactMethod === 'email') {
            ...
            addNotif({ type: 'info', icon: icon || 'briefcase', iconBg: 'bg-blue-50', iconClass: 'text-blue-600', name: name || 'Career Space', message: message || '', jobId: jobId || null });
          }
        }
        ...
        async function refreshCareerMatches(){
          ...
          try {
            ...
            // Only notify on a re-check that turned up something genuinely
            // new -- not on the very first match computation right after
            // submitting the quiz, which already lands the student
            // straight on this page anyway.
            if (hadMatchesBefore) {
            ...
            // Pro Plus: no recommendations, but still gets Claude's take
            // on the resume itself. Skipped entirely on Pro Max/Platinum
            // (they already got the fuller AI matching pass above) and
            // on Free/Pro (no cvAi at all).
        ...
        function handleCareerProfileResumeReplace(event){
          ...
          // Swap that local preview for the real Storage URL once it's up,
          // same as handleCareerStartResumeSelect above -- this is what
          // actually makes the replacement follow the account to other
          // devices instead of only living on this one.
          if (typeof uploadResumeFileToStorage === 'function') {
            uploadResumeFileToStorage(file).then(url => {
        ...
        // Trash icon next to the attached CV on the Career Profile page --
        // clears just the resume (file/text/data-url), keeping the rest of
        // the quiz answers intact, then re-screens matches on the answers
        // alone so the list doesn't just go stale.
        ...
        // Returns { matches, priority }, where matches is the ranked list
        // (best first, same as before) and priority is either null or
        // { job, reason } singling out the one match the candidate should
        // go after first -- see the "Prioritize this" callout at the top
        // of careerMatchesHTML.
        // Gate: full AI-ranked opportunity matching + recommendations is
        // a Pro Max/Platinum feature (see PLAN_FEATURES.cvRecommendations
        // in study.js -- Pro Plus gets AI resume reading, see
        // computeResumeAIFeedback below, but not this). Free/Pro/Pro
        // Plus students still get real results here, just from the
        // local keyword/filter scorer instead of a Claude call -- the
        // page should never sit empty just because a plan doesn't
        // include the AI pass.
        ...
        // ---------------- Pro Plus: AI resume feedback (no recommendations) ----------------
        // Pro Plus unlocks Claude actually reading the resume itself --
        // short, encouraging feedback on how it reads (strengths, gaps,
        // phrasing) -- without going as far as ranking or naming
        // specific posted opportunities against it (that's
        // computeCareerMatchesAI, Pro Max/Platinum only). Cached on the
        // profile like matches are, so it doesn't re-run on every open.
        ...
        async function computeCareerMatchesAI(profile, jobs){
          ...
          if (parsed.priority && parsed.priority.id) {
            ...
            // Only trust it if the AI's pick is actually one of the
            // matches we're about to show -- otherwise fall back to the
            // top-scored match below rather than surfacing a priority
            // pick for a listing that isn't even in the visible list.
        ...
        function computeCareerMatchesLocal(profile, jobs){
          ...
          // of a tailored match. No real signal to prioritize by here,
          // so skip the priority pick entirely rather than pretend one
          // recent listing is somehow the "best" one.
          if (!scored.length) {
          ...
          // Local scorer has no real "reasoning" to lean on beyond the
          // score itself, so the priority pick is simply the top-ranked
          // match -- still worth calling out explicitly rather than
          // making the student infer it from the ordering alone.
        ...
        // Highlighted callout the AI's single "act on this first" pick
        // (see the "priority" field computeCareerMatchesAI/Local return)
        // -- sits above the ranked list itself so the recommendation
        // doesn't get lost while scrolling through every match.
