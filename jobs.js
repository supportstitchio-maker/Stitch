        ...
        async function loadCareerStartProfile(){
          try {
            ...
            if (!key) { careerStartProfile = null; return; }
        ...
        function careerMatchFilterChip(label){
          return `<span class="text-xs font-medium">${escapeHtml(label)}</span>`;
        }
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto px-5 pb-6">
              ...
              <div class="bg-white rounded-3xl p-4 mb-5 shadow-sm">
                ...
                <div class="flex flex-wrap gap-x-1.5 gap-y-1 items-baseline w-full" style="color:${NAVY};">
                  ${filterChips.length ? filterChips.map((label, i) => careerMatchFilterChip(label) + (i < filterChips.length - 1 ? '<span class="text-xs text-gray-300">&middot;</span>' : '')).join('') : '<span class="text-xs text-gray-400">No filters set yet.</span>'}
                </div>
              </div>
