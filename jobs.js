        ...
        function posterApplicationFormHTML(){
          ...
          return `
            <div id="poster-app-form">
              <div class="mb-5">
                <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2 block">What brings you to Career Space?</label>
                <div class="flex gap-2">
                  <button onclick="setPosterAppIntent('post')" class="flex-1 px-4 py-3 rounded-2xl text-sm font-semibold text-center ${d.intent==='post' ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.intent==='post' ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">Post jobs &amp; opportunities</button>
                  <button onclick="setPosterAppIntent('explore')" class="flex-1 px-4 py-3 rounded-2xl text-sm font-semibold text-center ${d.intent==='explore' ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.intent==='explore' ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">Explore &amp; apply</button>
                </div>
              </div>
              ${d.intent === 'post' ? `
                <div class="mb-5">
                  ...
                  <div class="flex gap-2 flex-wrap">
                    ${POSTER_ROLE_OPTIONS.map(([key, label]) => `
                      <button onclick="setPosterAppRole('${key}')" class="px-4 py-2 rounded-full text-xs font-semibold ${d.role===key ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.role===key ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">${label}</button>
                    `).join('')}
                  </div>
                </div>
                <div class="mb-5">
