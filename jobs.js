const jobsTabs = [['all','All'],['opportunities','Opportunities'],['internships','Internships'],['courses','Courses'],['scholarships','Scholarships'],['others','Others']];
        ...
        let posterAppDraft = {
          ...
          businessEstablished: '', businessCategory: '', businessWebsite: '',
        ...
        function posterApplicationFormHTML(){
          ...
          return `
            <div id="poster-app-form">
              <div class="mb-5">
                <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2 block">Which best describes you?</label>
                <div class="grid grid-cols-2 gap-2">
                  ${POSTER_ROLE_OPTIONS.map(([key, label]) => `
                    <button onclick="setPosterAppRole('${key}')" class="px-4 py-2.5 rounded-full text-xs font-semibold text-center ${d.role===key ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.role===key ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">${label}</button>
                  `).join('')}
                </div>
              </div>
              <div class="mb-5">
              ...
              <div class="mb-5">
                <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2 block">When was it established?</label>
                <input type="date" oninput="updatePosterAppField('businessEstablished', this.value)" value="${escapeHtml(d.businessEstablished || '')}" max="${new Date().toISOString().slice(0,10)}" class="w-full bg-gray-100 border border-gray-300 rounded-2xl px-4 py-3 text-sm outline-none">
              </div>
              <div class="mb-5">
                <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2 block">What category fits best?</label>
                <div class="grid grid-cols-2 gap-2">
                  ${POSTER_CATEGORY_OPTIONS.map(([key, label]) => `
                    <button onclick="setPosterAppCategory('${key}')" class="px-4 py-2.5 rounded-full text-xs font-semibold text-center ${d.businessCategory===key ? '' : 'bg-gray-100 text-gray-500 border border-gray-300'}" style="${d.businessCategory===key ? `background:rgba(10,37,64,0.08);color:${NAVY};border:1.5px solid ${NAVY};` : ''}">${label}</button>
                  `).join('')}
                </div>
              </div>
              <div class="mb-5">
                <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2 block">Website or social link <span class="normal-case font-normal text-gray-400">(optional)</span></label>
                <input type="url" oninput="updatePosterAppField('businessWebsite', this.value)" placeholder="e.g. instagram.com/yourbusiness" value="${escapeHtml(d.businessWebsite || '')}" class="w-full bg-gray-100 border border-gray-300 rounded-2xl px-4 py-3 text-sm outline-none">
              </div>
              <div class="mb-5">
              ...
              <div class="mb-5">
                <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 block">Business or registration document <span class="normal-case font-normal text-gray-400">(optional)</span></label>
                <div class="text-xs text-gray-400 mb-2">Helps admins verify faster -- a registration certificate, permit, or similar.</div>
              ...
              <div class="mb-5">
                <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 block">Passport-size photo</label>
                <div class="text-xs text-gray-400 mb-2">A clear, recent photo of your face for verification. Kept private to admins.</div>
              ...
              <div class="mb-5">
                <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 block">Submit an ID</label>
                <div class="text-xs text-gray-400 mb-2">Any government-issued ID (national ID, passport, driver's license, etc.).</div>
        ...
        function posterApplicationOverlayHTML(){
          const deniedBanner = currentUserPosterStatus === 'denied' ? `
          ...
          const pendingBanner = currentUserPosterStatus === 'pending' ? `
            <div class="bg-amber-50 border border-amber-100 text-amber-700 rounded-2xl p-3 text-xs mb-5">Your application is already awaiting review -- you'll be notified once an admin decides.</div>` : '';
          return `
            <div aria-hidden="true" style="position:absolute;inset:0;z-index:-1;pointer-events:none;background:url('assets/apply-bg.webp') center / cover no-repeat;opacity:0.1;"></div>
            ...
            <div class="flex-1 overflow-y-auto px-5" style="padding-top:18px;padding-bottom:50px;">
              ${deniedBanner}${pendingBanner}
              ${posterApplicationFormHTML()}
              ${currentUserPosterStatus === 'pending' ? '' : `
                <div class="text-center mt-2" style="padding-bottom:10px;">
                  <button id="poster-app-submit-btn" onclick="submitPosterApplicationOverlay()" class="pill-cta inline-flex items-center justify-center text-white font-semibold text-center rounded-full text-sm" style="background:linear-gradient(135deg, ${NAVY} 0%, ${ROYAL} 100%);box-shadow:0 4px 14px rgba(65,105,225,0.35);padding:0.5rem 1.1rem;">${posterApplicationFormHTML_submitBtnLabel()}</button>
                </div>`}
            </div>`;
        }
