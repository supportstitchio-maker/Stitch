        ...
        // ---------------- "Don't know where to start?" intake form ----------------
        // Reached from the Career Space floating pill (#career-start-pill in
        // core.js -> openCareerStartForm()). A short, one-screen intake --
        // desired role, experience level, and a resume/CV -- for students
        // who land on the opportunity list without a clear idea of what to
        // search for, instead of leaving them to figure it out from a blank
        // search bar. No dedicated backend table for this yet, so it rides
        // the same Formspree pipe as the suggestion pill/Contact Us/Report
        // an Issue forms (see FORMSPREE_FORM_ID in core.js). Same caveat as
        ...
        let careerStartDraft = { fullName: '', email: '', phone: '', jobTitle: '', experienceLevel: '', resumeFileName: '', resumeFile: null };
        let careerStartErrors = { fullName: '', email: '', phone: '', jobTitle: '', experienceLevel: '', resume: '' };
        function resetCareerStartDraft(){
          careerStartDraft = { fullName: '', email: '', phone: '', jobTitle: '', experienceLevel: '', resumeFileName: '', resumeFile: null };
          careerStartErrors = { fullName: '', email: '', phone: '', jobTitle: '', experienceLevel: '', resume: '' };
        }
        // Called by #career-start-pill in the app shell.
        ...
        function setCareerStartExperience(id){
          careerStartDraft.experienceLevel = id;
          const ov = document.getElementById('overlay');
        ...
        function careerStartResumePickerHTML(){
          ...
          return `
            <div class="mb-4">
              <label class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-1 block">Resume / CV</label>
        ...
        async function submitCareerStartForm(){
          ...
          if (!d.jobTitle.trim()) careerStartErrors.jobTitle = 'Tell us what role or keyword to search for.';

          if (!d.experienceLevel) careerStartErrors.experienceLevel = 'Pick the experience level that fits you best.';

          if (!d.resumeFileName) careerStartErrors.resume = 'Attach your resume or CV so we can match you to the right roles.';
          ...
          try {
            ...
            // spells out the resume's filename and asks the student to
            // attach it by hand before sending.
          ...
          resetCareerStartDraft();
          closeOverlay();
          openAppAlertModal("Thanks! We've got your details and resume -- we'll follow up at " + email + " with roles worth a look.");
        }
        function careerStartFormHTML(){
          const d = careerStartDraft;
          return `
            ${overlayHeader("Don't Know Where to Start?", '20px', 'closeOverlay()')}
            <div class="flex-1 overflow-y-auto px-5">
              <p class="text-sm text-gray-500 mb-5">Tell us a bit about what you're looking for and share your resume/CV -- we'll use it to point you toward roles worth applying to.</p>
              ...
              ${careerStartFieldHTML('What role or keyword are you looking for?', 'jobTitle', 'text', 'e.g. Project Manager')}
              <div class="mb-2">
                <div class="text-xs font-bold uppercase tracking-wide text-gray-400 mb-2">Which experience level fits you best?</div>
                <div class="flex flex-col gap-2.5 mb-1">
                  ${careerExperienceLevels.map(l => `
                    <button onclick="setCareerStartExperience('${l.id}')" class="w-full px-4 py-2.5 rounded-2xl text-left bg-white${d.experienceLevel === l.id ? '' : ' border border-gray-100'}" style="${d.experienceLevel === l.id ? `background:rgba(10,37,64,0.08);border:1.5px solid ${NAVY};` : ''}">
                      <div class="text-sm font-semibold" style="color:${d.experienceLevel === l.id ? NAVY : '#1f2937'};">${l.label}</div>
                      <div class="text-xs text-gray-400">${l.desc}</div>
                    </button>
                  `).join('')}
                </div>
                ${careerStartErrors.experienceLevel ? `<div class="field-error">${escapeHtml(careerStartErrors.experienceLevel)}</div>` : ''}
              </div>
              <div class="mt-3">${careerStartResumePickerHTML()}</div>
              <button id="career-start-submit-btn" onclick="submitCareerStartForm()" class="w-full font-semibold text-sm py-3 rounded-2xl text-white" style="background:rgba(30,144,255,0.5);margin-bottom:16px;">Submit</button>
            </div>`;
        }
