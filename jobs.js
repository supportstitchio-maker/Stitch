const jobsTabs = [['all','All'],['opportunities','Opportunities'],['internships','Internships'],['courses','Courses'],['scholarships','Scholarships'],['others','Others']];
        ...
        function careerMenuPostItemHTML(){
          if (isCurrentUserAdmin() || currentUserPosterStatus === 'approved') {
            return `<button onclick="closeCareerMenuThen(() => openPostOpportunity())" class="text-left px-3 py-2 text-sm whitespace-nowrap text-gray-700 menu-item-pill">Post an Opportunity</button>`;
          }
          if (currentUserPosterStatus === 'pending') {
            return `<button onclick="closeCareerMenuThen(() => openAppAlertModal('An admin is reviewing your application to post -- you\\'ll get a notification once it\\'s decided.'))" class="text-left px-3 py-2 text-sm whitespace-nowrap text-gray-400 menu-item-pill">Poster application pending</button>`;
          }
          return `<button onclick="closeCareerMenuThen(() => openPosterApplicationForm())" class="text-left px-3 py-2 text-sm whitespace-nowrap text-gray-700 menu-item-pill">Apply to post</button>`;
        }
