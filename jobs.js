        ...
        function jobCard(job){
          return `
            <div class="rounded-3xl overflow-hidden mb-4 shadow-sm" style="background-image:linear-gradient(135deg, rgba(30,144,255,0.07) 0%, rgba(65,105,225,0.07) 100%);background-color:#ffffff;">
              ${job.coverImage ? `
                <button onclick="openJobOrClassroom('${job.id}')" class="block w-full">
                  <img src="${job.coverImage}" class="w-full object-cover" style="height:96px;" alt="">
                </button>` : ''}
              <div class="p-4 flex items-center gap-4">
                <button onclick="openJobOrClassroom('${job.id}')" class="flex items-center gap-4 flex-1 min-w-0 text-left">
                  ${!job.coverImage ? `<div class="w-12 h-12 bg-blue-50 rounded-2xl flex items-center justify-center text-gray-600 flex-shrink-0">${Icon(job.icon,'w-6 h-6')}</div>` : ''}
        ...
        function openJobOrClassroom(id){
          const job = findJob(id);
          const app = jobApplication(job);
          if (job && job.type === 'Course' && app && app.classId && !isCurrentUserAdmin()) {
            openClassDetailWithLoading(app.classId);
            return;
          }
          openJobDetail(id);
        }
        ...
        // -- re-enrolling reuses the same class via app.classId) so it's
        // indistinguishable from a class created or joined normally.
        function enrollCourseIntoClassroom(job, app){
          if (app.classId && myClasses.find(c => c.id === app.classId)) return app.classId;
        ...
        function finalizeJobApplication(id){
          ...
          app.statusUpdatedDate = Date.now();
          if (job.type === 'Course') {
            enrollCourseIntoClassroom(job, app);
          }
          queueSaveUserState();
          if (job.email) {
          ...
          return `
            ...
            <div class="flex-1 overflow-y-auto px-5 pb-8">
              ...
              <div class="text-center mt-2" style="padding-bottom:10px;">
                ${isJobApplied(job)
                  ? (isCourse
                      ? `<button onclick="openClassDetailWithLoading('${jobApplication(job).classId}')" class="pill-cta inline-flex items-center justify-center gap-2 text-white font-semibold text-center rounded-2xl text-sm" style="background:rgba(30,144,255,0.5);padding:0.5rem 1.1rem;">${Icon('book','w-4 h-4')} Go to Classroom</button>`
        ...
        async function submitNewOpportunity(){
          ...
          if (newOppEditingId) {
            ...
            if (existing) {
              ...
              postOpportunityInsertRemote(existing);
              newOppEditingId = null;
              resetNewOppDraft();
              closeOverlay();
