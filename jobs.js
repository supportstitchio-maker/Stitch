const jobsTabs = [['all','All'],['opportunities','Opportunities'],['internships','Internships'],['courses','Courses'],['scholarships','Scholarships'],['others','Others']];
        ...
        // NOTE (security): admin identity now comes ONLY from the DB
        // (profiles.role === 'admin'), never from a hardcoded email list.
        // Two reasons: (1) this file ships to every visitor's browser, so a
        // hardcoded list previously exposed both admins' real email
        // addresses to anyone who opened devtools -- a ready-made target
        // list for phishing/credential-stuffing; (2) it's a second source
        // of truth that can drift from the DB. This client-side flag only
        // controls which buttons render, though -- it is NOT what makes
        // admin actions safe. Every actual write gated by
        // isCurrentUserAdmin() (see postOpportunity / deleteOpportunity
        // below) still calls Supabase directly from the browser, so the
        // real enforcement HAS to live in Postgres Row Level Security
        // policies on the opportunities table (e.g. "allow insert/update/
        // delete only if exists (select 1 from profiles where user_id =
        // auth.uid() and role = 'admin')"). Without that RLS policy, any
        // signed-in user can call sb.from('opportunities').insert(...)
        // straight from the console and post/delete listings regardless of
        // what this function returns -- hiding a button is UX, not access
        // control. Set role='admin' on the two admin accounts' profiles
        // rows to restore their access under this change.
